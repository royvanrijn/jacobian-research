...
| Degreewise divisor-count theorem | Proved for every `N>=4`: at least `tau(N-1)` pairwise stably inequivalent maps of generic degree `N`, comprising one cancellation type per proper divisor of `N-1` plus one weighted type | None recorded; component constructions and boundary exhaustion have repository regressions | [Marked-root Keller maps](MARKED_ROOT_KELLER_MAPS.md) |
