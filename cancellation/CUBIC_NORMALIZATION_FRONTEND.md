...
Tan's nonflat triple-cover correspondence states that a rank-two reflexive
sheaf `M` with
\[
 \operatorname{Sym}^3(M)\otimes\det(M)^{-2}
\]
generated by global sections occurs as the trace-free sheaf of an integral
triple cover, obtained by normalization of the corresponding cubic
construction.  The generation condition is automatic on the affine scheme
`X`.  Thus there is such a normal triple cover with
