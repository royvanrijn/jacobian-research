...
| Positive-control calibration plus fail-closed descent gates | Elkies rank-25--29 fibres and the rank-32 search | Detector sensitivity to `jump>=1` is not an extreme-rank gate.  Before candidate promotion, freeze an ordinal score, validate score--jump association on an independent multi-stratum panel, and predeclare an upper-tail enrichment endpoint; then require the actual residual Selmer quotient on the same minimal curve before any expensive point search.  A timeout, binary escape, signature, or incomplete local-class list authorizes nothing. |
