...
| Coverage-certified sharded enumeration | Rootless bisections and singular rational-normalization searches | A distributed negative result needs half-open domain ranges, source hashes, gap/overlap checks, exact exception accounting, and a fail-closed merged certificate.  Individual no-hit chunks do not carry theorem scope. |
...
| Promoting a modular or formal lift to a rational source | A smooth GF(7) point and compatible Hensel data for NS0031 did not by themselves give an algebraic `QQ` family. | Certify the formal branch at its exact scope, then solve algebraization, rationality, Picard-rank, and marking gates independently. |
...
| Elliptic curves over `Q` | Curve 302 has unconditional rank at least 31 with exact local arithmetic; curves 273 and 356 have rank at least 30 and 29; the complete pinned norm-twelve atlas supplies exact native published-R17 and alternate-Q80 calibration fibres | Exact rank 31 or rank at least 32; completed residual descent for the high-rank and low-conductor targets; sharper exact conductor/rank records. |
| Elkies--K3 reconstruction and foundry | Published R17, direct alternate Q80, and the noncyclic `4A1/MW13` bridge are equation-explicit over `QQ` with saturated bases.  Smooth and singular-genus-one character maps are exhausted on the two direct norm-twelve charts, and the pinned 43-chart/474-curve atlas gives native controls. | The next construction milestone is arithmetic MW17 on a different NS.  NS0024 is excluded over `QQ`; NS0031 has a formally smooth marked source branch and physical corridor but still needs algebraization, a rational rank-19 marking, target-free endpoint selection, and exact compilation.  Rank `>=32`, nonzero product quotient classes, and global genus-two injectivity remain open. |
