...
| Positive-control calibration plus fail-closed descent gates | Elkies rank-25--29 fibres and the rank-32 search | The redacted sixteen-fibre refresh ladder now validates exact half-lattice recovered rank against displayed jumps `+3` through `+12`: tied-margin Kendall `tau_b=.7503`, exact one-sided `p=60852/2421619200`, and score at least ten on 7/8 of the `+10/+11/+12` tail versus 0/8 below (`p=1/1430`).  This supports extreme-jump scheduling, but the actual residual Selmer quotient on the same minimal curve remains mandatory before any serious point search.  A timeout, score, binary escape, signature, or incomplete local-class list authorizes nothing. |
