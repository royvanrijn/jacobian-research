...
endpoint-kernel identification.  These results construct the concrete
bridge in Lean and prove Theorem~\ref{thm:gvc3}, including its exact mixed
scalar.  Coefficient base change to every characteristic-zero field and
unused-variable padding then verify the negative dimension-$n\ge3$ half of
...
intermediate-value step for the moving envelopes.  It also derives the
full-profile implications from an explicitly named bridge record; for
Theorem~\ref{thm:family}, the record contains only the multivariate phase
coefficients, after which Lean proves the pure identity and complete exact
multiplier ladder.  The present
audit does not yet formalize the full-profile phase extraction, the
number-field good-prime transfer and shifted-ray theorem, or the binary
Hall/no-reversal/common-threshold bridge.  The spherical realization of the
...
Thus Theorems~\ref{thm:main} and \ref{thm:family}, and the complete
biconditional in Theorem~\ref{thm:classification}, are not yet fully
formally verified.
