...
the resulting linear derivative deficit.  Apart from the constant-term
theorem of Duistermaat and van der Kallen, the argument is elementary and
does not use a degree bound or computer algebra.  We then give a homogeneous
...
There were two particularly important precursors.  De Bondt proved GVC for
every product of linear differential forms, and therefore for every
homogeneous operator in two variables~\cite{deBondt}.  Much more recently,
Long found a five-term quartic in three real Gaussian variables for which
...
The proof came from trying to understand a repeated pattern in finite-degree
calculations.  At each Newton wall, the surviving coefficient ideals looked
like staircase, or Ferrers, ideals: an operator term could move left, and a
polynomial term could move right, but the two moves were never allowed to
close the whole gap.  The calculations suggested a local statement stronger
than any one radical computation: two unequal-weight faces satisfying all
pure vanishing identities cannot overlap horizontally.

Once that statement is written down, the global proof is short.  Give both
...
There are three points worth emphasizing.  First, the operator may be
nonhomogeneous.  Second, the argument is not an induction on degree.  Third,
the earlier carry, factorial, and bounded-radical calculations are not used
in the proof.  They were useful in finding the horizontal-separation
principle, but the final argument bypasses them.

The proof is internal and, at the date on this manuscript, has not been
externally reviewed.
...
\begin{proposition}[Degree cutoff]\label{prop:cutoff}
Suppose that $\Lambda$ has no constant term, and let $r$ be the least
...
it as the $y$-axis gives \eqref{eq:hall-normal} and
\eqref{eq:hall-poly}.
\end{proof}
...
\begin{proof}
Suppose the intersection is nonempty.  It lies on one affine weight line.
Because $u\ne v$, ordinary degree is strictly monotone on that line; hence
the intersection has a unique point $\rho$ of least ordinary degree.
Lemma~\ref{lem:expose}, applied in two independent sets of variables, gives
an integer $k$ for which $k\rho$ and $k(\rho+\delta)$ are integral and
\[
 c=[X^{k\rho}]A^k\ne0,
 \qquad b=[x^{k(\rho+\delta)}]B^k\ne0.
\]
Put $A_0=A^k$, $B_0=B^k$, $\alpha=k\rho$, and $\eta=k\delta$.

All coefficients involved lie in a finitely generated
characteristic-zero domain.  Specialize it to a number field while keeping
$bc\ne0$, and choose a sufficiently large unramified rational prime $p$
at which $b$ and $c$ are units.  The coefficient on the ray $p\eta$ is
\begin{equation}\label{eq:coefficient-sum}
 \sum_\gamma
 \frac{(\gamma+p\eta)!}{(p\eta)!}
 [X^\gamma]A_0^p\,
 [x^{\gamma+p\eta}]B_0^p,
\end{equation}
where factorials are taken coordinatewise.  Every contributing $\gamma$
belongs to
\[
 p\Newt(A_0)\cap\bigl(p\Newt(B_0)-p\eta\bigr).
\]

Let $s=\alpha_x+\alpha_y$.  Choose $p$ larger than every coordinate in
the fixed Newton segments.  If $\gamma$ is not divisible by $p$
coordinatewise, Frobenius makes each of the two coefficients in
\eqref{eq:coefficient-sum} divisible by $p$.  The factorial quotient has
$p$-adic valuation at least
\[
 \left\lfloor\frac{\gamma_x}{p}\right\rfloor+
 \left\lfloor\frac{\gamma_y}{p}\right\rfloor
 \geq s-1.
\]
Thus such a summand has valuation at least $s+1$.

If $\gamma=p\beta$, the factorial quotient has valuation
$\beta_x+\beta_y$.  The unique point with this value equal to $s$ is
$\beta=\alpha$.  At that point the two coefficients reduce to $c^p$ and
$b^p$, while the factorial quotient has valuation exactly $s$ and a
nonzero unit part.  Every other summand has valuation at least $s+1$.
Therefore \eqref{eq:coefficient-sum} has one term of uniquely least
$p$-adic valuation and cannot vanish.  This contradicts
\eqref{eq:shifted-vanishing} at scale $kp$.
\end{proof}

The specialization in this proof is only a device for proving a polynomial
identity is nonzero.  It therefore proves the result over every
...
Their order cannot reverse while $\Delta>0$.  They begin with the operator
interval strictly to the right by \eqref{eq:initial}.  A change can occur
only at one of the finitely many rational breakpoints of the envelopes.  At
the first hypothetical reversal, the complete breakpoint faces contain
both the old and new endpoints.  Their projected intervals must overlap,
contrary to Corollary~\ref{cor:horizontal}.  Hence, for every rational
$s>1$ with $\Delta(s)>0$,
\begin{equation}\label{eq:ordered}
 \min\{\alpha_x:\alpha\in\Supp A_s\}
 >
 \max\{\beta_x:\beta\in\Supp B_s\}.
\end{equation}

The function $\Delta$ cannot stay positive forever.  After its last
breakpoint, $L$ is realized by an operator monomial with globally least
$X$-exponent $a_{\min}$, while $U$ is realized by a polynomial monomial
with globally greatest $x$-exponent $i_{\max}$.  Equation
\eqref{eq:ordered} gives $a_{\min}>i_{\max}$.  Thus on this final interval
\[
 \Delta(s)=C+s(i_{\max}-a_{\min})
\]
has strictly negative slope, so it must cross zero.

Let $s_*>1$ be the first zero.  All breakpoints and zeroes of these
...
\section{How the witness was found and the full family}

The construction can be read backwards from the coefficient calculation.
...
For $n>3$, use the same $P,Q$, and $\Lambda$ in the first three variables
and let them ignore the remaining variables.  This preserves both
identities in \eqref{eq:gvc3-pure-mixed}.
\end{proof}
...
\section{What the binary proof explains}

The envelope argument explains the staircase ideals that appeared in the
finite calculations.  Suppose a wall begins with a horizontal gap $g>0$.
...
\]
This is why bounded moment calculations repeatedly returned coordinate
components and staircase products.  They were seeing finite shadows of a
