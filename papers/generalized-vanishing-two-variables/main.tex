...
certificate and the uniform cutoff
$m>(\deg P+\deg\lambda)\deg Q$, together with a finite decision procedure
for rational inputs.
...
\subsection{Finite support certificates and a degree-only cutoff}
...
The sharper certificate cutoff can be attained.  For $d,q\geq1$, take
\[
 \Lambda=\partial_x+\partial_y^{d+1},\qquad P=y^d,\qquad Q=x^q.
\]
All pure contractions vanish.  In a mixed contraction, a term with $j$
copies of $\partial_x$ requires $j\leq q$ and $m\leq(d+1)j$.
Thus the last nonzero mixed power is $m_0=(d+1)q$, with
\[
 \Lambda^{m_0}(QP^{m_0})=\binom{m_0}{q}\,q!\,(dm_0)!.
\]
The separator $(d+1,1)$ has gap one and gives precisely the cutoff $m_0+1$.
This excludes a degree-independent constant multiplying $\deg Q$; it
does not assert optimality of the general coefficient $d+R$.
