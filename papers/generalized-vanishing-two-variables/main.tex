...
\date{4 August 2026\\Revised 5 September 2026}
\hypersetup{
...
In fact, the binary premise is equivalent to strict support separation
after a linear change over the original field.  Section 6.1 extracts this
certificate and the optimal operator-independent cutoff
$m>(1+\lfloor(\deg P+1)^2/4\rfloor)\deg Q$, together with a finite
decision procedure for rational inputs and an additional operator-degree bound.
...
\subsection{Finite support certificates and optimal uniform cutoffs}

The same argument characterizes the pure-vanishing premise and makes the
mixed cutoff effective.  In the following statement the coordinate change
acts contragrediently on the differential symbol.

\begin{theorem}[Binary support certificate]\label{thm:support-certificate}
Let $0\ne\lambda\in k[X,Y]$ have zero constant term, let $0\ne P\in k[x,y]$,
and put $R=\deg\lambda$ and $d=\deg P$.  Then
$\Lambda^m(P^m)=0$ for every $m\geq1$ if and only if there are a linear
coordinate change over $k$ and positive integers $u,v\leq d+R$ such that
\begin{equation}\label{eq:strict-certificate}
 g:=\min_{\alpha\in\Supp\lambda}(u\alpha_x+v\alpha_y)
       -\max_{\beta\in\Supp P}(u\beta_x+v\beta_y)>0.
\end{equation}
If $d\geq r$, the Hall direction is unique and defined over $k$; any
coordinate frame over $k$ whose first vector has that direction admits
such a certificate with $u>v$.
\end{theorem}

\begin{proof}
If $d<r$, use the ordinary weight $(1,1)$.  Suppose $d\geq r$.
Lemma~\ref{lem:hall} also applies when $d=r$: a deficient subset with two
nonparallel directions would see all $d\geq r\geq |I|$ polynomial factors,
which is still impossible.

There cannot be two Hall directions.  If their full multiplicities in
$\lambda_r$ are $e_1,e_2$, the distinct annihilating factors in $P_d$
would have multiplicities $h_i\geq d-e_i+1$.  But
\[
 e_1+e_2\leq r,\qquad h_1+h_2\leq d,\qquad
 h_1+h_2\geq2d-r+2\geq d+2,
\]
a contradiction.  The unique direction is invariant under all
automorphisms of $\overline k/k$.  Its projective coordinate is algebraic
and separable over $k$, so it belongs to $k$.  Choose it as the first
coordinate direction, with any complement over $k$.  The initial Hall
gap $e>t$ holds in this frame.

For $d=r$, $\Delta(1)=0$ and its right derivative is $t-e<0$, so
$\Delta(s)<0$ for some rational $s>1$.  For $d>r$, the envelope argument
above gives a first rational $s_*>1$ with $\Delta(s_*)=0$.
The zero-output case of Proposition~\ref{prop:shifted} separates the
complete equality faces at $s_*$.  They contain the limiting active
exponents from the left, where the operator lies to the right of the
polynomial.  Hence the complete equality faces have that same order.
The right derivative of $\Delta$ at $s_*$ is therefore negative, and
again $\Delta(s)<0$ for some rational $s>1$.

We bound an integral weight directly from the support inequalities.
Every pair $\alpha\in\Supp\lambda$, $\beta\in\Supp P$ imposes
\[
 (\alpha_x-\beta_x)s+(\alpha_y-\beta_y)>0.
\]
Together with $s>1$ these define a nonempty open interval $(\ell,h)$.
Write $\ell=p/q$ in lowest terms.  Either $\ell=1$, or it is a lower
endpoint $(\beta_y-\alpha_y)/(\alpha_x-\beta_x)$ with positive denominator.
In both cases $1\leq p\leq d$ and $1\leq q\leq R$.
If $h$ is finite, write $h=p'/q'$ in lowest terms.  It comes from an
upper endpoint $(\alpha_y-\beta_y)/(\beta_x-\alpha_x)$, so
$1\leq p'\leq R$ and $1\leq q'\leq d$.
The mediant weight $(u,v)=(p+p',q+q')$ lies strictly inside the interval
and has both entries at most $d+R$.  If $h=+\infty$, use
$(u,v)=(p+q,q)$ instead.  In either case $u>v>0$ and the integral gap
in \eqref{eq:strict-certificate} is at least one.

Conversely, a strict support gap makes the differential weight in every
monomial selection of $\Lambda^m(P^m)$ exceed the input weight by at
least $mg>0$.  Each selection therefore vanishes.
\end{proof}

\begin{corollary}[Effective mixed cutoff]\label{cor:effective-cutoff}
Under the pure-vanishing premise, every nonzero $Q\in k[x,y]$ satisfies
\[
 \Lambda^m(QP^m)=0\qquad\bigl(m>(d+R)\deg Q\bigr).
\]
More precisely, a certificate \eqref{eq:strict-certificate} gives vanishing
for $m\geq\lfloor K_Q/g\rfloor+1$, where
$K_Q=\max_{\gamma\in\Supp Q}(u\gamma_x+v\gamma_y)$ in the chosen frame.
\end{corollary}

\begin{proof}
A mixed monomial selection vanishes once $mg>K_Q$.
Linear changes preserve total degree, and
$K_Q\leq\max(u,v)\deg Q\leq(d+R)\deg Q$, while $g\geq1$.
\end{proof}

For rational inputs, Theorem~\ref{thm:support-certificate} gives a finite
decision procedure for the pure premise: factor $\lambda_r(T,1)$ over
$\Q$, include the possible direction at infinity, test the rational
linear factors for the Hall multiplicity condition, and solve the support
inequalities in the unique surviving frame.  A nonrational Hall direction
would have two distinct conjugates and is ruled out by uniqueness.
An empty feasible interval means the pure premise fails.  The procedure
uses no finite prefix of moments to infer all-power vanishing.

\begin{theorem}[Optimal operator-independent cutoff]\label{thm:optimal-cutoff}
Let $0\ne P$, $d=\deg P$, and $C_d=1+\lfloor(d+1)^2/4\rfloor$.
Under the binary pure-vanishing premise, every nonzero $Q$ satisfies
\[
 \Lambda^m(QP^m)=0\qquad(m>C_d\deg Q).
\]
For every $d\ge0$, no smaller coefficient works uniformly over all
operators and degree-$d$ polynomials.
\end{theorem}

\begin{proof}
We first improve the normalized margin of a strict support separator.
Swap axes if necessary and write its weight as $(1,z)$, $0<z\le1$.
The concave piecewise-linear function
\[
 h(z)=\min_{(a,b)\in\Supp\lambda}(a+bz)
       -\max_{(i,j)\in\Supp P}(i+jz)
\]
is positive somewhere. We claim that some rational $z\in(0,1]$ gives
$h(z)\ge1/C_d$. If $d=0$, use $z=1$. For $d\ge1$, the endpoint values
are integers. A positive $h(1)$ suffices; if $h(0)>0$, then
$h(1/(d+1))\ge1-d/(d+1)\ge1/C_d$, since operator slopes are nonnegative
and polynomial slopes are at most $d$.

Otherwise a positive maximum occurs at an interior breakpoint. At a
polynomial-envelope breakpoint the reduced denominator is at most $d$,
so the positive gap is at least $1/d$. This covers simultaneous
breakpoints as well. At any remaining maximum one polynomial monomial
$(i,j)$ is active. Write $(c,e)$ and $(a,b)$ for the operator monomials
active on the left and right. Then $a>c$, $e>b$, and
\[
 z_0=\frac{a-c}{e-b},\qquad e\ge j\ge b.
\]
If $c>i$, the gap is at least one. Otherwise set
$A=a-i>0$, $B=j-b\ge0$, $C=i-c\ge0$, and $E=e-j\ge0$.
The integer $N=AE-BC$ is positive and $B+C\le d$, whence
\[
 \frac1{h(z_0)}=\frac{B+E}{N}
 =\frac1A+\frac B N+\frac{BC}{AN}
 \le1+B(C+1)\le1+\left\lfloor\frac{(d+1)^2}{4}\right\rfloor.
\]
This proves the claimed margin. Every multiplier of degree $q$ has weight
at most $q$, so $m>C_dq$ kills every mixed monomial selection.

For sharpness choose $e=\lceil(d+1)/2\rceil$, $j=\lfloor(d+1)/2\rfloor$,
and $R=ej+1=C_d$. Put
\[
 \Lambda=\partial_x^e+\partial_y^R,\qquad P=x^{e-1}y^j,
 \qquad Q=x^q\quad(q\ge1).
\]
The weight $(R,e)$ has gap one, so all pure contractions vanish.
A mixed term with $k$ copies of $\partial_x^e$ requires
$ek\le(e-1)m+q$ and $R(m-k)\le jm$. Together these give $m\le Rq$.
At $m_0=Rq$ exactly $k=(R-j)q$ survives, with nonzero value
\[
 \Lambda^{m_0}(QP^{m_0})
 =\binom{m_0}{jq}\bigl((e-1)m_0+q\bigr)!\,(jm_0)!.
\]
Thus $C_d$ is optimal, including $d=0$.
\end{proof}

Both degree bounds remain available: one may use
$m>\min(d+R,C_d)\deg Q$, or the particular support-certificate cutoff.
For $d=3$, the pair $\Lambda=\partial_x^2+\partial_y^5$, $P=xy^2$
with $Q=x$ has last nonzero mixed power five; a proposed general
coefficient $d+1$ would already fail here.
