...
\[
 ...
 \setminus\bigl(\{\operatorname{Res}=0\}
 \cup\{[LQ]_{T^2S}=0\}\bigr).
\]
This is the projective source open from
Section~\ref{sec:geometric-construction}.

\begin{lemma}[unique affine normalization]\label{lem:unique-factor-normalization}
Projectivization induces an isomorphism
\[
 X_{\mathrm{fac}}\xrightarrow{\sim}U.
\]
\end{lemma}

\begin{proof}
Choose representatives of $([L],[Q])\in U$ and put
\[
 m=[LQ]_{T^2S},\qquad r=\operatorname{Res}(L,Q).
\]
Under $(L,Q)\mapsto(\lambda L,\mu Q)$ these functions have weights $(1,1)$
and $(2,1)$, respectively.  The unique solution of
\[
 \lambda\mu m=1,
 \qquad
 \lambda^2\mu r=1
...
\[
 ...
 \mu=\frac r{m^2}.
\]
If the original representatives are replaced by $(\alpha L,\beta Q)$, the
new normalization scalars are $\lambda/\alpha$ and $\mu/\beta$.  Hence the
normalized pair is independent of representatives.  The formulas are regular
on $U$ and invert projectivization.
\end{proof}
...
\subsection{Unequal factor degrees}

The differential argument extends uniformly.  Put
\[
...
and define
\[
 \Theta_{p,q}:V_p\times V_q\longrightarrow V_{p+q}\times\A^1,
 \qquad
 (A,B)\longmapsto\bigl(AB,\operatorname{Res}(A,B)\bigr).
\]

\begin{proposition}[unequal-degree coefficient--resultant theorem]
If $p\ne q$, then $\Theta_{p,q}$ is etale on the coprime locus.
\end{proposition}

\begin{proof}
The kernel of the differential of multiplication at a coprime pair is again
spanned by $(A,-B)$.  Since the resultant has bidegree $(q,p)$,
\[
 d\operatorname{Res}_{(A,B)}(A,-B)
 =(q-p)\operatorname{Res}(A,B).
\]
This is nonzero in characteristic zero when $p\ne q$, and the source and
target dimensions are both $p+q+2$.  Therefore
$d(\Theta_{p,q})_{(A,B)}$ is an isomorphism, so $\Theta_{p,q}$ is etale at
$(A,B)$.
\end{proof}

There is a second distinction at the normalization stage.  Suppose $q>p$,
let $m$ be a nonzero linear functional of $AB$, and put
$r=\operatorname{Res}(A,B)$.  The conditions
\[
 \lambda\mu m=1,
 \qquad
 \lambda^q\mu^p r=1
...
are equivalent to
\[
 \lambda^{q-p}=\frac{m^p}{r},
 \qquad
 \mu=\frac1{\lambda m}.
\]
For consecutive degrees $q-p=1$, this gives the unique normalization
\[
 \lambda=\frac{m^p}{r},
 \qquad
 \mu=\frac{r}{m^{p+1}}.
\]
For $q-p>1$ there remains a residual $\mu_{q-p}$ ambiguity.  When $p=q$, the
resultant has weight zero along relative scaling and does not repair the
multiplication kernel.  Thus unequal degrees explain etaleness, while
consecutive degrees explain the canonical normalized slice used above.
