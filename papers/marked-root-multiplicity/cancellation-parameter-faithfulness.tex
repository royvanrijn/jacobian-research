...
$\bar\beta(R)=u^{-m}R$.  This proves \eqref{6.3}.
\end{proof}
...
\[
 q=q'\qquad\hbox{and}\qquad h_q=h_{q'}.
                                                               \tag{6.7}
\]
Equivalently, the common finite normalization of \eqref{6.2}, together with
...
stabilization, so \eqref{6.8} cannot exchange the two components.  Unique
factorization and \eqref{6.9} therefore give a constant $c\in k^*$ such that
\[
 \alpha^*A'=cA,
 \qquad
...
nondomain fiber $A'=1$ under \eqref{6.10} is $A=c^{-1}$; hence $c=1$.
...
If $m>1$, comparison of prime multiplicities in \eqref{6.13} gives
\[
...
$Q$-identity in \eqref{6.8} modulo $B$ gives $Y\equiv uy\pmod B$, while
\[
...
$(B)$.  Consequently \eqref{6.14} holds for every $m$.  The same reduction
modulo $B$ now gives $\lambda=u$.

Using \eqref{6.10} with $c=1$, the full $Q$-identity is
\[
 uy+a u^{-m}xB
 =u(y+xB)+AB\,F_q^*(\varphi).                          \tag{6.16}
\]
Cancel $B$.  If $a u^{-m}-u$ were nonzero, \eqref{6.16} would imply
...
\]
Equations \eqref{6.10}, \eqref{6.14}, and \eqref{6.17} carry the analogous
residue for $F_{q'}$ to
\[
...
On the inverse chart $D=A^{-1}$, $B=PD$, and $y=Q-sP$, so \eqref{6.18} is
\[
