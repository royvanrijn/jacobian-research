...
marked target divisors.  The weighted contact is
...
proof of the finite degreewise stable-multiplicity theorem.  The decorated
discriminant normalization and its positive-dimensional moduli, and the
Hurwitz--LL compactification of rerooting, are separate papers.  Other
repository documents may summarize the result or audit individual inputs,
but should defer to the formulation here for the finite count.
...
The following theorem is the discrete cross-construction statement: it distinguishes
...
the cancellation type belonging to each proper divisor $r\mid N-1$.  Its
proof uses only four inputs: complete codimension-one boundary lists, stable
functoriality, the unconditional thick-contact formula, and parameter
faithfulness.  The refined pairwise intersection diagram is not a dependency.
...
The common plane layer is the
\href{../../verified/TANGENT_MAP_CORE.md}{tangent-map core}.  After replacing
$\gamma$ by the first target coordinate $s=H'(W)+c\gamma$, its normal form is
\[
 (W,s)\longmapsto\bigl(s,Ws-H(W)\bigr).                 \tag{3.0}
\]
It simultaneously gives the inverse pencil $H(W)-sW+t=0$, the discriminant
normalization, the Hessian Fitting divisor, and the critical-value incidence.
The weighted construction below is its three-dimensional suspension; we use
those consequences without rederiving them separately.
...
$(2+\kappa)a_0+(1+\kappa)=0$.  The weighted-suspension clause of the
tangent-map core gives
\[
...
\begin{proposition}[regular weighted fibers away from critical values]
\label{prop:weighted-explicit-fiber}
Suppose $H\in\C[W]$ is an admissible seed of degree $d$ and $c\ne0$.  Choose
$\alpha\in\C$ outside the finite set
\[
 \left\{-\frac{H(w)}c:H'(w)=0\right\}.
\]
Then the target
\[
...
\]
By the choice of $\alpha$, $E$ and $E_W$ have no common root.  Thus $E$ is a
degree-$d$ polynomial with $d$ distinct complex roots.
...
map of generic degree $N$ and has a target fiber consisting of $N$ distinct
points.
\end{lemma}
...
Jacobian $2$, and exact generic degree $N$.  Choose any $\alpha$ outside the
finite critical-value set and apply
Proposition~\ref{prop:weighted-explicit-fiber}.  This supplies an $N$-point
fiber with every reconstruction denominator nonzero.  Since
$H_N\in\mathbb Q[W]$, taking $\alpha=\pi$ is a deterministic exact
certificate.
\end{proof}
...
There are $\tau(n)-1$ cancellation types.  Boundary exhaustion, stable
functoriality, and Lemma~\ref{lem:contact} attach to type $(m,r)$ the stable
signature
\[
 (e_\Delta,\mu)=
 \left(r+1,\,n\left(\frac nr-1\right)\right).             \tag{8.1}
\]
