...
- `index_three_inverse_degree_model.json` records an exact triangular
  ...
  `scripts/verify_index_three_inverse_model.py` and makes degree nine the
  first possible sharp uniform bound for the full index-three attack.
- `index_three_inverse_tree_obstruction.json` is the exact rational
  ...
  quotient and one explicit nonzero normal form for `K_11`; it is generated
  by `scripts/derive_index_three_tree_obstruction.py`.
