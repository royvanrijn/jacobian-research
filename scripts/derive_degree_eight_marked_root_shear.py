#!/usr/bin/env python3
"""Derive the degree-eight monic marked-root completing shear exactly."""
...
def shear_at(point: tuple[int, int]):
    ...
    factor = (
        ...
        + sigma * w**4
        + tau * w**3
        + (-sp.Rational(15, 2) - 4 * sigma - 3 * tau) * w
        + sp.Rational(11, 2)
        + 3 * sigma
        + 2 * tau
    )
...
def main() -> None:
    ...
    certificate = {
        "scope": "exact degree-eight classical rank-two shear interpolation",
        "family_factor": (
