#!/usr/bin/env python3
"""Modular interpolation of the degree-nine order-five pivot chart.

The exact one-line probe fixes denominator degree 72.  This full bivariate
job reconstructs four residual numerators with total-degree bounds
74, 76, 78, and 80, validates them on the unused part of the rectangular
grid, and saturates their ideal by the chart denominator.  Its output remains
modular discovery until several images are reconstructed over Q.
"""
...
from probe_degree_nine_order_five_chart import PIVOT_COLUMNS, pivot_data
from verify_degree_nine_relative_quantization_obstruction import family_presentation


DENOMINATOR_DEGREE = 72
NUMERATOR_DEGREES = (74, 76, 78, 80)
TARGET_INDICES = (149, 150, 151, 152)
...
def ambient_data():
    ...
    return tuple(monomials[:149]), tuple(
...
def main():
    ...
    parser.add_argument("--sigma-samples", type=int, default=150)
    parser.add_argument("--tau-lines", type=int, default=84)
    ...
    if args.sigma_samples < 150 or args.tau_lines < 84:
        raise SystemExit("grid is too small for the certified degree bounds")
    tasks = [
    ...
    certificate = {
        "scope": "modular degree-nine pivot-chart interpolation",
        "prime": args.prime,
        "grid": {
    ...
    print(
        f"PASS: degree-nine chart reconstructed over GF({args.prime}); "
