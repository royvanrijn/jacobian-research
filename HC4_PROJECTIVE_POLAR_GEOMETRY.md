...
gives a finite numerical atlas for quadratic and cubic affine gradients of
geometric degree two or three.
...
be \(14,13,79,78\), respectively.  These are necessary configurations,
not existence results.
...
## 8. Consequences and next finite tests
...
3. Discard every \(m=3\), affine-degree-two or affine-degree-three row by
   ...
   are needed.
4. Begin the genuinely open projective atlas at \(m=4\), corresponding to
   quintic potentials.  Intersect its degree-two/three signatures with the
   repository's quintic Schur strata before constructing base ideals.
...
\(m=3\) row.  The next nontrivial projective frontier is therefore
\(m=4\), i.e. quintic potentials.
...
log-concave degree lists, tests the smooth-curve numerical rows, verifies
the finite consequences of Wang's theorem and `HC4CQ1`, verifies the
cotangent and Meng--Yang aggregate corrections, and writes the JSON
artifact.  The second independently computes the graph and full-polar
projective degrees in (1.4) over \(\mathbb Q\).

The generated artifact currently has SHA-256

```text
434e27d8f5231058ef38ebc36883564bf55fc097ddfd67cf4cfb1749b1160031
```
