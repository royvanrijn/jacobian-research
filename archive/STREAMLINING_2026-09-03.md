...
```text
f47f1a1872881680d9296170f3f9619bcc943c29
```

Immutable browser form:

```text
https://github.com/royvanrijn/jacobian-research/tree/f47f1a1872881680d9296170f3f9619bcc943c29
```

Use that snapshot when reconstructing an old narrative, command surface, or handoff that was intentionally removed from the active README/AGENTS pages.
...
A route may be operationally archived while its detailed note remains at the old path when a current certificate or ledger still references it. In that case the active README/AGENTS pages determine that it is historical; the retained file is evidence, not an active task list.

No mathematical theorem status was changed by this cleanup. `MATH_STATUS.json` therefore remains unchanged and `STATUS.md` was not edited manually.
