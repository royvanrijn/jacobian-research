...
``--check`` replays retained positive cover, section and quotient witnesses.
The exhaustive split census remains pinned discovery evidence; its negative
obstruction inventory is integrity checked, not regenerated during point-proof
replay. ``--check --regenerate`` explicitly rebuilds the complete census.
"""
...
def replay_retained_covers(payload, selected):
    ...
    for chart in payload['charts']:
        ...
        for name,expected in chart['inputs'].items():retained_source(ROOT,name,expected)
...
def main():
    ...
    if args.check and not args.regenerate:
        ...
        if args.witness_worker:
            ...
            result=replay_retained_covers(json.loads(args.output.read_text()),selected)
