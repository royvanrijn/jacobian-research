# Foundational geometry: foundational-through-weighted core

This note isolates the complete foundational proof chain.  It uses no claim
after weighted marked-root theorem.  The four inputs are elementary polynomial identities, two explicit
...
zero.  Appending identity coordinates proves the foundational Keller map in every dimension at least
three.
...
nonosculating hyperplane used in Cubic Marked-Root Model; normalizing that coefficient to `-2`
...
Thus `I^simp` is the abstract open `X` in Cubic Marked-Root Model, and Theorem B proves both
$X\cong\mathbb A^3$ and the advertised coordinate realization of the
addition map.

## Cubic Image and Nonproperness Theorem as a direct corollary
...
Hence the nonproperness set is exactly `V(Q)`.  This proves Cubic Image and Nonproperness Theorem without using
the weighted theory.
...
Add Dean Cureton's pinned Lean foundational-map build with
