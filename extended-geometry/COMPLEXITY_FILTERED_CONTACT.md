...
For the ordinary-source-degree face of `F_2`, this computation is now
complete in the invariant-ring-saturated module.  The target invariant
algebra has a three-generator finite SAGBI basis, but linear lifting is not
Rees-strict: in weight one,
`3 ell_F(AC partial_A)-4 ell_F(C^2 partial_C)` drops from degree `39` to
degree `34`; adding `28 ell_F(B partial_A)` cancels that face too and leaves
a genuinely new degree-`29` initial-module generator.  Thus the required
commutation fails already on this face.
See [the finite LR Rees/SAGBI computation](LR_REES_SAGBI_MODULE_COMPUTATION.md).
...
That classification is now complete for the same saturated face.  Only
`p=1,2` survive and every `|p|>=3` matrix vanishes by a semigroup-annihilator
cutoff.  The `p=1` and `p=2` matrices have respectively eleven and eight
nonzero columns.  Exact module reduction leaves one new independent
`p=2` class,

\[
 \operatorname{II}_{F_2,2,-2}(\partial_A,A^2\partial_A)
 \equiv-\frac{987}{395}e_C
 \pmod{\operatorname{im}\operatorname{II}_{F_2,1,-1}}.
\]

Hence neither the minimal `p=1` column nor the full `p=1` matrix generates
the quadratic normal image.  More precisely, the extra quotient is the
reduced length-one module `R/(gamma,6u-1)`, generated by the displayed
`p=2` class.
