...
Makar--Limanov invariant is still only bounded above by the kernels of the
known actions; all three displayed LNDs fix `a`, but an undiscovered LND may
move `a,U,K`, or `J`.  Flexibility also remains open: saturation repairs the
artificial rank-one drop, but the known orbit distribution still has rank
only two at the displayed boundary point.

### Exact kernels and the Hilbert--14 test

The primitive pair has a completely explicit invariant ring.  Besides
(49), define the regular functions

\[
 \begin{aligned}
 G={J-4U\over a}
  &=4(2a^2x-acy+5cpz-6z+2c^2d),\\
 H={K^2+8J-16U\over a^2}
  &=16z^2p^2+64czpd+64c^2d^2+64c^2py\\
  &\quad-32azpy+16a^2y^2-128zd-64cy+64ax,
 \end{aligned}                                      \tag{59}
\]

and put \(V=4G-aH\).  They are killed by both \(D_{10}\) and \(D_7\), and

\[
 V^2-HK^2=1024a.                                    \tag{60}
\]

Consequently

\[
 B=k[a,U,K,J,G,H]=k[K,H,V]\simeq k^{[3]}.           \tag{61}
\]

Indeed (60) first recovers \(a\), then
\[
 G={V+aH\over4},\qquad
 U={a^2H-K^2-8aG\over16},\qquad J=4U+aG.
\]
After inverting \(a\), formula (52) becomes

\[
 R_a=B_a[p,s],\qquad
 D_{10}={\partial\over\partial s},\qquad
 D_7=a{\partial\over\partial p},                    \tag{62}
\]

where \(R=k[X_{2,3}]\).  Thus the common kernel in \(R_a\) is \(B_a\).
There are no extra invariants supported at the missing divisor.  To see this
without a normality shortcut, use

\[
 R/(a)=k[c^{\pm1},d,y,z],\qquad p=c^{-1},
\]

and set \(q=2c^2d-z\).  On this boundary

\[
 K=4c,\qquad G=4q,\qquad H={16q^2\over c^2},
 \qquad V=16q.                                      \tag{63}
\]

The induced map \(B/(a)\to R/(a)\) is injective.  If
\(f=b/a^n\in R\cap B_a\), with \(n\) minimal, reduction of
\(a^nf=b\) modulo \(a\) forces \(b\in aB\), a contradiction unless \(n=0\).
Therefore

\[
 \boxed{\ker D_{10}\cap\ker D_7=B=k[K,H,V].}         \tag{64}
\]

This is an exact finitely generated invariant ring for the genuine
\(\mathbb G_a^2\)-action generated by the commuting primitive derivations.
It is not merely a bounded search result.

The same boundary-valuative argument gives the individual kernels relevant
to the saturation experiment.  Since \(D_7(s)=0\), adjoining the boundary
value

\[
 s|_{a=0}={-14cd^2+2y-35dz/c\over10}
\]
preserves injectivity, and hence

\[
 \ker D_7=B[s]\simeq k^{[4]}.
\]
For the Euclidean shear set

\[
 t=a^2s-hp,\qquad
 W={10t-K\over a}
  =-35czp^2-14c^2pd-35azpd-14acd^2+30zp+4cd+5ay.
\]

Then \(D_{22}(W)=0\), \(B_a[t]=B_a[W]\), and on \(a=0\)

\[
 W=-10cd-{5z\over c}.
\]

The Jacobian of the boundary functions
\((K,V,W)\) with respect to \((c,d,z)\) is \(-1280c\).  Thus
\((B/(a))[W]\to R/(a)\) is injective, and the same minimal-pole argument
proves

\[
 \boxed{\ker D_7=B[s]\simeq k^{[4]},\qquad
        \ker D_{22}=B[W]\simeq k^{[4]}.}             \tag{65}
\]

Finally, \(D_{10}\) has the global slice \(s\), so its kernel is finitely
generated as well, although no comparably small polynomial presentation is
claimed here.  The Hilbert--14 outcome for every action and intersection
computed in this section is therefore **finite generation**.  A
non-finitely-generated ring attached to this slice would have to use a new
action, a different intersection, or a larger Keller boundary algebra.
Equation (64) does not compute the full Makar--Limanov invariant, because it
does not classify all locally nilpotent derivations.
