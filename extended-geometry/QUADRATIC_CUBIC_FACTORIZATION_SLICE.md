...
Makar--Limanov and Derksen invariants and flexibility remain the first
potentially separating invariants.

## 11. Two additive actions and a global cylinder
...
Thus the generic action acquires a genuine candidate pole.  Whether a
vertical correction in the `p,s` directions cancels all such poles is exactly
an extension problem over `a=0`; (55) is not a proof that no lift exists.

These formulas also delimit what the standard additive invariants currently
prove.

* The intersection of the two displayed kernels is an upper bound for the
  full Makar--Limanov invariant, and it contains the trinomial ring in (50).
  An undiscovered LND may still move `a,U,K`, or `J`.
* Since `ker(D_(10))` belongs to the Derksen algebra, and
  `h=D_(22)(s)=(7J-12U)/40` on `X`, the element
  `a^2s-ph` lies in `ker(D_(22))`.  Hence the subalgebra generated by these
  two kernels contains `ker(D_(10))[a^2s]` and becomes all of `k[X]_a` after
  inverting `a`.  Any Derksen obstruction is therefore supported on `a=0`.
* At `(a,c,z,p,d,y,x)=(1,0,0,0,1,0,1)` the two vectors have rank two.  At
  `(0,1,0,1,0,0,-1)` they are proportional and have rank one.  This proves a
  rank drop for the **known** actions, not failure of flexibility, whose
  definition uses every `Ga`-action.

A homogeneous coefficient search found no third LND among ambient-preserving
homogeneous derivations whose generator images have total degree at most two.
That is useful negative evidence but not a classification: derivations of the
quotient need not preserve the two defining ambient polynomials separately,
and higher-degree homogeneous components remain possible.  Consequently
neither `ML`, the Derksen invariant, nor nonflexibility currently proves that
`X_(2,3)` is not an abstract product.
...
smooth fourfold `Y` and isolate the divisor `a=0` as the only place where the
visible Derksen algebra and the generic `Ga^2` quotient can differ.  See
...
constructed, and the full ML/Derksen invariants and flexibility remain open
globally.  What is now localized to `a=0` is any Derksen obstruction visible
from the two displayed kernels and the failure of their generic `Ga^2`
product chart to extend.
...
The two commuting LNDs, global slice, trinomial quotient, visible Derksen
localization, and rank drop in Section 11 are checked by
