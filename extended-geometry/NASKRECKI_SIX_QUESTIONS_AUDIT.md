...
## 5. Dual-number deformation: explicit and partly separated
...
is now a proved repository result.  Two questions remain: whether `H` is
tangent to the orbit generated by arbitrary polynomial vector fields, and
whether it becomes trivial after stabilization.  The comparison target in
higher degree is the differential of the decorated-normalization map.  The
existing generic-finiteness theorem controls generic fibers set-theoretically;
it does not by itself prove that this differential is injective or that the
moduli image is generically reduced.

## 6. The `pq=0` boundary: partly resolved

The full unsaturated thirteen-variable Keller ideal is now generated
explicitly by the sixteen coefficients of the invariant determinant.  On
`p=q=0` it contains a reduced affine line and a reduced affine three-space of
triangular automorphisms:

\[
 (A,B,C)=(v,u+\lambda v,2)
\]
...
These are checked directly and show that the omitted divisor has genuine
reduced geometry.  Its primary decomposition has not yet been completed.  A
direct thirteen-variable Buchberger computation exceeded the practical
range of the lightweight symbolic backend, so the next computation should
use Singular or Macaulay2 and proceed in this order:

1. primary decomposition of `I+(p)` and `I+(q)`;
2. degree profiles and Jacobian constants on every component;
3. intersection of each component with the foundational maximal ideal;
4. comparison of its tangent cone with the claimed dual-number direction;
5. exact collision tests only on components that remain Keller and
   generically finite.

This ordering prevents a nilpotent embedded direction from being mistaken
for a new reduced family.
...
nilpotent tangent.  No conclusion about the full polynomial or stable orbit
is inferred from either quotient.
