...
through \(\hbar^7\).  The fifth-order solution scheme initially appears
much larger: its standard basis has dimension 27 in the 41 lower-lift
coordinates.  Exact radicalization makes that number tractable.  Ten
coordinates are absent, eight are eliminated by linear equations, and the
remaining nonlinear core uses only \(u_{26},\ldots,u_{41}\).  Its eleven
quadrics have an irreducible dimension-ten radical generated by

\[
\begin{gathered}
3u_{35}+4u_{36},\\
4194304u_{34}+203337542430a^2+487549466415a+295586113680,\\
3u_{32}+10u_{33},\\
9u_{30}+24u_{31}+(872a+1856)u_{41},\\
9u_{28}+18u_{29}+(392a+752)u_{39}+(848a+1496)u_{40},\\
27u_{26}+36u_{27}+(456a+672)u_{37}+(896a+1184)u_{38}.
\end{gathered}
\tag{QR19n'}
\]

Thus the reduced \(K\)-scheme of genuine fifth-order lifts is one affine
\(\mathbb A^{27}_K\), not a union of hidden components; the extra scheme
structure in the original quadrics is nilpotent.  On the explicit line
...
complete \(K(r)\) calculation writes the fifth-order correction with
degrees \(0,1,2\) in \(r\).  Three rank-six pivot charts have unit residual
gcd, and their pivot determinants have unit common gcd.  Hence every
algebraic value of \(r\), not just \(r=0,1\), is obstructed at order seven.

The full affine component has also been compressed.  Although all 27
coordinates enter the lower lift, the 20-column order-seven correction
matrix depends only on
\(u_{37},u_{38},u_{39},u_{40},u_{41}\).  The six columns numbered
\(9,11,13,15,17,19\), restricted to the pure-\(X\) rows
\(X^8,\ldots,X^{13}\), have a nonzero constant determinant.  Over the full
27-parameter function field every other correction column lies identically
in their span, so the variation rank is exactly six on every fiber.
Eliminating those six variables leaves 401 consistency polynomials
involving only

\[
u_{27},u_{29},u_{31},u_{33},u_{36},
u_{37},u_{38},u_{39},u_{40},u_{41}.
\tag{QR19n''}
\]
...
\((\kappa,\tau)=(0,1),(2,3/2),(-1/2,2)\), the 4065-variable all-pole lower
space induces exactly the same 41 nonzero conditions as the bounded
complex: 14 correction-image rows, three linear variations, and 24
quadratic variations.  Their exact rank is 15 and their common kernel is
the bounded period line.  This proves equality of the bounded and Laurent
constraint modules on these fibers and isolates a 41-by-16 module to lift
over the parameter ring.  It does not yet prove equality on every Fitting
stratum or for the four replacement supports.
