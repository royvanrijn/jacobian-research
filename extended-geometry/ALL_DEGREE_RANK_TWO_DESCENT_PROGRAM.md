...
argument then proves completion in every degree on both adapted charts,
including the formerly conjectural fixed-\(\kappa=-9\) theorem.  These are internal
...
by \(c=-16\nu(H)\).  The degree-five and degree-six formulas above are the
first two explicit coefficient specializations of this uniform identity.
...
every fixed slice.  More strongly, the full faithful seed open gives
\(N-3\) stable dimensions in \(\mathbb A^4\) for every \(N\ge5\).
