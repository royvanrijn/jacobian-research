...
- the geometric primes above it with \((e,f)\);
...
SHA-256 hashes of all three replayed scripts are emitted in the certificate.
...
`unknown`, but the generic cycle profile can now be sharpened.

The affine \(L\)-prime contributes local degree two.  The cover has total
degree four, leaving degree two over the generic target branch.  Moreover,
generic inertia lies in \(A_4\) and contains a 2-cycle from \(L\).  The only
such nontrivial cycle type in the natural \(A_4\)-action is
\((2,2)\).  Hence the finite normalization has a second index-two prime,
necessarily missing from the displayed affine cone source.  The abstract
double-transposition **cycle profile is therefore matched**.

The coloring is not matched: the package asks that both index-two primes lie
on the normalization boundary, as a Keller realization requires, whereas
the cone contains \(L=0\) inside its affine source.  The next normalization
calculation must construct the inferred second valuation explicitly and
compute its residue and conductor data.  The construction problem must then
move the \(L\)-ramification outside the affine source without changing the
\(A_4\) field.
...
inferred_missing_boundary_degree = 2
...
ledger, recognizes the pure-target log-Keller lift, and identifies the
generic double-transposition cycle profile as one affine \(L\)-prime plus
one missing boundary prime.  The next \(A_4\) task is to construct that
missing valuation explicitly and solve the boundary-coloring mismatch.
...
The verifier checks four surviving `unknown` packages and four intrinsic
...
extracts their divisor orders, compares them to the abstract package, and
emits the remaining realization gates.
