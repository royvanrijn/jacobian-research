...
seven remains a useful regression for that frontier.  The sharpest
current attack restores the generic-translation relations between Taylor
channels.  If the ratio of two character twists is flat along every
Taylor edge, it is necessarily a scalar times a torus character; its
moving coefficients differ by an \(N\)-th-power phase and are already
scale-compatible.  A nonflat ratio has a minimal collinear-triple or
unit-square curvature witness, matching the isolated Bessel and beta
atoms already proved terminal.  The missing lemma is to expose that atom
after differentiating the all-order period identity while preserving its
common high-digit quotient.  An exact \(C_2\) sign-twist search through
degree 12 and depth \(2d+4\) finds no unexplained isoperiodic pair after
quotienting scalar, torus, and reversal symmetries; this is bounded
