...
| [JC2_GLOBAL_COX_PACKET_ATTACK.md](JC2_GLOBAL_COX_PACKET_ATTACK.md) | Global quartic Cox and boundary-deletion reduction: all target groupings miss the same primitive ramification/canonical character; the canonical normal Gorenstein source bridge is the Zariski--Main open in the finite threefold normalization; the exact cusp/connector normalization, conductor, dualizing, and transition data are compatible, so the revised rank-one target is a degree-zero global-section/endpoint-pairing obstruction |
