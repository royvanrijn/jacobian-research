# Positive invariance regression for C01

Most boundary checks compare examples expected to differ.  This audit tests
the opposite requirement: hostile presentations of the same C01 map must
...
Write the C01 map as `F=(a,b,c)` and put
...
```bash
.venv/bin/python scripts/audit_c01_invariance_regression.py
```
