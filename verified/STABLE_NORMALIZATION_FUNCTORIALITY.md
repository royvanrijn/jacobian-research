...
1. The reduced boundary is
   ...
   of `partial`.
2. If `E` lies over a target prime divisor `Z` whose generic local ring in
   `Y` is a DVR (in particular, if `Y` is normal in codimension one), the
   stabilized valuation is
   the Gauss extension
   \[
...
5. For every finitely presented module `M` on any affine stratum,
   ...
   \]
6. Scheme-theoretic images of finite stratum maps (more generally, of
   quasi-compact maps between the affine finite-type strata under
   consideration), fiber products, diagonal subschemes, saturated
   off-diagonal closures, and conductor ideals commute with this flat base
   change whenever they are formed from the finite normalization morphism.
   Hence target stratum schemes, node-pairing, and conductor decorations
   simply acquire an `A^s` factor.
...
minimal primes of the same height.  For the target primes covered by (2),
the old uniformizer is still a uniformizer and the residue-field extension is
...
differentials gives (5).  For a quasi-compact map between affine finite-type
strata, its scheme-theoretic image is defined by the kernel of the
corresponding ring map; flatness identifies the base-changed kernel with its
polynomial extension.  Fiber products and diagonals commute with arbitrary
base change;
ideal quotients defining the relevant saturated closures commute here with
the faithfully flat polynomial extension.  Finally the conductor is the
annihilator of the finite quotient `nu_*O_X/O_D`; flat base change extends
that annihilator.  These observations prove the last assertion.
