...
5. For every finitely presented module `M` on any affine stratum,
   ...
   \]
6. Scheme-theoretic images of finite (more generally, quasi-compact) stratum
   maps, fiber products, diagonal subschemes, saturated off-diagonal closures,
   and conductor ideals commute with this flat base change whenever they are
   formed from the finite normalization morphism.  Hence target stratum
   schemes, node-pairing, and conductor decorations simply acquire an `A^s`
   factor.
...
differentials gives (5).  For an affine quasi-compact stratum map, its
scheme-theoretic image is defined by the kernel of the corresponding ring
map; flatness identifies the base-changed kernel with its polynomial
extension.  Fiber products and diagonals commute with arbitrary base change;
