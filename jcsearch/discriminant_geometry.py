...
@dataclass(frozen=True)
class DecoratedNormalization:
    """Exact decorated normalization of a polynomial discriminant pencil.

    ``fitting_polynomial`` defines the full zeroth-Fitting divisor, including
    multiplicities.  ``off_diagonal_ideal`` defines the ordered equal-image
    scheme away from the diagonal, and ``node_pair_ideal`` is its quotient in
    the elementary coordinates (sum, product) of an unordered pair.  The
    conductor in the normalization line is generated by
    ``conductor_polynomial``; ``ordinary`` records when its simpler
    cusp-squared-times-node-branches factorization applies.
    """

    degree: int
    variable: sp.Symbol
    primitive: sp.Expr
    fitting_polynomial: sp.Expr
    fitting_factorization: tuple[tuple[sp.Expr, int], ...]
    ordered_variables: tuple[sp.Symbol, sp.Symbol]
    off_diagonal_ideal: tuple[sp.Expr, ...]
    pair_coordinates: tuple[sp.Symbol, sp.Symbol]
    node_pair_ideal: tuple[sp.Expr, ...]
    node_branch_polynomial: sp.Expr
    node_jacobian: sp.Expr
    node_scheme_transverse: bool
    implicit_variables: tuple[sp.Symbol, sp.Symbol]
    implicit_equation: sp.Expr
    conductor_polynomial: sp.Expr
    conductor_map: "ConductorMap | None"
    boundary_divisor_polynomial: sp.Expr
    boundary_marks: tuple[tuple[str, sp.Expr], ...]
    infinity_mark: str
    ordinary: bool
...
@dataclass(frozen=True)
class ConductorMap:
    """Finite conductor subscheme upstairs and downstairs.

    The downstairs ring is presented by ``k[s,t]/downstairs_ideal``.  Its
    finite map to ``k[r]/(upstairs_polynomial)`` sends ``s,t`` to
    ``map_images``.  This packages branch identifications rather than merely
    returning the support of the conductor upstairs.
    """

    normalization_variable: sp.Symbol
    upstairs_polynomial: sp.Expr
    downstairs_variables: tuple[sp.Symbol, sp.Symbol]
    downstairs_ideal: tuple[sp.Expr, ...]
    map_images: tuple[sp.Expr, sp.Expr]
...
def _saturate_polynomial_ideal(equations, factor, variables, prefix="_sat"):
    """Saturate an exact polynomial ideal by ``factor`` using one gate.

    The elimination identity
    ``I : factor^infinity = (I, 1-gate*factor) intersect k[variables]``
    avoids heuristic factor deletion and retains all scheme multiplicities.
    """
    variables = tuple(variables)
    gate = sp.Dummy(f"{prefix}_gate")
    basis = sp.groebner(
        tuple(sp.expand(equation) for equation in equations)
        + (1 - gate * sp.expand(factor),),
        gate,
        *variables,
        order="lex",
    )
...
def _monic_univariate(expression, variable):
    """Return the monic associate of a nonzero univariate expression."""
    polynomial = sp.Poly(sp.expand(expression), variable)
    return sp.factor(polynomial.monic().as_expr())


def _ideal_is_unit(equations, variables):
    """Return whether exact equations generate the unit ideal."""
    basis = sp.groebner(
        tuple(sp.expand(equation) for equation in equations),
        *tuple(variables),
        order="lex",
...
def quadratic_zero_cluster_boundary_chart(H, W, scale=1, parameter=None):
    """Return the special quadratic chart on the discriminant boundary.

    For a seed with an exact double zero at ``W=0``, put ``W=C*k`` in

    ``B=H'(W)/C`` and ``A=(W*H'(W)-H(W))/(scale*C**2)``.

    The returned pair is the specialization ``(A,B)`` at ``C=0``.  Besides
    extracting the boundary conic exactly, this calculation certifies that
    its center on the discriminant normalization line is the canonical mark
    ``W=0``.
    """
    polynomial = sp.Poly(sp.expand(H), W)
    if polynomial.coeff_monomial(1) != 0 or polynomial.coeff_monomial(W) != 0:
        raise ValueError("the seed must vanish to order at least two at zero")
    quadratic_coefficient = polynomial.coeff_monomial(W**2)
    if quadratic_coefficient == 0:
        raise ValueError("the seed must have an exact double zero at zero")
    if sp.sympify(scale) == 0:
        raise ValueError("the target scale must be nonzero")
    C = sp.Dummy("zero_cluster_C")
    k = parameter if parameter is not None else sp.Dummy("zero_cluster_k")
    slope = sp.diff(H, W)
    intercept = sp.expand(W * slope - H)
    boundary_B = sp.cancel(slope.subs(W, C * k) / C).subs(C, 0)
    boundary_A = sp.cancel(
        intercept.subs(W, C * k) / (sp.sympify(scale) * C**2)
    ).subs(C, 0)
    return sp.factor(boundary_A), sp.factor(boundary_B)


def intrinsic_second_boundary_divisor(H, W):
    """Return the normalization divisor centered over the second boundary.

    A finite limit of the normalized discriminant chart at ``C=0`` forces
    ``H=H'=0``.  Hence the full center scheme on the normalization line is
    the monic gcd ``gcd(H,H')``; unlike a hard-coded point, this retains
    multiple centers and their scheme multiplicities.
    """
    polynomial = sp.Poly(sp.expand(H), W)
    derivative = sp.Poly(sp.diff(polynomial.as_expr(), W), W)
    divisor = sp.gcd(polynomial, derivative)
    return sp.factor(divisor.monic().as_expr())


def _inferred_boundary_marks(H, W, boundary_divisor):
    """Infer only boundary points certified by the weighted normalization."""
    if boundary_divisor == 1:
        return ()
    support = sp.Poly(sp.sqf_part(boundary_divisor, W), W)
    if support.degree() != 1:
        return ()
    root = sp.solve(support.as_expr(), W)
    if len(root) != 1:
        return ()
    root = sp.factor(root[0])
    # The label ``zero_cluster`` belongs to the normalized weighted seed
    # schema, not to an arbitrary polynomial parametrization.
    weighted_zero = (
        root == 0
        and sp.expand(H).subs(W, 0) == 0
        and sp.diff(H, W).subs(W, 0) == 0
        and sp.expand(H).subs(W, 1) == 0
        and sp.diff(H, W).subs(W, 1) != 0
    )
    label = "zero_cluster" if weighted_zero else "second_boundary_center"
    return ((label, root),)


def implicit_parametric_curve_equation(x, y, parameter, prefix="curve"):
    """Eliminate a birational polynomial parameter from its plane image."""
    s, t = sp.symbols(f"{prefix}_s {prefix}_t")
    equation = sp.resultant(s - x, t - y, parameter)
    polynomial = sp.Poly(sp.expand(equation), s, t)
    _, primitive = polynomial.primitive()
    return (s, t), sp.factor(primitive.as_expr())


def plane_parametric_conductor_polynomial(x, y, parameter, prefix="conductor"):
    """Compute a normalized plane curve's conductor exactly.

    For a birational plane parametrization ``x=x(r), y=y(r)`` with implicit
    equation ``f(x,y)=0``, the adjunction/conductor formula is

    ``c_B = (f_y(x(r),y(r))/x'(r))``.

    The quotient is a polynomial when ``k[r]`` is the normalization.  This
    formula remains valid for nonordinary plane-curve singularities and thus
    avoids imposing a cusp/node classification on the conductor extractor.
    """
    implicit_variables, implicit = implicit_parametric_curve_equation(
        x, y, parameter, prefix=prefix
    )
    s, t = implicit_variables
    numerator = sp.diff(implicit, t).subs({s: x, t: y})
    quotient = sp.cancel(numerator / sp.diff(x, parameter))
    if sp.denom(quotient) != 1:
        raise ValueError(
            "the parametrization did not produce a polynomial conductor; "
            "verify that it is the normalization"
        )
    conductor = _monic_univariate(sp.numer(quotient), parameter)
    return implicit_variables, implicit, conductor


def implicit_discriminant_equation(H, W, prefix="discriminant"):
    """Eliminate the normalization coordinate from its discriminant image."""
    slope, intercept = discriminant_param(H, W)
    return implicit_parametric_curve_equation(
        slope, intercept, W, prefix=prefix
    )


def plane_curve_conductor_polynomial(H, W, prefix="conductor"):
    """Compute the discriminant normalization conductor exactly."""
    slope, intercept = discriminant_param(H, W)
    return plane_parametric_conductor_polynomial(
        slope, intercept, W, prefix=prefix
    )


def conductor_subscheme_map(H, W, conductor_polynomial, prefix="conductor"):
    """Present the finite conductor map for a known conductor generator.

    For ``A=k[H',WH'-H] subset B=k[W]`` and conductor extension ``(c)`` in
    ``B``, the downstairs conductor quotient is the image of ``A`` in
    ``B/(c)``.  Its kernel is computed exactly by elimination.
    """
    conductor_polynomial = _monic_univariate(conductor_polynomial, W)
    s, t = sp.symbols(f"{prefix}_s {prefix}_t")
    slope = sp.expand(sp.diff(H, W))
    intercept = sp.expand(W * slope - H)
    basis = sp.groebner(
        (conductor_polynomial, s - slope, t - intercept),
        W,
        s,
        t,
        order="lex",
    )
    downstairs_ideal = tuple(
        sp.factor(polynomial.as_expr())
        for polynomial in basis.polys
        if W not in polynomial.as_expr().free_symbols
    )
    return ConductorMap(
        normalization_variable=W,
        upstairs_polynomial=conductor_polynomial,
        downstairs_variables=(s, t),
        downstairs_ideal=downstairs_ideal,
        map_images=(slope, intercept),
    )


def decorated_normalization(H, W, boundary_marks=None, prefix="decorated"):
    """Extract the scheme-theoretic decorated normalization of ``H-sW+t``.

    The routine is exact over the coefficient domain understood by SymPy.  It
    computes the Fitting divisor ``(H'')`` without squarefree reduction.  The
    ordered double scheme is obtained by diagonal division followed by
    saturation by the diagonal; merely dividing is insufficient because it
    leaves isolated cusp points on the diagonal.

    The conductor generator is computed from the implicit plane equation by
    the adjunction formula, including outside the ordinary cusp/node locus.
    The ``ordinary`` flag separately records whether the simpler
    ``cusp_support**2 * node_projection`` factorization applies.
    """
    H = sp.expand(H)
    degree = int(sp.Poly(H, W).degree())
    if degree < 3:
        raise ValueError("the primitive must have degree at least three")
    r, u = sp.symbols(f"{prefix}_r {prefix}_u")
    pair_sum, pair_product = sp.symbols(
        f"{prefix}_pair_sum {prefix}_pair_product"
    )

    fitting = _monic_univariate(sp.diff(H, W, 2), W)
    factorization = tuple(
        (sp.factor(factor), int(exponent))
        for factor, exponent in sp.factor_list(fitting, W)[1]
    )

    divided = bitangent_equations(H, W, r, u)
    off_diagonal = _saturate_polynomial_ideal(
        divided, r - u, (r, u), prefix=f"{prefix}_ordered"
    )
    symmetric = symmetric_bitangent_equations(
        H, W, pair_sum, pair_product
    )
    node_pair_ideal = _saturate_polynomial_ideal(
        symmetric,
        pair_sum**2 - 4 * pair_product,
        (pair_sum, pair_product),
        prefix=f"{prefix}_unordered",
    )
    node_jacobian = sp.factor(
        sp.det(
            sp.Matrix(
                [
                    [sp.diff(divided[0], r), sp.diff(divided[0], u)],
                    [sp.diff(divided[1], r), sp.diff(divided[1], u)],
                ]
            )
        )
    )
    node_scheme_transverse = bool(
        off_diagonal
        and _ideal_is_unit(
            off_diagonal + (node_jacobian,),
            (r, u),
        )
    )

    if off_diagonal:
        projection_basis = sp.groebner(
            off_diagonal, u, r, order="lex"
        )
        projection_generators = [
            polynomial.as_expr()
            for polynomial in projection_basis.polys
            if polynomial.as_expr().free_symbols <= {r}
        ]
        if not projection_generators:
            node_projection = sp.Integer(0)
        else:
            node_projection = _monic_univariate(
                sp.gcd_list(projection_generators), r
            )
    else:
        node_projection = sp.Integer(1)

    cusp_support = _monic_univariate(sp.sqf_part(fitting, W), W)
    expected_branch_degree = (degree - 2) * (degree - 3)
    projection_in_W = sp.expand(node_projection.subs(r, W))
    fitting_is_squarefree = sp.gcd(fitting, sp.diff(fitting, W)) == 1
    projection_is_expected = (
        projection_in_W != 0
        and sp.degree(projection_in_W, W) == expected_branch_degree
        and sp.gcd(projection_in_W, sp.diff(projection_in_W, W)) == 1
    )
    disjoint = sp.gcd(cusp_support, projection_in_W) == 1
    ordinary = bool(
        fitting_is_squarefree
        and projection_is_expected
        and disjoint
        and node_scheme_transverse
    )
    implicit_variables, implicit_equation, conductor = (
        plane_curve_conductor_polynomial(
            H, W, prefix=f"{prefix}_implicit"
        )
    )
    if ordinary:
        ordinary_conductor = _monic_univariate(
            cusp_support**2 * projection_in_W, W
        )
        if sp.factor(conductor - ordinary_conductor) != 0:
            raise AssertionError(
                "the adjunction conductor disagrees with the ordinary local formula"
            )
    conductor_map = conductor_subscheme_map(
        H, W, conductor, prefix=f"{prefix}_conductor"
    )

    boundary_divisor = intrinsic_second_boundary_divisor(H, W)
    if boundary_marks is None:
        marks = _inferred_boundary_marks(H, W, boundary_divisor)
    else:
        marks = tuple(
            (str(label), sp.factor(value))
            for label, value in boundary_marks.items()
        )
    return DecoratedNormalization(
        degree=degree,
        variable=W,
        primitive=H,
        fitting_polynomial=fitting,
        fitting_factorization=factorization,
        ordered_variables=(r, u),
        off_diagonal_ideal=off_diagonal,
        pair_coordinates=(pair_sum, pair_product),
        node_pair_ideal=node_pair_ideal,
        node_branch_polynomial=projection_in_W,
        node_jacobian=node_jacobian,
        node_scheme_transverse=node_scheme_transverse,
        implicit_variables=implicit_variables,
        implicit_equation=implicit_equation,
        conductor_polynomial=conductor,
        conductor_map=conductor_map,
        boundary_divisor_polynomial=boundary_divisor,
...
def pointed_quadratic_divisor_invariant(polynomial, variable, mark):
    """Affine invariant of an unordered quadratic divisor and one mark.

    For roots ``m +/- d`` it returns ``(mark-m)^2/d^2``.  It is unchanged by
    every affine reparameterization and works over the coefficient field,
    without adjoining the two roots.
    """
    poly = sp.Poly(sp.expand(polynomial), variable)
    if poly.degree() != 2:
        raise ValueError("a quadratic divisor is required")
    a, b, _ = poly.all_coeffs()
    discriminant = sp.discriminant(poly.as_expr(), variable)
    if discriminant == 0:
        raise ValueError("the quadratic divisor must be reduced")
    midpoint = -b / (2 * a)
    radius_squared = discriminant / (4 * a**2)
    return sp.factor((sp.sympify(mark) - midpoint) ** 2 / radius_squared)


def paired_quadratic_divisor_invariants(first, second, variable):
    """Affine invariants of two unordered reduced quadratic divisors.

    If the pairs have midpoints ``m_i`` and squared radii ``d_i^2``, return
    ``((m_2-m_1)^2/d_1^2, d_2^2/d_1^2)``.  This is independent of ordering
    inside either pair and of every affine change of ``variable``.
    """
    data = []
    for expression in (first, second):
        polynomial = sp.Poly(sp.expand(expression), variable)
        if polynomial.degree() != 2:
            raise ValueError("two quadratic divisors are required")
        a, b, _ = polynomial.all_coeffs()
        discriminant = sp.discriminant(polynomial.as_expr(), variable)
        if discriminant == 0:
            raise ValueError("both quadratic divisors must be reduced")
        data.append((-b / (2 * a), discriminant / (4 * a**2)))
    (first_midpoint, first_radius), (second_midpoint, second_radius) = data
    return (
        sp.factor((second_midpoint - first_midpoint) ** 2 / first_radius),
        sp.factor(second_radius / first_radius),
    )
