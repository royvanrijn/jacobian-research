#!/usr/bin/env python3
"""Exact finite-reduction screen for independence on the rational component.

For each bounded rational pair ``(r,T)``, construct the split six-root
specialization of the exact rational two-section component, map the twelve
visible quartic points and the two labelled affine points to the Jacobian,
and compare their combined exact mod-3 finite-reduction ranks.  A strict rank
increase on adjoining the affine points is an exact specialization witness
against a generic relation with the visible subgroup.  No numerical heights,
expanded residuals, or point search is used.

Failure to find an increase in the declared grid is not an upper bound.
"""
...
def specialization_record(
    root_parameter: Fraction,
    mestre_parameter: Fraction,
    *,
    prime_bound: int,
) -> dict[str, object] | None:
    """Return one exact quotient comparison, or None at a bad specialization."""

    roots = split_roots(root_parameter)
    if len(set(roots)) != 6:
        return None
    construction = SixRootMestreConstruction(roots)
    z = 12 * (root_parameter + 3) / (1 - root_parameter**2)
    coordinates = component_coordinates(z)
    quartic = construction.primitive_quartic_coefficients(mestre_parameter)
    visible = tuple(
        quartic_point_to_jacobian(construction, mestre_parameter, point)
        for point in primitive_visible_points(construction, mestre_parameter)
    )
    affine = []
    for intercept, slope in ((coordinates[4], coordinates[5]), (coordinates[6], coordinates[7])):
        x_value = intercept + slope * mestre_parameter
        y_value = rational_square_root(quartic_value(quartic, x_value))
        if y_value in (None, 0):
            return None
        affine.append(quartic_point_to_jacobian(construction, mestre_parameter, (x_value, y_value)))
    coefficients = construction.primitive_jacobian_coefficients(mestre_parameter)
    ...
    return {
        "r": rational_text(root_parameter),
        "T": rational_text(mestre_parameter),
...
def screen(
    *,
    ...
    roots = tuple(
        value
        for value in bounded_rationals(root_height)
        if value not in (-1, 0, 1, Q(-1, 3))
    )
    parameters = bounded_rationals(parameter_height, nonzero=True)
    pairs = tuple((root, parameter) for root in roots for parameter in parameters)
    selected = pairs[start:] if count is None else pairs[start : start + count]
    records = []
    for root, parameter in selected:
        try:
            record = specialization_record(root, parameter, prime_bound=prime_bound)
    ...
    best = max(
        records,
        default=None,
        key=lambda record: (record["augmented_rank"], record["visible_rank"], record["r"], record["T"]),
    )
    result = {
        "status": "bounded exact component independence screen completed",
...
def merge_batches(paths: Iterable[Path]) -> dict[str, object]:
    """Merge a complete set of disjoint bounded screen summaries."""
    ...
    reference = batches[0]
    keys = (
        "root_height", "mestre_parameter_height", "prime_bound",
        "candidate_pair_count", "scope",
    )
    ...
    for batch in batches:
        offset = int(batch["pair_offset"])
        batch_count = int(batch["pair_batch_count"])
    ...
    gains = [
        record
        for batch in batches
        for record in batch["strict_quotient_rank_gains"]
    ]
    ...
    return {
        ...
        "admissible_specialization_count": sum(
            int(batch["admissible_specialization_count"]) for batch in batches
        ),
        "strict_quotient_rank_gain_count": len(gains),
        "strict_quotient_rank_gains": gains,
        "best_observed": best,
...
def main() -> None:
    ...
    parser.add_argument("--root-height", type=int, default=8)
    parser.add_argument("--parameter-height", type=int, default=8)
    parser.add_argument("--prime-bound", type=int, default=151)
    ...
    result = (
        ...
        if args.merge_batches is not None
            ...
            parameter_height=args.parameter_height,
