#!/usr/bin/env python3
"""One fixed301-chart adaptive attempt on an independently certified retention rank25 curve."""
import argparse,sys
...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';LOCAL=ROOT/'artifacts/local/elliptic-curves';PROOF=ART/'scaled13_24_rank25_minimal_proof_v1.json'

def configure(identifier):
    global ID,D,SEED,CENSUS
    if identifier!='11952-300':raise ValueError('only the unique fixed scaled13_24>=25 result is in scope')
    ID=identifier;D=LOCAL/('scaled13-rank25-'+ID+'-adaptive-v1');SEED=LOCAL/'scaled13_24-r17-pari-v1'/ID/'result.json';CENSUS=LOCAL/'compact-r17-fresh-generic-census-v1'/ID.split('-')[0]/'generic-census.json'

def sources():
    names=['memory_rank_certificate.py','followup_scaled13_rank25.py','prepare_scaled13_rank25_adaptive.sage','prepare_fresh_r17_pari_batch.sage','fresh_r17_pari_batch.py','prospective_half_lattice_v2.sage','research_runtime/rotated_observation_state.py','research_runtime/cached_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
    return {**backend.sources(),**{str((CAS/n).relative_to(ROOT)):cert.hashed(CAS/n) for n in names}}

def prepare():
    if (D/'protocol.json').exists():raise FileExistsError('preserve adaptive rank25 protocol')
    seed,census=cert.read(SEED),cert.read(CENSUS);proof=next(r for r in cert.read(PROOF)['curves'] if r['id']==ID)
    if seed['status']!='COMPLETE_DECLARED_POINT_ATTEMPT' or seed['rank_lower_bound']!=25 or proof['icarm_matches'] or proof['previous_matches'] or seed['curve']!=proof['discovery_curve']:raise ArithmeticError('certified new25 gate failed')
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.scaled13-rank25-adaptive.v1','sources':sources(),'seed_path':str(SEED.relative_to(ROOT)),'seed_sha256':cert.hashed(SEED),'proof_sha256':cert.hashed(PROOF),'generic_census_path':str(CENSUS.relative_to(ROOT)),'generic_census_sha256':cert.hashed(CENSUS),'generic_pool':[{'mask':r['mask'],'computed_generic_norm':r['norm']} for r in pool],'initial_rank':25,'generic_rank':17,'charts':301,'height':125000,'seconds_per_chart':10,'target_rank':28,'gp_sha256':cert.hashed(Path('/usr/bin/gp')),'geometry_wall_seconds':300,'worker_wall_seconds':3600,'rss_bytes':1610612536,'maximum_workers':1,'prior_mw16_gate_sha256':cert.hashed(ART/'new_mw16_rank27_portable_replay_v1.json'),'gate':'The complete24-address13-scaling-conditioned batch and all1080 boxes produced this unique new rank25 curve at102/1525 with independently replayed point-cloud rank, search histories, global minimality and no catalogue/prior equation match. Its generic17 search has discovered8 further certified directions. This separately frozen follow-up tests301 parity centres involving those directions at125000, at the unchanged initial height, aiming for further exactly independent points. The prior MW16 follow-up recovered a new direction without increasing height, providing a finite centre-coverage gate; this does not guarantee recovery on R17.','centre_policy':'These301 pairings are not all2^25 classes or the full generic/quotient product. Take301 largest computed generic census norms, mask ties. Pair them cyclically with nonzero8-bit quotient words ordered by Hamming weight then integer. Compute384-bit numerical heights on the exact25-point subgroup and numerical CVP representatives in each declared parity; order descending numerical norm, generic mask, quotient word. These representatives are search choices, not specialized lattice-minimum certificates.','boundaries':'Only points discovered on this prospective curve enter centres. Freeze all rational maps before point searching. Preserve all histories and timeout outputs; no automatic continuation or rank upper bound. Stop at the first worker lower bound28 pending exact replay.'})
...
def initial(cache):
    ...
    if state.rank!=25:raise ArithmeticError('adaptive initial subgroup differs')
    return seed,state
...
def replay():
    ...
    words=sorted(range(1,256),key=lambda w:(w.bit_count(),w));expected={(r['mask'],words[i%len(words)]) for i,r in enumerate(p['generic_pool'])}
    ...
    for c in maps['centres']:
        if c['parity']!=c['generic_mask']|(c['quotient_word']<<17) or len(c['representative'])!=25 or any((c['representative'][j]-(c['parity']>>j))%2 for j in range(25)):raise ArithmeticError('adaptive parity differs')
    for i,row in enumerate(data['charts']):
        ...
        rep=m['centre']['representative']+[0]*(state.rank-25);search=PointedQuarticSearch(state=state,centre={'coefficients':rep},coordinate_policy=m['coordinate_policy']);r=row['search']
    ...
    checked_rank(model,state.basis,state.reductions.primes,state.no_two_torsion_prime);print('REPLAYED ADAPTIVE NEW25',len(data['charts']),'rank >=',state.rank,flush=True)

def launch():
    p=protocol()
    for name,cmd,seconds in [('maps',['/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python',str(CAS/'prepare_scaled13_rank25_adaptive.sage'),'--id',ID],p['geometry_wall_seconds']),('worker',[sys.executable,str(Path(__file__).resolve()),'worker','--id',ID],p['worker_wall_seconds'])]:
        path=D/(name+'.supervisor.json')
        if path.exists():raise FileExistsError('preserve adaptive supervisor')
        s=supervise(cmd,limits=Limits(seconds,p['rss_bytes']),log_path=D/(name+'.log'),checkpoint_path=path,cwd=ROOT);print('NEW25 ADAPTIVE STAGE',name,s['outcome'],s['returncode'],flush=True)
