#!/usr/bin/env python3
"""Bounded trace-engine calibration on five prospective retained MW16 addresses."""
import argparse
...
ROOT=Path(__file__).resolve().parents[2];LOCAL=ROOT/'artifacts/local/elliptic-curves';D=LOCAL/'mw16-extended-prime-benchmark-v1';PARENT=LOCAL/'prospective-mw16-h4096-v1';GP=Path('/usr/bin/gp')
...
def prepare():
    ...
    for f in atlas['families']:
        path=PARENT/f['fibration_id']/'population.json';t=cert.read(path)['retained_candidates'][4]['parameter'];model,pts=spec.specialize(f,t)
        if any(q.denominator!=1 for q in model):raise ArithmeticError('integral family model required')
        rows.append({'family':f['fibration_id'],'parameter':t,'model':list(map(str,model)),'population_sha256':cert.hashed(path),'retained_index':4})
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.mw16-extended-prime-benchmark.v1','sources':sources(),'gp_sha256':cert.hashed(GP),'gp_version':'2.15.4','rows':rows,'primes':PRIMES,'seconds_per_curve':20,'rss_bytes':536870912,'outer_seconds':150,'gate':'The current562-prime selector has incomplete retrospective recovery. Before another selector campaign, measure cost and exact sample agreement of prime extension on five predetermined retained addresses, index4 in each family. No record labels, exceptional points or catalogue equations enter this benchmark.','direct_validation_indices':[0,1,2,len(PRIMES)//4,len(PRIMES)//2,3*len(PRIMES)//4,len(PRIMES)-2,len(PRIMES)-1],'claim_boundary':'Trace-engine benchmark only; no selected candidate, point search or rank prediction. Singular displayed reductions are omitted, not mistaken for good trace data.'})
