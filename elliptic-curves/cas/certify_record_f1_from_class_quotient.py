...
def quotient_dimension_upper(presentation_dimension: int, span_rank: int) -> int:
    """Bound a quotient of a group presented by a certified cover."""
    if not 0 <= span_rank <= presentation_dimension:
        raise ArithmeticError("quotient dimensions are inconsistent")
    return presentation_dimension - span_rank
...
def main() -> None:
    ...
    # The certified PARI presentation C' surjects onto the true class group C.
    # If B' is generated by the replayed bad-prime classes, the induced map
    # C'/B' -> C/B is still surjective.  Thus dim(C'/2)-dim(B') is an
    # unconditional upper bound for the true S-class 2-rank; equality of the
    # full class-group dimensions is needed only to call the bad span itself
    # exact, not to subtract it in this quotient upper bound.
    s_class_two_rank_upper = quotient_dimension_upper(
        computed_class_two_rank, computed_bad_class_rank
    )
    ...
    result = {
        ...
        "bad_prime_class_span": {
            ...
            "certified_s_class_two_rank_upper_bound": s_class_two_rank_upper,
        ...
        "certification": {
            ...
            "s_class_upper_bound_by_quotient_surjectivity": True,
        ...
        "claim_boundary": (
            ...
            "bound. Quotienting the computed presentation by its replayed bad-prime "
            "classes gives an unconditional upper bound for the true S-class quotient, "
            "because quotienting preserves the certified surjection. The bad-prime "
            "span itself is called exact only when the full class dimensions match. "
            "The 2-Selmer group is contained in the resulting norm-square envelope; "
            "Selmer parity may sharpen an envelope of dimension 30 to MW29, with no "
            "local-image calculation or higher descent."
        ),
    }
