...
import benchmark_11952_annulus_cache_v3 as repair
...
D=ROOT/'artifacts/local/elliptic-curves/11952-new-annulus-scores-v2'
OUT=ART/'new_annulus_11952_selection_v2.json'
...
def sources():
    paths=[Path(__file__).resolve(),Path(scan.__file__),Path(cache.__file__),cache.OUT,cache.TABLE,repair.BINARY,repair.OUT,Path(repair.__file__),Path(engine.__file__),Path(scoring.__file__),Path(models.__file__),models.spec.ATLAS,OLD,CAT,scan.D/'protocol.json',scan.D/'result.json',scan.D/'replay.json',CAS/'research_runtime/store.py',CAS/'research_runtime/supervisor.py']
...
def prepare():
    ...
    upstream=cert.read(scan.D/'controller/ledger.json');raw=cert.read(scan.D/'result.json');replay=cert.read(scan.D/'replay.json');proof=cert.read(cache.OUT);fixed=cert.read(repair.OUT)
    if upstream['status']!='PASS' or replay['status']!='PASS' or replay['result_sha256']!=cert.hashed(scan.D/'result.json') or proof['status']!='PASS' or cert.hashed(cache.TABLE)!=proof['cache_sha256'] or fixed['status']!='PASS' or cert.hashed(repair.BINARY)!=fixed['binary_sha256']:raise ArithmeticError('annular and exact cached-score gates required')
    ...
    checkpoint(out,{'schema':'elliptic-curves.11952-new-annulus-bank.v2','sources':sources(),'rows':rows,'cached_seconds':120,'cached_primes':2948,'rss_bytes':2147483648,'gate':'All eight disjoint131072<H<=524288 slices and32768 canonical short scores replay. The existing exact11952 projective extension cache is parameter-independent and previously agrees with scalar traces and complete encoded-table replay. Retain every short survivor for this cached extension, then freeze one bounded scalar cohort.','scope':'The v1 reader rejected the immutable bank before scoring because it capped input height at131072. Preserve that failure and use the separately proven explicit524288 reader on the same32768 addresses. One cached4099..32749 score call on all32768 addresses, then4096 distinct equations selected by combined S1 through32749, with977 prior and593 catalogue equations used only as exclusions. No public rank, point or validation score enters ordering. A separate immutable scalar protocol binds its complete4096 roster before new scalar calls.'})
...
def cached(check=False):
    ...
    cmd=[str(repair.BINARY),str(cache.TABLE),str(input_path),'524288']
    if not check:
...
def select():
    ...
    checkpoint(OUT,{'schema':'elliptic-curves.11952-new-annulus-finalists.v2','status':'PASS_FROZEN64_SELECTION',
