#!/usr/bin/env python3
"""Read-only relocated replay of the strict scanner certificate; no build or scan."""
import argparse
from pathlib import Path
import benchmark_periodic_nagao_scanner as b
import verify_periodic_nagao_scanner as original
...
def recorded(path):
    # Relocate only declared workspace paths; never read the recorded checkout.
    old_root=Path(cert.read(b.D/'build.json')['command'][-1]).parents[4]
    return str(old_root/path.relative_to(b.ROOT))
...
def main(check):
    ...
    if build['binary_sha256']!=cert.hashed(binary) or build['supervision']['outcome']!='completed' or build['supervision']['returncode']!=0 or build['command']!=['g++','-O3','-std=c++17',recorded(b.CPP),'-o',recorded(binary)]:raise ArithmeticError('prototype build binding differs')
    bindings={}
    for case in p['cases']:
        ...
        for v,path in [('v1',b.old.BINARY),('v2',binary)]:
            rawpath=b.D/case['name']/(v+'.json');raw=cert.read(rawpath);expected=[recorded(path),recorded(b.ROOT/p['table']['path']),*[str(case[k]) for k in ('numerator_bound','denominator_bound','keep','shard','shards')]];rows,summary=b.old.parse(raw['stdout'],-1)
    ...
    for family in parent['families']:
        for sign in (-1,1):
            ...
            for version,path in [('v1',b.old.BINARY),('v2',binary)]:
                folder=D/family/str(sign);out=folder/(version+'.json');command=[recorded(path),recorded(input_path),'31','29','1000','0','1']
                if not check:
