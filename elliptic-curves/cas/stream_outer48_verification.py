#!/usr/bin/env python3
"""Exact history and point proofs as the fixed48 cohort finishes."""
...
def prepare():
    ...
    if out.exists():raise FileExistsError('preserve fixed48 streaming verification')
    paths=[Path(__file__).resolve(),CAS/'verify_outer48_r17_pari_batch.py',CAS/'replay_outer48_geometry.py',CAS/'audit_recorded_point_mod2_rank_v3.py',CAS/'audit_retained_cloud_modl.py',batch.D/'protocol.json',batch.D/'maps-ledger.json']
    checkpoint(out,{'schema':'elliptic-curves.outer48-stream-verification.v1','sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},'rows':p['rows'],'maximum_workers':2,'seconds_per_curve':1500,'overall_wall_seconds':60000,'rss_bytes_per_worker':2147483648,'gate':'All48 exact map files precede the fixed point batch. Verify each completed worker independently as its immutable result becomes available. This changes verification timing only, not selection, points, charts, stopping conditions or search budgets.','early_high_rank_check':'For any returned lower bound at least28, also replay rational map/point provenance and build/check a separate complete-cloud modulo3/5 certificate, with300 seconds per stage. Catalogue novelty and model normalization remain separate before inventory promotion.','scope':'At most48 exact history/mod2 cloud proofs and one early odd-prime check per provisional>=28 curve. Failed or censored source workers and proof failures remain explicit; no point search or automatic retry occurs here.'})
...
def launch():
    ...
    def one(index):
        ...
        s=run([sys.executable,str(CAS/'verify_outer48_r17_pari_batch.py'),'--index',str(index)],limits=Limits(p['seconds_per_curve'],p['rss_bytes_per_worker']),log_path=folder/'verification.log',checkpoint_path=folder/'verification.supervisor.json',cwd=ROOT)
        ...
        if cloud['rank_lower_bound']>=28:
            import replay_outer48_geometry as geometry
            ...
            odd=batch.ART/('outer48_r17_'+row['id'].replace('-','_')+'_early_modl_v1.json')
    ...
    with ThreadPoolExecutor(max_workers=p['maximum_workers']) as pool:
        ...
        for f in as_completed(pending):
            i=pending[f];data['rows'][i]=f.result();checkpoint(out,data);print('OUTER48 EXACT PROOF',p['rows'][i]['id'],data['rows'][i]['status'],data['rows'][i].get('rank_lower_bound'),flush=True)
    ...
    if data['status']=='PASS':
        ...
        if final.exists():raise FileExistsError('preserve final48 verification ledger')
