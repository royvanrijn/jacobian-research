...
def verify():
    ...
    coverage=ART/'new_rank26_tail_coverage_v1.json';cloud=ART/'new_rank26_all_retained_mod2_v1.json'
    checkpoint(folder/'protocol.json',{'tail_input_sha256':cert.hashed(path),
        'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in (Path(__file__).resolve(),CAS/'replay_new_rank26_tails.py',CAS/'audit_recorded_point_mod2_rank_v2.py')},
        'chart_replay_wall_seconds':900,'cloud_build_wall_seconds':300,'cloud_check_wall_seconds':180,'rss_bytes':1610612736,
        'scope':'Replay all retained tail charts and contiguous coverage. Separately combine raw points from the initial43, adaptive301 and retained tail charts for one exact finite-quotient audit. The combined point-only input is not a synthetic state-history replay.'})
    ...
    def stage(name,args,seconds):
        ...
        return ok
    stage('charts',[str(CAS/'replay_new_rank26_tails.py'),'--input',str(path),'--output',str(coverage)],900)
    protocol=cert.read(D/'protocol.json');parent_path=ROOT/protocol['input_path'];parent=cert.read(parent_path)
    pp=cert.read(parent_path.parents[1]/'protocol.json');initial_path=ROOT/pp['input_path'];initial=cert.read(initial_path)
    if cert.hashed(parent_path)!=protocol['input_sha256'] or cert.hashed(initial_path)!=pp['input_sha256']:raise ArithmeticError('point-cloud ancestry changed')
    inputs=[(initial_path,initial),(parent_path,parent),(path,data)]
