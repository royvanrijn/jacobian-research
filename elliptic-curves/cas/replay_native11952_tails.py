#!/usr/bin/env python3
"""Exact replay of fixed native11952 tails and their union with the original intervals."""
import argparse
...
from search_native11952_tails import GEOMETRY_KEYS
...
def replay(path,output):
    ...
    parent_path=root/protocol['input_path']
    parent=cert.read(parent_path)
    control_protocol=cert.read(parent_path.parents[1]/'protocol.json')
    boundary_path=parent_path.parents[1]/'redacted-input.json'
    boundary=cert.read(boundary_path)
    if cert.hashed(boundary_path)!=control_protocol['input_sha256']:raise ArithmeticError('generic control input changed')
    model=tuple(map(cert.F,data['curve']));points=tuple(tuple(map(cert.F,p)) for p in data['generic_points'])
    original=tuple(map(cert.F,boundary['boundary_short_model']));u=cert.F(boundary['boundary_to_curve_scale_u'])
    if not u or model!=(cert.F(0),cert.F(0),cert.F(0),original[3]/u**4,original[4]/u**6):raise ArithmeticError('control model transport failed')
    if points!=tuple((cert.F(x)/u**2,cert.F(y)/u**3) for x,y in boundary['boundary_generic_points']):raise ArithmeticError('generic control transport failed')
    ...
    checkpoint(output,{'schema':'elliptic-curves.new-native11952-combined-box-coverage.v1','status':'PASS',
        'input_path':str(path.relative_to(root)),'input_sha256':cert.hashed(path),'parent_input_sha256':cert.hashed(source_path),
        'sources':{str(p.relative_to(root)):cert.hashed(p) for p in (Path(__file__).resolve(),root/'elliptic-curves/cas/search_native11952_tails.py')},
    ...
    print('REPLAYED FIXED NATIVE11952 CONTROL TAILS',data['family'],data['parameter'],'charts',len(data['charts']),'rank >=',state.rank,flush=True)
