...
ROOT=batch.ROOT;ART=batch.ART;D=batch.D;OUT=ART/'higher24_r17_results_v1.json';DATABASE=ROOT/'artifacts/local/elliptic-curves/retention24-current-catalogue-v1/database.json';OLD=ART/'extended20_mw16_union_results_v1.json'
...
def build():
    ...
    for entry in ledger['rows']:
        ...
        model=tuple(map(cert.F,data['curve']));row={'id':entry['id'],'arms':entry['arms'],'retained_index':entry['retained_index'],'family':data['family'],'parameter':data['parameter'],'curve':data['curve'],'points':cloud['independent_points'],'generic_points':data['generic_points'],'family_to_curve_scale_u':data['family_to_curve_scale_u'],'rank_certificate':cloud['rank_certificate'],'rank_lower_bound':cloud['rank_lower_bound'],'icarm_matches':[r['id'] for r in projection if cert.isomorphic(model,r['ainvs'])],'previous_matches':[r['address'] for r in previous if cert.isomorphic(model,r['curve'])],'attempted_charts':len(data['charts']),'declared_charts':len(data['centres']),'completed_boxes':sum(c['search']['status']=='bounded_search_complete' for c in data['charts']),'search_status':data['status'],'supervision_status':entry['status'],'discovery_witness':{'path':str(path.relative_to(ROOT)),'sha256':cert.hashed(path)},'complete_cloud_certificate':{'path':str(cloud_path.relative_to(ROOT)),'sha256':cert.hashed(cloud_path)}};verify(row,families,projection,previous);rows.append(row);print('CERTIFIED HIGHER24 R17',row['id'],row['parameter'],'rank >=',row['rank_lower_bound'],'catalogue',row['icarm_matches'],'previous',len(row['previous_matches']),flush=True)
    pairs=[[j,i] for i,r in enumerate(rows) for j,s in enumerate(rows[:i]) if cert.isomorphic(r['curve'],s['curve'])]
    cert.write(OUT,{'schema':'elliptic-curves.higher24-r17-results.v1','sources':sources(),'curves':rows,'within_batch_isomorphic_pairs':pairs,'point_protocol_sha256':cert.hashed(D/'protocol.json'),'point_ledger_sha256':cert.hashed(D/'ledger.json'),'verification_ledger_sha256':cert.hashed(D/'verification-ledger.json'),'previous_equations':previous,'previous_source':{'path':str(OLD.relative_to(ROOT)),'sha256':cert.hashed(OLD)},'catalogue':{'url':'https://elliptic-rank.icarm.cloud/database.json','raw_sha256':cert.hashed(DATABASE),'curve_count':len(projection),'equations':projection,'acknowledgement':'ICARM, supported by NSF Grant DMS2425401'},'claim_boundary':'Exact point lower bounds for the frozen higher24 candidate cohort. Q-isomorphism and catalogue comparison occurs only after all attempts/replays. No exact rank, global minimality, exact conductor or universal novelty is inferred. Validation-prime enrichment and discovery yield are finite experimental outcomes.'})
...
def check():
    ...
    print('REPLAYED HIGHER24 R17 CERTIFICATES',len(d['curves']),flush=True)
