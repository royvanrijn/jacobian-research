...
from search_new_rank26_pari43 import ROOT,PARENT,MAPS,sources
...
def replay(path):
    ...
    if not u or model!=(0,0,0,original[3]/u**4,original[4]/u**6) or points!=tuple((x/u**2,y/u**3) for x,y in generic):raise ArithmeticError('generic transport failed')
    if data['initial_state']!=parent['final_state'] or data['centres']!=parent['centres']:raise ArithmeticError('initial generic state or fixed centres changed')
    ...
    if state.rank!=26 or tuple(tuple(map(cert.F,p)) for p in state.basis)!=initial_points or initial_points[:17]!=points:raise ArithmeticError('generic basis changed')
    ...
    if data['status']=='COMPLETE_DECLARED_PILOT' and len(data['charts'])!=43:raise ArithmeticError('incomplete43 plan')
    print('REPLAYED NEW RANK26 PARI43 MAPS AND POINTS',len(data['charts']),'rank >=',state.rank,flush=True)
