...
OUT=ART/'compact192_followup_supplement_evidence_v1.json'
...
def main():
    ...
    paths={Path(__file__).resolve(),CAS/'verify_compact192_followup_supplement.py',CAS/'replay_retention24_geometry.py',
    ...
    for row in cert.read(follow.BATCH/'verification-ledger.json')['rows']:
        for key in ('mod2_certificate','modl_certificate'):paths.add(ROOT/row[key])
    ...
    cert.write(OUT,{'schema':'elliptic-curves.compact192-followup-supplement-evidence.v1','archive':str(archive.relative_to(ROOT)),
