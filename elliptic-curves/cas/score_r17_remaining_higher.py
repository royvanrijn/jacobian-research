#!/usr/bin/env python3
"""Target-free scalar selection on10240 new higher R17 equations."""
...
D=ROOT/'artifacts/local/elliptic-curves/r17-remaining-higher-scores-v1'
OUT=ART/'r17_remaining_higher_selection_v1.json'
FRESH_PRIMES=[q for q in _primes_up_to(131071) if q>=65537]

def sources():
    paths=[Path(__file__).resolve(),Path(scan.__file__),Path(engine.__file__),Path(models.__file__),Path(scoring.__file__),Path(cert.__file__),engine.spec.ATLAS,scan.D/'protocol.json',scan.D/'result.json',scan.D/'replay.json',CAS/'research_runtime/store.py',CAS/'research_runtime/supervisor.py',CAS/'mod2_reduction_independence.py']
    return {str(p.relative_to(ROOT)):cert.hashed(p) for p in paths}

def prospective_rows(raw):
    families={r['family']:r for r in cert.read(engine.spec.ATLAS)['families'] if r['family']!='11952'};pools=[];seen={};skips=[]
    for band in (2,3):
        for family in sorted(families):
            candidates=[{**r,'band':band,'family':family,'slice_id':s['id']} for manifest in raw['shards'] if manifest['band']==band and manifest['family']==family for s in [scan.load_shard(manifest)] for r in s['rows']]
            candidates.sort(key=lambda r:(-r['combined_selection_units'],-r['combined_good'],r['denominator'],r['numerator']))
            if len(candidates)!=131072 or len({r['parameter'] for r in candidates})!=131072:raise ArithmeticError('fixed131072 prospective band/family pool required')
            pool=[]
            for i,r in enumerate(candidates):
                row={**r,'id':f'r17-b{band}-{family}-{i:04}','retained_index':i,'model':models.model_at(families[family],r['parameter'])}
...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.r17-remaining-higher-scores.v1','sources':sources(),'rows':rows,'prospective_skips':skips,
        ...
        'gate':'All3059808912 primitive addresses in 320 untouched higher slices have been scored through32749 before retention, with all1310720 retained scores replayed. The complete five-family cache matches all4831 earlier scalar cases. Choose1024 within-roster distinct equations per higher band/family from the fixed131072-address signed pool, giving equal scalar exposure without public-record targets.',
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=p['maximum_workers']) as pool:
            for start in range(p['cost_gate_rows'],len(p['rows']),p['checkpoint_block']):
                ...
                if len(result['rows'])%800==20:print('R17 BROADER TRACE PROGRESS',len(result['rows']),flush=True)
...
def check():
    ...
    print('REPLAYED ALL10240 R17 BROADER SCORES',flush=True)
...
def validate(check=False):
    ...
    print('R17 BROADER FINALISTS FRESH VALIDATION60 PASS',flush=True)
