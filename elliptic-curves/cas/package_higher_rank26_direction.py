...
ROOT=Path(__file__).resolve().parents[2];ART=ROOT/'artifacts/generated-results/elliptic-curves';LOCAL=ROOT/'artifacts/local/elliptic-curves';OUT=ART/'higher_rank26_direction_evidence_v1.json'
FOLDERS=('higher-rank26-direction-preflight-v1','higher-rank26-11952-069-newdirection-v2')
...
def main():
    ...
    paths={Path(__file__).resolve(),ROOT/'elliptic-curves/notes/HIGHER_RANK26_FOLLOWUP_2026-09-06.md'}
    for name in FOLDERS:paths.update(p for p in (LOCAL/name).rglob('*') if p.is_file())
    for pattern in ('higher_rank26_direction_*_v1.json',):paths.update(ART.glob(pattern))
    names=('followup_higher_rank26_direction.py','prepare_higher_rank26_direction.sage','followup_higher_rank26_direction_v2.py','prepare_higher_rank26_direction_v2.sage','audit_higher_rank26_direction.py','replay_higher_rank26_direction_geometry.py','verify_higher_rank26_direction_bundle.py')
    ...
    cert.write(OUT,{'schema':'elliptic-curves.higher-rank26-direction-evidence.v1','archive':str(archive.relative_to(ROOT)),'archive_sha256':cert.hashed(archive),'archive_bytes':archive.stat().st_size,'required_base_archives':bases,'files':members,'inherited_exact_members':inherited,'claim_boundary':'Extract sixteen pinned base archives and this supplement. Retains the failed seed-path preflight, corrected frozen protocol, all301 new-direction maps and completed boxes, histories and965-point full-cloud proofs modulo2,3,5. The bound remains26 on existing inventory ID99. No new27/28/32, rank upper bound, saturation or point-absence claim.'})
    print('PACKAGED HIGHER26 DIRECTION',len(members),'new',len(inherited),'inherited',archive.stat().st_size,'bytes',flush=True)
