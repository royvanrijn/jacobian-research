...
import endpoint_specialized_parity_trial as control
import certify_compact_r17_candidates as cert
from replay_retention24_geometry import geometry,cloud_check,tuples
from research_runtime.store import digest

def main():
    rank=control.dimension();p=control.protocol();D=control.D;maps=cert.read(D/'maps.json');data=cert.read(D/'result.json');seed=cert.read(control.SEED);initial=tuples(seed['points'])
    ...
    if [r['parity'] for r in maps['sample']]!=control.masks(p):raise ArithmeticError('fixed256 masks differ')
    ...
    selected=sorted(maps['sample'],key=lambda r:(-r['metric_norm'],r['parity']))[:p['charts']]
    if maps['centres']!=selected or len(maps['rows'])!=p['charts']:raise ArithmeticError('fixed selected roster differs')
    geometry(data,maps,initial,{**p,'maps_path':D/'maps.json'})
    cloud_check(data,data['charts'],control.ART/('endpoint_specialized_'+control.ROW['id'].replace('-','_')+'_mod2_v1.json'),D/'result.json')
    if data['status']=='COMPLETE_DECLARED_ADAPTIVE_ATTEMPT' and len(data['charts'])!=p['charts']:raise ArithmeticError('complete status on short roster')
    print('EXACT256 PARITIES;12 SELECTED;RATIONAL MAP/POINT PROVENANCE',len(data['charts']),'SEARCHED',flush=True)
if __name__=='__main__':
    for index in range(len(cert.read(control.BATCH/'protocol.json')['rows'])):control.configure(index);main()
