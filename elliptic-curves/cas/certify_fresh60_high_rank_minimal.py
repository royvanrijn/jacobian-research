...
ROOT=Path(__file__).resolve().parents[2];ART=ROOT/'artifacts/generated-results/elliptic-curves';INPUT=ART/'fresh60_mw16_results_v1.json';OUT=ART/'fresh60_high_rank_minimal_proof_v1.json'
def sources():
...
def expected():
    ...
    if len(selected)!=4 or sorted(r['rank_lower_bound'] for r in selected)!=[23,24,24,26]:raise ArithmeticError('fixed four-curve cohort differs')
    for q in selected:
    ...
    return {'schema':'elliptic-curves.fresh60-high-rank-minimal.v1','status':'PASS','sources':sources(),'input_sha256':cert.hashed(INPUT),'curves':rows,'catalogue':d['catalogue'],'previous_equations':d['previous_equations'],'claim_boundary':'Exact independent-point lower bounds, proved global minimal integral models and exact point transports for the unmatched>=22 portion of the completed fresh outer60 MW16 cohort. Absence from593 pinned catalogue equations and977 earlier measured equations is not universal novelty. No exact rank or conductor is asserted.'}
if __name__=='__main__':
    ...
    if a.check:
        ...
        checkpoint(OUT,r)
    print('EXACT FRESH60 HIGH-RANK MINIMAL PROOFS',[(q['id'],q['rank_lower_bound'],q['minimality']['invariant_gcd']) for q in r['curves']],flush=True)
