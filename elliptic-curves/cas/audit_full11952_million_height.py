...
import full11952_million_height as million
...
def inputs(kind,index):
    ...
    paths=[ROOT/q for q in p['prior_point_histories']]
...
def main(kind,index,check):
    p,row,folder,data,paths,charts,sources=inputs(kind,index);stem='full11952_million_'+row['id'].replace('-','_');out=folder/'all-retained-point-cloud-only.json';mod2=ART/(stem+'_all_retained_mod2_v1.json');modl=ART/(stem+'_all_retained_modl_v1.json');summary=ART/(stem+'_coverage_v1.json')
