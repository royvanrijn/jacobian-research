...
def run_gp_once(
    ...
    """Run one foreground PARI process with joined TERM/KILL cleanup."""

    if timeout <= 0 or stack_bytes < 8_000_000:
        raise ValueError("timeout and PARI stack bounds must be positive")
    ...
    if process.returncode != 0 or fatal_lines:
        return None, {
            "status": "pari_error",
            "returncode": process.returncode,
            "wall_seconds": elapsed,
            "error": " ".join((fatal_lines or stderr.splitlines()))[:2_000],
        }
