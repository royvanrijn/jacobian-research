...
def launch():
    ...
    def one(index):
        ...
        s=run([sys.executable,str(CAS/'verify_full11952_64_r17_pari_batch.py'),'--index',str(index)],limits=Limits(p['seconds_per_curve'],p['rss_bytes_per_worker']),log_path=folder/'verification.log',checkpoint_path=folder/'verification.supervisor.json',cwd=ROOT)
        ...
        if cloud['rank_lower_bound']>=28:
            import replay_full11952_64_geometry as geometry
            ...
            odd=batch.ART/('full11952_64_r17_'+row['id'].replace('-','_')+'_early_modl_v1.json')
    ...
    with ThreadPoolExecutor(max_workers=p['maximum_workers']) as pool:
        ...
        for f in as_completed(pending):
            i=pending[f];data['rows'][i]=f.result();checkpoint(out,data);print('FULL11952_64 EXACT PROOF',p['rows'][i]['id'],data['rows'][i]['status'],data['rows'][i].get('rank_lower_bound'),flush=True)
