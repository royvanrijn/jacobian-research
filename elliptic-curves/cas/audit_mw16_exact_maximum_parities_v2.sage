...
D=ROOT/'artifacts/local/elliptic-curves/mw16-exact-maximum-parities-v2'
...
def run(family):
    ...
    scaled=[[2*cert.F(v) for v in r] for r in f['generic_height_gram']]
    if any(v.denominator!=1 for r in scaled for v in r):raise ArithmeticError('nonintegral scaled Gram')
    G=matrix(ZZ,[[int(v) for v in r] for r in scaled])
