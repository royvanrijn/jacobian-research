...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},
        ...
        'scope':'Freeze 320 untouched higher MW16 slices before the complete cache and all40960 saved-score gates finish. After all gates pass, score every address through32749 before retaining4096 per slice. Require 320 complete signed-frame regressions, first-slice-per-band runtime gates and all1310720 independent retained-score replays. Exactly3058897488 primitive addresses in16384<H<=65536 and65536<H<=262144; no compact-region rescan or prior outer slice reuse. No public-record input, adaptive refill or point search.'})
...
def launch():
    ...
    try:
        ...
        for name,seconds in p['stages']:
            ...
            print('BROAD MW16 SCAN',name,ledger['rows'][-1]['status'],flush=True)
