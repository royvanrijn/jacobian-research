...
ROOT=Path(__file__).resolve().parents[2];ART=ROOT/'artifacts/generated-results/elliptic-curves';D=ROOT/'artifacts/local/elliptic-curves/next24-r17-extended-pari-v1';OUT=ART/'next24_high_rank_minimal_proofs_v2.json';OLD=ART/'fresh_r17_paired_results_v1.json';DB=ROOT/'artifacts/local/elliptic-curves/next12-current-catalogue-v1/database.json';IDS=('103b2-021','11952-030','11952-041')

def sources():
    names=['certify_next24_high_rank_minimal_v2.py','memory_rank_certificate.py','certify_compact_r17_candidates.py','elliptic_candidate_record.py','mod2_reduction_independence.py','research_runtime/finite_reduction.py','research_runtime/memory_store.py','research_runtime/store.py']
...
def check_row(r,catalogue,previous):
    ...
    u=cert.F(r['discovery_to_minimal_scale_u'])
    if u<=0 or r['minimality']!=minimality(model) or short!=(0,0,0,-inv['c4']*u**4/48,-inv['c6']*u**6/864):raise ArithmeticError('minimal model or short transport differs')
    a1,a2,a3,a4,a6=model;back=[((x+inv['b2']/12)*u**2,(y+(a1*x+a3)/2)*u**3) for x,y in points]
