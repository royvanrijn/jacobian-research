...
OLD=ART/'new_high_rank_curve_index_v19.json'
ADDITIONAL=ART/'strata60_mw16_results_v1.json'

def sources():
    paths=(Path(__file__).resolve(),Path(cert.__file__).resolve(),ROOT/'elliptic-curves/cas/memory_rank_certificate.py',ROOT/'elliptic-curves/cas/mod2_reduction_independence.py',ROOT/'elliptic-curves/cas/elliptic_candidate_record.py',ART/'strata60_mw16_point_portable_replay_v1.json')
...
def promotion_gate():
    ...
    novelty=cert.read(ART/'strata60_mw16_novelty_v1.json')
    if report['status']!='COMPLETE_FIXED_COMPARISON' or audit['status']!='PASS' or novelty['status']!='PASS':raise ArithmeticError('terminal comparison and novelty required')
...
def build(output):
    ...
    cert.write(output,{'schema':'elliptic-curves.new-high-rank-index.v20','sources':sources(),
...
def check(path):
    ...
    for r in data['curves']:
        ...
        seen.add(j)
        print('REPLAYED V20 INVENTORY',r['id'],'rank >=',len(points),flush=True)
    print('EXACT CURVE INVENTORY',len(seen),flush=True)

if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('--output',type=Path,default=ART/'new_high_rank_curve_index_v20.json');p.add_argument('--check',type=Path)
