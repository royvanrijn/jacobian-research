...
"""Independent PARI Tate replay of the one-curve Sage generic local audit."""
...
INPUT=ART/'corrected60_conductor_bounds_v1.json'
OUT=ART/'corrected60_conductor_bounds_pari_replay_v1.json'
...
def expected():
    ...
    inventory=[r for r in cert.read(ART/'corrected60_mw16_results_v1.json')['curves'] if r['rank_lower_bound']>=22 and not r['icarm_matches'] and not r['previous_matches']]
    if len(inventory)!=1 or [r['id'] for r in inventory]!=[r['id'] for r in d['rows']]:raise ArithmeticError('whole inventory required')
    counts=[]
    ...
    return {'schema':'elliptic-curves.corrected60-conductor-pari-replay.v1','status':'PASS',
        'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in (Path(__file__).resolve(),INPUT,ART/'corrected60_mw16_results_v1.json')},
        'pari_version':str(pari.version()),'curves':1,'local_pairs':sum(r['local_pairs'] for r in counts),'rows':counts,
        'claim_boundary':'Independent PARI elllocalred agrees with every one of the Sage generic Tate local results and reconstructs the new-curve conductor upper bound. This does not factor the residual cofactors, verify external catalogue conductors, reprove ranks or assert conductor records.'}
