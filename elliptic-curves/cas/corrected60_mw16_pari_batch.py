#!/usr/bin/env python3
"""Six higher parameters per band and MW16 family from frozen extended scores."""
import argparse
from pathlib import Path
from concurrent.futures import ThreadPoolExecutor,as_completed
import certify_compact_r17_candidates as cert
import compact_mw16_specialization as spec
import score_corrected_mw16_higher as extension
...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';D=ROOT/'artifacts/local/elliptic-curves/corrected60-mw16-pari-v1';SAGE='/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python'

def sources():
    names=['memory_rank_certificate.py','corrected60_mw16_pari_batch.py','prepare_corrected60_mw16_pari_batch.sage','prospective_half_lattice_v2.sage','compact_mw16_specialization.py','research_runtime/search_state.py','research_runtime/cached_observation_state.py','research_runtime/preloaded_prime_state.py','research_runtime/rotated_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
...
def expected_roster():
    ...
    return [{'id':r['id'],'family':r['family'],'band':r['band'],'parameter':r['parameter'],'retained_index':r['retained_index'],'arms':['fresh_outer_mw16_band_'+str(r['band'])],'combined_selection_units':r['combined_late_units']} for r in d['selected']]

def freeze():
    if (D/'protocol.json').exists():raise FileExistsError('preserve corrected60 protocol')
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.corrected60-mw16-pari.v1','sources':sources(),'selection_sha256':cert.hashed(extension.OUT),'score_protocol_sha256':cert.hashed(extension.D/'protocol.json'),'score_ledger_sha256':cert.hashed(extension.D/'controller/ledger.json'),'validation_sha256':cert.hashed(extension.D/'fresh-validation.json'),'rows':rows,'generic_masks':roster,'height':125000,'seconds_per_chart':10,'admission_prime_bound':997,'stop_rank':28,'map_wall_seconds':120,'worker_wall_seconds':600,'rss_bytes':1610612736,'maximum_workers':2,'gp_sha256':cert.hashed(Path('/usr/bin/gp')),'gate':'All320 corrected-score higher slices, with restored good5/13 terms,10240 scalar scores,60-curve rational-isomorphism deduplication and wholly disjoint validation replay. Existing target-free MW16 prospective point proofs establish that this generic16-only detector can recover additional independent points. Finite point exposure tests fresh candidates at new parameter heights, not a guaranteed rank yield.','selection':'Six per band/family in16384<H<=65536 and65536<H<=262144 using combined quantized S1 through65521. Validation65537..131071, public ranks, exceptional points and catalogue data never affect selection or maps.','geometry':'Exactly43 recorded generic16 parity labels. Numerical384-bit heights choose representatives; exact rational centres and maps replay. All60 map files must precede every point search. No specialized maximum or covering theorem is claimed.','boundaries':'Exactly60 attempts and at most2580 initial boxes, each seeded only by16 generic sections. Stop an individual curve at observed28 pending independent proof. Failures/timeouts retained without retry. Novelty comparisons only after terminal batch and proof. No adaptive wave, exact rank, upper bound or universal novelty.'})
...
def protocol():
    ...
    if p['sources']!=sources() or any(p[k]!=cert.hashed(v) for k,v in bindings) or p['rows']!=expected_roster() or p['generic_masks']!=masks():raise ArithmeticError('frozen corrected60 inputs differ')
    return p
...
def replay(index):
    ...
    memory_checked_rank(model,state.basis,state.reductions.primes,state.no_two_torsion_prime);print('REPLAYED CORRECTED60 MW16',p['rows'][index]['id'],len(data['charts']),'rank >=',state.rank,flush=True)
...
def maps():
    ...
    def one(i):
        ...
        supervision=supervise([SAGE,str(CAS/'prepare_corrected60_mw16_pari_batch.sage'),'--index',str(i)],limits=Limits(p['map_wall_seconds'],p['rss_bytes']),log_path=folder/'maps.log',checkpoint_path=folder/'maps.supervisor.json',cwd=ROOT)
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        for f in as_completed(pending):
            i=pending[f];got[i]=f.result();ledger['rows']=[got[j] for j in sorted(got)];checkpoint(path,ledger);print('CORRECTED60 MAP',got[i]['id'],got[i]['status'],flush=True)
...
def batch():
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        for future in as_completed(futures):
            i=futures[future];r=future.result();ledger['rows'][i]=r;checkpoint(path,ledger);print('CORRECTED60 MW16 ATTEMPT',r['id'],r['status'],r.get('rank_lower_bound'),flush=True)
