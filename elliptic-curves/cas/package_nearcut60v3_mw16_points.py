#!/usr/bin/env python3
"""Standalone point evidence for a censor-aware retained near-finalist trial."""
...
OUT=ART/'nearcut60v2_mw16_point_evidence_v1.json'
CONTROL=batch.extension.D/'point-controller-v2'
AUDIT=batch.extension.D/'accounting-controller-v3'


def main():
    archive=OUT.with_suffix('.zip')
    if OUT.exists() or archive.exists():raise FileExistsError('preserve standalone comparison bundle')
    p=batch.protocol();report_path=ART/'nearcut60v2_mw16_experiment_v1.json'
    audit_path=ART/'nearcut60v3_mw16_accounting_replay_v1.json'
    report=cert.read(report_path);audit=cert.read(audit_path)
    if cert.read(CONTROL/'ledger.json')['status']!='COMPLETE_FIXED_RETAINED_TRIAL_AND_ACCOUNTING' or cert.read(AUDIT/'ledger.json')['status']!='PASS' or audit['status']!='PASS':
    ...
    paths={Path(__file__).resolve(),CAS/'verify_nearcut60v2_mw16_points_portable.py',report_path,audit_path,
    ...
    cert.write(OUT,{'schema':'elliptic-curves.nearcut60v2-point-evidence.v1','status':'PASS',
        ...
        'claim_boundary':'Standalone admission-history, exact generic16 transport, rational geometry, returned-point provenance and finite-quotient rank replay for every certified row of the60-curve retained near-finalist trial. All allocations, unresolved outcomes, terminal partial prefixes and immutable computation-accounting evidence remain included. An unresolved row is not a zero gain. The prior population scan and selection replay are bound context, not rerun. The local independent accounting audit is embedded; isolated point replay does not rerun that audit or measure search time again. No new search, point absence, exact rank, catalogue novelty or rank-density theorem.'})
    print('STANDALONE NEARCUT60V2',len(members),'FILES',archive.stat().st_size,'BYTES',flush=True)
