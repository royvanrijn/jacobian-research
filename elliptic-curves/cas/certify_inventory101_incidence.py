#!/usr/bin/env python3
"""Bind eight exact incidence cohorts to the current101 equations and duplicate map."""
import argparse
...
NAMES=['compact','latest7','latest8','latest23','retention','extended20_mw16','higher24','inventory101_added']
...
INDEX=ART/'new_high_rank_curve_index_v12.json'

def result():
    index=cert.read(INDEX);targets={r['id']:r for r in index['curves']};seen=set();counts=Counter();duplicates=[]
    if len(targets)!=101:raise ArithmeticError('fixed101 inventory differs')
    maps=None
    for path in PROOFS:
        data=cert.read(path)
    ...
    if seen!=set(targets) or counts!={'NO_RATIONAL_J_PREIMAGE':1090,'RATIONAL_J_PREIMAGES_CERTIFIED':122} or len(duplicates)!=21:raise ArithmeticError('fixed1212 coverage differs')
    ...
    return {'schema':'elliptic-curves.inventory101-incidence.v1','status':'PASS','sources':sources,'targets_checked':101,'pairs_checked':1212,'status_counts':dict(counts),'same_generic_subgroup_duplicates':duplicates,'historical_rank_metadata':'Incidence targets retain their historical lower bounds; only nondecreasing current bounds are accepted, and all curve, j, family and parameter fields must remain identical. Incidence itself is independent of rank.','claim_boundary':'Combines the eight separately exact-replayed rational j-incidence cohorts and the exact Q(t) unimodular17-section transport. Every additional recorded presentation is the same generic subgroup. The separate incidence/transport replays remain required; this summary does not replay ranks or exclude other families, nongeneric points or higher ranks.'}
...
if __name__=='__main__':
    ...
    print('EXACT COHORT BINDING101 CURVES1212 PAIRS21 DUPLICATE PRESENTATIONS',flush=True)
