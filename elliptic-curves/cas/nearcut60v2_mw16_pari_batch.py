...
def replay(index):
    ...
    memory_checked_rank(model,state.basis,state.reductions.primes,state.no_two_torsion_prime);print('REPLAYED NEARCUT60V2 MW16',p['rows'][index]['id'],len(data['charts']),'rank >=',state.rank,flush=True)
...
def maps():
    ...
    def one(i):
        ...
        supervision=supervise([SAGE,str(CAS/'prepare_nearcut60v2_mw16_pari_batch.sage'),'--index',str(i)],limits=Limits(p['map_wall_seconds'],p['rss_bytes']),log_path=folder/'maps.log',checkpoint_path=folder/'maps.supervisor.json',cwd=ROOT)
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        for f in as_completed(pending):
            i=pending[f];got[i]=f.result();ledger['rows']=[got[j] for j in sorted(got)];checkpoint(path,ledger);print('NEARCUT60V2 MAP',got[i]['id'],got[i]['status'],flush=True)
...
def batch():
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        for future in as_completed(futures):
            i=futures[future];r=future.result();ledger['rows'][i]=r;checkpoint(path,ledger);print('NEARCUT60V2 MW16 ATTEMPT',r['id'],r['status'],r.get('rank_lower_bound'),flush=True)
