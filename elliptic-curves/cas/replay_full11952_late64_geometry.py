...
def main():
    ...
    for row in p['rows']:
        ...
        geometry(data,maps,initial,{**p,'maps_path':folder/'maps.json'});cloud_check(data,data['charts'],ART/('full11952_late64_r17_'+row['id'].replace('-','_')+'_mod2_v1.json'),path);count+=len(data['charts'])
    ...
    print('REPLAYED FULL11952_LATE64 CHARTS',count,'GENERIC CENTRES, RAW POINTS AND CLOUD PROVENANCE',flush=True)
