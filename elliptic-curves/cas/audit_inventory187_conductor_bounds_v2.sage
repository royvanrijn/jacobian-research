...
D=ROOT/'artifacts/local/elliptic-curves/inventory187-conductor-bounds-v2'
OUT=ART/'inventory187_conductor_bounds_v2.json'
...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'sources':sources(),'roster':[r['id'] for r in rows],
        ...
        'superseded_failure':'V1 hit the Sage generic Tate uniformizer branch on a nonintegral short model before completing a row. V2 first translates to an exactly isomorphic integral equation; no source data or roster changes.',
...
def expected(write=False):
    ...
    result={'schema':'elliptic-curves.inventory187-conductor-bounds.v2','status':'RUNNING',
