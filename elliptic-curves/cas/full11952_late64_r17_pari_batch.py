...
import full11952_late_band_selection as extension
...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';D=ROOT/'artifacts/local/elliptic-curves/full11952-late64-r17-pari-v1';PARITY=ART/'r17_exact_maximum_parity_classes_v1.json';SAGE='/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python'

def sources():
    names=['verify_full11952_late_band_selection.py','memory_rank_certificate.py','full11952_late_band_selection.py','verify_full11952_late64_r17_pari_batch.py','full11952_late64_r17_pari_batch.py','prepare_full11952_late64_r17_pari_batch.sage','replay_full11952_late64_geometry.py','audit_full11952_late64_clouds_modl.py','stream_full11952_late64_verification.py','prospective_half_lattice_v2.sage','compact_atlas_specialization.py','research_runtime/search_state.py','research_runtime/cached_observation_state.py','research_runtime/preloaded_prime_state.py','research_runtime/rotated_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
...
def expected_roster():
    ...
    for r in selected['selected']:
        ...
        rows[-1]['arms']=['full11952_fresh4096_late_band_top64']
...
def freeze():
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.full11952-late64-r17-pari.v1','sources':sources(),'selection_sha256':cert.hashed(extension.OUT),'selection_protocol_sha256':cert.hashed(extension.D/'protocol.json'),'selection_replay_sha256':cert.hashed(extension.D/'selection-replay.supervisor.json'),'validation_sha256':cert.hashed(extension.D/'fresh-validation.json'),'validation_replay_sha256':cert.hashed(extension.D/'validation-check.supervisor.json'),'score_controller_sha256':cert.hashed(CONTROL/'ledger.json'),'control_sha256':cert.hashed(ROOT/'artifacts/local/elliptic-curves/native11952-height125-control-v1/125000/verification.json'),'rows':rows,'generic_masks':masks,'height':125000,'seconds_per_chart':10,'admission_prime_bound':997,'stop_rank':28,'map_wall_seconds':120,'worker_wall_seconds':600,'map_roster_wall_seconds':1800,'point_batch_wall_seconds':20000,'geometry_before_search':True,'rss_bytes':1610612736,'maximum_workers':2,'gp_sha256':cert.hashed(Path('/usr/bin/gp')),'gate':'The separately frozen4096 new equations all pass scalar4099..65521 traces, prior cached-score agreement and8192 direct character sums. The64 finalists are selected through65521 and then pass wholly disjoint65537..131071 validation. Exact exclusions remove all853 earlier cohort and593 pinned catalogue equations before the4096 roster; selected equations are distinct. The native generic17-only control recovers28 at125000. The retrospective unused-band ordering contrast motivates the finite trial, not a rank-density or sensitivity law.','selection':'The64 frozen Q-isomorphism-distinct nonsingular finalists from the fresh4096 second-band selector. Combined S1 through65521, total good count, denominator and signed numerator determine ordering. Fresh validation and public ranks/points never enter ordering or maps. Catalogue equations are exclusions only. The4096 roster is an explicit truncation of the retained population; no refill or new parameter scan.','geometry':'Every one of the49 generic maximum-parity classes on each curve. Numerical384-bit heights and rounded CVP choose representatives; exact parities, rational centres and quartic transports are checked. Every one of the64 map files precedes all point searches.','boundaries':'Exactly64 point attempts and at most3136 initial boxes, height125000 and ten seconds each, generic17 seeds and a per-curve provisional28 stop pending exact proof. No adaptive wave, automatic budget extension, exact rank, upper bound, saturation, point absence, score optimality or universal novelty.'})

def protocol():
    p=cert.read(D/'protocol.json')
    bindings={'selection_sha256':extension.OUT,'selection_protocol_sha256':extension.D/'protocol.json','selection_replay_sha256':extension.D/'selection-replay.supervisor.json','validation_sha256':extension.D/'fresh-validation.json','validation_replay_sha256':extension.D/'validation-check.supervisor.json','score_controller_sha256':CONTROL/'ledger.json','control_sha256':ROOT/'artifacts/local/elliptic-curves/native11952-height125-control-v1/125000/verification.json','gp_sha256':Path('/usr/bin/gp')}
...
def replay(index):
    ...
    memory_checked_rank(model,state.basis,state.reductions.primes,state.no_two_torsion_prime);print('REPLAYED FULL11952_LATE64 R17',p['rows'][index]['id'],len(data['charts']),'rank >=',state.rank,flush=True)
...
def maps():
    ...
    def one(index):
        ...
        r=supervise([SAGE,str(CAS/'prepare_full11952_late64_r17_pari_batch.sage'),'--index',str(index)],limits=Limits(p['map_wall_seconds'],p['rss_bytes']),log_path=folder/'maps.log',checkpoint_path=folder/'maps.supervisor.json',cwd=ROOT)
    ...
    with ThreadPoolExecutor(max_workers=p['maximum_workers']) as pool:
        ...
        for f in as_completed(pending):
            i=pending[f];ledger['rows'][i]=f.result();checkpoint(path,ledger);print('FULL11952_LATE64 FROZEN MAP',p['rows'][i]['id'],ledger['rows'][i]['status'],flush=True)
...
def batch():
    ...
    if path.exists():raise FileExistsError('preserve full11952_late64 point batch')
    ...
    with ThreadPoolExecutor(max_workers=p['maximum_workers']) as pool:
        pending={pool.submit(one,i):i for i in range(len(p['rows']))}
        for f in as_completed(pending):
            i=pending[f];r=f.result();ledger['rows'][i]=r;checkpoint(path,ledger);print('FULL11952_LATE64 POINT ATTEMPT',r['id'],r['status'],r.get('rank_lower_bound'),flush=True)
