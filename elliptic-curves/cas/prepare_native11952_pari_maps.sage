...
"""Freeze PARI-reduced horizontal maps from only the native control generic points."""
import sys
...
D=ROOT/'artifacts/local/elliptic-curves/native11952-pari-maps-v1'
...
def run():
    ...
    source=cert.read(PARENT/'redacted-input.json');previous=cert.read(PARENT/'candidate-00/result.json')
    if cert.hashed(PARENT/'redacted-input.json')!=protocol['input_sha256'] or cert.hashed(PARENT/'candidate-00/result.json')!=protocol['centre_source_sha256']:raise ArithmeticError('generic map input changed')
    model=tuple(map(cert.F,source['curve']));points=tuple(tuple(map(cert.F,p)) for p in source['generic_points']);rows=[]
    ...
    for c in previous['centres']:
        ...
        checkpoint(D/'maps.json',output);print('FROZEN NATIVE11952 PARI MAP',len(rows),flush=True)
