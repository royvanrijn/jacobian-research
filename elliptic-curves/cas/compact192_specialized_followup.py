#!/usr/bin/env python3
"""One fixed specialized-parity follow-up on the new compact192 rank26/27 curves."""
import argparse,sys
...
BATCH=LOCAL/'compact192-specialized-followup-v1';INDEX=ART/'new_high_rank_curve_index_v13.json';GATE=ART/'native28_specialized_parity_adaptive_coverage_v1.json'
...
def sources():
    names=['audit_compact192_specialized_followup.py','replay_compact192_specialized_geometry.sage','export_new_high_rank_curve_index_v13.py','memory_rank_certificate.py','compact192_specialized_followup.py','prepare_compact192_specialized_followup.sage','prepare_fresh_r17_pari_batch.sage','fresh_r17_pari_batch.py','prospective_half_lattice_v2.sage','research_runtime/search_state.py','research_runtime/preloaded_prime_state.py','research_runtime/rotated_observation_state.py','research_runtime/cached_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
    return {**backend.sources(),**{str((CAS/n).relative_to(ROOT)):cert.hashed(CAS/n) for n in names}}

def prepare():
    if (BATCH/'protocol.json').exists():raise FileExistsError('preserve one compact192 follow-up protocol')
    import export_new_high_rank_curve_index_v13 as inventory
    inventory.promotion_gate()
    replay=ART/'new_high_rank_curve_index_v13_memory_replay_v1.json'
    verified=cert.read(replay)
    if verified['status']!='PASS' or any(cert.hashed(ROOT/n)!=h for n,h in verified['sources'].items()):raise ArithmeticError('bound exact V13 replay required')
    gate=cert.read(GATE);rows=[];seeds={};oldpath=ART/'new_high_rank_curve_index_v12.json';paths=[INDEX,GATE,replay,oldpath]
    ...
    eligible=sorted((r for r in index['curves'] if r['id'] not in previous_ids and 26<=r['rank_lower_bound']<28 and r['source_certificate']=='compact192_r17_results_v1.json'),key=lambda r:r['id'])
    if not 1<=len(eligible)<=24:raise ArithmeticError('full eligible roster must contain1through24 curves; no truncation or refill')
    for r in eligible:
        ...
        if q['icarm_matches'] or q['previous_matches'] or r['points'][:len(g)]!=g or len(g)!=17 or rank!=len(r['points']):raise ArithmeticError('new catalogue-unmatched own26/27 seed and generic17 prefix required')
    ...
    checkpoint(BATCH/'protocol.json',{'schema':'elliptic-curves.compact192-specialized-followup.v1','sources':sources(),'inputs':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},'seed_hashes':seeds,'rows':rows,'sample_size':2048,'sample_domain':'compact192-specialized-followup-v1','charts':49,'height':125000,'seconds_per_chart':10,'target_rank':28,'gp_sha256':cert.hashed(Path('/usr/bin/gp')),'geometry_wall_seconds':300,'worker_wall_seconds':900,'replay_wall_seconds':600,'rss_bytes':2147483648,'maximum_workers':1,'maximum_point_boxes':49*len(rows),'gate':'The known native control recovered its29th direction from28 known points using the same2048-mask specialized-parity policy at125000 on chart18. The fixed compact192 initial searches use only17 generic-point centres; every new26/27 seed supplies additional exact independent directions not used to define those initial centres. Test this changed point exposure on the entire eligible new compact192 roster. The control proves finite recovery, not prospective sensitivity, rank density or solubility of other covers.','centre_policy':'Use only each curve own certified26/27-point seed. Generate2048 distinct SHA256 masks of that dimension with nonzero quotient above the17-point generic prefix, using the fixed sample domain and increasing index. Compute384-bit canonical heights, round at10^6, use a unimodular LLL change and numerical CVP, exactly check parities and rounded norms, then select49 largest computed norms with mask ties. Freeze every cohort map file before any point search. No covering or optimality theorem is claimed.','selection':'All V13 entries newly added beyond V12 from compact192_r17_results_v1.json with certified lower bound26or27, sorted by stable local ID. Require the whole eligible roster to contain at most24 curves; otherwise stop before writing any seed. No public points, oracle words, new parameter scans, score changes, selection among previous adaptive outcomes or result-dependent refill.','boundaries':'Exactly one finite follow-up of49 charts per eligible curve, at most1176 boxes,125000 height and ten seconds each, one point worker. Stop each curve at its first28-point lower bound pending independent replay; retain all timeouts and partial records. No second wave, larger heights, automatic retry, exact-rank, upper-bound, saturation, record or universal-novelty claim.'})
...
def initial(cache):
    ...
    if state.rank!=ROW['initial_rank']:raise ArithmeticError('certified own26/27 initial subgroup differs')
    return seed,state
...
def replay():
    ...
    checked_rank(model,state.basis,state.reductions.primes,state.no_two_torsion_prime);print('REPLAYED ADAPTIVE COMPACT192 SPECIALIZED',len(data['charts']),'rank >=',state.rank,flush=True)
...
def launch():
    ...
    for index in range(len(p['rows'])):
        ...
        s=supervise(['/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python',str(CAS/'prepare_compact192_specialized_followup.sage'),'--index',str(index)],limits=Limits(p['geometry_wall_seconds'],p['rss_bytes']),log_path=D/(name+'.log'),checkpoint_path=path,cwd=ROOT)
