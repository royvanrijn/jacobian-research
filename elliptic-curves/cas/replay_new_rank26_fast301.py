...
from search_new_rank26_fast301 import ROOT,SOURCE,MAPS,sources
...
def replay(path):
    ...
    f=next(r for r in cert.read(spec.ATLAS)['families'] if r['family']==data['family']);original,generic=spec.specialize(f,data['parameter']);u=cert.F(data['family_to_curve_scale_u']);model=tuple(map(cert.F,data['curve']));points=tuple(tuple(map(cert.F,p)) for p in data['generic_points'])
    if not u or model!=(0,0,0,original[3]/u**4,original[4]/u**6) or points!=tuple((x/u**2,y/u**3) for x,y in generic):raise ArithmeticError('generic transport differs')
    if data['initial_state']!=parent['initial_state'] or data['centres']!=parent['centres'] or data['initial_dimension']!=26:raise ArithmeticError('initial subgroup or centres differ')
    ...
    if state.rank!=26 or initial_points[:17]!=points:raise ArithmeticError('generic prefix differs')
    ...
    for i,row in enumerate(data['charts']):
        ...
        rep=c['representative']+[0]*(state.rank-26);search=PointedQuarticSearch(state=state,centre={'coefficients':rep},coordinate_policy=mapping['coordinate_policy']);found=backend.replay(search,mapping,row['search']);compression=compress(model,state.basis,rep,found)
    ...
    print('REPLAYED FAST301',len(data['charts']),'rank >=',state.rank,flush=True)
