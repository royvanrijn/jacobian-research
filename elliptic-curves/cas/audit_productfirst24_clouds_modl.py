#!/usr/bin/env python3
"""Bounded mod3/5 audit of every complete cloud in the fixed24 paired cohort."""
...
def main():
    ...
    if v['status']!='PASS' or len(p['rows'])!=24:raise ArithmeticError('all24 terminal exact histories and mod2 clouds required')
    rows=[]
    for r in p['rows']:
        path=ART/('productfirst24_r17_'+r['id'].replace('-','_')+'_mod2_v1.json')
        rows.append({'id':r['id'],'input':str(path.relative_to(ROOT)),'input_sha256':cert.hashed(path),'output':str((ART/('productfirst24_r17_'+r['id'].replace('-','_')+'_modl_v1.json')).relative_to(ROOT))})
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.productfirst24-odd-cloud-audit.v1','sources':{str(q.relative_to(ROOT)):cert.hashed(q) for q in (Path(__file__).resolve(),ROOT/'elliptic-curves/cas/audit_retained_cloud_modl.py',batch.D/'protocol.json',batch.D/'verification-ledger.json')},'rows':rows,'prime_bound':997,'moduli':[3,5],'maximum_workers':2,'seconds_per_stage':180,'rss_bytes_per_worker':1073741824,'gate':'Every point attempt completed and replayed. A finite mod2 dependence need not be rational dependence; check all recorded points modulo3 and5 before interpreting the low admission counts. No new point, trace or selection.','boundaries':'Exact finite-quotient ranks are lower bounds only; matching mod2/3/5 ranks do not prove saturation, exact rank or absence of more directions.'})
    ...
    def one(row):
        ...
        for label,args in [('build',['--input',str(ip),'--output',str(op)]),('check',['--check',str(op)])]:
            folder=D/row['id'];s=run(['/usr/bin/python3',str(ROOT/'elliptic-curves/cas/audit_retained_cloud_modl.py'),*args],limits=Limits(180,1073741824),log_path=folder/(label+'.log'),checkpoint_path=folder/(label+'.supervisor.json'),cwd=ROOT)
