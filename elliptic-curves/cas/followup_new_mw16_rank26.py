#!/usr/bin/env python3
"""One fixed301-chart follow-up on each of three independently certified new MW16 rank26 curves."""
import argparse,sys
...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';LOCAL=ROOT/'artifacts/local/elliptic-curves';PROOF=ART/'extended20_mw16_high_rank_minimal_v1.json'

IDS=('a1-fibration-01-015','a1-fibration-01-052','a1-fibration-02-014')
def configure(identifier):
    global ID,D,SEED,CENSUS
    if identifier not in IDS:raise ValueError('only the three certified new26 MW16 curves are in scope')
    ID=identifier;D=LOCAL/('mw16-new26-'+ID+'-adaptive-v1');SEED=LOCAL/'extended20-mw16-pari-v1'/ID/'result.json';CENSUS=LOCAL/'prospective-mw16-h4096-v1'/ID.rsplit('-',1)[0]/'generic-census.json'

def sources():
    names=['memory_rank_certificate.py','followup_new_mw16_rank26.py','prepare_new_mw16_rank26_adaptive.sage','prepare_fresh_r17_pari_batch.sage','fresh_r17_pari_batch.py','prospective_half_lattice_v2.sage','research_runtime/rotated_observation_state.py','research_runtime/cached_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
...
def prepare():
    ...
    if seed['status']!='COMPLETE_DECLARED_POINT_ATTEMPT' or seed['rank_lower_bound']!=26 or proof['icarm_matches'] or proof['previous_matches'] or seed['curve']!=proof['discovery_curve']:raise ArithmeticError('certified new26 gate failed')
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.new-mw16-rank26-adaptive.v1','sources':sources(),'seed_path':str(SEED.relative_to(ROOT)),'seed_sha256':cert.hashed(SEED),'proof_sha256':cert.hashed(PROOF),'completed_mw16_gate_sha256':cert.hashed(gate),'generic_census_path':str(CENSUS.relative_to(ROOT)),'generic_census_sha256':cert.hashed(CENSUS),'generic_pool':[{'mask':r['mask'],'computed_generic_norm':r['norm']} for r in pool],'initial_rank':26,'generic_rank':16,'charts':301,'height':100000,'seconds_per_chart':10,'target_rank':28,'gp_sha256':cert.hashed(Path('/usr/bin/gp')),'geometry_wall_seconds':300,'worker_wall_seconds':3600,'rss_bytes':1610612736,'maximum_workers':1,'gate':'The fixed20 MW16 cohort completed and independently replayed all860 generic16 point boxes at100000. This curve has26 exactly independent points, global minimality and no prior-equation or catalogue match. Its ten newly discovered directions have not been used as point-search centres. Test one fixed301-centre follow-up at the unchanged100000 height; exactly three certified new26 curves are in scope, at most two concurrent workers. Earlier R17 follow-ups are finite controls, not a guarantee of success here.','centre_policy':'This is301 declared pairings, not the full generic/quotient product or all2^26 parity classes. Take301 largest computed generic census norms, mask ties. Pair them cyclically with nonzero10-bit quotient words ordered by Hamming weight then integer. Compute384-bit numerical heights on the exact26-point subgroup and numerical CVP representatives in each declared parity; order descending numerical norm, generic mask, quotient word. These representatives are search choices, not specialized lattice-minimum certificates.','boundaries':'Only points discovered on this prospective curve enter centres. Freeze all rational maps before point searching. Preserve all histories and timeout outputs; no automatic continuation or rank upper bound. Stop at the first worker lower bound28 pending exact replay.'})
...
def replay():
    ...
    for c in maps['centres']:
        if c['parity']!=c['generic_mask']|(c['quotient_word']<<16) or len(c['representative'])!=26 or any((c['representative'][j]-(c['parity']>>j))%2 for j in range(26)):raise ArithmeticError('adaptive parity differs')
...
def launch():
    p=protocol()
    for name,cmd,seconds in [('maps',['/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python',str(CAS/'prepare_new_mw16_rank26_adaptive.sage'),'--id',ID],p['geometry_wall_seconds']),('worker',[sys.executable,str(Path(__file__).resolve()),'worker','--id',ID],p['worker_wall_seconds'])]:
