...
"""Twelve exact images of the new marked native genus-one carrier."""
...
ART=ROOT/'artifacts/generated-results/elliptic-curves';D=ROOT/'artifacts/local/elliptic-curves/native-rank3-carrier-images-v1';OUT=ART/'native_rank3_carrier_images_v1.json'
HELPER=ROOT/'elkies-k3/scripts/prepare_r17_norm12_11952_alternate_v4_rank_one_bases.sage'
NAMES=['rank_jump_minimal_native_block_carrier_v1.json','rank_jump_minimal_native_block_carrier_inputs_v1.json','rank_jump_native_triple_intersection_inputs_v1.json','rank_jump_native_triple_carrier_verification_v1.json']
...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.native-rank3-carrier-images.v1','sources':sources(),'words':WORDS,'maximum_parameter_bits':512,'model_bits_gate':400,'compact_inner_height':4096,'seconds_per_stage':180,'rss_bytes':2147483648,'gate':'The independently checked marked degree4 genus-one carrier supplies two independent generic native directions beyond generic17; its Jacobian has exact rank3 but only two explicit points are recorded. Map twelve fixed words in those two points, without assuming they are a full basis. Compare model height with the current outer MW16 displayed sizes, which reach396 bits.','scope':'Exact pointed quartic inversion, simultaneous square roots, native polynomial identities, published-to-compact08234 transport and factor-free normalized integral height bounds. Record the known origin separately. Only new parameter heights beyond4096 with displayed integral models at most400 bits receive a finite19-point independence attempt through997. No additional auxiliary point search, saturation, original-curve point search, score selection, catalogue-driven word choice, retries or automatic enlargement. Failed height gates exclude these images only, never the full rank3 carrier.'})
def expected():
    p=cert.read(D/'protocol.json')
    if p['sources']!=sources() or p['words']!=WORDS:raise ArithmeticError('native carrier source binding changed')
    carrier,ci,inp,verification=[cert.read(ART/n) for n in NAMES]
    ...
    J=EllipticCurve(QQ,g['minimal_Jacobian_model']);iso=J.isomorphism_to(base);generators=[J([QQ(v) for v in P]) for P in des['points']]
...
if __name__=='__main__':
    ...
    if a.stage=='prepare':prepare()
        ...
        print('NATIVE RANK3 CARRIER IMAGES',[(r['word'],r['status'],r.get('parameter_bits'),r.get('normalized_coefficient_bits_lower_bound'),r.get('rank_lower_bound')) for r in d['rows']],flush=True)
