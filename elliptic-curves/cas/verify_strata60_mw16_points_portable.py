...
D=ROOT/'artifacts/local/elliptic-curves/strata60-mw16-point-portable-v1'
OUT=ART/'strata60_mw16_point_portable_replay_v1.json'


def main():
    path=ART/'strata60_mw16_point_evidence_v1.json';manifest=cert.read(path);workspace=D/'workspace';folder=D/'verification'
    ...
    report=cert.read(art/'retained_mw16_score_strata_experiment_v1.json')
    p=cert.read(workspace/'artifacts/local/elliptic-curves/strata60-mw16-pari-v1/protocol.json')
    ...
    for i,(case,row) in enumerate(zip(manifest['cases'],report['rows'])):
        ...
        if case['replay_point_proof']:
            ...
            verified=cert.read(workspace/'artifacts/local/elliptic-curves/strata60-mw16-pari-v1'/ident/'verification.json')
            ...
            jobs.extend([
                (ident+'-history',[str(cas/'strata60_mw16_pari_batch.py'),'replay','--index',str(i)],600),
                (ident+'-geometry',[str(cas/'verify_strata60_mw16_points.py'),'geometry-check','--index',str(i)],300),
    ...
    for name,args,seconds in jobs:
        ...
        print('ISOLATED STRATA60',name,ledger['rows'][-1]['status'],flush=True)
        if not ok:
    ...
    checkpoint(OUT,{'schema':'elliptic-curves.strata60-point-portable-replay.v1','status':'PASS',
    ...
    print('ISOLATED STRATA60',len(jobs),'PROOF STAGES PASS;',len(unresolved),'UNRESOLVED ALLOCATIONS',flush=True)
