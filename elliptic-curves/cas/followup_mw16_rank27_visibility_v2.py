...
D = LOCAL / 'mw16-rank27-visibility-followup-v2'
OLD = LOCAL / 'new27-specialized-parity-six-v1/new-20260906-90'
PROTOCOL = D / 'protocol.json'
RESULT = D / 'result.json'
MOD2 = ART / 'mw16_rank27_visibility_followup_mod2_v2.json'
ODD = ART / 'mw16_rank27_visibility_followup_modl_v2.json'
REPORT = ART / 'mw16_rank27_visibility_followup_v2.json'
...
def prepare():
    ...
    checkpoint(PROTOCOL, {
        'schema': 'elliptic-curves.mw16-rank27-visibility-followup-protocol.v2',
        'supersedes': 'v1 stopped before any point invocation: exact rational values were compared to serialized strings. v2 normalizes those types; all mathematical inputs and budgets are unchanged.',
        'failed_v1': {str(p.relative_to(ROOT)): cert.hashed(p) for p in
                      (LOCAL / 'mw16-rank27-visibility-followup-v1/protocol.json',
                       LOCAL / 'mw16-rank27-visibility-followup-v1/worker.log',
                       LOCAL / 'mw16-rank27-visibility-followup-v1/worker.supervisor.json',
...
def initial():
    ...
    if state.rank != 27 or [tuple(map(cert.F, q)) for q in state.basis] != points:
...
def report():
    ...
    return {'schema': 'elliptic-curves.mw16-rank27-visibility-followup.v2', 'status': 'PASS',
