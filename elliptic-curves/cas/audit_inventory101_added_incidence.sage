...
"""Exact twelve-presentation incidence for the new101st inventory curve."""
import argparse
...
def run(output):
    ...
    targets = [{k:r[k] for k in ('id', 'curve', 'j_invariant', 'family', 'parameter', 'rank_lower_bound')}
               for r in cert.read(source_paths[3])['curves'] if int(r['id'].split('-')[-1])>100]
    if len(targets)!=1:raise ArithmeticError('exactly one new101 inventory addition required')
    maps = []
