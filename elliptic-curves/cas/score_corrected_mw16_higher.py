...
import scan_corrected_mw16_higher_annuli as scan
...
OUT=ART/'corrected_mw16_higher_selection_v1.json'
    ...
    for band in (2,3):
        for family in sorted(families):
            ...
            for i,r in enumerate(candidates):
                row={**r,'id':f'corrected-b{band}-{family}-{i:04}','retained_index':i,'model':models.model_at(families[family],r['parameter'])}
...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.mw16-corrected-higher-scores.v1','sources':sources(),'rows':rows,'prospective_skips':skips,
        ...
        'gate':'All3059269468 primitive addresses in320 corrected-score higher slices, with one fixed benchmark reuse and319 new slices, have been scored through32749 including restored good5/13 terms before retention, with all1310720 retained scores replayed. The complete five-family cache matches all40960 earlier scalar cases. Choose1024 within-roster distinct equations per higher band/family from the fixed131072-address signed pool, giving equal scalar exposure without public-record targets.',
        'ordering':'The short score includes the exactly restored5/13 good-prime terms. Candidate1024 pools use combined S1 through32749,good count,denominator,signed numerator in band/family order; only prospective Q-isomorphism deduplication is permitted. Final six per band/family use S1 through65521 with the same tie rules. Disjoint65537..131071 validation cannot alter selection.',
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=p['maximum_workers']) as pool:
            for start in range(p['cost_gate_rows'],len(p['rows']),p['checkpoint_block']):
                ...
                if len(result['rows'])%800==20:print('MW16 CORRECTEDER TRACE PROGRESS',len(result['rows']),flush=True)
...
def check():
    ...
    print('REPLAYED ALL10240 MW16 CORRECTEDER SCORES',flush=True)
...
def validate(check=False):
    ...
    print('MW16 CORRECTEDER FINALISTS FRESH VALIDATION60 PASS',flush=True)
