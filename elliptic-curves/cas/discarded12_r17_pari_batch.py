...
import extend_r17_discarded_shard_scores as extension
...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';D=ROOT/'artifacts/local/elliptic-curves/discarded12-r17-pari-v1';PARITY=ART/'r17_exact_maximum_parity_classes_v1.json';SAGE='/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python'

def sources():
    names=['memory_rank_certificate.py','discarded12_r17_pari_batch.py','prepare_discarded12_r17_pari_batch.sage','prospective_half_lattice_v2.sage','compact_atlas_specialization.py','research_runtime/search_state.py','research_runtime/cached_observation_state.py','research_runtime/preloaded_prime_state.py','research_runtime/rotated_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
    return {**backend.sources(),**{str(p.relative_to(ROOT)):cert.hashed(p) for p in [*(CAS/n for n in names),spec.ATLAS,PARITY]}}

PARENT=ROOT/'artifacts/local/elliptic-curves/next24-r17-extended-pari-v1'
...
def expected_roster():
    ...
    for f in sorted(selected['selection']):
        ...
        if [r['retained_index'] for r in available[:2]]!=selected['selection'][f]['extended_top_two']:raise ArithmeticError('fixed selection differs')
        for r in available[:2]:rows.append({'id':f+f"-{r['retained_index']:03}",'family':f,'parameter':r['parameter'],'retained_index':r['retained_index'],'arms':['discarded_shard_top_two'],'combined_selection_units':r['combined_selection_units']})
    if len(rows)!=12 or len({r['id'] for r in rows})!=12:raise ArithmeticError('fixed12 roster differs')
    return rows

def freeze():
    if (D/'protocol.json').exists():raise FileExistsError('preserve next24 protocol')
    gate=cert.read(ART/'next24_portable_replay_v1.json')
    ...
    control=cert.read(ROOT/'artifacts/local/elliptic-curves/native11952-height125-control-v1/125000/verification.json');trace=cert.read(extension.D/'replay.supervisor.json')
    if control['status']!='PASS' or control['completed_boxes']!=49 or control['rank_lower_bound']!=28 or trace['outcome']!='completed' or trace['returncode']!=0:raise ArithmeticError('height and score replay gates incomplete')
    rows=expected_roster();masks={f['family']:[r['mask'] for r in f['classes']] for f in cert.read(PARITY)['families']}
    if sum(len(masks[r['family']]) for r in rows)!=540:raise ArithmeticError('fixed generic chart count differs')
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.discarded12-r17-pari.v1','sources':sources(),'selection_sha256':cert.hashed(extension.D/'result.json'),'prior_protocol_sha256':cert.hashed(PARENT/'protocol.json'),'prior_replay_sha256':cert.hashed(ART/'next24_portable_replay_v1.json'),'rows':rows,'generic_masks':masks,'height':125000,'seconds_per_chart':10,'admission_prime_bound':997,'stop_rank':32,'map_wall_seconds':120,'worker_wall_seconds':600,'rss_bytes':1610612736,'maximum_workers':2,'gp_sha256':cert.hashed(Path('/usr/bin/gp')),'gate':'Exactly768 previously discarded saved sign-shard finalists have completed the same extended-prime traces and exact selection replay. Their fixed top two per family now receive equally exposed generic17 point attempts. A separately completed native control certified28 at125000 compared with27 at100000 on identical49 maps, giving a finite visibility/cost gate for125000.','selection':'Use the frozen top two per family from the discarded-shard extension, ordering by selection score through32749, good count, denominator, numerator. Validation32771..65521, catalogue data, public points and measured ranks do not enter ordering. No refill or automatic continuation.','geometry':'Use all43or49 exact generic maximum parity classes. Numerical384-bit heights choose representatives only; exact rational centres, horizontal maps and point independence are replayed.','replay_change':'The final stand-alone rank proof uses the tested explicit memory cache, avoiding per-point disk fact publication. Arithmetic and certificate schema are unchanged; the original worker/checker remain frozen.','boundaries':'Exactly12 declared attempts and540 initial boxes. Every candidate starts from only its17 exact generic sections. All raw points, state archives and failures remain. Catalogue and earlier-equation comparison only after the terminal fixed batch. No exact rank, rank upper bound or universal novelty.'})
...
def replay(index):
    ...
    memory_checked_rank(model,state.basis,state.reductions.primes,state.no_two_torsion_prime);print('REPLAYED DISCARDED12 R17',p['rows'][index]['id'],len(data['charts']),'rank >=',state.rank,flush=True)
...
def batch():
    ...
    def one(index):
        row=p['rows'][index];folder=D/row['id'];mapping=supervise([SAGE,str(CAS/'prepare_discarded12_r17_pari_batch.sage'),'--index',str(index)],limits=Limits(p['map_wall_seconds'],p['rss_bytes']),log_path=folder/'maps.log',checkpoint_path=folder/'maps.supervisor.json',cwd=ROOT)
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        for future in as_completed(futures):
            i=futures[future];r=future.result();ledger['rows'][i]=r;checkpoint(path,ledger);print('DISCARDED12 R17 ATTEMPT',r['id'],r['status'],r.get('rank_lower_bound'),flush=True)
