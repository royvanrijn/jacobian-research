...
OLD=ART/'new_high_rank_curve_index_v1.json'
ADDITIONAL=ART/'prospective_mw16_next12_results_v1.json'
...
def build(output):
    ...
    cert.write(output,{'schema':'elliptic-curves.new-high-rank-index.v2','sources':sources(),
        ...
        'claim_boundary':'Exact independent-point lower bounds and absence from the pinned 584-equation catalogue. Existing local curve IDs are preserved. These are neither exact ranks nor a proof of universal novelty; no new rank-28/32 curve is claimed.'})
...
def check(path):
    ...
    if data['curves']!=expected or data['catalogue']!=previous['catalogue']:
    ...
    for r in data['curves']:
        ...
        seen.add(j)
        print('REPLAYED V2 INVENTORY',r['id'],'rank >=',len(points),flush=True)
    print('EXACT CURVE INVENTORY',len(seen),flush=True)

if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('--output',type=Path,default=ART/'new_high_rank_curve_index_v2.json');p.add_argument('--check',type=Path)
