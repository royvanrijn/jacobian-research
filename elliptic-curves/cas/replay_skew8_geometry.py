...
def main():
    ...
    for row in p['rows']:
        ...
        geometry(data,maps,initial,{**p,'maps_path':folder/'maps.json'});cloud_check(data,data['charts'],ART/('skew8_r17_'+row['id'].replace('-','_')+'_mod2_v1.json'),path);count+=len(data['charts'])
    if count>368 or sum(len(p['generic_masks'][r['family']]) for r in p['rows'])!=368:raise ArithmeticError('fixed368 maximum chart roster differs')
    print('REPLAYED SKEW8 CHARTS',count,'GENERIC CENTRES, RAW POINTS AND CLOUD PROVENANCE',flush=True)
