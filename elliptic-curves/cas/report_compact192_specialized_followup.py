#!/usr/bin/env python3
"""Bind the completed fixed own26-point follow-up without another search."""
import argparse
import json
from pathlib import Path
import certify_compact_r17_candidates as cert
import compact192_specialized_followup as batch
from research_runtime.store import checkpoint

ROOT=batch.ROOT
ART=batch.ART
CONTROL=batch.LOCAL/'compact192-specialized-followup-controller-v1'
OUT=ART/'compact192_specialized_followup_v1.json'
...
def expected():
    ...
    if len(roster)!=5 or execution['status']!='PASS' or proof['status']!='PASS' or [r['id'] for r in execution['rows']]!=roster or [r['id'] for r in proof['rows']]!=roster:raise ArithmeticError('all five follow-up attempts and clouds required')
    if [r['id'] for r in execution['maps']]!=roster or any(r['status']!='PASS' for r in execution['maps']):raise ArithmeticError('all five prior geometry stages required')
    rows=[]
    for r,worker,check in zip(p['rows'],execution['rows'],proof['rows']):
        ...
        if cloud['rank_lower_bound']!=26 or bounds!={'3':26,'5':26} or check['rank_lower_bound']!=26 or bounds!=check['odd_modulus_lower_bounds']:raise ArithmeticError('observed matching26 bounds differ')
        rows.append({'id':r['id'],'family':r['family'],'parameter':r['parameter'],'initial_rank':r['initial_rank'],'rank_lower_bound':26,'odd_modulus_lower_bounds':bounds,'completed_boxes':49,'retained_points':len(cloud['points'])})
    return {'schema':'elliptic-curves.compact192-specialized-followup-result.v1','status':'PASS','sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in sorted(paths)},'rows':rows,'declared_point_boxes':245,'completed_point_boxes':245,'total_retained_point_witnesses':sum(r['retained_points'] for r in rows),'claim_boundary':'One complete245-box specialized-parity follow-up of all five new compact192 curves with26 certified points. All2048-mask samples and49-map rosters precede points; exact histories, rational geometry and full-cloud proofs modulo2,3,5 pass. Every retained cloud still certifies26, giving no rank gain or new inventory entry. These are lower bounds, not exact ranks, upper bounds, saturation, covering optimality, point absence or a universal sensitivity claim.'}
...
if __name__=='__main__':
    ...
    if args.check:
        ...
        checkpoint(OUT,d)
    print('FIVE26 FOLLOWUP: ALL245 BOXES PASS;',d['total_retained_point_witnesses'],'WITNESSES; NO BOUND GAIN',flush=True)
