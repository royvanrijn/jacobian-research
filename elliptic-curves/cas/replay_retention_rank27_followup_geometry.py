...
def main():
    ...
    for identifier in IDS:
        ...
        words=sorted(range(1,1024),key=lambda w:(w.bit_count(),w));expected={(r['mask'],words[i%len(words)]) for i,r in enumerate(pool)}
        ...
        for c in maps['centres']:
            if c['parity']!=c['generic_mask']|(c['quotient_word']<<17) or len(c['representative'])!=27 or any((c['representative'][j]-(c['parity']>>j))%2 for j in range(27)):raise ArithmeticError('exact centre parity differs')
        ...
        cloud_check(data,seed['charts']+data['charts'],follow.ART/('retention_rank27_'+identifier.replace('-','_')+'_all_retained_mod2_v1.json'),D/'all-retained-point-cloud-only.json')
    ...
    print('EXACT RETENTION RANK27 FOLLOWUP GEOMETRY AND CLOUD PROVENANCE',count,'charts',flush=True)
