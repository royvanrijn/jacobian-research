...
"""Freeze301 exact PARI maps using only the newly discovered27-point subgroup."""
import sys,argparse
...
def main():
    ...
    model=tuple(map(cert.F,seed['curve']));points=tuple(tuple(map(cert.F,P)) for P in seed['final_state']['state']['reductions']['points']);gram,asymmetry=mapper.geometry.canonical_height_gram(model,points);oracle=mapper.geometry.CosetOracle(mapper.geometry.rounded_gram(gram,1000000));words=[1024+w for w in sorted(range(1024),key=lambda w:(w.bit_count(),w))];centres=[]
    for i,g in enumerate(p['generic_pool']):
        word=words[i%len(words)];mask=g['mask']|(word<<16);residue=[(mask>>j)&1 for j in range(27)];norm,rep,error=oracle.solve(residue)
        if len(rep)!=27 or any((rep[j]-residue[j])%2 for j in range(27)):raise ArithmeticError('adaptive parity mismatch')
    ...
    data['status']='COMPLETE_DECLARED_MAPS';checkpoint(out,data);print('FROZEN NEW27 ADAPTIVE MAPS',len(data['rows']),flush=True)
