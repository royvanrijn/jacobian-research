#!/usr/bin/env python3
"""Replay every terminal wide-R17 chart prefix and complete recorded point cloud."""
import argparse
...
def verify(directory):
    ...
    if ledger['status'] != 'COMPLETE_FIXED_BATCH_ATTEMPTS' or len(ledger['rows']) != 24:
    ...
    checkpoint(folder/'protocol.json', {'point_ledger_sha256': cert.hashed(directory/'point-ledger.json'),
        'sources': {str(p.relative_to(ROOT)): cert.hashed(p) for p in (Path(__file__).resolve(),
            ROOT/'elliptic-curves/cas/replay_compact_r17_wide.py', ROOT/'elliptic-curves/cas/audit_recorded_point_mod2_rank_v2.py')},
        'workers': 4, 'chart_replay_wall_seconds': 600, 'cloud_build_wall_seconds': 300,
    ...
    def worker(row):
        ...
        artifact = ART/f"compact_r17_wide_recorded_mod2_{row['family']}_{row['index']:02}_v1.json"
        stages = [('charts', [sys.executable, str(ROOT/'elliptic-curves/cas/replay_compact_r17_wide.py'), str(path)], 600),
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        for future in as_completed([pool.submit(worker, row) for row in ledger['rows']]):
            ...
            checkpoint(folder/'ledger.json', result)
            print('VERIFIED WIDE R17 ATTEMPT', row['family'], row['index'], row['status'], flush=True)
