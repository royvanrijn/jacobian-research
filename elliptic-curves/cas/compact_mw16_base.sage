...
DIRECTORY = ROOT/'artifacts/local/elliptic-curves/compact-mw16-base-v1'
...
def prepare(directory):
    ...
    checkpoint(directory/'protocol.json', {
        'schema': 'elliptic-curves.compact-mw16-base.v1', 'sources': sources(),
        ...
        'composite_factor_bit_gate': 160, 'prime_root_bit_gate': 512,
        ...
        'method': 'Weighted constant scaling; exact perfect-power extraction from gcd of primitive A/B discriminants and resultant; bounded proved factorization; prime-local auxiliary minimization and reduction; exact weighted coefficient identities.',
...
def run(directory, presentation):
    ...
    if root == 1:
        factors = []
    elif root.nbits() <= protocol['prime_root_bit_gate'] and root.is_prime(proof=True):
        factors = [(root, 1)]
    elif root.nbits() <= protocol['composite_factor_bit_gate']:
        factors = list(root.factor(proof=True))
    else:
        raise ArithmeticError('composite power root exceeds frozen factor gate')
    restored = ZZ(1)
    for p, e in factors:
