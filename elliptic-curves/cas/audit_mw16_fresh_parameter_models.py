...
P=ROOT/'artifacts/local/elliptic-curves/mw16-fresh-retention-scores-v1/protocol.json'
SCAN=ROOT/'artifacts/local/elliptic-curves/mw16-fresh-outer-retention-v1/result.json'
OUT=ART/'mw16_fresh_parameter_models_v1.json'
BANDS={1:(4096,16384)}
...
def expected():
    ...
    if len(p['rows'])!=40960 or scan['status']!='COMPLETE_FIXED_STRATIFIED_POPULATION':raise ArithmeticError('fixed retained population required')
    ...
    if len(source)!=40960:raise ArithmeticError('duplicate retained address')
    ...
    if len(records)!=5 or any(r['curves']!=8192 or r['positive']!=4096 or r['negative']!=4096 for r in records):raise ArithmeticError('balanced signed band/family counts differ')
...
if __name__=='__main__':
    ...
    if a.check:
        ...
        checkpoint(OUT,d)
    print('FRESH OUTER MODEL AUDIT',d['curves_checked'],d['distinct_j_invariants'],d['distinct_Q_isomorphism_classes'],'PASS',flush=True)
