...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';LOCAL=ROOT/'artifacts/local/elliptic-curves';PROOF=ART/'higher24_rank25_minimal_proof_v1.json'

def configure(identifier):
    global ID,D,SEED,CENSUS
    if identifier!='11952-069':raise ValueError('only the unique fixed higher24>=25 result is in scope')
    ID=identifier;D=LOCAL/('higher-rank25-'+ID+'-adaptive-v1');SEED=LOCAL/'higher24-r17-pari-v1'/ID/'result.json';CENSUS=LOCAL/'compact-r17-fresh-generic-census-v1'/ID.split('-')[0]/'generic-census.json'

def sources():
    names=['memory_rank_certificate.py','followup_higher_rank25.py','prepare_higher_rank25_adaptive.sage','prepare_fresh_r17_pari_batch.sage','fresh_r17_pari_batch.py','prospective_half_lattice_v2.sage','research_runtime/rotated_observation_state.py','research_runtime/cached_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
...
def launch():
    p=protocol()
    for name,cmd,seconds in [('maps',['/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python',str(CAS/'prepare_higher_rank25_adaptive.sage'),'--id',ID],p['geometry_wall_seconds']),('worker',[sys.executable,str(Path(__file__).resolve()),'worker','--id',ID],p['worker_wall_seconds'])]:
