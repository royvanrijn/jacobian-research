#!/usr/bin/env python3
"""Bind completed retained outer26 exposures, proofs and measured cost."""
import argparse
from pathlib import Path
import recent_outer26_followup as batch
from research_runtime.store import checkpoint
ROOT,ART=batch.ROOT,batch.ART
OUT=ART/'recent_outer26_followup_v1.json'
...
def expected():
    ...
    for metadata,execution,verified in zip(p['rows'],run['rows'],audit['rows']):
        ...
        assert raw['status']=='COMPLETE_DECLARED_ADAPTIVE_ATTEMPT' and raw['initial_dimension']==26
        ...
        rows.append({'id':ident,'family':metadata['family'],'parameter':metadata['parameter'],
            'initial_rank_lower_bound':26,'rank_lower_bound':cloud['rank_lower_bound'],
    ...
    return {'schema':'elliptic-curves.recent-outer26-followup-result.v1','status':'PASS_VERIFIED_DECLARED_ATTEMPTS',
        ...
        'total_supervised_seconds':total,'shared_geometry_replay_seconds':geo['wall_seconds'],
        'certified_added_directions':sum(r['rank_lower_bound']-26 for r in rows),
        'boundary':'Two fixed retained outer26 curves, both complete maps before points,49 declared125000-height ten-second charts each, no rank stopping. Full histories, clouds and rational geometry replay. Lower bounds only; a null result is not exact rank, saturation, absence or a sensitivity theorem. No new parameter scan or automatic next wave.'}
