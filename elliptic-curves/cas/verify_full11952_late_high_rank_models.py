...
D=ROOT/'artifacts/local/elliptic-curves/full11952-late-high-rank-models-portable-v1'
OUT=ART/'full11952_late_high_rank_models_portable_replay_v1.json'


def main():
    path=ART/'full11952_late_high_rank_models_evidence_v1.json';manifest=cert.read(path);workspace=D/'workspace';folder=D/'verification'
    ...
    jobs=[('models',sys.executable,[str(workspace/'elliptic-curves/cas/export_full11952_late_high_rank_models.py'),'--check']),
          ('sage-export','/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python',[str(workspace/'artifacts/generated-results/elliptic-curves/new_full11952_late_high_rank_curves.sage')])]
    ...
    checkpoint(OUT,{'schema':'elliptic-curves.full11952-late-high-rank-models-portable-replay.v1','status':'PASS','logical_stages':2,'curves_checked':len(cert.read(ART/'full11952_late_high_rank_models_v1.json')['curves']),'independent_point_witnesses':sum(r['rank_lower_bound'] for r in cert.read(ART/'full11952_late_high_rank_models_v1.json')['curves']),'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},'archive_sha256':manifest['archive_sha256'],'ledger':ledger,'claim_boundary':manifest['claim_boundary']})
