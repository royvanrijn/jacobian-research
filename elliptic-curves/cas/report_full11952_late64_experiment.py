#!/usr/bin/env python3
"""Bind the completed second-band selection and fixed64 point outcomes."""
import argparse,json
from pathlib import Path
import certify_compact_r17_candidates as cert
import full11952_late64_r17_pari_batch as batch
from research_runtime.store import checkpoint
ROOT=batch.ROOT;ART=batch.ART;D=batch.D;CONTROL=ROOT/'artifacts/local/elliptic-curves/full11952-late64-controller-v1'
INPUT=ART/'full11952_late64_r17_results_v1.json';OUT=ART/'full11952_late64_experiment_v1.json'
def expected():
    p=batch.protocol();ledger=cert.read(CONTROL/'ledger.json');source=cert.read(INPUT);odd=cert.read(D/'odd-cloud-audit/ledger.json')
    if ledger['status']!='PASS' or odd['status']!='PASS' or len(source['curves'])!=64 or len(odd['rows'])!=64:raise ArithmeticError('all selection, maps, points, histories and clouds required')
    required=['selection-replay','freeze','maps','stream-freeze','stream','points','odd','geometry']
    ...
    for r,o in zip(source['curves'],odd['rows']):
        ...
        rows.append({'id':r['id'],'parameter':r['parameter'],'rank_lower_bound':r['rank_lower_bound'],
    ...
    return {'schema':'elliptic-curves.full11952-late64-experiment.v1','status':'PASS_STRONGER_ODD_BOUNDS_REQUIRE_REVIEW' if stronger else 'PASS',
        ...
        'attempted_charts':sum(r['attempted_charts'] for r in rows),'maximum_declared_boxes':3136,
        'retained_point_witnesses':sum(r['retained_points'] for r in rows),
        'claim_boundary':'A finite second-prime-band selection experiment on4096 fresh equations from the existing retained population, followed by64 fixed generic17 point searches and exact complete-cloud proofs. All853 prior cohort and593 pinned catalogue equations were excluded before the4096 roster. The wholly disjoint final validation band does not alter selection or maps. Matching finite bounds do not prove exact rank, saturation or absence. No rank-density, selector sensitivity, universal novelty or isolated portability claim is made here.'}
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--check',action='store_true');a=p.parse_args();d=expected()
    if a.check:
        if cert.read(OUT)!=json.loads(json.dumps(d)):raise ArithmeticError('late64 aggregate differs')
    else:
        if OUT.exists():raise FileExistsError('preserve late64 aggregate')
        checkpoint(OUT,d)
    print('LATE64 OUTCOMES',d['status'],d['completed_point_boxes'],[(r['id'],r['rank_lower_bound']) for r in d['rows'] if r['rank_lower_bound']>=22],flush=True)
