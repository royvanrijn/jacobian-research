...
index enlargement restarts at weight one in the enlarged lattice. Rank-changing
and saturation-only restarts have separate frozen budgets, so saturation cannot
consume the rank-growth allowance. A miss at a sparse stage is never reported
as a rank upper bound.
"""
...
RESTART_AMENDMENT = ART / "curve385_sparse_restart_budget_v2.json"
...
from curve385_sparse_restart_policy import (  # noqa: E402
    POLICY_ID as RESTART_POLICY_ID,
    charge as charge_restart,
...
def load_protocol() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    # Protocol v1 pins the historical runner.  The v2 amendment below pins
    ...
    if amendment.get("status") != "FROZEN_INDEPENDENT_RESTART_BUDGETS_BEFORE_FUTURE_SEARCH":
        raise ArithmeticError("the restart-budget amendment is not frozen")
    definition = amendment.get("definition", {})
    if policy_hash(definition) != amendment.get("definition_sha256"):
        raise ArithmeticError("the restart-budget amendment hash is invalid")
    if definition.get("amends_protocol_sha256") != digest(PROTOCOL):
        raise ArithmeticError("the restart amendment identifies another protocol")
    if definition.get("replacement", {}).get("policy_id") != RESTART_POLICY_ID:
...
def initial_state(protocol: dict[str, Any], args) -> tuple[
    ...
    payload = {
        ...
        "restart_budget_amendment": {
            "path": relative(RESTART_AMENDMENT),
            "whole_file_sha256": digest(RESTART_AMENDMENT),
            "definition_sha256": args.restart_amendment["definition_sha256"],
        },
        "configuration": {
            ...
            "maximum_rank_changing_restarts": args.max_rank_changing_restarts,
            "maximum_saturation_only_restarts": args.max_saturation_only_restarts,
        ...
        "restart_accounting": {
            ...
            "saturation_only_group_change_count": 0,
        },
        "generation": {
        ...
        "claim_boundary": [
            *protocol["claim_boundary"],
            (
                "Rank-changing and saturation-only group changes consume separate "
                "v2 restart budgets; no combined lattice-state cap is used."
            ),
        ],
    }
...
def resumed_state(protocol: dict[str, Any], args) -> tuple[
    ...
    if payload.get("restart_budget_amendment") != expected_amendment:
        raise ArithmeticError("the checkpoint uses a different restart-budget amendment")
    ...
    try:
        validate_accounting(payload.get("restart_accounting", {}))
    except ValueError as exc:
        raise ArithmeticError(f"invalid v2 restart accounting: {exc}") from exc
...
def main() -> None:
    ...
    parser.add_argument("--max-rank-changing-restarts", type=int)
    parser.add_argument("--max-saturation-only-restarts", type=int)
    parser.add_argument("--max-lattice-states", type=int, help=argparse.SUPPRESS)
    ...
    if args.max_lattice_states is not None:
        raise SystemExit(
            "the combined --max-lattice-states budget is retired; use separate "
            "--max-rank-changing-restarts and --max-saturation-only-restarts"
        )
    protocol, restart_amendment = load_protocol()
    args.restart_amendment = restart_amendment
    default_budgets = restart_amendment["definition"]["replacement"]["budgets"]
    if args.max_rank_changing_restarts is None:
        args.max_rank_changing_restarts = default_budgets[
            "maximum_rank_changing_restarts"
        ]
    if args.max_saturation_only_restarts is None:
        args.max_saturation_only_restarts = default_budgets[
            "maximum_saturation_only_restarts"
        ]
    if (
        args.max_rank_changing_restarts < 0
        or args.max_saturation_only_restarts < 0
    ...
    while True:
        ...
        if saturation["status"] != "PASS_BASIS_EQUALS_DISCOVERED_GROUP":
            ...
            checkpoint_state(args, payload, basis, discoveries, searched_keys)
            return
        restart_kind = classify_group_change(
            rank_before=active_state["basis_rank"],
            rank_after=len(basis),
            basis_before_sha256=old_basis_hash,
            basis_after_sha256=active_stage["basis_after_sha256"],
            finite_index_saturation_event_count=active_stage[
                "finite_index_saturation_event_count"
            ],
        )
        active_stage["restart_kind"] = restart_kind
        if active_stage["group_changed"] != (restart_kind is not None):
            raise ArithmeticError("group-change flag and restart classification disagree")
        if restart_kind is not None:
            payload["restart_accounting"] = charge_restart(
                payload["restart_accounting"], restart_kind
            )
        ...
        exceeded = exceeded_budget(
            payload["restart_accounting"],
            maximum_rank_changing_restarts=args.max_rank_changing_restarts,
            maximum_saturation_only_restarts=args.max_saturation_only_restarts,
        )
        if exceeded is not None:
            active_state["status"] = "RESTART_BUDGET_EXCEEDED"
            payload["status"] = f"STOPPED_AT_{exceeded}_RESTART_LIMIT"
            payload["stop"] = {
                "basis_rank": len(basis),
                "exceeded_restart_category": exceeded,
                "restart_accounting": payload["restart_accounting"],
                "no_rank_upper_bound_claimed": True,
            }
            checkpoint_state(args, payload, basis, discoveries, searched_keys)
            return
