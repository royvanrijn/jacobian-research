...
ROOT=engine.ROOT;CAS=engine.CAS;ART=engine.ART;LOCAL=ROOT/'artifacts/local/elliptic-curves';D=LOCAL/'outer131072-r17-stratified-v2';BINARY=engine.D/'scanner-v2';REFERENCE=engine.old.BINARY;SALT='elliptic-stratified-height-131072-v1'
def sources():
    return {**engine.sources(),str(Path(__file__).resolve().relative_to(ROOT)):cert.hashed(Path(__file__).resolve()),str(Path(strict.__file__).resolve().relative_to(ROOT)):cert.hashed(Path(strict.__file__).resolve()),str(BINARY.relative_to(ROOT)):cert.hashed(BINARY),str(REFERENCE.relative_to(ROOT)):cert.hashed(REFERENCE),str((CAS/'scan_outer131072_r17.py').relative_to(ROOT)):cert.hashed(CAS/'scan_outer131072_r17.py')}
def roster():
