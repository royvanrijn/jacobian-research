...
import higher26_million_height_pilot as million
from replay_retention24_geometry import geometry,cloud_check,tuples
from research_runtime.store import checkpoint
from research_runtime.supervisor import run,Limits
ROOT=million.ROOT;ART=million.ART;CAS=million.CAS

def inputs(kind,index):
    module=million;p=module.protocol();row=p['rows'][index];folder=module.D/row['id'];path=folder/'result.json';data=cert.read(path)
    if cert.read(module.D/'ledger.json')['status']!='PASS':raise ArithmeticError('terminal history replay required')
    paths=[ROOT/'artifacts/local/elliptic-curves/higher-rank26-11952-069-newdirection-v2/all-retained-point-cloud-only.json']
    ...
    mp=ROOT/row['maps_path'];maps=cert.read(mp);initial=tuples(data['initial_state']['state']['reductions']['points'])
...
def main(kind,index,check):
    p,row,folder,data,paths,charts,sources=inputs(kind,index);stem='higher26_million_'+row['id'].replace('-','_');out=folder/'all-retained-point-cloud-only.json';mod2=ART/(stem+'_all_retained_mod2_v1.json');modl=ART/(stem+'_all_retained_modl_v1.json');summary=ART/(stem+'_coverage_v1.json')
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--kind',choices=['million'],required=True);p.add_argument('--index',type=int,default=0);p.add_argument('--check',action='store_true');a=p.parse_args();main(a.kind,a.index,a.check)
