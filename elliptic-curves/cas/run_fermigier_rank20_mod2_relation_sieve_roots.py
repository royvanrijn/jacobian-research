...
def main():
    ...
    # Scan near the three real roots of f. Around m=0, |f(m)| is essentially
    # the 158-bit constant term; near a real root, nearest-integer values are
    # typically around 110--115 bits, vastly improving smoothness probability.
    RF = RealField(200)
    roots = sorted(r for r, mult in f.change_ring(RF).roots())
    centers = [ZZ(r.round()) for r in roots]
    print(
        f"{PROTOCOL}|stage=root_windows|roots={roots}|centers={centers}"
        f"|half_width={args.m_bound}",
        flush=True,
    )

    total = len(centers) * (2 * args.m_bound + 1)
    n = 0
    for window_id, center in enumerate(centers):
        lo = int(center - args.m_bound)
        hi = int(center + args.m_bound)
        for m0 in range(lo, hi + 1):
            n += 1
            m = ZZ(m0)
            N = abs(m**3 - A*m + C)
            if N == 0:
            ...
            if args.progress_every and n % args.progress_every == 0:
                ...
                for i in s_cols:
                    insert_row(kill, 1 << i)
                print(
                    f"{PROTOCOL}|stage=progress|window={window_id}"
                    f"|processed={n}/{total}|m={m0}|norm_bits={N.nbits()}"
                    f"|rank={len(pivots)}|after_killing_S={len(fb)-len(kill)}"
                    f"|full={full}|partial_seen={partial_seen}|matched={matched}"
