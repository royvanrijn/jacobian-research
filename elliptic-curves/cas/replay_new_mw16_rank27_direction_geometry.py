...
def main():
    ...
    for identifier in IDS:
        ...
        words=[1024+w for w in sorted(range(1024),key=lambda w:(w.bit_count(),w))];expected={(r['mask'],words[i%len(words)]) for i,r in enumerate(pool)}
        ...
        for c in maps['centres']:
            if not (c['parity']>>26)&1:raise ArithmeticError('new direction must be odd')
            if c['parity']!=c['generic_mask']|(c['quotient_word']<<16) or len(c['representative'])!=27 or any((c['representative'][j]-(c['parity']>>j))%2 for j in range(27)):raise ArithmeticError('exact centre parity differs')
        if len(initial)!=27 or tuples(data['initial_state']['state']['reductions']['points'])!=initial or data['protocol_hash']!=digest(p) or maps['protocol_hash']!=digest(p):raise ArithmeticError('initial subgroup/protocol differs')
        geometry(data,maps,initial,{**p,'maps_path':D/'maps.json'})
        prior=cert.read(follow.SEED.parent/'all-retained-point-cloud-only.json')
        cloud_check(data,prior['charts']+data['charts'],follow.ART/('mw16_new27_direction_'+identifier.replace('-','_')+'_all_retained_mod2_v1.json'),D/'all-retained-point-cloud-only.json')
    ...
    print('EXACT NEW MW16 FOLLOWUP GEOMETRY AND CLOUD PROVENANCE',count,'charts',flush=True)
