#!/usr/bin/env python3
"""Exact rank certificate for the rank-21-family lead ``T=531/2``.

This independently replays the uniform height-1,000,000 quartic search,
checks and maps every point exactly, and reconstructs the 114-point Jacobian
pool.  Two height-matrix precisions select seventeen candidates but do not
certify them.  Small-prime saturation followed by exact finite reductions
provides the unconditional rank lower bound.  PARI independently recomputes
the minimal model, conductor, and root number.
"""
...
EXPECTED_SIGNED_POINTS = 228
EXPECTED_NEW_IMAGES = 102
EXPECTED_POOL_SIZE = 114
EXPECTED_HEIGHT_SUBSET = tuple(range(1, 12)) + (13, 14, 15, 16, 17, 19)
EXPECTED_CONDUCTOR = 17363106716312881727260568367360064041745970511465978238830
TARGET_RANK = 21
REPRODUCING_COMMAND = (
    "PYTHONPATH=elliptic-curves/cas .venv/bin/python "
    "elliptic-curves/cas/verify_nagao_rank21_t531.py"
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=(
            root
            / "artifacts/generated-results/elliptic_nagao_rank21_t531_rank17_certificate.json"
        ),
    )
    return parser
...
def main() -> None:
    ...
    if len(saturated_basis) != 17 or any(
    ...
    if conductor["conductor"] != EXPECTED_CONDUCTOR:
    ...
    artifact: dict[str, Any] = {
        ...
        "theorem": (
            "the Nagao rank-21-family specialization T=531/2 has "
