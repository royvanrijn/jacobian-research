...
def replay(path, output):
    ...
    original = cert.read(ROOT/'artifacts/generated-results/elliptic-curves/new_high_rank_curve_index_v7.json')
    if data['targets'] != [{k:r[k] for k in ('id','curve','j_invariant','family','parameter','rank_lower_bound')} for r in original['curves'] if int(r['id'].split('-')[-1])>70]:
    ...
    if len(maps) != 12 or not 1<=len(targets)<=24 or len(data['pairs']) != 12*len(targets) or {(r['target'],r['family']) for r in data['pairs']} != {(i,f) for i in targets for f in maps}:
