#!/usr/bin/env python3
"""Exact integral models and point transports for unmatched MW16 follow-up curves>=27."""
import argparse
from pathlib import Path
import certify_compact_r17_candidates as cert
import certify_discarded_rank26_minimal as arithmetic
from research_runtime.store import checkpoint
ROOT=Path(__file__).resolve().parents[2];ART=ROOT/'artifacts/generated-results/elliptic-curves';INPUT=ART/'new_mw16_followup_results_v1.json';OUT=ART/'new_mw16_rank27_minimal_proof_v1.json'
def sources():
    return {**arithmetic.sources(),str(Path(__file__).resolve().relative_to(ROOT)):cert.hashed(Path(__file__).resolve())}
def expected():
    d=cert.read(INPUT);selected=[r for r in d['curves'] if r['rank_lower_bound']>=27 and not r['icarm_matches'] and not r['previous_matches']];rows=[]
    if not selected:raise ArithmeticError('no unmatched>=27 cohort')
    for q in selected:
    ...
    return {'schema':'elliptic-curves.new-mw16-rank27-minimal-minimal.v1','status':'PASS','sources':sources(),'input_sha256':cert.hashed(INPUT),'curves':rows,'catalogue':d['catalogue'],'previous_equations':d['previous_equations'],'claim_boundary':'Exact independent-point lower bounds, proved global minimal integral models and exact point transports for the unmatched>=27 portion of the fixed three-curve follow-up cohort. Absence from593 pinned catalogue equations and382 earlier measured equations is not universal novelty. No exact rank or conductor is asserted.'}
if __name__=='__main__':
    ...
    if a.check:
        ...
        checkpoint(OUT,r)
    print('EXACT NEW MW16 HIGH-RANK MINIMAL PROOFS',[(q['id'],q['rank_lower_bound'],q['minimality']['invariant_gcd']) for q in r['curves']],flush=True)
