...
ROOT=batch.ROOT;ART=batch.ART;D=batch.D;OUT=ART/'compact192_r17_results_v1.json';DATABASE=ROOT/'artifacts/local/elliptic-curves/retention24-current-catalogue-v1/database.json';OLD=ART/'skew8_r17_results_v1.json';ENDPOINTS=ART/'compact_atlas_endpoints_v2.json'
...
def sources():
    return {str(p.relative_to(ROOT)):cert.hashed(p) for p in (Path(__file__).resolve(),ENDPOINTS,Path(cert.__file__).resolve(),Path(spec.__file__).resolve(),spec.ATLAS,ROOT/'elliptic-curves/cas/memory_rank_certificate.py',ROOT/'elliptic-curves/cas/mod2_reduction_independence.py',ROOT/'elliptic-curves/cas/elliptic_candidate_record.py')}

def build():
    if OUT.exists():raise FileExistsError('preserve compact192 certificate')
    ...
    projection=[{'id':r['id'],'ainvs':r['ainvs']} for r in cert.read(DATABASE)['curves']];old=cert.read(OLD);previous=old['previous_equations']+[{'address':OLD.name+':'+r['family']+':'+r['parameter'],'curve':r['curve']} for r in old['curves']]+[{'address':'endpoint:'+r['family']+':'+r['endpoint'],'curve':r['curve']} for r in cert.read(ENDPOINTS)['rows'] if r['status']=='CERTIFIED_SPECIALIZED_SUBGROUP'];families={f['family']:f for f in cert.read(spec.ATLAS)['families']};rows=[]
    for entry in ledger['rows']:
        ...
        model=tuple(map(cert.F,data['curve']));row={'id':entry['id'],'arms':entry['arms'],'retained_index':entry['retained_index'],'family':data['family'],'parameter':data['parameter'],'curve':data['curve'],'points':cloud['independent_points'],'generic_points':data['generic_points'],'family_to_curve_scale_u':data['family_to_curve_scale_u'],'rank_certificate':cloud['rank_certificate'],'rank_lower_bound':cloud['rank_lower_bound'],'icarm_matches':[r['id'] for r in projection if cert.isomorphic(model,r['ainvs'])],'previous_matches':[r['address'] for r in previous if cert.isomorphic(model,r['curve'])],'attempted_charts':len(data['charts']),'declared_charts':len(data['centres']),'completed_boxes':sum(c['search']['status']=='bounded_search_complete' for c in data['charts']),'search_status':data['status'],'supervision_status':entry['status'],'discovery_witness':{'path':str(path.relative_to(ROOT)),'sha256':cert.hashed(path)},'complete_cloud_certificate':{'path':str(cloud_path.relative_to(ROOT)),'sha256':cert.hashed(cloud_path)}};verify(row,families,projection,previous);rows.append(row);print('CERTIFIED COMPACT192 R17',row['id'],row['parameter'],'rank >=',row['rank_lower_bound'],'catalogue',row['icarm_matches'],'previous',len(row['previous_matches']),flush=True)
    pairs=[[j,i] for i,r in enumerate(rows) for j,s in enumerate(rows[:i]) if cert.isomorphic(r['curve'],s['curve'])]
    cert.write(OUT,{'schema':'elliptic-curves.compact192-r17-results.v1','sources':sources(),'curves':rows,'within_batch_isomorphic_pairs':pairs,'point_protocol_sha256':cert.hashed(D/'protocol.json'),'point_ledger_sha256':cert.hashed(D/'ledger.json'),'verification_ledger_sha256':cert.hashed(D/'verification-ledger.json'),'previous_equations':previous,'previous_source':{'path':str(OLD.relative_to(ROOT)),'sha256':cert.hashed(OLD)},'catalogue':{'url':'https://elliptic-rank.icarm.cloud/database.json','raw_sha256':cert.hashed(DATABASE),'curve_count':len(projection),'equations':projection,'acknowledgement':'ICARM, supported by NSF Grant DMS2425401'},'claim_boundary':'Exact point lower bounds for the frozen compact192 candidate cohort. Q-isomorphism and catalogue comparison occurs only after all attempts/replays. No exact rank, global minimality, exact conductor or universal novelty is inferred. Validation-prime enrichment and discovery yield are finite experimental outcomes.'})

def check():
    d=cert.read(OUT)
    if d['sources']!=sources():raise ArithmeticError('compact192 certificate source differs')
    ...
    print('REPLAYED COMPACT192 R17 CERTIFICATES',len(d['curves']),flush=True)
