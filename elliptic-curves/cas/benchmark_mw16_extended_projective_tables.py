#!/usr/bin/env python3
"""Balanced fifteen-table cost gate for longer scoring before MW16 retention."""
import argparse
import time
...
D = ROOT / 'artifacts/local/elliptic-curves/mw16-extended-projective-benchmark-v1'
OUT = ART / 'mw16_extended_projective_benchmark_v1.json'
...
def prepare():
    ...
    for row in cert.read(spec.ATLAS)['families']:
        ...
        families.append({'family':row['fibration_id'], 'model':model, 'model_hash':digest(model)})
    ...
    checkpoint(D/'protocol.json', {
        'schema':'elliptic-curves.mw16-extended-projective-benchmark.v1',
        ...
        'gate':'Deeper retention in fresh outer MW16 slices has recovered strong independent-point subgroups beyond the former signed512 prefix. A separate11952 audit measures substantial loss when short scores are truncated before longer scoring. Test whether projective trace tables could remove the same software selection bottleneck on all five MW16 families, with equal cost exposure and no record targets. A projective table is independent of rational parameter height and therefore can serve new outer territory.',
        'scope':'Exactly three predetermined prime tables per family, all five families equally represented; compute every projective residue, including infinity. One worker,20 seconds per case and300 seconds total; preserve failure without retry. Verify complete framing, singularity markers and Hasse bounds, plus five independent character sums per table. No catalogue, known record equation/parameter/point/rank/j-invariant or jump label is read. No full cache, new parameter sweep or point search is authorized by this benchmark. Runtime extrapolations are descriptive, not mathematical bounds.'})
    ...
    try:
        for f in p['families']:
            for prime in p['primes']:
                ...
                print('MW16 PROJECTIVE TABLE',f['family'],prime,row['wall_seconds'],'PASS',flush=True)
        ...
        artifact = {'schema':'elliptic-curves.mw16-extended-projective-benchmark-result.v1',**data,'sources':sources(),'claim_boundary':p['scope']}
        if check:
