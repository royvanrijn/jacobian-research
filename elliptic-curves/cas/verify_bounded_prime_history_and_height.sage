 ...
 for row in p['rows']:
  ...
  for i,m in enumerate(maps['rows']):
   ...
   for (prior_maps,H),record in zip(prior,m['history_comparison']['comparisons']):
    ...
    T=matrix(QQ,2,list(map(QQ,oldmap['matrix']))).inverse()*phi;flat=primitive(list(T));assert flat==record['transition'] and H==record['height'];T=matrix(ZZ,2,flat);transitions.append((T,H))
