...
OLD=ART/'new_high_rank_curve_index_v12.json'
ADDITIONAL=ART/'compact192_r17_results_v1.json'

def sources():
    paths=(Path(__file__).resolve(),Path(cert.__file__).resolve(),ROOT/'elliptic-curves/cas/memory_rank_certificate.py',ROOT/'elliptic-curves/cas/mod2_reduction_independence.py',ROOT/'elliptic-curves/cas/elliptic_candidate_record.py')
    return {str(p.relative_to(ROOT)):cert.hashed(p) for p in paths}

def promotion_gate():
    base=ROOT/'artifacts/local/elliptic-curves/compact192-r17-pari-v1'
    protocol=cert.read(base/'post-batch/protocol.json')
    if any(cert.hashed(ROOT/n)!=h for n,h in protocol['sources'].items()):
        raise ArithmeticError('fixed compact192 finalization sources differ')
    ledger=cert.read(base/'post-batch/ledger.json')
    stages=['certify','proof-replay','geometry','odd-clouds','aggregate-build','aggregate-check']
    if ledger['status']!='PASS' or ledger.get('stronger_odd_prime_bounds')!=[] or [r['name'] for r in ledger['rows']]!=stages:
        raise ArithmeticError('complete compact192 proof and review of any stronger odd bounds required')
    ...
    aggregate=cert.read(ART/'compact192_experiment_v1.json')
    if aggregate['status']!='PASS' or len(aggregate['rows'])!=192:
        raise ArithmeticError('complete192 aggregate required')
    if any(cert.hashed(ROOT/n)!=h for n,h in aggregate['sources'].items()):
        raise ArithmeticError('compact192 aggregate inputs changed')
    incoming=cert.read(ADDITIONAL)
    if incoming['within_batch_isomorphic_pairs'] or len(incoming['curves'])!=192:
        raise ArithmeticError('complete distinct192 point proof required')
...
def build(output):
    ...
    cert.write(output,{'schema':'elliptic-curves.new-high-rank-index.v13','sources':sources(),
...
def check(path):
    ...
    for r in data['curves']:
        ...
        seen.add(j)
        print('REPLAYED V13 INVENTORY',r['id'],'rank >=',len(points),flush=True)
    print('EXACT CURVE INVENTORY',len(seen),flush=True)

if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('--output',type=Path,default=ART/'new_high_rank_curve_index_v13.json');p.add_argument('--check',type=Path)
