...
UPSTREAM=scan.corrected.D/'controller'
...
def prepare():
    ...
    paths=[Path(__file__).resolve(),Path(scan.__file__),Path(scan.old.__file__),
           scan.old.D/'protocol.json',scan.narrow.D/'controller/protocol.json',Path(scan.narrow.__file__),Path(scan.binary.__file__),UPSTREAM/'protocol.json',
    ...
    checkpoint(D/'protocol.json',{'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},
        ...
        'wait_seconds':18000,'rss_bytes':4294967296,
        'stages':[('prepare',180),('benchmark',2400),('run',7200),('replay',1800)],
        'scope':'Freeze320 corrected-score higher MW16 slices after complete base/local cache proofs and the exact corrected benchmark pass. Exclude all previous outer, narrow and broader slices; reuse the fixed corrected benchmark once and scan319 new slices. After all gates pass, score every address through32749 with restored good5/13 terms before retaining4096 per slice. Require 320 complete signed-frame regressions, first-slice-per-band runtime gates and all1310720 independent retained-score replays. Exactly3059269468 primitive addresses in16384<H<=65536 and65536<H<=262144; no compact-region rescan or prior outer slice reuse. No public-record input, adaptive refill or point search.'})
...
def launch():
    ...
    try:
        ...
        for name,seconds in p['stages']:
            ...
            print('CORRECTED MW16 SCAN',name,ledger['rows'][-1]['status'],flush=True)
