...
ROOT=batch.ROOT;ART=batch.ART;D=batch.D;CONTROL=ROOT/'artifacts/local/elliptic-curves/remaining60-r17-controller-v1'
INPUT=ART/'remaining60_r17_results_v1.json';OUT=ART/'remaining60_r17_experiment_v1.json'
def expected():
    ...
    return {'schema':'elliptic-curves.remaining60-r17-experiment.v1','status':'PASS_STRONGER_ODD_BOUNDS_REQUIRE_REVIEW' if stronger else 'PASS',
        ...
        'attempted_charts':sum(r['attempted_charts'] for r in rows),'maximum_declared_boxes':2652,
        'retained_point_witnesses':sum(r['retained_points'] for r in rows),
        'claim_boundary':'320 untouched signed denominator slices in32768<H<=131072 and131072<H<=524288 on the five R17 families other than11952, with explicit exclusion of all earlier outer-square and skew-rectangle residues. All3510 selection primes precede the first retention cut; no removable-good-prime defect occurs through131071 by the separate exact arithmetic theorem. All10240 retained extended scores precede60 distinct finalists; wholly disjoint validation never changes selection. Up to2652 generic17-only point charts with all maps before points. Catalogue and1345 prior-equation comparison follows terminal exact point proofs. Matching finite bounds do not prove exact rank, saturation or absence. No universal novelty, rank density, calibrated sensitivity or isolated portability claim.'}
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--check',action='store_true');a=p.parse_args();d=expected()
    if a.check:
        if cert.read(OUT)!=json.loads(json.dumps(d)):raise ArithmeticError('remaining60 aggregate differs')
    else:
        if OUT.exists():raise FileExistsError('preserve remaining60 aggregate')
        checkpoint(OUT,d)
    print('REMAINING60 OUTCOMES',d['status'],d['completed_point_boxes'],[(r['id'],r['rank_lower_bound']) for r in d['rows'] if r['rank_lower_bound']>=22],flush=True)
