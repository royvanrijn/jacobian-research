#!/usr/bin/env python3
"""Bind five exact incidence cohorts to the current89 equations and duplicate map."""
import argparse
...
NAMES=['compact','latest7','latest8','latest23','retention']
...
INDEX=ART/'new_high_rank_curve_index_v7.json'

def result():
    index=cert.read(INDEX);targets={r['id']:r for r in index['curves']};seen=set();counts=Counter();duplicates=[]
    if len(targets)!=89:raise ArithmeticError('fixed89 inventory differs')
    maps=None
    for path in PROOFS:
        data=cert.read(path)
        ...
        for r in data['targets']:
            if r['id'] in seen or any(r[k]!=targets[r['id']][k] for k in r):raise ArithmeticError('cohort overlap or target model mismatch')
            seen.add(r['id'])
    ...
    if seen!=set(targets) or counts!={'NO_RATIONAL_J_PREIMAGE':958,'RATIONAL_J_PREIMAGES_CERTIFIED':110} or len(duplicates)!=21:raise ArithmeticError('fixed1068 coverage differs')
    ...
    return {'schema':'elliptic-curves.inventory89-incidence.v1','status':'PASS','sources':sources,'targets_checked':89,'pairs_checked':1068,'status_counts':dict(counts),'same_generic_subgroup_duplicates':duplicates,'claim_boundary':'Combines the five separately exact-replayed rational j-incidence cohorts and the exact Q(t) unimodular17-section transport. Every additional recorded presentation is the same generic subgroup. The separate incidence/transport replays remain required; this summary does not replay ranks or exclude other families, nongeneric points or higher ranks.'}
...
if __name__=='__main__':
    ...
    print('EXACT COHORT BINDING89 CURVES1068 PAIRS21 DUPLICATE PRESENTATIONS',flush=True)
