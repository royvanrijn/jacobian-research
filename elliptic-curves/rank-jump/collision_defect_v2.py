...
OUTPUT = r.OUT / 'rank_jump_collision_defect_v2.json'
WORK = r.ROOT / 'artifacts/local/rank-jump-collision-defect-v2'
    ...
    for pstr in source['collision_primes']:
        ...
        for i in range(4):
            for j in range(i):
                ...
                roots = sorted(int(a) for a in g.roots(multiplicities=False)) if g.degree() > 0 else []
...
def run():
    ...
    r.write_new(OUTPUT, {'schema': 'rank-jump.collision-defect.v2', 'layer': 'solubility', 'rows': rows,
