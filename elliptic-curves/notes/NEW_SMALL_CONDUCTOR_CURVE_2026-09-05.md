...
## Targeted arithmetic study, 2026-09-06

The original rank, minimality and conductor proof was replayed. The same
ordered 22-point subgroup is now **proved saturated at 2, 3 and 5**.
The mod-2 certificate already supplies injectivity for this subgroup;
the new check recomputes the mod-3 and mod-5 finite signatures on these
22 generators alone, using primes from the earlier complete-cloud audit.
Both matrices have column rank 22, with separate good-reduction witnesses
excluding rational torsion of the respective prime orders.

Here is the saturation argument, which does not assume the full rank is
known. Write H for the subgroup generated by the displayed P_i. If
ell*Q = sum a_i*P_i, its image in every E(F_p)/ell*E(F_p) is zero.
Independent columns force ell to divide every a_i. Consequently
ell*(Q - sum (a_i/ell)*P_i) = O. The rational ell-torsion exclusion gives
Q in H. Apply this for ell = 2, 3, 5. This excludes missing divisions by
these primes inside the rational span of H; it neither proves full
saturation nor excludes independent directions outside that span.

The exact local root-number calculation was also rerun. The global sign
is **+1**, including the infinite factor -1. Thus the analytic order is
even. Under the algebraic parity conjecture, a 23rd independent point
would imply rank at least 24; the unconditional point target remains 23.
The sign is equally compatible with rank 22 and supplies no evidence
that the extra point exists. See the
[PARI documentation](https://pari.math.u-bordeaux.fr/dochtml/html/Elliptic_curves.html#ellrootno).

A fresh [ICARM database](https://elliptic-rank.icarm.cloud/database.json)
download at **2026-09-06 17:50:21 UTC** contains **614 curves**, with SHA256
`2048f046f13bc709c2a3b4c56a02c4c3e5d410e1774492576434398ad611688d`.
The exact rational-isomorphism comparison still finds no match. Its
conductor comparison is unchanged:

| Rank threshold | Minimum recorded conductor's ICARM ID | Minimum divided by this curve's conductor |
|---|---:|---:|
| at least 22 | 376 | about 0.03126 |
| at least 23 | 539 | about 16,849,507 |
| at least 24 | 548 | about 63,995,845,864 |

Only IDs 376 and 575 have smaller recorded rank-at-least-22 conductors.
IDs 537, 543, 545 and 581 still lack conductors at all three thresholds.
These are comparisons with recorded values in this pinned download;
the original 586-row proof and its historical comparison remain intact.

The next mathematical discriminator is an unconditional rank/Selmer upper
bound: an upper bound 22 would close the extra-point question negatively,
whereas a larger Selmer space alone would still require rational points
or a further obstruction calculation. The previous 180-second descent
initialization timed out before producing any bound. No new descent is
claimed by this study. Saturating H cannot by itself increase its rank.

### Twelve larger-height attempts

The fixed experiment reused the first twelve distinct exceptional quotient
words in the already ordered 301-map roster, at zero-based indices
`0,1,2,3,4,5,7,8,9,11,12,14`. Every centre and rational chart map was
frozen before search. The input was this curve's own certified 22-point
subgroup; no new parameter or public exceptional point entered the trial.
The height bound increased from 100,000 to **1,000,000**, with one worker,
30 seconds per PARI call, a 600-second worker cap and 1.5 GiB memory limit.

**Four boxes completed and eight timed out. No 23rd independent point was
certified.** All twelve attempt records and exact admission histories
replayed. The worker took 366.21 seconds and replay 27.56 seconds. The
2,034 retained point occurrences, together with the original generators,
give 2,048 distinct points up to sign. Independent complete-cloud audits
modulo 3 and 5 through prime 997 both still give column rank 22; the
search's mod-2 admission proof also retains lower bound 22. These finite
tests supply no rank upper bound. In particular, the eight timeouts make
no completed-box or denominator-prefix claim.

The V1 preflight failed before any point search because it compared stored
rational strings directly with Fraction objects. V2 converts both sides
to exact rationals for that check. The original source, frozen protocol
and failure log are preserved; the twelve-map selection and budgets did
not change.

The [study certificate](../../artifacts/generated-results/elliptic-curves/small_conductor_targeted_study_v2.json),
[odd-modulus cloud proof](../../artifacts/generated-results/elliptic-curves/small_conductor_targeted_cloud_modl_v2.json)
and [evidence manifest](../../artifacts/generated-results/elliptic-curves/small_conductor_targeted_study_evidence_v3.json)
retain the arithmetic and bounded experiment. The bundle includes both
catalogue snapshots, exact maps, raw outputs, source dependencies and
the failed preflight. It requires Python and the pinned PARI binary;
replay performs no point search or descent.
All [three isolated replay stages](../../artifacts/generated-results/elliptic-curves/small_conductor_targeted_study_portable_replay_v3.json)
pass from the 3,937,474-byte, 125-file bundle. The earlier packaging V2
omitted the `ecsearch` import dependency and failed before replay; its
archive and failed report are retained. V3 changes packaging only.

```sh
python3 elliptic-curves/cas/study_small_conductor_target.py replay
python3 elliptic-curves/cas/study_small_conductor_target.py check
python3 elliptic-curves/cas/audit_small_conductor_targeted_cloud.py --check
```
...
The [targeted descent study](SMALL_CONDUCTOR_DESCENT_SHORTCUT_2026-09-06.md)
specializes an exact class-group stopping criterion and verifies 18 principal
relations from reduced norm coordinates. It supplies no rank upper bound.
