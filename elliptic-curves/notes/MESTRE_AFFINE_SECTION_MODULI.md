...
its truncations.  The same exact component calculation proves that its
leading invariant is a rational square.  Exact degree-eight modular
function-field calculations recover the selected visible-dependence relation;
its direct characteristic-zero determinant was stopped before completion.
...
rank-seven/one-dimensional pair at this seed.  The recovered surface now
explains this: the six companions are etale sheets over one common
two-dimensional moduli locus, so each unordered pair has a two-dimensional
germ.
...
is explained by the recovered relation on the surface and is treated as
structural rather than a weakness of the chosen quotient panel.  The
characteristic-zero relation determinant remains incomplete.
...
branch.  The incidence-germ note now supplies the surrounding
...
Shioda Gram matrix, saturation, or a rank upper bound.  It does establish
that the selected pair is closed as a route to the missing generic rank-14
direction.
