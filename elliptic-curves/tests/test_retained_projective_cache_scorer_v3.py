...
class RetainedCacheScorerV3(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.temp=tempfile.TemporaryDirectory();cls.folder=Path(cls.temp.name);cls.binary=cls.folder/'scorer'
        subprocess.run(['g++','-O3','-std=c++17',str(ROOT/'elliptic-curves/cas/newfamily/score_retained_projective_cache_v3.cpp'),'-o',str(cls.binary)],check=True,capture_output=True,timeout=20)
    @classmethod
    def tearDownClass(cls):cls.temp.cleanup()
    def fixture(self):
        primes=[5,4099,4111];tables=[];data=b'R17XS001'+struct.pack('<I',len(primes))
        for p in primes:
    ...
    def test_explicit_new_height_and_signed_boundaries(self):
        cache,path,tables,pairs=self.fixture()
        pairs=[(131072,131073),(-131072,131073),(524287,524288),(-524287,524288),(524288,524287),(-524288,524287),(1,4099*127),(-1,4099*127),(524288,1),(-524288,1),(1073741824,1073741823),(-1073741824,1073741823)]
        path.write_text('R17-CANDIDATES-V1 '+str(len(pairs))+'\n'+''.join(f'{n} {d}\n' for n,d in pairs))
        r=subprocess.run([str(self.binary),str(cache),str(path),'1073741824'],text=True,capture_output=True,timeout=5)
        self.assertEqual(r.returncode,0,r.stderr);expected=[]
        for i,(n,d) in enumerate(pairs):
            score=good=0
            for p,units,flags in tables:
            ...
            expected.append(f'R {i} {score} {good}')
        self.assertEqual(r.stdout.splitlines(),expected+[f'S {len(pairs)} {len(tables)}'])
        for args in ([],['524288'],['0'],['1073741825'],['524288junk']):
            q=subprocess.run([str(self.binary),str(cache),str(path),*args],text=True,capture_output=True,timeout=5)
            self.assertNotEqual(q.returncode,0);self.assertEqual(q.stdout,'')
