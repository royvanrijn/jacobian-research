...
This root later produced the strongest rank-10 evidence.
...
`audit_rank17_extension_chain.py` was added to expose the index-2 parity class,
find a minimal saturation bridge, and export exact `|pairing|=2` continuation
anchors.  The bridge/closure output is still a pending runtime result at this
checkpoint.

## Rank-10 continuation: first attempt and correction
...
bug but a symmetry/labeling ambiguity.
...
### Root-000029 scan: strong numerical hit
...
The Coxeter surface was held completely fixed during this scan; only the added
quartic section and one primary quadratic slope were nonlinear variables, with
secondary slopes eliminated linearly.  The fit is therefore at essentially the
same numerical scale as `root-000029` itself (`~8e-13`).

The winning fingerprint is exactly a permutation of the **negative** of the
original section-961 signature.  Numerically, the evidence therefore points to

```text
Q_10 = -P_961
```

under a nontrivial relabeling of the Coxeter generators.

Because the winning fingerprint has multiplicities `2 zeros, 3 twos, 4 ones`,
there remain
...
full-lattice Coxeter mappings compatible with rank 10.  Rank 11 should be used
to distinguish these mappings instead of restarting a full `S9` scan.

This is currently the strongest numerical evidence that an explicit
rank-growth step of the missing rank-17 fibration has been located.  It is
**not yet an exact proof of rank 10**.

## Current experiment

`refine_rank10_winning_fingerprint.py` has been added to refine the winning
`root-000029` / fingerprint-2452 configuration.  It:

1. re-finds the added section on the fixed surface;
2. lets the 104 Coxeter variables and the section move jointly;
3. imposes all nine exact Coxeter fingerprint anchors;
4. monitors discriminant and non-isotriviality rather than allowing collapse;
5. computes the Jacobian nullity of the combined rank-10 system;
6. writes the 288 compatible Coxeter mappings for the rank-11 stage.

The geometric expectation is that a genuine generic rank-10 elliptic K3 locus
has post-gauge dimension about 8.  Thus a healthy machine-precision refinement
with numerical nullity near 8 would be an important independent confirmation.

## Next steps

If the rank-10 refinement succeeds:

1. freeze the surviving 288 lattice labelings;
2. use the next exact lattice section, signed index `2402`, to filter those
   mappings and reconstruct rank 11;
3. continue through the selected chain

   ```text
   961 -> 2402 -> 1642 -> 1300 -> 1023 -> 2216 -> 2392 -> 2610;
   ```

4. expect the numerical moduli dimension to decrease roughly
   `9 -> 8 -> ... -> 1` as MW rank grows `9 -> 10 -> ... -> 17`;
...
- the fixed-surface rank-10 hit with score `5.850e-13` for fingerprint 2452.

What remains unproved:

- an exact Weierstrass model for the intended rank-17 fibration;
- exact generic coordinates for sections 10 through 17;
- exact certification that the current rank-10 numerical hit belongs to the
  desired rank-17 component;
