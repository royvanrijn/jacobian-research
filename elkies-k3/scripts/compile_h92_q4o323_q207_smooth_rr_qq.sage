#!/usr/bin/env sage
"""Compile the exact lifted q323/q207 candidate by the smooth chord module.

For the exact QQ section candidate with P.O=10, use D=O+P-4F.  The ambient
degrees are deg(AA)<=16 and deg(BB)<=4, and AA*X=BB*Y mod Z^2 is a direct
22-to-2 linear calculation.  Compile the pencil to its binary quartic and
minimal Jacobian without resolved vertical rows, elimination, or a Groebner
basis.  The output remains candidate-level until the independent marked q207
class gate passes.
"""
...
TARGET = LOCAL / "q4o323-q207-deflated-horizontal-qq.json"
SURFACE = LOCAL / "q4o323-component2-pointing-qq.json"
...
def parse_args():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=LOCAL / "q4o323-q207-smooth-rr-qq.json",
    )
...
assert target_artifact["status"] == "PASS_EXACT_QQ_Q4O323_DEFLATED_CANDIDATE_SECTION"
assert target_artifact["section"]["P_dot_O"] == 10
assert target_artifact["section"]["exact_projective_weierstrass_identity"]

R = PolynomialRing(QQ, "v")
v = R.gen()
K = R.fraction_field()
A = R(surface_artifact["global_short_model"]["A_coefficients_low_to_high"])
B = R(surface_artifact["global_short_model"]["B_coefficients_low_to_high"])
target = target_artifact["section"]
X = R(target["X_coefficients_low_to_high"])
Y = R(target["Y_coefficients_low_to_high"])
Z = R(target["Z_coefficients_low_to_high"])
assert (X.degree(), Y.degree(), Z.degree()) == (24, 34, 10)
...
print(
    "Q4O323Q207SMOOTHQQ|preflight_root_rank={}|fibres={}|infinity={}".format(
        root_rank,
...
assert root_rank == 6
assert all(item["kodaira"] in ("I1", "I2", "II") for item in finite_fibres)
assert infinity_kodaira in ("smooth", "I1", "I2", "II")
...
payload = {
    "schema": "h92-q4o323-q207-smooth-rr-qq-v1",
    "reproducing_command": (
        "/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python "
        "elkies-k3/scripts/compile_h92_q4o323_q207_smooth_rr_qq.sage "
        "--output artifacts/local/elkies-k3/q4o323-q207-smooth-rr-qq.json"
    ),
    "inputs": {
    ...
    "method": {
        ...
        "marked_q207_class_gate_pending": True,
    },
    "runtime_seconds": time.monotonic() - started,
    "proof_boundary": "Exact QQ h0=2 pencil, binary quartic, minimal Jacobian, and 6A1 fibre classification for candidate 5887. Its marked Abel trace differs from q207 at the good fibre (p,tau)=(61,3), so this is a certified negative route result, not the fixed-corridor edge.",
    "status": "REJECTED_EXACT_QQ_Q4O323_CANDIDATE5887_WRONG_6A1_CHILD",
}
args.output.parent.mkdir(parents=True, exist_ok=True)
args.output.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
print(
    "Q4O323Q207SMOOTHQQ|ambient=22|rank={}|h0={}|quartic_degree={}|"
