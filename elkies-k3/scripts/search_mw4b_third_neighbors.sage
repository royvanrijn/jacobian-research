...
ap=argparse.ArgumentParser(description="Third-generation low-q search from MW4 node B (q=9 child of q=90 MW7 node).")
...
FRAME=Path("elkies-k3/data/fibrations/q90_mw4b_frame.txt")
M=matrix(ZZ,[[ZZ(x) for x in ln.split()] for ln in FRAME.read_text().splitlines() if ln.strip()])
assert M.det()==948
U=matrix(ZZ,[[0,1],[1,0]])
NS=block_diagonal_matrix(U,-M)

print(f"MW4BNBR|stage=start|q={a.qmin}..{a.qmax}|det={M.det()}",flush=True)
...
def vectors_of_qnorm(n):
    ...
    rec(dim-1,RR(0))
    print(f"MW4BNBR|stage=enumerate|q={n}|pairs={len(found)}",flush=True)
    return found
...
for q in range(a.qmin,a.qmax+1):
    for v in vectors_of_qnorm(q):
        for aa in divisors(q):
            ...
            if mw<=4:
                print(f"MW4BNBR|cand={len(res)}|q={q}|ab={aa},{bb}|root_rank={rr}|roots={rc}|rootdet={rd}|MW={mw}",flush=True)

res.sort(key=lambda z:(z[0],-z[2],z[3],z[4]))
print(f"MW4BNBR|stage=summary|unique={len(res)}",flush=True)

for i,z in enumerate(res[:a.report],1):
    mw,rr,rc,rd,q,aa,bb,v,P=z
    print(f"MW4BNBR_BEST|rank={i}|MW={mw}|root_rank={rr}|roots={rc}|rootdet={rd}|q={q}|ab={aa},{bb}|v={v}",flush=True)
    print(f"MW4BNBR_BEST|frame_gram={P}",flush=True)
