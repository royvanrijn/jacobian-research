...
PROBE=ROOT/'elkies-k3/scripts/probe_curve302_triangle_mw14.sage'
SOURCE=ROOT/'artifacts/generated-results/elkies-k3-r17-norm12-orbit11952-direct-fibration-v1.json'
OUT=ROOT/'artifacts/generated-results/elkies-k3-curve302-triangle-mw14-qq-v1.json'
...
def build():
    ...
    O=section(vector(ZZ,17));P=section(e[3]);Q=section(e[10]);Z=section(e[6]);F=O+P+Q
    ...
    p,q=[E(decode(d['sections']['records'][i]['X']),decode(d['sections']['records'][i]['Y'])) for i in [3,10]]
    rp,rq,lam=meet
    ...
    for prime in [1009,1013,1021]:
        path=OUT.with_name(f'elkies-k3-curve302-triangle-mw14-mod{prime}-v1.json')
    ...
    return {'schema':'curve302.triangle-mw14-qq.v1','status':'PASS_QQ_MW14_AND_TARGET_EXCLUSION' if exclusions else 'PASS_QQ_MW14_TARGET_UNRESOLVED',
        ...
        'pencil_description':'s=c1*(y+yP4)/(x-xP4)+c2*(y+yP11)/(x-xP11)+c0',
