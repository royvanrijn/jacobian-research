...
"""Export an exact D12 state returned by a selected zero-changing loop."""
...
parser.add_argument(
    "--search", type=Path,
    default=GENERATED / "elkies-k3-h3-d12-zero-changing-a11-presentations-full.json",
)
parser.add_argument(
    "--source", type=Path,
    default=GENERATED / "elkies-k3-h3-current_D12-marked-frame.json",
)
parser.add_argument("--q", type=int, default=4)
parser.add_argument("--degree", type=int, default=2)
parser.add_argument("--orbit", type=int, default=20)
parser.add_argument("--zero", default="old_D12_component_1")
...
def load_matrix(path):
    return matrix(
        ZZ,
        [[ZZ(x) for x in line.split()] for line in path.read_text().splitlines()
...
assert search["status"] == "PASS_EXACT_D12_ZERO_CHANGING_A11_PRESENTATION_SEARCH"
...
transition = matrix(ZZ, selected["source_to_returned_D12_basis"])
...
assert targets["current_A11"] == vector(ZZ, selected["exit_A11_fibre_in_returned_D12"])
...
payload = {
    "schema": "elkies-k3.h3-d12-zero-loop-returned-marking.v1",
    "status": "PASS_EXACT_D12_ZERO_LOOP_RETURNED_MARKING",
    "hub": "{}_q{}_orbit{}_{}".format(
        source.get("hub", "current_D12"), args.q, args.orbit, args.zero
    ),
    "candidate_id": selected["first_edge_candidate_id"],
    "explicit_zero_curve": selected["explicit_zero_curve"],
    "prefix_raw_score": int(
        source.get("prefix_raw_score", 0)
        + selected["first_edge_score"]
        + selected["return_score"]
    ),
    "prefix_operational_score": int(
        source.get("prefix_operational_score", 0)
        + sum(max(500, int(selected[key])) for key in ("first_edge_score", "return_score"))
    ),
    "root_data": [12, 264, 4],
    "frame_output": str(FRAME_OUTPUT.relative_to(ROOT)),
    "frame_sha256": hashlib.sha256(FRAME_OUTPUT.read_bytes()).hexdigest(),
    "current_D12_to_root_adapted_hub_basis": rows(transition),
    "root_adapted_hub_to_current_D12_basis": rows(inverse),
    "target_fibres_in_root_adapted_hub": {
        name: [int(x) for x in value] for name, value in targets.items()
    },
    "proof_boundary": (
        "Exact returned D12 frame, marked U, full bidirectional current-D12 transport, "
...
print(
    "D12RETURNMARK|prefix_raw={}|prefix_operational={}|det={}|status={}|output={}".format(
        payload["prefix_raw_score"], payload["prefix_operational_score"],
