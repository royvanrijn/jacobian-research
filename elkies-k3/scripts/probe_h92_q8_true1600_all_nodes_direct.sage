...
"""
Exact all-six finite E7 node probe on the H92 q=8 true-1600 global survivor.

This avoids Singular entirely.

For each actual resolved edge chart:
  * start from the already-computed 18-dimensional E8+H+generic-E7 kernel;
  * discard ambient columns with common-clearing t exponent >= T=17;
  * combine the remaining 286 ambient columns into the 18 survivors;
...
This gives the exact modular node quotient image on the 18-dimensional
survivor. It also stacks all six node images and reports their combined rank.

Run:
  sage -python ~/Downloads/probe_h92_q8_true1600_all_nodes_direct.sage
...
OUTPUT_PATH = GEN / f"zz-h92-q8-true1600-all-e7-nodes-direct-mod-{P}.json"

for path in (
    P1_PATH, PULLBACKS_PATH, GLUING_PATH, AMBIENT_PATH,
    CLEARINGS_PATH, KERNEL_PATH,
):
    if not path.exists():
...
clearing_nodes = {entry["chart"]: entry for entry in clearings["nodes"]}
assert set(NODES) == set(charts) == set(edges) == set(clearing_nodes)
...
print(
    f"ALLNODESDIRECT|prime={P}|ambient=1600|survivors=18|nodes=6|T={T}|K={K}",
    flush=True,
)

node_records = []
combined_rows = []
previous_combined_rank = 0
...
for node_index, node_name in enumerate(NODES):
    ...
    for (a, b), terms in sorted(active_by_family.items()):
        ...
        for factor in (
            ...
            # H_reverse^(K-k)=1 because all true-ambient columns have k=18.
        ):
    ...
    node_matrix = matrix(finite, len(coordinates), 18)
...
payload = {
    "schema": "elkies-k3.h92-q8-true1600-all-e7-nodes-direct-modp.v1",
    "status": "EXPERIMENTAL_MODULAR_TRUE1600_EXACT_ALL_E7_NODE_IMAGES",
    "prime": int(P),
    "inputs": {
    ...
    "common": {
        ...
        "T": int(T),
        "K": int(K),
        "columns_zero_before_surface_reduction": 1314,
        "active_ambient_columns": 286,
        "method": (
    ...
    "nodes": node_records,
    "combined": {
        "restricted_rank": int(previous_combined_rank),
        "remaining_dimension": int(18 - previous_combined_rank),
    },
    "boundary": (
        "This stacks the exact modular quotient images at the six E7 edge "
        "nodes on the already-computed generic global survivor. It does not "
...
print(
    "Q8TRUEALLFINITENODESDIRECT|"
    f"prime={P}|global_survivor=18|nodes=6|"
    f"combined_rank={previous_combined_rank}|"
    f"remaining={18-previous_combined_rank}|"
    "status=EXPERIMENTAL_MODULAR_TRUE1600_EXACT_ALL_E7_NODE_IMAGES",
    flush=True,
)
