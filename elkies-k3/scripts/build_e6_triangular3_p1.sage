...
# Third triangular solve: P1_5 for y2 in RR.
target="P1_5"
if target not in tags:
    raise RuntimeError("P1_5 missing")
E5=eqs[tags.index(target)]
y2r=rrd["y2"]
if E5.degree(y2r)!=1:
    raise RuntimeError("P1_5 not linear in y2")
coef5=E5.derivative(y2r)
const5=E5.subs({y2r:0})
y2_expr=-const5/coef5

print(f"E6TRI3|elim_from=P1_5|variable=y2|coef_terms={len(coef5.monomials())}|expr_num_terms={len(y2_expr.numerator().monomials())}|expr_den_terms={len(y2_expr.denominator().monomials())}",flush=True)

# Substitute y2 in all existing equations except P1_5.
new_names2=[n for n in new_names if n!="y2"]
R3=PolynomialRing(K,new_names2,order="degrevlex")
r3d=R3.gens_dict()

def sub_to_R3(e):
    # Substitute y2 in Frac(RR), then explicitly evaluate coefficients
    # into the new polynomial ring. This avoids Sage parent/subs issues.
    RRF=FractionField(RR)
    ee=RRF(e).subs({RRF(y2r):RRF(y2_expr)})
    num=RR(ee.numerator())
    den=RR(ee.denominator())

    vals=[r3d[n] for n in new_names2]

    # y2 no longer occurs after substitution, so evaluate the remaining
    # RR variables positionally into R3.
    def convert(poly):
        result=R3(0)
        for mon,coeff in poly.dict().items():
            ...
            for i,exp in enumerate(mon):
                ...
                if oldname=="y2":
                    if exp:
                        raise RuntimeError("y2 survived substitution")
                    continue
                term *= r3d[oldname]**exp
            result += term
        return result

    return convert(num), convert(den)
...
print(f"E6TRI3|field={field}|vars={R3.ngens()}|eqs={len(eqs3)}|naive_dim={R3.ngens()-len(eqs3)}",flush=True)

for i,(tag,e) in enumerate(zip(tags3,eqs3)):
    print(f"E6TRI3_EQ|i={i}|tag={tag}|degree={e.degree()}|terms={len(e.monomials())}",flush=True)

# Diagnose next equations.
for target in ("P1_4","P1_3","P1_2"):
    if target not in tags3:
        continue
    e=eqs3[tags3.index(target)]
    vals=[]
    for nm in ["x1","y1","a4","a3","a2","x0","y0","a1"]:
        if nm not in r3d:
            continue
        v=r3d[nm]
        if e.degree(v)==1:
    ...
    print(f"E6TRI3_NEXT|eq={target}|linear={vals}",flush=True)

print("E6TRI3|expected_geometry_dim=3",flush=True)
...
if args.export:
    ...
    with out.open("w") as h:
        h.write(",".join(new_names2)+"\\n"+str(args.p)+"\\n")
        for i,e in enumerate(eqs3):
            h.write(str(e).replace("**","^"))
            h.write(",\\n" if i+1<len(eqs3) else "\\n")
    print(f"E6TRI3|export={out}",flush=True)
