...
if args.screen_levels and kernel.nrows() == 2:
    ...
    for level in F:
        ...
        # A zero discriminant means this level is a generically doubled /
        # singular member of the new pencil.  That is legitimate and should
        # not be sent to squarefree_decomposition(), which is undefined on 0.
        if not disc:
            singular.append(int(level))
            records.append({
                "level": int(level),
                "singular_reason": "quadratic_discriminant_identically_zero",
            })
            continue
        ...
        for side, value in (("n",num),("d",denp)):
            if not value:
                # Defensive guard; disc!=0 should make the numerator nonzero.
                continue
    ...
    branch = {
        ...
        "genus_one_levels": sorted(good),
        "singular_levels": sorted(set(singular)),
        "records": records,
    }
