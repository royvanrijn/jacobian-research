...
from half the supplied precision to the full supplied precision and
exactifies immediately if possible.
...
if precision < 128 or precision % 2:
    raise ValueError("precision must be an even p-adic digit count >= 128")
...
data_by_level = {}
for k in (half_precision, precision):
...
rhalf = data_by_level[half_precision][1]
rfull = data_by_level[precision][1]
stable = [
    i for i, (u, v) in enumerate(zip(rhalf, rfull))
    if u is not None and u == v
]
print(
    f"Q6REBASE_STABLE|{half_precision}->{precision}|"
...
complete = all(v is not None for v in rfull)
exact = False
if complete:
    Z = QS(list(rfull[:21]) + [QQ(1)])
    X = QS(list(rfull[21:68]))
    Y = QS(list(rfull[68:]))
    exact = (
        Z.degree() == 21
        and X.degree() == 46
...
payload = {
    ...
    "rr_full_precision": {
        "precision": precision,
        "recovered": int(sum(v is not None for v in rfull)),
        "Z": int(sum(v is not None for v in rfull[:21])),
        "X": int(sum(v is not None for v in rfull[21:68])),
        "Y": int(sum(v is not None for v in rfull[68:])),
    },
    "stable_half_to_full": {
...
if exact:
    exact_values = list(Z.list()) + list(X.list()) + list(Y.list())
    max_num_digits = max(
        digits_int(QQ(v).numerator()) for v in exact_values if v
    )
    max_den_digits = max(
        digits_int(QQ(v).denominator()) for v in exact_values if v
    )
    payload["Z"] = [str(v) for v in Z.list()]
    payload["X"] = [str(v) for v in X.list()]
    payload["Y"] = [str(v) for v in Y.list()]
    payload["exact_identity"] = True
    payload["max_numerator_decimal_digits"] = int(max_num_digits)
    payload["max_denominator_decimal_digits"] = int(max_den_digits)
    print(
        f"Q6REBASE_EXACT_HEIGHTS|max_num_digits={max_num_digits}|"
