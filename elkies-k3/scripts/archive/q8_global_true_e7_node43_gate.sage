...
def build_prime(prime):
    ...
    # --------------------------------------------------------------
    # Safe necessary leading-term gate at the actual E7_4--E7_3 node.
    #
    # This is the same Pareto-minimal procedure as
    # probe_h92_q8_e7_4_3_node_module_modp.sage, but applied directly
    # to the true 18-dimensional global survivor.
    # --------------------------------------------------------------

    candidate = matrix(F, M8H7g.right_kernel().basis_matrix())
    assert candidate.nrows() == ncols - rankall

    def node_bidegree_from_global_column(column):
        bidx, degree = columns[column]
        a, b = generic_meta[bidx]
        # The generic node formula is written in the q6^9 helper convention.
        # Our ambient entry uses i=degree-9, k=18, exactly as E7_CONDITIONS.
        i = degree - 9
        k = H_POWER
        d = 4*k - i
        return (
    ...
    def support_pairs(rows):
        ...
        for col in range(rows.ncols()):
            if any(rows[r, col] for r in range(rows.nrows())):
                pairs.setdefault(
                    node_bidegree_from_global_column(col), []
                ).append(col)
        return pairs
    ...
    while candidate.nrows():
        ...
        coefficient = vector(
            F,
            [candidate[r, label] for r in range(candidate.nrows())],
        )
        assert coefficient

        node_records.append((degree, label))
        cut = matrix(
            F,
            matrix(F, 1, candidate.nrows(), [coefficient])
            .right_kernel()
            .basis_matrix(),
        )
    ...
    print(
        "Q8TRUENODE43|prime={}|generic_survivor={}|"
        "constraints={}|node_survivor={}|"
        "degrees={}|status=TRUE_E7_4_3_NODE_LEADING_GATE".format(
            prime,
            ncols-rankall,
            len(node_records),
            candidate.nrows(),
            ";".join(
                "{},{}".format(degree[0], degree[1])
                for degree, unused in node_records
            ),
        ),
        flush=True,
    )
