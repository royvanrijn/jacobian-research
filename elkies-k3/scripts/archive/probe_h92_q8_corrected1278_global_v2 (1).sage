...
def patched(source_name, replacements, target_dir):
    ...
    # The repository evaluators normally infer ROOT from __file__.  These
    # patched copies live in a temporary directory, so pin ROOT explicitly
    # to the repository located by this wrapper.
    root_anchor = 'ROOT = Path(__file__).resolve().parents[2]'
    if root_anchor not in source:
        raise RuntimeError(
            f"ROOT anchor not found in {source_name}"
        )
