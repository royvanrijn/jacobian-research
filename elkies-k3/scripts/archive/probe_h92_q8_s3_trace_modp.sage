...
ii = R(next(item for item in q6["finite_fibres"] if item["kodaira"] == "II*")["factor"]).monic()
iv = R(next(item for item in q6["finite_fibres"] if item["kodaira"] == "IV*")["factor"]).monic()
...
for entry in kernel_polys:
    sp = R([modq(v) for v in PolynomialRing(QQ, "T")(entry["s"]).list()])
    tp = R([modq(v) for v in PolynomialRing(QQ, "T")(entry["t"]).list()])
    Bcoef = K(sp)/K(h)
    Acoef = (
        -K(sp)*p_fun/K(h)
...
# D13 child coefficients at U=tau.
tau = F(args.tau)
...
A13 = eval_coeff(A13_poly, tau)
B13 = eval_coeff(B13_poly, tau)
E13 = EllipticCurve(F, [0, 0, 0, A13, B13])
if not E13.discriminant():
    raise ArithmeticError("tau gives singular D13 child fibre")

# Degree-52 fibre of S3 over U=tau.
H = R(U3_num - tau*U3_den)
if H.degree() != 52:
    raise ArithmeticError(f"S3 q8 fibre degree dropped to {H.degree()}")
H = H.monic()
if H.gcd(H.derivative()).degree() != 0:
    raise ArithmeticError("degree-52 S3 fibre is not etale")
print(
    f"Q8S3TRACE|prime={p}|tau={int(tau)}|degree=52|stage=setup|status=PASS",
    flush=True,
)
...
for label, origin in (("plus", Qplus), ("minus", Qminus)):
    ...
    if len(halves) != 1:
        raise ArithmeticError(
            f"origin {label}: doubled AJ has {len(halves)} rational halves, expected one"
        )
    AJ = halves[0]
    assert 2*AJ == doubled
...
print(
    ...
    "L53_trace=PASS|two_cover_halving=PASS|"
