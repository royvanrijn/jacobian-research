...
def native_binary_quartic_covariants(quartic):
    """Covariants over the ACTUAL finite base field, with no QQ scalars."""
    base = quartic.base_ring()
    BR = PolynomialRing(base, names=("x", "z"))
    bx, bz = BR.gens()
    f = sum(
        BR(quartic[index]) * bx**index * bz**(4-index)
        for index in range(5)
    )
    H = (
        f.derivative(bx, 2) * f.derivative(bz, 2)
        - f.derivative(bx).derivative(bz)**2
    ) / base(3)
    G = f.derivative(bx) * H.derivative(bz) - f.derivative(bz) * H.derivative(bx)
...
def native_transport_binary_quartic_point(
    covariants, x_value, z_value, w_value, minimizing_unit, extension
):
    """Same covariant map as the core, but ring-native over GF(p^d)."""
    f, H, G, I, J = covariants
    x_value = extension(x_value)
    z_value = extension(z_value)
    w_value = extension(w_value)
    unit = extension(minimizing_unit)

    fv = extension(f(x=x_value, z=z_value))
    if not fv or w_value**2 != fv:
        raise ValueError("binary-quartic point failed native w^2=f check")

    Hv = extension(H(x=x_value, z=z_value))
    Gv = extension(G(x=x_value, z=z_value))

    raw_x = -extension(3)/extension(4) * Hv/fv
    raw_y = extension(9)/extension(32) * Gv*w_value/fv**2

    standard_a = -extension(27) * extension(I)
    standard_b = -extension(27) * extension(J)
    if raw_y**2 != raw_x**3 + standard_a*raw_x + standard_b:
        raise ArithmeticError("native covariant map missed standard Jacobian")

    child_x = unit**2 * raw_x
    child_y = unit**3 * raw_y
    child_a = unit**4 * standard_a
    child_b = unit**6 * standard_b
    if child_y**2 != child_x**3 + child_a*child_x + child_b:
        raise ArithmeticError("native covariant map missed minimized child")
...
def eval_rf_limit_at(value, alpha):
    """Evaluate a removable rational-function singularity at u=alpha."""
    top = R(value.numerator())
    bottom = R(value.denominator())
    linear = u - F(alpha)
    while not top(F(alpha)) and not bottom(F(alpha)):
        top, rem_top = top.quo_rem(linear)
        bottom, rem_bottom = bottom.quo_rem(linear)
        if rem_top or rem_bottom:
            raise ArithmeticError("failed to cancel common local factor")
    if not bottom(F(alpha)):
        raise ZeroDivisionError("rational function has a genuine pole")
    return top(F(alpha)) / bottom(F(alpha))
...
def point_from_old_section(
    alpha, quartic, covariants, square_factor, minimizing_unit,
    ...
    w = raw_sqrt / sf
    if w**2 != eval_poly_at(quartic, alpha):
    ...
    mapped = native_transport_binary_quartic_point(
        covariants, alpha, extension(1), w, minimizing_unit, extension
    )
...
def trace_factor_point(factor, factor_index, quartic, covariants, square_factor,
                       minimizing_unit, q6_m, child_a, child_b):
    degree = factor.degree()
    if degree == 1:
        alpha = -factor[0] / factor[1]
        E, Q = point_from_old_section(
            F(alpha), quartic, covariants, square_factor, minimizing_unit,
...
def oriented_quartic_pole_points(
    tau, quartic, covariants, square_factor, q6_m,
    minimizing_unit, child_a, child_b
):
    """
    Return the old-O and A=-P1 points on the child, with sign fixed locally.

    At the common pole branch m->infinity, the residual quadratic has one root
    tending to O with raw discriminant square root +m^2, and the other tending
    to -P1 with -m^2.  Therefore w_O is the removable limit m^2/square_factor.
    """
    ...
    quartic_value = quartic(u0)
    if not quartic_value:
        raise ArithmeticError("pole branch landed on quartic branch point")

    wO = eval_rf_limit_at(q6_m**2 / square_factor, u0)
    if wO**2 != quartic_value:
        raise ArithmeticError("local old-O sign does not square to quartic value")
    wA = -wO
    ...
    def mapped_point(w):
        mapped = native_transport_binary_quartic_point(
            covariants, F(u0), F(1), F(w), minimizing_unit, F
        )
        assert mapped["child_a"] == child_a
        assert mapped["child_b"] == child_b
        return Ebase(mapped["child_x"], mapped["child_y"])

    QO = mapped_point(wO)
    QA = mapped_point(wA)
    if QO == QA:
        raise ArithmeticError("old O and -P1 collapsed on child")

    return F(u0), F(wO), QO, QA
...
for tau_integer in tau_candidates:
    try:
        ...
        covariants = native_binary_quartic_covariants(quartic)

        # Fix the old-O / A=-P1 orientation intrinsically from the local
        # m->infinity asymptotics, then fail early if the covariant map is wrong.
        u0, wO, QO, QA = oriented_quartic_pole_points(
            tau, quartic, covariants, square_factor, q6_m,
            minimizing_unit, child_a, child_b
        )
        ...
        for factor, multiplicity in factorization:
            ...
            traceC += trace_factor_point(
                factor, factor_index, quartic, covariants, square_factor,
                minimizing_unit, q6_m, child_a, child_b
            )
        if sum(factor_degrees) != 50:
            raise ArithmeticError("factor degrees do not sum to 50")

        # Literal-support Picard identity on a q6 fibre:
        # AJ(C)=S3+24*A, while the covariant embedding uses QO rather than
        # old O as the group origin. Therefore
        #   S3 = Trace(C) - 25*QO - 24*QA.
        S3 = traceC - 25*QO - 24*QA
        wrong_branch = traceC - 25*QA - 24*QO
        A_child = QA - QO
        assert S3 - wrong_branch == A_child
        ...
        print(
            ...
            f"prime={p}|tau={int(tau)}|u0={int(u0)}|wO={int(wO)}|"
            f"factor_degrees={','.join(map(str, factor_degrees))}|"
            f"traceC={point_text(traceC)}|"
            f"QO={point_text(QO)}|QA={point_text(QA)}|"
            f"A_child={point_text(A_child)}|"
            f"S3={point_text(S3)}|wrong={point_text(wrong_branch)}|status=PASS",
            flush=True,
        )
        print(
            "Q6P2TRACE_RESULT|"
            "degree50_trace=PASS|quartic_transport=PASS|frobenius_descent=PASS|"
            "orientation=PASS_LOCAL_POLE_ASYMPTOTIC|"
            "status=PASS_ORIENTED_S3_SMOKE_TEST",
            flush=True,
        )
        break
