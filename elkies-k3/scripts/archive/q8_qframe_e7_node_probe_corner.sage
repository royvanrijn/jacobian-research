...
parser.add_argument(
    "--mode",
    choices=("finite-corner", "local-normal-form"),
    default="finite-corner",
    help=(
        "finite-corner is the fast one-way Artinian obstruction used by the "
        "repository; local-normal-form asks Singular for the exact ds local basis"
    ),
)
...
for chart in sorted(pullbacks["charts"], key=lambda item: item["name"]):
    ...
    if args.mode == "local-normal-form":
        ...
        # q8 E7-node finite-corner mode.  Because t is monomial-times-unit at
        # the node, membership in (surface,t^T) implies vanishing in this
        # finite Artinian corner quotient.  Failure here is therefore a
        # rigorous rejection; survival still needs the exact local ds test.
...
payload = {
    ...
    "status": (
        ...
        else "EXPERIMENTAL_Q8_QFRAME_E7_NODE_LOCAL_NORMAL_FORM"
    ),
    ...
    "boundary": (
        (
            ...
            "q-frame. Characteristic-zero lifting and a complete Cech overlap "
