...
for twist in twists:
    ...
    tag = f"p{P}-twist-{'m'+str(-twist) if twist < 0 else 'p'+str(twist)}"
    prefix = f"zz-h92-q8-twistladder-{tag}"
    ambient_basis = build_ambient(twist)

    print(
        f"TWIST_START|prime={P}|n={twist}|ambient={len(ambient_basis)}",
        flush=True,
    )
    global_data = build_global_matrix(twist, ambient_basis, prefix)
    K = global_data["kernel"]

    combined_rows = []
    chart_records = []
    combined_rank = 0
    for chart_name in TRANSLATED_CHARTS:
        M, Lold = translated_matrix(twist, ambient_basis, K, chart_name)
        rank = int(M.rank())
        before = combined_rank
        combined_rows.extend([list(row) for row in M.row_space().basis()])
        combined_rank = (
            int(matrix(finite, combined_rows).rank()) if combined_rows else 0
        )
        gain = combined_rank - before
        ...
        print(
            "TWIST_TRANSLATED|"
            f"prime={P}|n={twist}|chart={chart_name}|L={Lold}|"
            f"rank={rank}|gain={gain}|combined={combined_rank}",
            flush=True,
        )

    final_h0 = int(K.nrows()) - combined_rank
    elapsed = time.perf_counter() - started
    result = {
        "twist": twist,
        "ambient": len(ambient_basis),
        "H_rank": global_data["H_rank"],
        "E8_rank": global_data["E8_rank"],
        "E7_rank": global_data["E7_rank"],
        "generic_rank": global_data["global_rank"],
        "generic_h0": global_data["generic_h0"],
        "translated_rank": combined_rank,
        "final_h0": final_h0,
    ...
    print(
        "Q8TWIST|"
        f"prime={P}|n={twist}|ambient={result['ambient']}|"
        f"generic_h0={result['generic_h0']}|"
        f"translated_rank={combined_rank}|final_h0={final_h0}|"
...
if all(n in by_twist for n in (-1,0,1)):
    hm1 = by_twist[-1]["final_h0"]
    h0 = by_twist[0]["final_h0"]
    hp1 = by_twist[1]["final_h0"]
    if (hm1,h0,hp1) == (0,10,28):
        diagnosis = "FORCED_SPLITTING"
        degree = -8
        splitting = "O^10 + O(-1)^8"
    print(
        "Q8TWISTLADDER|"
        f"prime={P}|hm1={hm1}|h0={h0}|hp1={hp1}|"
        f"diagnosis={diagnosis}|"
        f"splitting={splitting if splitting else 'NA'}|"
        f"degree={degree if degree is not None else 'NA'}",
        flush=True,
    )

out = GEN / f"zz-h92-q8-true-twist-ladder-mod-{P}.json"
out.write_text(json.dumps({
    "schema": "elkies-k3.h92-q8-true-twist-ladder-modp.v1",
    "status": "EXPERIMENTAL_MODULAR_Q8_COMPILED_BUNDLE_TWIST_LADDER",
    "prime": P,
    "results": results,
    "diagnosis": {
        "kind": diagnosis,
