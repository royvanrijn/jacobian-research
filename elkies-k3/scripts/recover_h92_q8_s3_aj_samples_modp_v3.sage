...
This script loads the generic q8 quartic and square factor serialized by
the global-orientation certificate.  Every finite-field specialization is obtained by reducing those exact
...
# ===========================================================================
# Load the serialized exact generic q8 transport normalization.
#
# The v1 sampler rebuilt the huge QQ(U) quartic here before its first print.
# The v2 global-orientation artifact now stores that quartic and its square
# factor, so sampling can start without repeating the expensive exact build.
# ===========================================================================
...
if orient.get("schema") != "elkies-k3.h92-q8-global-orientation.v2":
    raise SystemExit(
        "q8-global-orientation.json is the old v1 artifact. "
        "Regenerate it with derive_h92_q8_global_orientation_v2.sage first."
    )
transport = orient.get("generic_transport")
if not transport:
    raise SystemExit("global orientation artifact has no serialized generic transport")
...
# Small replay checks on the loaded transport data.
...
print(
    "Q8S3AJORIENT|generic_transport=LOADED|global_wIV=PASS|"
