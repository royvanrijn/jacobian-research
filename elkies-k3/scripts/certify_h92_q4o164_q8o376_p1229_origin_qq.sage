...
This combines two independent exact facts:

1. ``identify_h92_q4o164_transported_degree1_marked_classes_qq.sage`` assigns
   a transported equation section to the inherited marked curve P1229 using
   exact heights and pairings in MW9;
2. ``point_h92_q4o164_q8o376_from_known_sections_qq.sage`` restricts that same
   equation section to the exact q8 pencil and, when the restriction has
   degree one, constructs an exact rational point on the binary quartic and
   verifies the pointed Jacobian invariants.

The result is the preferred equation-level zero selected by the certified
q8->q12 lattice route.  This does not yet identify all four I2 components of
the q8 child; that physical component attachment remains a separate gate.
"""
...
q8_frames = [
    record
    for route_name in ("promoted_q323_free_pinned_route", "promoted_q12_direct_route")
    if route_name in handoff
    for record in handoff[route_name].get("compiler_frames", [])
    if record.get("edge") == "q8/o376"
]
if not q8_frames:
    # Older handoff schema keeps the compiler frames under one promoted route.
...
negative_matches = [
    record for record in identification["sections"] if record["negative_matches_P1229"]
]
if len(matches) + len(negative_matches) != 1:
    payload = {
        "schema": "elkies-k3.q4o164-q8o376-p1229-origin-qq.v1",
        "status": "OPEN_Q4O164_Q8O376_P1229_EQUATION_ORIGIN",
    ...
    print(f"Q8O376P1229|status={payload['status']}|output={OUTPUT}", flush=True)
    raise SystemExit(2)
...
point_candidates = [
    record
    for record in pointing.get("degree_one", [])
    if record.get("name") == name and int(record.get("sign", 0)) == required_sign
]
if len(point_candidates) != 1:
    # Some versions only retain all candidates, not a separate degree_one list.
    point_candidates = [
        record
        for record in pointing.get("candidates", [])
        if record.get("name") == name
        and int(record.get("sign", 0)) == required_sign
        and int(record.get("new_base_degree", -1)) == 1
    ]
if len(point_candidates) != 1:
    payload = {
        "schema": "elkies-k3.q4o164-q8o376-p1229-origin-qq.v1",
        "status": "OPEN_Q4O164_Q8O376_P1229_RESTRICTION_NOT_DEGREE_ONE",
...
payload = {
    "schema": "elkies-k3.q4o164-q8o376-p1229-origin-qq.v1",
    "status": "PASS_EXACT_QQ_Q4O164_Q8O376_P1229_POINTED_ORIGIN",
    "child": {
        "ADE": "4A1",
        "MW_rank_if_rho19": 13,
        "selected_zero": "P1229",
    ...
    "next_required": (
        "Attach the four physical I2 root components to the P1229-pointed 4A1 equation, "
        "then import the certified q8 NS transport and compile q12/orbit5867."
    ),
    "proof_boundary": (
        ...
        "Thus the invariant 4A1 Jacobian is now pointed by the route-selected zero. "
        "The four child I2 component labels and complete equation-to-lattice frame remain open."
    ),
    "inputs": {
        "paths": [str(path.relative_to(ROOT)) for path in INPUTS],
        "sha256": {str(path.relative_to(ROOT)): sha256(path) for path in INPUTS},
...
OUTPUT.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n")
print(
    "Q8O376P1229|transported={}|sign={}|status={}|output={}".format(
        transported_index, required_sign, payload["status"], OUTPUT
    ),
    flush=True,
)
