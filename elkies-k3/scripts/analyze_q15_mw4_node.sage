...
print(f"Q15MW4|stage=input|rank={F.rank()}|det={F.det()}|positive={F.is_positive_definite()}",flush=True)
...
print(f"Q15MW4|stage=roots|half={len(half)}|signed={len(roots)}|rank={R.rank()}|det={abs(G.det())}",flush=True)
...
print(f"Q15MW4|stage=ADE|solutions={sols}",flush=True)
...
print(f"Q15MW4|stage=shioda|MW_rank={mw_rank}|rootdet={rootdet}|regulator_if_torsion1={reg_t1}",flush=True)

out=Path("artifacts/local/elkies-k3/q15-mw4-node.txt")
out.parent.mkdir(parents=True,exist_ok=True)
out.write_text(
    f"ADE={sols}\nMW_rank={mw_rank}\nrootdet={rootdet}\nregulator_t1={reg_t1}\n"
    f"root_basis=\n{B}\nroot_gram=\n{G}\n"
)
print(f"Q15MW4|stage=done|out={out}",flush=True)
