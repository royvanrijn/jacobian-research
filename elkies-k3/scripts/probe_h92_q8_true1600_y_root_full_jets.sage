...
18-dimensional survivor, not merely rank over GF(p)(s).
...
Run:
  sage -python ~/Downloads/probe_h92_q8_true1600_y_root_full_jets.sage
...
CONDITIONS_PATH = GEN / "zz-h92-q8-true1600-generic-conditions.json"
TRANSLATED_PATH = GEN / f"zz-h92-q8-true1600-two-translated-divisors-mod-{P}.json"
OUTPUT_PATH = GEN / f"zz-h92-q8-true1600-y-root-full-jets-mod-{P}.json"
...
def compute_component(component, chart_name, solved_name):
    ...
    # Build all 18 obstruction series modulo e^M.
    ...
    Mmat = matrix(finite, rows) if rows else matrix(finite, 0, 18)
    rank = int(Mmat.rank())
...
print(
    f"YROOTFULLJETS_START|prime={P}|ambient=1600|survivors=18|components=3",
    flush=True,
)
...
payload = {
    "schema": "elkies-k3.h92-q8-true1600-y-root-full-jets-modp.v1",
    "status": "EXPERIMENTAL_MODULAR_TRUE1600_Y_ROOT_FULL_NORMAL_JETS",
    "prime": int(P),
    "common": {
    ...
    "boundary": (
        ...
        "rank below 16 would mean the remaining finite E7 information is not "
