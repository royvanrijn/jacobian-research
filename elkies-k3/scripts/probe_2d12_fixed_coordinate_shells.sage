...
"""Scan the nonidentity residual-class fixed-coordinate shell in N(2D12).

For the component-swap class of the exact glue-preserving S2 residual group,
test every rank-seven coordinate direct summand of a pinned LLL basis of the
primitive rank-twelve fixed lattice. Apply the determinant-5000,
...
RESIDUAL = (
    ROOT
    / "artifacts/generated-results/elkies-k3-2d12-glue-residual-group-v1.json"
)
DEFAULT_OUTPUT = (
    ROOT
...
def build(catalog, residual, determinant_bound, minimum_mw_rank):
    ...
    assert residual["schema"] == "elkies-k3.2d12-glue-residual-group.v1"
    assert residual["status"] == "PASS_EXACT_2D12_GLUE_AND_RESIDUAL_GROUP"
    ...
    assert class_scans[0]["residual_conjugacy_class_id"] == "2D12-C02"
    assert class_scans[0]["accounting"][
        "high_mw_mod2_accepted_seeds"
    ] == 0
    return {
        "schema": "elkies-k3.2d12-fixed-coordinate-shell-probe.v1",
        "status": "PASS_EXACT_PRE_RESIDUAL_QUOTIENT_2D12_COORDINATE_SHELL_SCAN",
        "proof_scope": {
            "proved": (
        ...
        "parameters": {
            ...
            "seed_language": (
                ...
                "for the component-swap fixed lattice"
            ),
        },
...
def main():
    ...
    if arguments.check:
        if not output.exists() or output.read_text() != encoded:
            raise SystemExit("2D12 coordinate-shell probe is stale")
    else:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(encoded)
    print(
        "2D12SHELL|classes={}|subsets={}|seeds={}|status=PASS_EXACT".format(
