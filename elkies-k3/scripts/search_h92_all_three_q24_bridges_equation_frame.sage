...
OUT=args.output.resolve() if args.output else LOCAL/"q24-all-three-equation-bridge-search.json"
...
# All three direct q24 -> D12 targets in this same current equation D13 frame.
ALL3 = LOCAL/"q24-three-d12-current-equation-profiles.json"
if not ALL3.exists():
    raise SystemExit(f"Missing prerequisite: {ALL3}")
all3 = json.loads(ALL3.read_text())
assert all3["status"] == "PASS_EXACT_THREE_Q24_D12_CURRENT_EQUATION_PROFILES"

target_vectors = {
    int(item["orbit"]): vector(ZZ, item["equation_mw"])
    for item in all3["profiles"]
}
assert sorted(target_vectors) == [73,85,184]
assert target_vectors[85] == zQ24

for orbit,target in sorted(target_vectors.items()):
    assert target*H13*target == 52
    print(
        "Q24ALLBRIDGE_TARGET|"
        f"orbit={orbit}|mw={','.join(map(str,target))}|height=52|status=PASS",
        flush=True,
    )
...
for a in range(int(lower[0]),int(upper[0])+1):
    for b in range(int(lower[1]),int(upper[1])+1):
        for c in range(int(lower[2]),int(upper[2])+1):
            ...
            matches=[]
            for orbit,target0 in sorted(target_vectors.items()):
                for sign,target in ((+1,target0),(-1,-target0)):
                    residual=target-z
...
for rank,rec in enumerate(compatible[:args.top],1):
    corrections=";".join(
        f"orbit={m['orbit']},orient={m['orientation']:+d},"
        f"add={m['G1']}*G1+{m['G3']}*G3"
...
# Per-orbit exact minima. The enumeration above is exhaustive below threshold.
best_by_orbit={}
for orbit in sorted(target_vectors):
    hits=[
        rec for rec in compatible
        if any(m["orbit"]==orbit for m in rec["q24_matches"])
    ]
    hits.sort(key=lambda r:(r["q8_degree"],sum(abs(x) for x in r["q6_word"])))
    best_by_orbit[orbit] = None if not hits else hits[0]

for orbit in sorted(target_vectors):
    rec=best_by_orbit[orbit]
    if rec is None:
        print(
            "Q24ALLBRIDGE_BEST|"
            f"orbit={orbit}|best_degree=none|threshold={args.threshold}|"
            "status=NO_BRIDGE_BELOW_THRESHOLD",
            flush=True,
        )
        continue
    match=next(m for m in rec["q24_matches"] if m["orbit"]==orbit)
    print(
        "Q24ALLBRIDGE_BEST|"
        f"orbit={orbit}|best_degree={rec['q8_degree']}|"
        f"word={','.join(map(str,rec['q6_word']))}|"
        f"AJ={','.join(map(str,rec['AJ_equation_D13']))}|"
        f"orient={match['orientation']:+d}|"
        f"add={match['G1']}*G1+{match['G3']}*G3|"
        f"q6Oold={rec['q6_P_dot_Oold']}|q6Ostd={rec['q6_P_dot_Ostd']}|"
        "status=PASS_MINIMUM_BELOW_THRESHOLD",
        flush=True,
    )
...
payload={
    "schema":"elkies-k3.h92-q24-all-three-equation-bridge-search.v1",
    "status":status,
    ...
    "equation_D13":{
        ...
        "all_three_q24_targets":{
            str(orbit): list(map(int,target))
            for orbit,target in sorted(target_vectors.items())
        },
    },
    "best_by_orbit":{
        str(orbit): (
            None if best_by_orbit[orbit] is None else best_by_orbit[orbit]
        )
        for orbit in sorted(target_vectors)
    },
    "search":{
...
def _best_degree(orbit):
    rec=best_by_orbit[orbit]
    return "none" if rec is None else str(rec["q8_degree"])

numeric=[
    (best_by_orbit[o]["q8_degree"],o)
    for o in sorted(target_vectors)
    if best_by_orbit[o] is not None
]
numeric.sort()
winner="none" if not numeric else str(numeric[0][1])
print(
    "Q24ALLBRIDGE_RESULT|"
    f"orbit73={_best_degree(73)}|orbit85={_best_degree(85)}|"
    f"orbit184={_best_degree(184)}|winner={winner}|"
    f"threshold={args.threshold}|exhaustive_below_threshold=1|status=PASS",
    flush=True,
)
