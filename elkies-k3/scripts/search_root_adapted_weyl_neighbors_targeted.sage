...
parser.add_argument(
    "--target-root-rank",
    type=int,
    help="with streaming first-hit mode, only stop on this exact child root rank",
)
parser.add_argument(
    "--target-root-count",
    type=int,
    help="with streaming first-hit mode, only stop on this exact child root count",
)
parser.add_argument(
    "--target-root-determinant",
    type=int,
    help=(
        "optional exact child root determinant; evaluated only after rank/count "
        "match, so it does not slow the bulk screening"
    ),
)
parser.add_argument(
...
if args.target_root_rank is not None and args.target_root_rank < 0:
    parser.error("--target-root-rank must be nonnegative")
if args.target_root_count is not None and args.target_root_count < 0:
    parser.error("--target-root-count must be nonnegative")
if args.target_root_determinant is not None and args.target_root_determinant <= 0:
    parser.error("--target-root-determinant must be positive")
if (
    any(v is not None for v in (
        args.target_root_rank,
        args.target_root_count,
        args.target_root_determinant,
    ))
    and not (args.stream_first_growth and args.stop_after_first_growth)
):
    parser.error(
        "exact target-root screening requires "
        "--stream-first-growth --stop-after-first-growth"
    )
...
for q in sorted(set(args.q)):
    ...
    for mw in mw_vectors:
        ...
        for choices in combined_weights_by_norm.get(target - mw_norm, ()):
            ...
            if (
                args.target_root_rank is not None
                and int(child_root_rank) != args.target_root_rank
            ):
                continue
            if (
                args.target_root_count is not None
                and int(child_root_count) != args.target_root_count
            ):
                continue
            if 17 - int(child_root_rank) < adapt_mw_at_least:
                continue

            if args.target_root_determinant is not None:
                _, _, target_data = roots_and_data(child)
                if int(target_data[2]) != args.target_root_determinant:
                    continue
