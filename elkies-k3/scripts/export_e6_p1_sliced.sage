...
# Start from section-first variables.
...
# Solve b6,b7 from B(1)=2s1^3 and B'(1)+s1*A'(1)=0.
...
# Safe elimination 2: P1_6 -> b5.
E6=RF(S1a[6])
c6p=E6.derivative(b5)
if c6p==0 or c6p.derivative(b5)!=0:
    raise RuntimeError("P1_6 not affine-linear in b5")
...
# Safe elimination 3: P1_5 -> y2.
E5=RF(S2[5])
c5=E5.derivative(y2)
if c5==0 or c5.derivative(y2)!=0:
    raise RuntimeError("P1_5 not affine-linear in y2")
...
# Safe elimination 4: P1_4 -> a3.
E4=RF(S3[4])
c4=E4.derivative(a3)
if c4==0 or c4.derivative(a3)!=0:
    raise RuntimeError("P1_4 not affine-linear in a3")
a3_expr=RF(-E4.subs({a3:0})/c4)

subs4={a3:a3_expr}
A4=subpoly(A3,subs4)
B4f=subpoly(B3,subs4)
X4=subpoly(X3,subs4)
Y4=subpoly(Y3,subs4)
...
def convert(fr):
    ...
    for mon,coef in num.dict().items():
        ...
        for i,exp in enumerate(mon):
            ...
            nm=names[i]
            if nm not in rrd:
                raise RuntimeError(f"eliminated variable survived: {nm}")
            term*=rrd[nm]**exp
        out+=term
    return out
...
# Fiber equations still not used in triangular section chain.
add("I4_1_D2",D2(1))
add("I4_1_D3",D3(1))

for pref,a,s in [
    ("I2_lam",settle(lam),settle(sl)),
    ("I2_mu",settle(mu),settle(sm))
]:
    add(pref+"_A",A4(a)+3*s**2)
    add(pref+"_B",B4f(a)-2*s**3)
    add(pref+"_d1",Bp(a)+s*Ap(a))

# P1_7..P1_4 vanish by construction; keep P1_0..P1_3.
for k in range(4):
    add(f"P1_{k}",S4[k])

base_eq_count=len(eqs)

# Three generic linear slices for expected 3-dimensional P1 locus.
varsRR=list(RR.gens())
for j in range(args.slices):
    form=RR(K.random_element())
    # dense-ish but simple random affine hyperplane
    for v in varsRR:
        c=K.random_element()
        if c:
            form += c*v
    eqs.append(form)
    tags.append(f"SLICE_{j+1}")
...
# Record eliminated expressions for reconstruction.
meta=Path(args.out).with_suffix(".meta.txt")
