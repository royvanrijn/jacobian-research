...
print(
    ...
    f"status={'PASS' if deg2 else 'NO_DESCENDING_DEG2_TARGET'}",
    flush=True,
)
if not deg2:
    raise SystemExit("no GF(p^2) combination descends to an F_p target with Z-degree 2")

target_exports=[]
for i,(X,Y,Z) in enumerate(deg2):
    ...
    print(
        ...
        "field=Fp|status=PASS_EXPLICIT_PDOT_O_2",
        flush=True,
    )
