...
The occurrence of five here has an exact but limited Kummer interpretation.
The normalizer of the inertia subgroup generated by the `(5,1)` cycle is
`C_5 semidirect C_4 = AGL(1,5)`, the same permutation group as a generic
fifth-root splitting field.  Multiplier parity cuts out
`Q(sqrt(5))` inside `Q(zeta_5)`, so its quadratic character agrees with the
arithmetic sign character above.  However, `A_6` is perfect and the full
marked residue triple has trivial centralizer; there is no global `C_5`
character or deck action.  The exact comparison and the resulting conditional
rational-source degree-eleven fivefold-spectator model are proved in
