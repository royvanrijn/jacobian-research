...
line=(R/'factor_q0_exact_clean.out').read_text().splitlines()[0];x=line.split('=',1)[1];x=re.sub(r'u(\d+)',r'u**\1',x);x=re.sub(r'(\d)u',r'\1*u',x);s=sp.Symbol('s');sv=ec.eval_u(-sp.expand(sp.sympify(x,locals={'s':s,'u':ec.U})-s));red=[ss(q,1,sv) for q in qs]
...
for e in out:
 ...
 ser.append(ee)
(R/'hne0_deg35.pkl').write_bytes(pickle.dumps(ser,protocol=4));print([len(e) for e in ser],[max(map(sum,e)) for e in ser])
