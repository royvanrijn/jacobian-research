...
for br,line in enumerate(fl[:2],1):
 ...
 for i in keep:
  equation={}
  for monomial,coefficient in ww[i].d.items():
   # r and s were eliminated; retain (h,u1,u2,u3).
   assert monomial[0]==monomial[1]==0
   target=(monomial[2],monomial[3],monomial[4],monomial[5])
   equation[target]={
    j:(int(coefficient.p[j].p),int(coefficient.p[j].q))
    for j in range(len(coefficient.p)) if coefficient.p[j]
   }
  machine.append(equation)
 (R/f'case1_branch{br}_after_w.pkl').write_bytes(pickle.dumps(machine,protocol=4))
