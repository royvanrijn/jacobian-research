...
SOURCES = {
    ...
    'support': 'archive/non-elliptic/verified/SUPPORT_SATURATION_PATHS.json',
...
def records(root: Path, entries: list[dict]) -> list[dict]:
    ...
    def add(identifier, kind, heading, source, payload, group, *, summary=None):
        ...
        result.append(dict(id=identifier, kind=kind, title=heading, source=source,
                           ...
                           programme_status='archived' if kind == 'support' else 'active',
...
def filtered(rows: list[dict], kind: str | None = None, unknown_only: bool = False,
             *, history: bool = False) -> list[dict]:
    if not history:
        rows = [row for row in rows if is_active(row)]
    if kind:
