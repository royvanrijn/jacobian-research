...
def audit_existing() -> None:
    ...
    assert payload["status"]["id"] == "HC4RSD8"
    assert payload["rank_two_passive_split"]["new_rank_two_frontier"] == (
        "rank Hess_(z,w)(B)=1"
    )
    assert payload["open_frontier"] == (
        "rank-one and rank-two quadratic pivots with x-moving generic kernel lines"
    )
    print(
        "PASS: committed HC4RSD8 stage artifact is intact; its rank-one/two "
        "frontier is historical and closed by HC4RSD9--10; no symbolic replay "
        "or rewrite"
    )
