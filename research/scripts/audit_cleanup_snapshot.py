...
import re
from research_programmes import relocate_reference
...
def audit() -> None:
    ...
    def rebase(match):
        ...
        return "](" + os.path.relpath((ROOT / relocate_reference(path)).resolve(), destination.parent) + (sep + anchor if sep else "") + ")"
