#!/usr/bin/env python3
"""Bounded curve92 M26 discovery with native productive anchors and two map policies.
...
PREP = LOCAL/'curve92-v3-preparation-v1'
BANK = PREP
...
def freeze(folder):
    ...
    require(prepared['files']['seed-M26.json'] == sha(PREP/'seed-M26.json'), 'M26 seed changed')
    require(all(sha(PREP/n) == h for n,h in prepared['files'].items()), 'prepared native inputs changed')
    names = ('run_curve92_productive_v3.py', 'visibility_productive92.py',
    ...
    inputs = [PREP/'seed-M26.json', PREP/'prepared.json', BANK/'anchor-bank.json',
              BANK/'protocol.json', BANK/'gains.json', PREP/'generic-cvp-proofs.json']
    protocol = {'schema': 'curve92-productive-adaptive.v1', 'initial_rank': 26,
        ...
        'max_epochs': 7, 'mapping_order': list(MAPPERS),
        ...
        'scope': 'New directions beyond the certified M26 on curve92. Historical productive '
...
def search(folder):
    ...
    proof = seed['proof']
    require(len(points) == 26, 'must start at M26')
    ...
    for epoch in range(protocol['max_epochs']):
        ...
        snapshot = {'status': 'RETAINED_PRODUCTIVE_EPOCH_CLOUD', 'family': 'a1-fibration-02',
            'parameter': '3161/432', 'curve': seed['curve'], 'charts': charts, 'final_state': state.record()}
        snap = wd/'cloud.json'
        if snap.exists():
