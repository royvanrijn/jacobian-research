...
def check(D, protocol_path, cloud_path):
    p=cert.read(protocol_path);rank=len(cert.read(D/"seed.json")["points"]);
    control=SimpleNamespace(SEED=D/"seed.json")
    def masks(p):
        result=[];i=0
        floor=next((r.get("mask_floor",r["generic_dimension"]) for r in p.get("rows",[]) if r["id"]==D.name),17)
        while len(result)<p["sample_size"]:
            domain=next((r.get('sample_domain',p['sample_domain']) for r in p.get('rows',[]) if r['id']==D.name),p['sample_domain'])
            m=int(digest([domain,i]),16)%(1<<rank);i+=1
            if m>>floor and m not in result:result.append(m)
        return result
    maps=cert.read(D/'maps.json');data=cert.read(D/'result.json');seed=cert.read(control.SEED);initial=tuples(seed['points'])
    ...
    if [r['parity'] for r in maps['sample']]!=masks(p):raise ArithmeticError('fixed2048 masks differ')
    ...
    cloud_check(data,data['charts'],cloud_path,D/'result.json')
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--run',type=Path,required=True);parser.add_argument('--protocol',type=Path,required=True);parser.add_argument('--cloud',type=Path,required=True);args=parser.parse_args()
    check(args.run,args.protocol,args.cloud)
