...
ROOT=batch.ROOT;ART=batch.ART;D=batch.D;CONTROL=ROOT/'artifacts/local/elliptic-curves/annulus64-controller-v2'
INPUT=ART/'annulus64_r17_results_v1.json';OUT=ART/'annulus64_experiment_v1.json'
def expected():
    ...
    return {'schema':'elliptic-curves.annulus64-experiment.v1','status':'PASS_STRONGER_ODD_BOUNDS_REQUIRE_REVIEW' if stronger else 'PASS',
        ...
        'claim_boundary':'Eight fresh signed denominator slices in131072<H<=524288 on11952, disjoint from the previous complete square. All32768 retained cached scores precede4096 exact equation exclusions/scalar scores and64 distinct finalists; wholly disjoint validation never changes selection. Up to3136 generic17-only point charts with all maps before points. The977 prior-equation and593 catalogue exclusions are independently rechecked after terminal exact point proofs. Matching finite bounds do not prove exact rank, saturation or absence. No universal novelty, rank density, calibrated sensitivity or isolated portability claim.'}
if __name__=='__main__':
    ...
    if a.check:
        ...
        checkpoint(OUT,d)
    print('ANNULUS64 OUTCOMES',d['status'],d['completed_point_boxes'],[(r['id'],r['rank_lower_bound']) for r in d['rows'] if r['rank_lower_bound']>=22],flush=True)
