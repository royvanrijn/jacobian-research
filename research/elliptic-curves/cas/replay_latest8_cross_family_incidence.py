...
def replay(path, output):
    ...
    original = cert.read(ROOT/'artifacts/generated-results/elliptic-curves/new_high_rank_curve_index_v4.json')
    if data['targets'] != [{k:r[k] for k in ('id','curve','j_invariant','family','parameter','rank_lower_bound')} for r in original['curves'] if int(r['id'].split('-')[-1])>39]:
    ...
    if len(maps) != 12 or len(targets) != 8 or len(data['pairs']) != 96 or {(r['target'],r['family']) for r in data['pairs']} != {(i,f) for i in targets for f in maps}:
    ...
    checkpoint(output, {'schema':'elliptic-curves.compact-cross-family-j-incidence-replay.v1',
        ...
        'pairs_checked':96,'status_counts':counts,
    ...
    print('REPLAYED LATEST8 CROSS-FAMILY J INCIDENCE', counts, flush=True)
