...
OUT=ART/'nearcut60v3_mw16_accounting_replay_v1.json'
...
def check_exact_roster(protocol, selected):
    # Independently spell the exact boundary; do not import the worker projection.
    fields=('id','family','band','sign','parameter','numerator','denominator',
            'model','arm','block','retained_index','retained_rank','late_rank',
            'original_scalar_id','j_invariant','j_height','parameter_height',
            'combined_late_units','combined_late_good')
    expected=[{key:row[key] for key in fields} for row in selected]
    same(protocol['rows'],expected)
    def exact(value):
        if isinstance(value,float):raise ArithmeticError('float in exact roster')
        if isinstance(value,dict):
            for v in value.values():exact(v)
        elif isinstance(value,list):
            for v in value:exact(v)
    exact(protocol['rows'])
...
def audit():
    ...
    return {'schema':'elliptic-curves.nearcut60v3-independent-accounting.v1','status':'PASS','sources':bindings,
