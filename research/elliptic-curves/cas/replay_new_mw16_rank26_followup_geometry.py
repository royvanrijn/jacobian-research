...
def main():
    ...
    for identifier in IDS:
        ...
        for c in maps['centres']:
            if c['parity']!=c['generic_mask']|(c['quotient_word']<<16) or len(c['representative'])!=26 or any((c['representative'][j]-(c['parity']>>j))%2 for j in range(26)):raise ArithmeticError('exact centre parity differs')
        ...
        cloud_check(data,seed['charts']+data['charts'],follow.ART/('mw16_new26_'+identifier.replace('-','_')+'_all_retained_mod2_v1.json'),D/'all-retained-point-cloud-only.json')
