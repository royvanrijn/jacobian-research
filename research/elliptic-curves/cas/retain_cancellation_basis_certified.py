#!/usr/bin/env python3
"""Retain the certified-bank control and inspect all bound receipts.
...
def bindings():
    ...
    for receipt in supervision['records']:
        ...
        need(verification['status'] == 'PASS' and verification['new_directions'] == result['new_directions'] and verification['rank'] == result['rank_lower_bound'], 'unverified arm')
...
def portable():
    ...
    with tempfile.TemporaryDirectory(prefix='basis-certified-empty-root-') as temp:
        ...
        code = "import sys;sys.path.insert(0,'elliptic-curves');sys.path.insert(0,'elliptic-curves/cas');import retain_cancellation_basis_certified as r;from verify_cancellation_basis_certificate import verify_arm;import ecsearch.q12o5867_specialization;a=r.bindings();print(a['status'],len(a['rows']))"
