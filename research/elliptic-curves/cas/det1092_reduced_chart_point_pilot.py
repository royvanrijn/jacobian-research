...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';BATCH=ROOT/'artifacts/local/elliptic-curves/det1092-reduced-chart-point-pilot-v1';SAGE='/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python'
def sources():
 return {**engine.sources(),**{str((CAS/n).relative_to(ROOT)):cert.hashed(CAS/n) for n in ['det1092_reduced_chart_point_pilot.py','prepare_det1092_reduced_chart_point_pilot.sage','verify_det1092_point_seeds.sage']}}
def freeze():
 ...
 checkpoint(BATCH/'protocol.json',dict(schema='elliptic-curves.det1092-reduced-chart-point-pilot.v1',sources=sources(),inputs={str(x.relative_to(ROOT)):cert.hashed(x) for x in [parentfile,gate,chartproof,ROOT/'artifacts/local/elliptic-curves/det1092-parameter-lattice-reduction-v1/result.json']},rows=rows,sample_size=2048,sample_domain='full11952-specialized-followup-v1',charts=49,height=125000,seconds_per_chart=10,rank_stop=False,target_rank=32,maximum_point_boxes=98,geometry_wall_seconds=180,worker_wall_seconds=600,replay_wall_seconds=300,rss_bytes=2147483648,maximum_workers=1,gp_sha256=cert.hashed(Path('/usr/bin/gp')),
 previous_attempt=None,gate='The factor-free original27-only blind49 control recovered28. An independent generic isomorphism and all17 section transports certify the equation-only reduced parameter chart; its90/135-bit coefficient sizes justify matched point exposure after the huge-model pilot returned no finite points.',
...
def intake():
 ...
 for row in p['rows']:
  ...
  seed=dict(original_parameter=str(original_t),family=row['family'],parameter=row['parameter'],curve=list(map(str,model)),points=[list(map(str,P)) for P in points],generic_points=[list(map(str,P)) for P in points],rank_certificate=proof,long_curve=list(map(str,a)),long_points=[list(map(str,P)) for P in images],protocol_hash=digest(p),model_coefficient_bits=max(max(abs(v.numerator).bit_length(),v.denominator.bit_length()) for v in model))
...
def launch():
 ...
 for i,row in enumerate(p['rows']):
  configure(i);s=run([SAGE,str(CAS/'prepare_det1092_reduced_chart_point_pilot.sage'),'--index',str(i)],limits=Limits(p['geometry_wall_seconds'],p['rss_bytes']),log_path=D/'maps.log',checkpoint_path=D/'maps.supervisor.json',cwd=ROOT)
