...
import kihara_fresh_point_pilot as control
...
def main():
    ...
    model=tuple(map(cert.F,seed['curve']));E=EllipticCurve(QQ,[QQ(str(c)) for c in model])
    old=[E([QQ(c) for c in P]) for P in seed['original_points']];new=[E([QQ(c) for c in P]) for P in seed['points']]
    if new[1:]!=old[1:] or 6*new[0]!=sum(old[:12],E(0)):raise ArithmeticError('index-six seed identity differs')
    ...
    cloud_check(data,data['charts'],control.ART/('kihara_fresh_point_pilot_'+control.ROW['id'].replace('-','_')+'_mod2_v1.json'),D/'result.json')
