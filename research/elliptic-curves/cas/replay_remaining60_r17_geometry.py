...
def main():
    ...
    for row in p['rows']:
        ...
        geometry(data,maps,initial,{**p,'maps_path':folder/'maps.json'});cloud_check(data,data['charts'],ART/('remaining60_r17_'+row['id'].replace('-','_')+'_mod2_v1.json'),path);count+=len(data['charts'])
    if sum(len(cert.read(batch.D/r['id']/'maps.json')['rows']) for r in p['rows'])!=2652:raise ArithmeticError('fixed2652 declared roster differs')
    print('REPLAYED',count,'OF2652 DECLARED R17 CHARTS, GENERIC CENTRES, RAW POINTS AND CLOUD PROVENANCE',flush=True)
