...
import followup_next24_rank27 as follow
...
def main():
    ...
    mod2=ART/'next24_rank27_all_retained_mod2_v1.json';modl=ART/'next24_rank27_all_retained_modl_v1.json';stages=[('mod2-build',[str(CAS/'audit_recorded_point_mod2_rank_v3.py'),'--input',str(output),'--input-sha256',cert.hashed(output),'--output',str(mod2),'--prime-bound','997'],120),('mod2-check',[str(CAS/'audit_recorded_point_mod2_rank_v3.py'),'--check',str(mod2)],120),('modl-build',[str(CAS/'audit_retained_cloud_modl.py'),'--input',str(mod2),'--output',str(modl)],180),('modl-check',[str(CAS/'audit_retained_cloud_modl.py'),'--check',str(modl)],180)];ledger={'status':'RUNNING','rows':[]};checkpoint(D/'cloud-verification-ledger.json',ledger)
