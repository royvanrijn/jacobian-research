...
from cancellation_basis_early import ROOT, CAS, OUT, RAW, read, sha, need, new_write, guard
...
def audit():
    ...
    start = time.process_time(); rows=[]; counts=Counter(); blocks={str(i):Counter() for i in (0,1)}
    ...
    result={'status':'PASS','rows':rows,'counts':dict(counts),'blocks':{k:dict(v) for k,v in blocks.items()},
        'cpu_seconds':time.process_time()-start,'cpu_limit':POSTMORTEM_CPU_LIMIT,'source_sha256':sha(Path(__file__)),
