#!/usr/bin/env python3
"""Six higher parameters per band and R17 family from frozen extended scores."""
import argparse
from pathlib import Path
from concurrent.futures import ThreadPoolExecutor,as_completed
import certify_compact_r17_candidates as cert
import compact_atlas_specialization as spec
import score_r17_remaining_higher as extension
...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';D=ROOT/'artifacts/local/elliptic-curves/remaining60-r17-pari-v1';SAGE='/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python'

def sources():
    names=['memory_rank_certificate.py','remaining60_r17_pari_batch.py','prepare_remaining60_r17_pari_batch.sage','prospective_half_lattice_v2.sage','compact_atlas_specialization.py','research_runtime/search_state.py','research_runtime/cached_observation_state.py','research_runtime/preloaded_prime_state.py','research_runtime/rotated_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
...
PARITY=ART/'r17_exact_maximum_parity_classes_v1.json'
def census_paths():
    return [PARITY]
def masks():
    d=cert.read(PARITY)
    if not d['parity_upper_bounds_replayed']:
        raise ArithmeticError('proved generic parity census required')
    result={r['family']:[c['mask'] for c in r['classes']] for r in d['families'] if r['family']!='11952'}
    expected={'103b2':43,'074d9':43,'07ca9':43,'08234':43,'08f72':49}
    if {f:len(v) for f,v in result.items()}!=expected or any(len(set(v))!=len(v) for v in result.values()):
        raise ArithmeticError('complete remaining-five generic17 mask roster required')
    return result

def expected_roster():
    d=cert.read(extension.OUT)
    if d['status']!='PASS_FROZEN60_SELECTION' or len(d['selected'])!=60:raise ArithmeticError('fixed60 selection required')
    return [{'id':r['id'],'family':r['family'],'band':r['band'],'parameter':r['parameter'],'retained_index':r['retained_index'],'arms':['fresh_remaining_r17_band_'+str(r['band'])],'combined_selection_units':r['combined_late_units']} for r in d['selected']]

def freeze():
    if (D/'protocol.json').exists():raise FileExistsError('preserve broad60 protocol')
    gate=cert.read(extension.D/'controller/ledger.json')
    if gate['status']!='PASS':raise ArithmeticError('all selection and disjoint validation stages required')
    extension.selection_check();rows=expected_roster();roster=masks()
    if sum(len(roster[r['family']]) for r in rows)!=2652:raise ArithmeticError('fixed2652 maps differ')
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.remaining60-r17-pari.v1','sources':sources(),'selection_sha256':cert.hashed(extension.OUT),'score_protocol_sha256':cert.hashed(extension.D/'protocol.json'),'score_ledger_sha256':cert.hashed(extension.D/'controller/ledger.json'),'validation_sha256':cert.hashed(extension.D/'fresh-validation.json'),'rows':rows,'generic_masks':roster,'height':125000,'seconds_per_chart':10,'admission_prime_bound':997,'stop_rank':28,'map_wall_seconds':120,'worker_wall_seconds':600,'rss_bytes':1610612736,'maximum_workers':2,'gp_sha256':cert.hashed(Path('/usr/bin/gp')),'gate':'All 320 higher full-score slices,10240 scalar scores,60-curve rational-isomorphism deduplication and wholly disjoint validation replay. Existing target-free R17 prospective point proofs establish that this generic17-only detector can recover additional independent points. Finite point exposure tests fresh candidates at new parameter heights, not a guaranteed rank yield.','selection':'Six per band/family in32768<H<=131072 and131072<H<=524288 using combined quantized S1 through65521. Validation65537..131071, public ranks, exceptional points and catalogue data never affect selection or maps.','geometry':'Exactly43 recorded generic17 parity labels on103b2,074d9,07ca9,08234 and49 on08f72. Numerical384-bit heights choose representatives; exact rational centres and maps replay. All60 map files must precede every point search. No specialized maximum or covering theorem is claimed.','boundaries':'Exactly60 attempts and at most2652 initial boxes, each seeded only by16 generic sections. Stop an individual curve at observed28 pending independent proof. Failures/timeouts retained without retry. Novelty comparisons only after terminal batch and proof. No adaptive wave, exact rank, upper bound or universal novelty.'})

def protocol():
    p=cert.read(D/'protocol.json')
    bindings=[('selection_sha256',extension.OUT),('score_protocol_sha256',extension.D/'protocol.json'),('score_ledger_sha256',extension.D/'controller/ledger.json'),('validation_sha256',extension.D/'fresh-validation.json')]
    if p['sources']!=sources() or any(p[k]!=cert.hashed(v) for k,v in bindings) or p['rows']!=expected_roster() or p['generic_masks']!=masks():raise ArithmeticError('frozen broad60 inputs differ')
    return p
...
def replay(index):
    ...
    memory_checked_rank(model,state.basis,state.reductions.primes,state.no_two_torsion_prime);print('REPLAYED REMAINING60 R17',p['rows'][index]['id'],len(data['charts']),'rank >=',state.rank,flush=True)
...
def maps():
    ...
    def one(i):
        ...
        supervision=supervise([SAGE,str(CAS/'prepare_remaining60_r17_pari_batch.sage'),'--index',str(i)],limits=Limits(p['map_wall_seconds'],p['rss_bytes']),log_path=folder/'maps.log',checkpoint_path=folder/'maps.supervisor.json',cwd=ROOT)
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        for f in as_completed(pending):
            i=pending[f];got[i]=f.result();ledger['rows']=[got[j] for j in sorted(got)];checkpoint(path,ledger);print('REMAINING60 MAP',got[i]['id'],got[i]['status'],flush=True)
...
def batch():
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        for future in as_completed(futures):
            i=futures[future];r=future.result();ledger['rows'][i]=r;checkpoint(path,ledger);print('REMAINING60 R17 ATTEMPT',r['id'],r['status'],r.get('rank_lower_bound'),flush=True)
