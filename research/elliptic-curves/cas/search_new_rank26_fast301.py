...
D=LOCAL/'new-rank26-fast301-v1';SOURCE=LOCAL/'compact-r17-new26-followup-v1/candidate-00/result.json';MAPS=LOCAL/'new-rank26-adaptive-pari-maps-v1/maps.json'
...
def prepare():
    ...
    if source['rank_lower_bound']!=26 or len(source['centres'])!=301 or maps['status']!='COMPLETE_DECLARED_MAPS' or [r['centre'] for r in maps['rows']]!=source['centres']:raise ArithmeticError('fixed301 geometry differs')
    if len(gate['rows'])!=43 or any(r['status']!='PASS' for r in gate['rows']):raise ArithmeticError('backend equality gate failed')
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.new-rank26-fast301.v1','sources':sources(),'source_sha256':cert.hashed(SOURCE),'maps_sha256':cert.hashed(MAPS),'backend_gate_sha256':cert.hashed(ART/'new_rank26_engine_comparison_v2.json'),'gp_binary_sha256':cert.hashed(Path('/usr/bin/gp')),'gp_version':'2.15.4','height':100000,'charts':301,'seconds_per_chart':5,'worker_wall_seconds':600,'worker_rss_bytes':1610612736,'maximum_workers':1,'admission_prime_bound':997,'gate':'Native11952 fixed-coordinate comparison improved certified25 to27 with the same point engine. PARI/GMP affine hit sets agree on all49 native and43 new26 boxes. Keep the new26 curve, its original certified26-point subgroup and301 adaptive centres fixed; change only the exactly recorded coordinate maps and calibrated point backend. No known-record exceptional points enter this follow-up.','claim_boundary':'No rank upper bound. Each timeout has no invented denominator prefix; raw output and exact infinity witnesses remain. Stop only at rank>=32 pending independent replay, otherwise attempt all301 within the outer cap.'})
...
def run():
    ...
    if state.rank!=26:raise ArithmeticError('initial subgroup differs')
    output=D/'candidate-00/result.json'
    if output.exists():raise FileExistsError('preserve fast301 attempt')
    data={k:source[k] for k in ('family','parameter','curve','generic_points','family_to_curve_scale_u','initial_state','initial_dimension','centres','metric_gram')};data.update(schema='elliptic-curves.new-rank26-fast301-result.v1',protocol_hash=digest(p),charts=[],status='RUNNING',rank_lower_bound=26,final_state=state.record(),arithmetic_facts=cache.store.snapshot());checkpoint(output,data)
    model=tuple(map(cert.F,data['curve']));primes=tuple(q for q in range(3,998) if _is_prime(q))
    for mapping in maps['rows']:
        c=mapping['centre'];rep=c['representative']+[0]*(state.rank-26);search=PointedQuarticSearch(state=state,centre={'coefficients':rep},coordinate_policy=mapping['coordinate_policy']);record,points=backend.execute(search,mapping,100000,5,p['gp_binary_sha256']);compression=compress(model,state.basis,rep,points)
