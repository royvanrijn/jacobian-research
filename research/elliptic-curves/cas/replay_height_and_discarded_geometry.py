...
import discarded12_r17_pari_batch as batch
...
def geometry(data,maps,initial,p):
    ...
    for index,row in enumerate(data['charts']):
        m=maps['rows'][index];r=row['search'];rep=m['centre']['representative'];C=linear_combination(model,initial,rep+[0]*(len(initial)-len(rep)))
...
def main():
    ...
    for row in p['rows']:
        ...
        geometry(data,maps,initial,{**p,'maps_path':folder/'maps.json'});cloud_check(data,data['charts'],ART/('discarded12_r17_'+row['id'].replace('-','_')+'_mod2_v1.json'),path);count+=len(data['charts'])
    import new27_height125_followup as follow
    ap=follow.protocol()
    old_names=('paired_rank27_all_retained_mod2_v1.json','paired_second27_all_retained_mod2_v1.json','next24_rank27_all_retained_mod2_v1.json')
    for index,row in enumerate(ap['rows']):
        folder=follow.D/row['id'];data=cert.read(folder/'result.json');maps=cert.read(ROOT/row['maps_path']);seed=cert.read(ROOT/row['seed_path']);initial=tuples(seed['final_state']['state']['reductions']['points'])
        if tuples(data['initial_state']['state']['reductions']['points'])!=initial or data['protocol_hash']!=digest(ap):raise ArithmeticError('prospective follow-up binding differs')
        geometry(data,maps,initial,{**ap,'maps_path':ROOT/row['maps_path']})
        old=cert.read(ART/old_names[index]);charts=[{'search':{'finite_curve_points':[{'x':x,'y':y} for x,y in old['points']]}}]+data['charts'];cloud_check(data,charts,ART/('new27_height125_'+row['id'].replace('-','_')+'_mod2_v1.json'),folder/'all-retained-point-cloud-only.json');count+=len(data['charts'])
    from certify_native29_height_control import FOLDERS
    mp=ROOT/'artifacts/local/elliptic-curves/native11952-pari-maps-v1/maps.json';native=cert.read(mp);maps={**native,'centres':[r['centre'] for r in native['rows']]}
    for height,folder in FOLDERS.items():
        data=cert.read(folder/'result.json');ap=cert.read(folder.parent/'protocol.json');initial=tuples(native['generic_points'])
        if data['protocol_hash']!=digest(ap) or tuples(data['initial_state']['state']['reductions']['points'])!=initial:raise ArithmeticError('control input differs')
        geometry(data,maps,initial,{**ap,'height':height,'maps_path':mp});cloud_check(data,data['charts'],ART/f'native11952_height{height}_mod2_v1.json',folder/'result.json');count+=len(data['charts'])
    print('REPLAYED HEIGHT AND DISCARDED GEOMETRY, EXACT CENTRES, RAW POINTS AND CLOUD PROVENANCE',count,'charts',flush=True)
