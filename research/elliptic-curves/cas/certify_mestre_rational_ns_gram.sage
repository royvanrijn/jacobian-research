...
OUT=ART/'mestre_rational_ns_gram_v1.json'
LOCAL=ROOT/'artifacts/local/elliptic-curves/mestre-rational-ns-gram-v1'
...
def compute():
    ...
    for row in data['rows']:
        ...
        for vv in kernel:
            ...
            glues.append({'fixed_numerator':list(v),'replace_basis_index':j,'geometric_Gram':[[int(c) for c in r] for r in geo.rows()], 'Galois_action':[[int(c) for c in r] for r in galois.rows()]})
        result={'outer_u':row['outer_u'],'basis':['F','O']+['I2_minus','I2_plus','I4_1','I4_2','I4_3']+['C'+str(i) for i in h['seed_indices']],
          ...
          'two_primary_kernel_dimension':kernel.dimension(),'eligible_index_two_geometric_glues':glues,
    ...
    return {'schema':'elliptic-curves.mestre-rational-ns-gram.v1','status':'PASS','rows':rows,
