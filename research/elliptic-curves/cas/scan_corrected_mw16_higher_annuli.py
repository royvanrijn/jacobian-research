...
import scan_broad_mw16_higher_annuli as previous
import benchmark_corrected_mw16_annulus as corrected
import encode_mw16_local_corrections as local
import build_mw16_local_score_corrections as local_models
...
D=ROOT/'artifacts/local/elliptic-curves/corrected-mw16-higher-annuli-v1'
SALT='corrected-mw16-higher-fresh320-v1';KEEP=4096

def sources():
    paths=[Path(__file__).resolve(),Path(old.__file__),old.D/'protocol.json',
           Path(previous.__file__),previous.D/'protocol.json',previous.D/'replay.json',
           Path(corrected.__file__),corrected.BINARY,corrected.CPP,corrected.TEST,corrected.OUT,corrected.D/'controller/ledger.json',
           corrected.D/'new-full-slice.json',Path(local.__file__),local.OUT,local_models.OUT,Path(local_models.__file__),
...
def roster():
    rows=[];prior=cert.read(previous.D/'protocol.json')['rows'];fixed=cert.read(corrected.D/'protocol.json')['real_case']
    for r in cert.read(narrow.D/'controller/protocol.json')['rows']:
        group=(r['band'],r['family'],r['sign']);modulus=r['shards']
        excluded=sorted({r['shard'],r['excluded_previous_shard']} | {q['shard'] for q in prior if (q['band'],q['family'],q['sign'])==group})
        choices=[]
        ...
        reuse_group=group==(fixed['band'],fixed['family'],fixed['sign'])
        if reuse_group:
            if fixed['shard'] in excluded:raise ArithmeticError('benchmark reused a prior campaign slice')
            chosen=[('fixed-cost-benchmark:'+fixed['choice_sha256'],fixed['shard'])]+[x for x in sorted(choices) if x[1]!=fixed['shard']][:15]
        else:chosen=sorted(choices)[:16]
        for slot,(h,shard) in enumerate(chosen):
            count=old.counts.population(r['N'],r['M'],shard,modulus)-old.counts.population(r['inner'],r['inner'],shard,modulus)
            rows.append({'id':f"corrected-{r['id']}-{slot:02}",'parent_slice_id':r['id'],'slot':slot,
                ...
                'sha256_choice':h,'primitive_population':count,'reuse_benchmark':reuse_group and slot==0})
    return rows
...
def prepare():
    ...
    correction=cert.read(corrected.OUT)
    if cert.read(corrected.D/'controller/ledger.json')['status']!='PASS' or not correction['cost_gate_passed'] or cert.hashed(corrected.BINARY)!=correction['binary_sha256'] or cert.read(local.OUT)['status']!='PASS':raise ArithmeticError('complete corrected-scanner and local-cache gates required')
    ...
    rows=roster()
    if sum(r['reuse_benchmark'] for r in rows)!=1:raise ArithmeticError('exactly one fixed benchmark reuse required')
    ...
    for f in {r['family'] for r in rows}:
        ...
        path=local.D/(f+'.bin');meta=next(r for r in cert.read(local.OUT)['rows'] if r['family']==f)
        if cert.hashed(path)!=meta['sha256']:raise ArithmeticError('exact local correction cache changed')
        bindings[str(path.relative_to(ROOT))]=meta['sha256']
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.corrected-mw16-higher-annuli.v1','sources':sources(),
        ...
        'gate':'The exact family-wide scaling proof identifies omitted good terms at5and13; all1660 local cells and all10240 earlier scalar-model corrections replay. The corrected engine passes all twenty real-cache frames and a6.38-million-address cost gate. The user requests new territory and broader initial coverage. Keep sixteen slices per band/family/sign while moving to a new disjoint population, with one fixed cost-benchmark reuse. Restore intrinsic good5/13 terms before retention; score weights, prime ranges,4096 retention per signed slice and the later10240-scalar/sixty-fibre budgets remain fixed. Earlier narrow and broader experiments are preserved. Complete five-family cache proofs and40960 saved scalar agreements gate execution.',
        'population':'Exactly320 fresh signed denominator slices: sixteen per band/family/sign, across all five families and16384<H<=65536 or65536<H<=262144. Rank same-parity residues by a frozen SHA256 seed and take sixteen, excluding earlier outer-band residues, narrow-trial residues and all sixteen previous broader residues. Exactly one fixed corrected-benchmark slice is reused once; all other319 slices are untouched. All addresses are outside the compact region and mutually disjoint within each family. Parameter novelty does not establish equation novelty.',
        'selection':'Every primitive parameter receives all3510 quantized selection primes, including exactly restored good5/13 terms, before heap admission. Retain4096 per signed slice by units,good count,denominator,absolute numerator. There is no short-score prefilter. Public record equations,parameters,points,ranks,j-invariants and jump labels remain outside selection and execution.',
        'validation':'Complete signed actual-modulus frames and top7 ordering for all320 cases against the independent retained-list reader on both family caches. Check every retained score component and exact primitive population. The lower-band fixed corrected benchmark is reused once without another scan call. The first higher-band slice has the same45-second cost gate. Independent full-model5/13 scaling and finite-point counts verify every retained local correction. No retry, adaptive expansion or replacement slices.',
    ...
    print('FROZEN320 CORRECTED HIGHER MW16 SLICES',sum(r['primitive_population'] for r in rows),'FULL3510-PRIME ADDRESSES',flush=True)
...
def execute(name,cmd,check=False,reuse=False):
    path=D/(name+'.json')
    if not check:
        if path.exists():raise FileExistsError('preserve one fresh joint call')
        if reuse:checkpoint(path,cert.read(corrected.D/'new-full-slice.json'))
        else:
...
def scan_case(c,keep,name,check=False):
    ...
    fixed=cert.read(corrected.D/'protocol.json')['real_case']
    reuse=bool(c.get('reuse_benchmark')) and keep==4096 and all(c[k]==fixed[k] for k in ('family','sign','N','M','inner','shard','shards'))
    raw=execute(name,[str(corrected.BINARY),str(binary.cache(c['family'],'short')),str(binary.cache(c['family'],'extended')),*map(str,values),str(local.D/(c['family']+'.bin'))],check,reuse);lines=raw['stdout'].splitlines()
    ...
    if len(lines)!=kept+3 or lines[:2]!=['CORRECTED_MW16_ANNULUS_V1','P '+' '.join(map(str,values))] or lines[-1]!=f'S {count} {kept}':raise ArithmeticError('fresh joint population framing differs')
    rows=[]
...
def components(rows,family,name,check=False):
    ...
    for label,count in [('short',562),('extended',2948)]:
        ...
        sums.append(values)
    result=[];f=next(f for f in cert.read(local_models.spec.ATLAS)['families'] if f['fibration_id']==family)
    for row,a,b in zip(rows,*sums):
        A,B=local_models.coefficients(f,row['numerator'],row['denominator']);units=good=0
        for q in (5,13):
            aa,bb,exponent=A,B,0
            while aa%q**4==0 and bb%q**6==0:aa//=q**4;bb//=q**6;exponent+=1
            if exponent:
                ap=exact_trace(aa%q,bb%q,q)
                if ap is not None:units+=round((2-ap)/(q+1-ap)*math.log(q)*10**12);good+=1
        if 'combined_selection_units' in row and (row['combined_selection_units'],row['combined_good'])!=(a[0]+b[0]+units,a[1]+b[1]+good):raise ArithmeticError('corrected score differs from independent components and local scaling')
        result.append({**row,'displayed_score_units':a[0],'displayed_good_primes':a[1],
            'local_correction_units':units,'restored_good_primes':good,
            'score_units':a[0]+units,'good_primes':a[1]+good,'extension_selection_units':b[0],'extension_good':b[1],
            'combined_selection_units':a[0]+b[0]+units,'combined_good':a[1]+b[1]+good})
    return result

@lru_cache(maxsize=2048)
def exact_trace(a,b,q):
    return local_models.diagnostic.trace(a,b,q)
...
def benchmark(check=False):
    ...
    print('ALL320 CORRECTED MW16 FULL-SCORE FRAMES PASS',flush=True)
...
def load_shard(manifest):
    ...
    if path!=D/manifest['id']/'retained.json' or cert.hashed(path)!=manifest['retained_sha256']:
        raise ArithmeticError('immutable corrected retained shard changed')
    result=cert.read(path)
    if result['id']!=manifest['id'] or len(result['rows'])!=KEEP:
        raise ArithmeticError('corrected retained shard roster differs')
    return result
...
def checked(row,check=False):
    ...
    if count!=row['primitive_population'] or len(rows)!=KEEP:raise ArithmeticError('full corrected slice/count differs')
    ...
    if check:
        if cert.read(path)!=saved:raise ArithmeticError('corrected retained point/score vectors differ')
    else:
        if path.exists():raise FileExistsError('preserve corrected retained shard')
...
def run():
    ...
    try:
        for band in (2,3):
            i=next(i for i,r in enumerate(p['rows']) if r['band']==band and (band!=2 or r['reuse_benchmark']))
        ...
        with ThreadPoolExecutor(max_workers=4) as pool:
            ...
            for future in as_completed(pending):
                ...
                print('CORRECTED HIGHER MW16',done[i]['id'],done[i]['wall_seconds'],flush=True)
...
def replay():
    ...
    out=D/'replay.json';result={'status':'PASS','protocol_sha256':cert.hashed(D/'protocol.json'),'result_sha256':cert.hashed(D/'result.json'),'retained_scores':1310720,'selection_primes':3510,'primitive_population':p['total_primitive_population'],'disjoint_previous_outer_slices':True,'disjoint_narrow_and_broad_slices':True,'fixed_benchmark_reused_once':True}
    if out.exists():
        if cert.read(out)!=result:raise ArithmeticError('saved higher scanner replay differs')
    else:checkpoint(out,result)
    print('ALL1310720 CORRECTED HIGHER MW16 FULL-SCORE SURVIVORS REPLAY',flush=True)
