...
def run(folder,replay=False):
    if replay:
        ...
        protocol=dict(schema='fresh6-lowheight-r17-selection.v1',per_family=1,parameter_height_cap=1024,
            ...
            rule='One per family from the height-at-most1024 stratum of the6144 saved H4096 rows, descending combined_selection_units, '
    ...
    for family in sorted({r['family'] for r in pool}):
        ordered=sorted((r for r in pool if r['family']==family and max(abs(r['numerator']),r['denominator'])<=1024),key=lambda r:
        ...
        chosen.append({**row,'id':family+'-lowheight-001','score_order':position})
