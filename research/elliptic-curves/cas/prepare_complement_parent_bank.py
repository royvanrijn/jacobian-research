...
def run(preparation, output):
    ...
    complete=enumerate_bank([list(map(int,r)) for r in (u*g*u.transpose()).rows()],
        [list(map(int,r)) for r in u.rows()],bound=bound,shells=(bound,),node_limit=20000000)
    candidates=[r for r in complete['rows'] if reduce(r['mask'],original_pivots)]
    chosen=[]
    ...
    derivation={'original_masks':sorted(old_masks),'new_masks':sorted(words),
        ...
        'generic_scaled_shell':bound,'candidate_count_outside_original_span':len(candidates),
        'rule':'Enumerate the complete generic shell at the largest productive minimum. '
               'In ascending parity-mask order, first extend the productive F2 span; '
    ...
    protocol={'family':previous['family'],'parameter':previous['parameter'],'initial_rank':rank,
        ...
        'generic_enumeration_node_limit':20000000, 'scaled_bound':bound,
    ...
    checkpoint(output/'complete-shell-bank.json',complete)
    derivation['complete_shell_sha256']=sha(output/'complete-shell-bank.json')
