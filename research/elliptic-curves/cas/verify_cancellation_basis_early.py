...
"""Independent replay of latest-generator-first order and complete clouds."""
...
def verify_arm(case, arm, dest, plan, result):
    ...
    fresh_start = initial; order = []; active_index = None
    ...
    for event in events:
        ...
        if kind == 'bank_start':
            ...
            fresh_start = initial if epoch < 0 else len(bank['seed']['points'])
            ...
            if epoch == 0:
                order = list(range(len(bank['centres'])))
            else:
                latest = [i for i, c in enumerate(bank['centres']) if any(c['representative'][fresh_start:])]
                earlier = [i for i, c in enumerate(bank['centres']) if not any(c['representative'][fresh_start:]) and any(c['representative'][initial:])]
                old = [i for i, c in enumerate(bank['centres']) if not any(c['representative'][initial:])]
                order = latest + earlier + old
            need(event['fresh_generator_start'] == fresh_start and event['visit_order'] == order, 'latest-generator-first policy differs')
            ...
            need(next_anchor < len(order) and event['centre'] == order[next_anchor], 'anchor order differs')
            need(prepared is None or next_model == len(prepared['models']), 'neighbour exploration silently skipped')
            active_index = order[next_anchor]; word = bank['centres'][active_index]['representative']; next_anchor += 1
            ...
            if kind == 'exact_completed_duplicate':
                need(event['epoch'] == epoch and event['centre'] == active_index and event['model'] == next_model, 'duplicate schedule differs')
            ...
            need((row['epoch'], row['centre'], row['model']) == (epoch, active_index, next_model), 'call order differs')
...
def check():
    from cancellation_basis_early import OUT, RAW, guard
