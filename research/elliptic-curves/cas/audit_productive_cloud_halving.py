#!/usr/bin/env python3
"""Fixed eight-point diagnostic for finite-span UNKNOWNs in a completed V3 cloud."""
import argparse
import hashlib
import json
...
def run(folder, output):
    ...
    chosen = sorted(candidates, key=lambda p:(height(p), p))[:8]
    protocol = {'point_searches': 0, 'sample_size': 8, 'max_steps': 4,
        'selection': 'Eight smallest max-absolute-numerator times max-denominator scores, '
