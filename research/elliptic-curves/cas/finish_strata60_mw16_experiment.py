#!/usr/bin/env python3
"""Run the frozen retained-pool comparison only after original proofs and matching."""
import argparse
import sys
import time
...
ROOT=batch.ROOT; CAS=batch.CAS; D=batch.extension.D/'point-controller'


def sources():
    names=['finish_strata60_mw16_experiment.py','strata60_mw16_pari_batch.py',
           'prepare_strata60_mw16_pari_batch.sage','verify_strata60_mw16_points.py',
           'report_strata60_mw16_experiment.py','replay_corrected60_mw16_geometry.py',
...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.strata60-mw16-controller.v1',
        'sources':sources(),'wait_seconds':100000,'rss_bytes':4294967296,
        'stages':[
            ['freeze','strata60_mw16_pari_batch.py',['freeze'],120],
            ['maps','strata60_mw16_pari_batch.py',['maps'],4000],
            ['baselines','strata60_mw16_pari_batch.py',['baselines'],4000],
            ['points','strata60_mw16_pari_batch.py',['batch'],20000],
            ['verify','verify_strata60_mw16_points.py',['all'],43000],
            ['report','report_strata60_mw16_experiment.py',[],180],
            ['report-check','report_strata60_mw16_experiment.py',['--check'],180]],
        'scope':'Wait for original corrected MW16 terminal proofs and frozen matching/replay. All60 maps and independent16 baseline certificates precede all point attempts. Exactly43 generic charts per curve with identical125000 height,10sec/chart,600sec/curve,two workers. No rank stop/retry/refill. Exact histories, exposure geometry, full retained clouds mod2/3/5, provenance and censor-aware measured-yield reporting. Per-curve proof failures remain unresolved and do not discard allocated rows. No validation-driven adaptation or new parameter scan.'})
...
def launch():
    ...
    try:
        while True:
            ...
            if state=='PASS_FROZEN60_MATCHED_SELECTION':break
        ...
        for label,script,args,seconds in p['stages']:
            ...
            print('STRATA60 CONTROLLER',label,ledger['rows'][-1]['status'],flush=True)
            if not ok:
                if label in ('maps','baselines'):
                    ...
                    for reporting_args in ([],['--check']):
                        ...
                        r=run([sys.executable,str(CAS/'report_strata60_mw16_experiment.py'),*reporting_args],limits=Limits(180,p['rss_bytes']),cwd=ROOT,log_path=D/(suffix+'.log'),checkpoint_path=D/(suffix+'.supervisor.json'))
        ...
        ledger['status']='COMPLETE_FIXED_COMPARISON_AND_ACCOUNTING';checkpoint(D/'ledger.json',ledger)
