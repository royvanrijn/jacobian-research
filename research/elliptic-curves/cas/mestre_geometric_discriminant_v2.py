...
OUT=ART/'mestre_geometric_discriminant_v2.json'
PREVIOUS=ART/'mestre_geometric_discriminant_v1.json'
...
def compute():
    ...
    if any(r['geometric_NS_discriminant']!=-468 for r in previous['rows']):raise ArithmeticError('preserved V1 sign error differs')
    rows=[]
    for r,g in zip(source['rows'],geometry['rows']):
        ...
        rows.append({'outer_u':r['outer_u'],'quotient_full_MW_determinant':str(quotient_full_det),
          ...
          'geometric_NS_rank':19,'geometric_NS_discriminant':(-1)**(19-1)*int(geometric_det),
          'arithmetic_NS_rank':18,'arithmetic_NS_discriminant':(-1)**(18-1)*r['full_arithmetic_NS_absolute_determinant'],
    ...
    paths=[Path(__file__).resolve(),SOURCE,GEOMETRY,PREVIOUS]
    return {'schema':'elliptic-curves.mestre-geometric-discriminant.v2','status':'PASS','rows':rows,
      'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},
      'correction':'V1 retained the correct absolute determinant468 but incorrectly printed negative468 for the rank19 geometric NS determinant. Signature(1,18) gives positive468; signature(1,17) gives negative468 for the arithmetic fixed lattice. V1 is preserved as a rejected signed-discriminant record.',
...
if __name__=='__main__':
    ...
    print('PASS6 GEOMETRIC NS DET +468; ARITHMETIC NS DET -468; MW CAP16')
