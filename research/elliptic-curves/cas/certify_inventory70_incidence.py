#!/usr/bin/env python3
"""Bind four exact incidence cohorts to the current70 equations and duplicate map."""
import argparse
...
NAMES=['compact','latest7','latest8','latest23']
...
INDEX=ART/'new_high_rank_curve_index_v6.json'

def result():
    index=cert.read(INDEX);targets={r['id']:r for r in index['curves']};seen=set();counts=Counter();duplicates=[]
    if len(targets)!=70:raise ArithmeticError('fixed70 inventory differs')
    maps=None
    for path in PROOFS:
        data=cert.read(path)
    ...
    if seen!=set(targets) or counts!={'NO_RATIONAL_J_PREIMAGE':752,'RATIONAL_J_PREIMAGES_CERTIFIED':88} or len(duplicates)!=18:raise ArithmeticError('fixed840 coverage differs')
    ...
    return {'schema':'elliptic-curves.inventory70-incidence.v1','status':'PASS','sources':sources,'targets_checked':70,'pairs_checked':840,'status_counts':dict(counts),'same_generic_subgroup_duplicates':duplicates,'claim_boundary':'Combines the four separately exact-replayed rational j-incidence cohorts and the exact Q(t) unimodular17-section transport. Every additional recorded presentation is the same generic subgroup. The separate incidence/transport replays remain required; this summary does not replay ranks or exclude other families, nongeneric points or higher ranks.'}
...
if __name__=='__main__':
    ...
    print('EXACT COHORT BINDING70 CURVES840 PAIRS18 DUPLICATE PRESENTATIONS',flush=True)
