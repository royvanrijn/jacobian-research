...
"""Capped exact remainder trees, protected ideal anchors and known-point parity relations."""
import argparse
import hashlib
import json
...
D = ROOT/'artifacts/local/elliptic-curves/small-conductor-class-target-capped-v1'
...
SEED_SOURCE = ROOT/'elliptic-curves/cas/pursue_small_conductor_class_target_protected.sage'
...
def audit_path(wave):
    return ART/('small_conductor_class_target_capped_wave_%03d_v1.json'%wave)
...
def prepare(args):
    ...
    checkpoint(path,{'schema':'elliptic-curves.small-conductor-class-target-capped-protocol.v1',
...
def audit_all(last,write=False):
    ...
    for wave in range(1,last+1):
        ...
        record = {'schema':'elliptic-curves.small-conductor-class-target-capped-wave.v1','status':'PASS','wave':wave,
