...
BATCH=LOCAL/'curve302-focused-point-exposure-v2'
SAGE='/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python'

def sources():
    names=['curve302_focused_point_exposure_v2.py','prepare_curve302_focused_maps_v2.sage',
           'verify_curve302_focused_seeds.sage','verify_factor_free_rank.sage','run_curve302_focused_point_exposure_v2.py']
...
def prepare():
    ...
    def add(ident,model,points,old_generic,floor,role,parameter):
        ...
        assert tuple(tuple(map(cert.F,P)) for P in state.basis)==pts
...
def freeze():
    ...
    checkpoint(BATCH/'protocol.json',dict(schema='elliptic-curves.curve302-focused-point-exposure.v2',
...
def launch():
    ...
    for i,row in enumerate(p['rows']):
        configure(i)
        s=run([SAGE,str(CAS/'prepare_curve302_focused_maps_v2.sage'),'--index',str(i)],
