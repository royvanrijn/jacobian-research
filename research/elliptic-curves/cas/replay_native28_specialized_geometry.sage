...
def main():
    p=control.protocol();D=control.D;maps=cert.read(D/'maps.json');data=cert.read(D/'result.json');seed=cert.read(control.SEED);initial=tuples(seed['final_state']['state']['reductions']['points'])
    if maps['protocol_hash']!=digest(p) or data['protocol_hash']!=digest(p) or len(initial)!=28 or tuples(data['initial_state']['state']['reductions']['points'])!=initial:raise ArithmeticError('known-only subgroup/protocol differs')
    ...
    for r in maps['sample']:
        w=matrix(ZZ,1,28,r['representative']);v=matrix(ZZ,1,28,r['reduced_representative'])
        if w!=v*u or int((w*g*w.transpose())[0,0])!=r['metric_norm'] or any((int(w[0,j])-(r['parity']>>j))%2 for j in range(28)):raise ArithmeticError('exact representative parity/norm differs')
    ...
    cloud_check(data,seed['charts']+data['charts'],control.ART/'native28_specialized_parity_all_retained_mod2_v1.json',D/'all-retained-point-cloud-only.json')
