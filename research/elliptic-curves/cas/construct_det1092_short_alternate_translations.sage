...
def freeze():
    save('protocol.json',dict(classification='generic-only shortest alternate section construction',
        source_conic=47755,section_height='3/2',existing_source_count=130,
        rule='From the completed original-section source set of alternate degree1, take every section attaining the global lower height bound3/2. Translate the fixed bidegree2/2 conic once by each. Keep all distinct new image classes of minimum original degree greater than1. Build the whole tied minimum set only if at most8; otherwise retain UNKNOWN_MAP_CAP and do not choose orbit IDs.',
...
def lattice():
    ...
    for i,row in sources:
        ...
        image=action*C;degree=dot(image,F)
        assert degree>=0 and degree in ZZ and dot(image,D)==2 and dot(image,image)==-2
        record=dict(source_index=i,word=row['word'],height=str(height),component_bits=ints(bits),
            original_degree=int(degree),image_NS=ints(image),
            action=[ints(v) for v in action.rows()])
    ...
    for row in tied:
        if row['image_NS'] not in [r['image_NS'] for r in classes]:
            classes.append(dict(image_NS=row['image_NS'],original_degree=minimum,
                source_indices=[r['source_index'] for r in tied if r['image_NS']==row['image_NS']]))
    save('geometry.json',dict(status='PASS_GENERIC_MINIMUM_IMAGE_SET' if len(classes)<=p['limits']['maps'] else 'UNKNOWN_MAP_CAP',
        height_histogram=heights,short_sections=allrows,minimum_new_original_degree=minimum,
        selected_classes=classes,protocol_sha256=sha(OUT/'protocol.json'),
        boundary='Selection only among the complete old degree1 source set. Height3/2 is globally minimal, but completeness of all height3/2 alternate sections is not yet claimed. No target incidence or new rank.'))
