...
import retained_native19_trial_v2 as batch
...
def main():
    ...
    for index,row in enumerate(p['rows']):
        ...
        mod2=ART/('retained_native19_trial_v2_'+row['id'].split('-')[-1]+'_mod2_v1.json');modl=mod2.with_name(mod2.name.replace('_mod2_','_modl_'))
