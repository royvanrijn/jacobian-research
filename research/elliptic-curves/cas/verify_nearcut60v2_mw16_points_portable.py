...
D=ROOT/'artifacts/local/elliptic-curves/nearcut60-mw16-point-portable-v2'
OUT=ART/'nearcut60v2_mw16_point_portable_replay_v1.json'


def main():
    path=ART/'nearcut60v2_mw16_point_evidence_v1.json';manifest=cert.read(path);workspace=D/'workspace';folder=D/'verification'
    ...
    p=cert.read(workspace/'artifacts/local/elliptic-curves/nearcut60-mw16-pari-v2/protocol.json')
    ...
    for i,(case,row) in enumerate(zip(manifest['cases'],report['rows'])):
        ...
        if case['replay_point_proof']:
            ...
            verified=cert.read(workspace/'artifacts/local/elliptic-curves/nearcut60-mw16-pari-v2'/ident/'verification.json')
            ...
            jobs.extend([
                (ident+'-history',[str(cas/'nearcut60v2_mw16_pari_batch.py'),'replay','--index',str(i)],600),
                (ident+'-geometry',[str(cas/'verify_nearcut60v2_mw16_points.py'),'geometry-check','--index',str(i)],300),
    ...
    for name,args,seconds in jobs:
        ...
        print('ISOLATED NEARCUT60V2',name,ledger['rows'][-1]['status'],flush=True)
        if not ok:
    ...
    checkpoint(OUT,{'schema':'elliptic-curves.nearcut60v2-point-portable-replay.v1','status':'PASS',
    ...
    print('ISOLATED NEARCUT60V2',len(jobs),'PROOF STAGES PASS;',len(unresolved),'UNRESOLVED ALLOCATIONS',flush=True)
