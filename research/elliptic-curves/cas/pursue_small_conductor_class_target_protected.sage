...
"""Near-root relation waves with16 certified ideal anchors protected during elimination."""
import argparse
import hashlib
import json
...
D = ROOT/'artifacts/local/elliptic-curves/small-conductor-class-target-protected-v1'
...
SEED_SOURCE = ROOT/'elliptic-curves/cas/pursue_small_conductor_class_target_strips.sage'
...
def audit_path(wave):
    return ART/('small_conductor_class_target_protected_wave_%03d_v1.json'%wave)
...
def history(wave, smooth_bound):
    seen = seed['history'](SEED_WAVES+1) if smooth_bound<=50000 else {}
    for k in range(1,wave):
        p = protocol(k)
...
def selection(matrix, wave, box, count, smooth_bound):
    ...
    for i in matrix.free:
        ...
        if i in ANCHORS or i in matrix.inside or c['f']!=1 or c['e']!=1 or seen.get(i,0)>=box:
            continue
...
def prepare(args):
    ...
    paths = [SOURCE,SEED_SOURCE,CHAR_SOURCE,CHARACTERS,OLD_SOURCE,BASE,EXTRA,SMALL,PREVIOUS,Path(batch.__file__).resolve(),batch.forms.OUT,
    ...
    checkpoint(path,{'schema':'elliptic-curves.small-conductor-class-target-protected-protocol.v1',
        ...
        'gate':'User goal: reduce the GRH-certified small-base quotient to16. Protect the16 independently certified ideal classes and skip targets that become elimination pivots during the wave. Target other free columns using thin strips near real zeros of the transformed norm form, to test lower norms per enumerated candidate. Measure exact independent supported rank gain.',
        'population':'For each exact index-q lattice and each of three real norm roots, center u at floor(v*floor(root_in_lattice*2^96)/2^96). For each new integer v<=vmax, try center-1, center, center+1; deduplicate pairs and require primitive(u,v) and mapped(m,n). Skip a target whose column is already an elimination pivot at its turn. Stop after a completed target if dimension<=16.',
    ...
    for index,target in enumerate(p['selection']['targets']):
        ...
        if path.exists():
            ...
            for r in chunk['relations']:
                matrix.add(r['ideal_factorization'])
        elif target['column'] in matrix.inside:
            chunk = {'target':target,'candidate_occurrences':0,'relations':[],
                     'skipped':'column already an elimination pivot','ending_dimension':matrix.dimension(),
...
def audit_all(last,write=False):
    ...
    for wave in range(1,last+1):
        ...
        for index,item in enumerate(summary['chunks']):
            ...
            should_skip = target['column'] in matrix.inside
        ...
        record = {'schema':'elliptic-curves.small-conductor-class-target-protected-wave.v1','status':'PASS','wave':wave,
                  ...
                  'protected_anchor_columns':sorted(ANCHORS),'unconditional_class_two_rank_lower_bound':16,
