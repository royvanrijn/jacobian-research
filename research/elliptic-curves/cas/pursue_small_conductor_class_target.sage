...
"""Checkpointed, certificate-producing relation waves toward class-2-rank <=16."""
import argparse
import hashlib
import json
...
def audit_path(wave):
    return ART/('small_conductor_class_target_wave_%03d_v1.json'%wave)
...
def baseline():
    base = cert.read(BASE)
    matrix = Matrix(base)
    for path in [BASE, EXTRA, PREVIOUS]:
        for r in cert.read(path)['relations']:
            matrix.add(r['ideal_factorization'])
    if matrix.dimension()!=2879:
        raise ArithmeticError('baseline dimension differs')
    return matrix
...
def history(wave):
    seen = {}
    # Both historical target pilots used the same deterministic lattice reduction.
    for path in [old['D']/'protocol.json', ROOT/'artifacts/local/elliptic-curves/small-conductor-special-primes-v1/protocol.json']:
        p = cert.read(path)
        for t in p['selection']['targets']:
            seen[t['column']] = max(seen.get(t['column'],0),p['coefficient_box'])
    for k in range(1,wave):
        p = protocol(k)
        # Only completed target chunks count as searched.
        for item in cert.read(audit_path(k))['chunks']:
            t = cert.read(ROOT/item['path'])['target']
            seen[t['column']] = max(seen.get(t['column'],0),p['box'])
    return seen


def selection(matrix, wave, box, count):
    ...
    for i in matrix.free:
        c = matrix.base['columns'][i]; q = c['p']
        if i in matrix.inside or c['f']!=1 or c['e']!=1 or seen.get(i,0)>=box:
            continue
        if int(nf.disc())%q==0 or int(data['defining_order_index'])%q==0 or a%q==0:
            continue
        eligible.append(i)
    selected = eligible if len(eligible)<=count else [eligible[k*(len(eligible)-1)//(count-1)] for k in range(count)]
    targets = []
    for column in selected:
        ...
        targets.append({'column':column,'p':q,'hnf':col['hnf'],'theta_root':root,
                        'norm_slope':slope,'lattice_basis':old['reduce_lattice'](q,slope,data['reduced_hessian']),
                        'old_box':seen.get(column,0)})
    return {'eligible_columns':len(eligible),'targets':targets}
...
def prepare(args):
    ...
    if not 2<=args.targets<=4096 or not 1<=args.box<=2048 or not BOUND<=args.smooth_bound<=400000:
        raise ValueError('parameters outside declared bounds')
    matrix = prior_state(args.wave)
    choose = selection(matrix,args.wave,args.box,args.targets)
    if not choose['targets']:
        raise ArithmeticError('no eligible targets at this box; choose a larger bounded box')
    paths = [SOURCE,OLD_SOURCE,BASE,EXTRA,SMALL,PREVIOUS,Path(batch.__file__).resolve(),batch.forms.OUT,
             ROOT/'elliptic-curves/cas/audit_bnf_free_s_class_quotient.py']
    paths += [audit_path(k) for k in range(1,args.wave)]
    checkpoint(path,{'schema':'elliptic-curves.small-conductor-class-target-protocol.v1',
        'wave':args.wave,'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},
        'selection':choose,'starting_dimension':matrix.dimension(),'box':args.box,
        'smooth_bound':args.smooth_bound,'workers':1,'rss_bytes':1610612736,
        'worker_seconds':args.seconds,'audit_seconds':600,
        'gate':'User goal: reduce the GRH-certified small-base quotient to16. Target free columns of the current supported matrix and measure exact independent supported rank gain.',
        'population':'For each exact index-q lattice, coprime(u,v) in [-box,box] x [1,box], outside the already searched square; mapped(m,n) must be primitive. Stop after a completed target if dimension<=16.',
    ...
    print('PREPARED',args.wave,'TARGETS',len(choose['targets']),'DIMENSION',matrix.dimension(),'BOX',args.box,flush=True)
...
def worker(wave):
    ...
    b = p['box']; chunks = []; started = time.monotonic(); count = 0
        ...
        if path.exists():
            ...
            for u in range(-b,b+1):
                for v in range(1,b+1):
                    if max(abs(u),v)<=target['old_box'] or gcd(u,v)!=1:
                        continue
...
def audit_all(last,write=False):
    ...
    for wave in range(1,last+1):
        ...
        if matrix.dimension()!=p['starting_dimension'] or selection(matrix,wave,p['box'],len(p['selection']['targets']))!=p['selection']:
        ...
        for index,item in enumerate(summary['chunks']):
            ...
            for r in chunk['relations']:
                ...
                if not (-p['box']<=u<=p['box'] and 1<=v<=p['box'] and max(abs(u),v)>target['old_box'] and gcd(u,v)==gcd(m,n)==1):
        ...
        record = {'schema':'elliptic-curves.small-conductor-class-target-wave.v1','status':'PASS','wave':wave,
                  ...
                  'claim_boundary':p['claim_boundary']}
        if record['conditional_on_grh_curve_rank_upper_bound']<22:
...
if __name__=='__main__':
    ...
    parser.add_argument('--box',type=int,default=128)
    parser.add_argument('--targets',type=int,default=512)
    parser.add_argument('--smooth-bound',type=int,default=BOUND)
