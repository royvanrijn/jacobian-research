...
"""Fixed finite conductor bounds for the three new broader MW16 curves."""
...
INPUT=ART/'broad60_mw16_results_v1.json'
CAT=ROOT/'artifacts/local/elliptic-curves/retention24-current-catalogue-v1/database.json'
D=ROOT/'artifacts/local/elliptic-curves/broad60-conductor-bounds-v1'
OUT=ART/'broad60_conductor_bounds_v1.json'
...
def candidates():
    gate=cert.read(ROOT/'artifacts/local/elliptic-curves/broad60-mw16-pari-v1/post-batch/ledger.json')
...
def prepare():
    ...
    rows=candidates()
    if len(rows)!=3 or len({r['id'] for r in rows})!=3:raise ArithmeticError('fixed three new higher curves required')
    checkpoint(D/'protocol.json',{'sources':sources(),'roster':[r['id'] for r in rows],
        ...
        'scope':'Audit conductor upper bounds for the three newly certified broader-MW16 curves with lower bounds24,24,22, after terminal point proofs and novelty checks. Exact local Tate data at2,3 and every discriminant prime through10000; leave every larger cofactor unfactored. No parameter search, point search, new rank assertion, descent or automatic factorization escalation. A bound above a recorded benchmark does not exclude a smaller true conductor.'})
...
def expected(write=False):
    ...
    result={'schema':'elliptic-curves.broad60-conductor-bounds.v1','status':'RUNNING',
...
if __name__=='__main__':
    ...
    if a.stage=='prepare':prepare()
        ...
        print('BROAD60 NEW-CURVE CONDUCTOR BOUNDS',len(d['rows']),'STRICT IMPROVEMENTS',d['strict_improvement_ids'],flush=True)
