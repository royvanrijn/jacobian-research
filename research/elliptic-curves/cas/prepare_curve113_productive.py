#!/usr/bin/env python3
"""Exact native M25 and productive generic anchors for curve113; no point search."""
import argparse
...
def run(folder):
    ...
    paths = [ART/'compact192_r17_results_v1.json', spec.ATLAS,
        LOCAL/'compact192-r17-pari-v1/08f72-001/result.json']
    inputs = [json.loads(p.read_text()) for p in paths]
    proofs, atlas, initial = inputs
    proof = next(r for r in proofs['curves'] if r['id'] == '08f72-001')
    family = next(r for r in atlas['families'] if r['family'] == '08f72')
    protocol = {'family': '08f72', 'parameter': '-3292/1853', 'initial_rank': 25,
        ...
        'sources': {str((CAS/n).relative_to(ROOT)): sha(CAS/n) for n in (
            'prepare_curve113_productive.py', 'compact_atlas_specialization.py',
    ...
    original, generic = spec.specialize(family, '-3292/1853')
    scale = F(initial['family_to_curve_scale_u'])
    if (len(basis) != 25 or proof['parameter'] != '-3292/1853'
    ...
    print('CURVE113_PREPARED', len(rows), masks, flush=True)
