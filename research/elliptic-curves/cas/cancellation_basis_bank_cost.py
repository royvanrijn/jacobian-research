...
"""One frozen, point-search-free comparison on all 34 retained transitions."""
import argparse
...
def seal():
    ...
    names = source_closure([Path(__file__),CAS/'verify_cancellation_basis_bank.py',
                            CAS/'verify_parity_minimum.py',CAS/'check_cancellation_basis_bank.py',
                            CAS/'check_cancellation_basis_bank_bindings.py'])
    plan = {'status':'FROZEN_ALL_RETAINED_TRANSITIONS_COST_GATE','arms':ARMS,'transitions':34,
        ...
        'input_sha256':{n:sha(OUT/n) for n in ('inputs.json','environment.json','development.json','corruption-checks.json','binding-checks.json')}}
...
def worker(ident, arm):
    ...
    try:
        ...
        if arm == 'rational_rebuild':
            ...
            from verify_cancellation_basis_bank import rebuild
        ...
        need(bank == read(retained/'bank.json'), 'rebuilt exported bank differs from retained bank')
