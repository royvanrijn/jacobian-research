...
def expected():
    broad=True
    prefix='r17-remaining'
    scan=LOCAL/(prefix+'-higher-annuli-v1')
    scores=LOCAL/(prefix+'-higher-scores-v1')
    ...
    for row in protocol['rows']:
        ...
        lo,hi={2:(32768,131072),3:(131072,524288)}[row['band']]
    ...
    return {'schema':'elliptic-curves.higher-r17-remaining-model-and-retention-audit.v1',
        ...
        'variant':'remaining-five-r17','models_checked':model_count,
...
if __name__=='__main__':
    ...
    output=ART/'r17_remaining_higher_score_coverage_v1.json';data=expected()
    if a.check:
        if cert.read(output)!=json.loads(json.dumps(data)):raise ArithmeticError('remaining-five R17 model/coverage proof replay differs')
    else:
        if output.exists():raise FileExistsError('preserve remaining-five R17 model/coverage proof')
        checkpoint(output,data)
    print('REMAINING FIVE HIGHER R17 MODEL/RETENTION AUDIT',data['status'],data['models_checked'],data['distinct_j_invariants'],flush=True)
