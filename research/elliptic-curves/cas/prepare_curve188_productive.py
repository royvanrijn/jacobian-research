#!/usr/bin/env python3
"""Exact native M28 and productive generic anchors for curve188; no point search."""
import argparse
...
def run(folder):
    ...
    paths = [ART/'full11952_late64_r17_results_v1.json', spec.ATLAS,
        LOCAL/'full11952-late64-r17-pari-v1/11952-0959582/result.json',
        ART/'inventory188_fixed49_point_control_mod2_v1.json',
        LOCAL/'inventory188-fixed49-point-control-v1/result.json']
    inputs = [json.loads(p.read_text()) for p in paths]
    proofs, atlas, initial, cloud, adaptive = inputs
    proof = next(r for r in proofs['curves'] if r['id'] == '11952-0959582')
    old_basis = proof['points']
    if cloud['input_sha256'] != sha(paths[4]) or cloud['curve'] != proof['curve']:
        raise ArithmeticError('M28 cloud provenance differs')
    proof = dict(proof, points=cloud['independent_points'], rank_certificate=cloud['rank_certificate'])
    if proof['points'][:27] != old_basis:
        raise ArithmeticError('native old M27 prefix differs')
    family = next(r for r in atlas['families'] if r['family'] == '11952')
    protocol = {'family': '11952', 'parameter': '110314/102227', 'initial_rank': 28,
        'generic_rank': 17, 'prime_bound': 1000, 'cvp_node_limit': 2000000,
        'point_searches': 0,
        'rule': 'Freshly certify every gain in both retained attempts. Retain all productive '
        ...
        'sources': {str((CAS/n).relative_to(ROOT)): sha(CAS/n) for n in (
            'prepare_curve188_productive.py', 'compact_atlas_specialization.py',
    ...
    original, generic = spec.specialize(family, '110314/102227')
    scale = F(initial['family_to_curve_scale_u'])
    if (len(basis) != 28 or proof['parameter'] != '110314/102227'
    ...
    if json.loads(json.dumps(fresh)) != old:
        raise ArithmeticError('existing M28 certificate differs')
    checkpoint(folder/'seed-M28.json', {'curve': list(map(str, model)),
        'points': [list(map(str, p)) for p in basis], 'generic_rank': 17,
        'rank_lower_bound': 28, 'proof': fresh})
    gains = []
    for name, data, start, expected in (('initial', initial, basis[:17], 27),
                                       ('adaptive', adaptive, basis[:27], 28)):
    ...
    checkpoint(folder/'prepared.json', {'status': 'PASS_NATIVE_M28_AND_PRODUCTIVE_ANCHORS',
        'point_searches': 0, 'anchor_count': len(rows),
        'files': {p.name: sha(p) for p in sorted(folder.glob('*.json'))}})
    print('CURVE188_PREPARED', len(rows), masks, flush=True)
