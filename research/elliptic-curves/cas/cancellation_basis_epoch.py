"""Charged full-cloud epochs for a frozen fixed-basis / rebuilding ablation."""
...
def native_rank(packet):
    return SourceFileLoader('basis_epoch_native_rank', str(CAS/'verify_finite_cancellation_cpu.sage')).load_module().sage_rank(packet)


def rebuild(packet, case, plan, folder):
    """No search can use a bank until independent CVP and anchor checks pass."""
    ...
    certified_state(model, basis, packet['proof'])
    write(folder/'seed.json', packet)
    policy = {**plan['bank_policy'], 'generic_rank': case['parent_bank']['dimension'],
              'scaled_shells': case['parent_bank']['shells'],
              'anchor_count': len(case['parent_bank']['rows'])}
    need(len(basis)-policy['generic_rank'] <= plan['maximum_extension_dimension'], 'extension census exceeds protocol')
    tick = cpu()
    producer = landscape(model, basis, set(), folder/'producer', policy, case['parent_bank'])
    production = cpu()-tick
    tick = cpu()
    reference = landscape(model, basis, set(), folder/'reference', policy, case['parent_bank'], reference=True)
    ...
    for row in producer['centres']:
        P = sum((int(c)*p for c, p in zip(row['representative'], native)), E(0))
        need(not P.is_zero() and list(map(str, P.xy())) == row['point'], 'independent anchor identity differs')
    centres = producer['centres'][:plan['maximum_centres']]
    need(bool(centres), 'empty compatible bank')
    bank = {'seed': packet, 'centres': centres, 'rank': len(basis)}
    write(folder/'bank.json', bank)
    receipt = {'status': 'PASS_INDEPENDENT_BANK', 'rank': len(basis),
               'basis_sha256': digest(canonical(packet['points'])),
               'bank_sha256': sha(folder/'bank.json'), 'seed_sha256': sha(folder/'seed.json'),
               'producer_sha256': sha(folder/'producer/selection.json'),
               'reference_sha256': sha(folder/'reference/selection.json'),
               'parent_bank_sha256': digest(canonical(case['parent_bank'])),
               'exported_centres': len(centres), 'verified_centres': len(producer['centres']),
               'full_cosets_scored': producer['full_cosets_scored'],
               'production_cpu_seconds': production, 'reference_cpu_seconds': reference_cpu,
               'native_anchor_cpu_seconds': cpu()-tick, 'cpu_seconds': cpu()-start,
               'boundary': 'Exact CVP for the recorded rounded positive numerical metric and retained parent subset; no canonical-height certification.'}
...
def run(case, arm, plan, dest, protocol_hash):
    ...
    def open_bank():
        ...
        except (TimeoutError, ArithmeticError, ValueError, AssertionError) as error:
        ...
        record({'kind': 'bank_ready', **epochs[-1]})
        return True
    ...
    if remaining() <= 0:
        ...
        while bank is not None:
            ...
            refresh = False
            for ci, centre in enumerate(bank['centres']):
                if remaining() <= 0:
