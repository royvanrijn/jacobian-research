...
OUT=ART/'full11952_64_point_evidence_v1.json'

def main():
    archive=OUT.with_suffix('.zip')
    if OUT.exists() or archive.exists():raise FileExistsError('preserve standalone full64 point evidence')
    p=batch.protocol();done=cert.read(batch.D/'post-batch/ledger.json');summary=cert.read(ART/'full11952_64_experiment_v1.json')
    ...
    for path in [batch.D/'point-batch.supervisor.json',batch.D/'stream-verification-v1/stream.supervisor.json',batch.D/'post-batch/controller.supervisor.json']:
        s=cert.read(path)
        if s['outcome']!='completed' or s['returncode']!=0:raise ArithmeticError('all source jobs must be terminal before packaging')
    paths={Path(__file__).resolve(),CAS/'verify_full11952_64_points_portable.py',CAS/'certify_full11952_64_r17_results.py',CAS/'audit_recorded_point_mod2_rank_v3.py',CAS/'audit_retained_cloud_modl.py',ART/'full11952_64_r17_results_v1.json',ART/'full11952_64_experiment_v1.json',batch.extension.OUT,batch.extension.D/'protocol.json',batch.extension.D/'selected-validation/result.json',batch.CONTROL/'selection-check.supervisor.json',batch.CONTROL/'validation-check.supervisor.json',batch.CONTROL/'ledger.json',ROOT/'artifacts/local/elliptic-curves/native11952-height125-control-v1/125000/verification.json'}
    ...
    proofs=[ART/'full11952_64_r17_results_v1.json']
    for row in p['rows']:
        proofs.extend(ART/('full11952_64_r17_'+row['id'].replace('-','_')+'_'+suffix+'_v1.json') for suffix in ('mod2','modl'))
        early=ART/('full11952_64_r17_'+row['id'].replace('-','_')+'_early_modl_v1.json')
    ...
    cert.write(OUT,{'schema':'elliptic-curves.full11952-64-point-evidence.v1','archive':str(archive.relative_to(ROOT)),'archive_sha256':cert.hashed(archive),'archive_bytes':archive.stat().st_size,'files':members,'required_base_archives':[],'claim_boundary':'Standalone replay of all64 exact admission histories, complete point clouds modulo2,3,5, rational quartic maps, generic-section transports, and independent point proofs with embedded593-catalogue and789-prior-equation comparisons. Frozen score selection and control gates are hash-bound context; the20.89-billion scanner, million-row scoring, cache construction and historical controls are not rerun by this point-only supplement. No new search, exact rank, full saturation, point absence or universal novelty.'})
    print('STANDALONE FULL64 POINT BUNDLE',len(members),'FILES',archive.stat().st_size,'BYTES',flush=True)
