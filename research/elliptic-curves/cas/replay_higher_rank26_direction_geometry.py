...
def main():
    ...
    for identifier in IDS:
        ...
        words=[256+w for w in sorted(range(256),key=lambda w:(w.bit_count(),w))];expected={(r['mask'],words[i%len(words)]) for i,r in enumerate(pool)}
        ...
        for c in maps['centres']:
            ...
            if c['parity']!=c['generic_mask']|(c['quotient_word']<<17) or len(c['representative'])!=26 or any((c['representative'][j]-(c['parity']>>j))%2 for j in range(26)):raise ArithmeticError('exact centre parity differs')
        if len(initial)!=26 or tuples(data['initial_state']['state']['reductions']['points'])!=initial or data['protocol_hash']!=digest(p) or maps['protocol_hash']!=digest(p):raise ArithmeticError('initial subgroup/protocol differs')
        geometry(data,maps,initial,{**p,'maps_path':D/'maps.json'})
        prior=cert.read(follow.SEED.parent/'all-retained-point-cloud-only.json')
        cloud_check(data,prior['charts']+data['charts'],follow.ART/('higher_rank26_direction_'+identifier.replace('-','_')+'_all_retained_mod2_v1.json'),D/'all-retained-point-cloud-only.json')
    ...
    print('EXACT HIGHER26 FOLLOWUP GEOMETRY AND CLOUD PROVENANCE',count,'charts',flush=True)
