...
"""Finite conductor-upper-bound screen for the four next12 certified new rank>=22 curves."""
...
NAMES=('prospective_mw16_next12_results_v1.json',)
OUT=ROOT/'artifacts/local/elliptic-curves/next12-new-curve-conductor-bounds-v1/result.json'
...
def run():
    ...
    if len(inputs)!=4:raise ArithmeticError('frozen4-input roster changed')
    ...
    result={'schema':'elliptic-curves.next12-new-curve-conductor-bounds.v1','status':'RUNNING','rows':[],'benchmarks':benchmarks,
