...
"""Frozen301-chart adaptive follow-up on the new compact-MW16 rank25 curve."""
import argparse
...
geometry=SourceFileLoader('atlas_adaptive_general_geometry',str(CAS/'prospective_half_lattice.sage')).load_module()
PARENT=ROOT/'artifacts/local/elliptic-curves/prospective-mw16-h4096-v1'
DIRECTORY=ROOT/'artifacts/local/elliptic-curves/prospective-mw16-wide-rank25-followup-v1'


def sources():
    paths=(Path(__file__).resolve(),Path(spec.__file__).resolve(),spec.ATLAS,Path(cert.__file__).resolve(),
           CAS/'prospective_half_lattice.sage',CAS/'research_runtime/memory_store.py')
...
def prepare(directory):
    ...
    path=PARENT/'a1-fibration-04/candidate-01/result.json';result=cert.read(path)
    certificate=ROOT/'artifacts/generated-results/elliptic-curves/prospective_mw16_wide_results_v1.json'
    record=next(r for r in cert.read(certificate)['curves'] if r['family']==result['family'] and r['parameter']==result['parameter'])
    if result['status']!='COMPLETE_DECLARED_PILOT' or result['rank_lower_bound']!=25 or record['discovery_witness']['sha256']!=cert.hashed(path):raise ArithmeticError('rank25 input changed')
    if record['icarm_matches'] or record['previous_matches']:raise ArithmeticError('not the new prospective rank25 curve')
    ...
    checkpoint(directory/'protocol.json',{'schema':'elliptic-curves.prospective-mw16-rank25-followup.v1','sources':sources(),
        ...
        'family':result['family'],'parameter':result['parameter'],'initial_rank_lower_bound':25,'generic_dimension':16,
        'selection':'Sole new rank25 result of the completed fixed20-address wider MW16 pilot; adaptive points are only those discovered and independently certified in that pilot.',
    ...
    print('FROZEN MW16 RANK25 FOLLOWUP',result['family'],result['parameter'],flush=True)
