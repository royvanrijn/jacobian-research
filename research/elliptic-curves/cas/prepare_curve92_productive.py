#!/usr/bin/env python3
"""Exact native M26 and productive generic anchors for curve92; no point search."""
import argparse
...
def run(folder):
    ...
    paths = [ART/'extended20_mw16_high_rank_minimal_v1.json', spec.ATLAS,
        LOCAL/'extended20-mw16-pari-v1/a1-fibration-02-014/result.json',
        LOCAL/'mw16-new26-a1-fibration-02-014-adaptive-v1/result.json',
        LOCAL/'prospective-mw16-h4096-v1/a1-fibration-02/generic-census.json']
    inputs = [json.loads(p.read_text()) for p in paths]
    proofs, atlas, initial, adaptive, census = inputs
    proof = next(r for r in proofs['curves'] if r['id'] == 'a1-fibration-02-014')
    family = next(r for r in atlas['families'] if r['fibration_id'] == 'a1-fibration-02')
    protocol = {'family': 'a1-fibration-02', 'parameter': '3161/432', 'initial_rank': 26,
        ...
        'sources': {str((CAS/n).relative_to(ROOT)): sha(CAS/n) for n in (
            'prepare_curve92_productive.py', 'compact_mw16_specialization.py',
    ...
    if json.loads(json.dumps(fresh)) != old:
        raise ArithmeticError('existing M26 certificate differs')
    checkpoint(folder/'seed-M26.json', {'curve': list(map(str, model)),
        'points': [list(map(str, p)) for p in basis], 'generic_rank': 16,
        'rank_lower_bound': 26, 'proof': fresh})
    gains = []
    for name, data, start in (('initial', initial, basis[:16]), ('adaptive', adaptive, basis)):
    ...
    checkpoint(folder/'prepared.json', {'status': 'PASS_NATIVE_M26_AND_PRODUCTIVE_ANCHORS',
        'point_searches': 0, 'anchor_count': len(rows),
        'files': {p.name: sha(p) for p in sorted(folder.glob('*.json'))}})
    print('CURVE92_PREPARED', len(rows), masks, flush=True)
