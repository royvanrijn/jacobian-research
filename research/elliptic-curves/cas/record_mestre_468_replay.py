...
LOCAL = ROOT / 'artifacts/local/elliptic-curves/mestre-468-standalone-v1'
OUT = ART / 'mestre_468_portable_replay_v1.json'
...
def compute():
    ...
    files = {}
    for source in (ROOT / 'elliptic-curves/cas/verify_mestre_468_bundle.sage',
    ...
    return {
        'schema': 'elliptic-curves.mestre-468-portable-replay.v1',
        'status': 'PASS', 'parents': [11, 13, 17, 19, 23, 29],
        'portable_files': files,
        'command': 'sage -python verify_mestre_468_bundle.sage --input mestre_468_replay_bundle_v1.json',
        'supervisor': supervisor, 'transcript': transcript.decode(),
        'checks': '714 heights total, exact generic relations and base involutions, finite saturation witnesses, every saved finite fibre count, Picard bounds and determinant arithmetic',
