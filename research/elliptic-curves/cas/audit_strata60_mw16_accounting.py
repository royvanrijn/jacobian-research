...
D=LOCAL/'strata60-mw16-pari-v1'
ART=ROOT/'artifacts/generated-results/elliptic-curves'
REPORT=ART/'retained_mw16_score_strata_experiment_v1.json'
OUT=ART/'retained_mw16_score_strata_accounting_replay_v1.json'
...
def audit():
    ...
    selected=read(ART/'retained_mw16_score_strata_selection_v1.json')['selected']
    same(p['rows'],selected)
    same(len(selected),60)
    same(Counter(r['arm'] for r in selected),Counter(top=20,moderate=20,lower=20))
    ...
    for choice,reported in zip(selected,report['rows']):
        ...
        for key in ('id','arm','block','family','band','sign','parameter','retained_rank','j_height','parameter_height'):
    ...
    policy=report['policy_comparison'];top=compare_summary(policy['top_only'],[r for r in independent if r['arm']=='top'])
    pooled=compare_summary(policy['equal_three_arm_portfolio'],independent)
    leave=[]
    for family in sorted({r['family'] for r in independent}):
        other=[r for r in independent if r['family']!=family]
        t=totals([r for r in other if r['arm']=='top'])['gain_per_discovery_second'];q=totals(other)['gain_per_discovery_second']
        leave.append({'omitted_family':family,'top_gain_per_second':t,'pooled_gain_per_second':q,'pooled_advantage':t is not None and q is not None and q>t})
    same(len(policy['leave_one_family_out']),len(leave))
    for actual,expected in zip(policy['leave_one_family_out'],leave):
    ...
    resolved=all(row[k] is not None for row in (top,pooled) for k in keys)
    diversify=resolved and pooled[keys[0]]>top[keys[0]] and pooled[keys[1]]>top[keys[1]] and pooled[keys[2]]>=top[keys[2]] and all(r['pooled_advantage'] for r in leave)
    favoured=resolved and top[keys[0]]>pooled[keys[0]] and top[keys[1]]>=pooled[keys[1]] and top[keys[2]]>=pooled[keys[2]]
    recommendation='DIVERSIFY_WITHIN_RETAINED_POOL' if diversify else 'TOP_FAVOURED_IN_THIS_FINITE_SAMPLE' if favoured else 'INCONCLUSIVE'
    same(policy['recommendation'],recommendation);same(policy['diversification_criterion_met'],bool(diversify));same(policy['all_endpoints_resolved'],resolved)
    bindings[str(Path(__file__).resolve().relative_to(ROOT))]=sha256(Path(__file__).read_bytes()).hexdigest()
    return {'schema':'elliptic-curves.strata60-independent-accounting.v1','status':'PASS','sources':bindings,
        'allocated_curves':60,'allocated_boxes':2580,'independent_rows':independent,'recommendation':recommendation,
        'scope':'Independent read-only reconstruction of all60 allocated curve costs, certified gain bindings, terminal chart completion, missing CPU timing, arm/family rates and the frozen policy criterion. Exact rank and geometry proofs are separately checked by their recorded workers; this audit checks accounting, not point search completeness or a rank-density theorem. No report-generator functions imported and no new search.'}
...
if __name__=='__main__':
    ...
    print('INDEPENDENT STRATA ACCOUNTING',data['status'],data['recommendation'],flush=True)
