#!/usr/bin/env python3
"""Bind ten exact incidence cohorts to the current187 equations and duplicate map."""
import argparse
...
NAMES=['compact','latest7','latest8','latest23','retention','extended20_mw16','higher24','inventory101_added','inventory185_added','inventory187_added']
...
INDEX=ART/'new_high_rank_curve_index_v14.json'

def result():
    index=cert.read(INDEX);targets={r['id']:r for r in index['curves']};seen=set();counts=Counter();duplicates=[]
    if len(targets)!=187:raise ArithmeticError('fixed187 inventory differs')
    maps=None
    for path in PROOFS:
        data=cert.read(path)
    ...
    if seen!=set(targets) or counts!={'NO_RATIONAL_J_PREIMAGE':2032,'RATIONAL_J_PREIMAGES_CERTIFIED':212} or len(duplicates)!=25:raise ArithmeticError('fixed2244 coverage differs')
    ...
    return {'schema':'elliptic-curves.inventory187-incidence.v1','status':'PASS','sources':sources,'targets_checked':187,'pairs_checked':2244,'status_counts':dict(counts),'same_generic_subgroup_duplicates':duplicates,'historical_rank_metadata':'Incidence targets retain their historical lower bounds; only nondecreasing current bounds are accepted, and all curve, j, family and parameter fields must remain identical. Incidence itself is independent of rank.','claim_boundary':'Combines the ten separately exact-replayed rational j-incidence cohorts and the exact Q(t) unimodular17-section transport. Every additional recorded presentation is the same generic subgroup. The separate incidence/transport replays remain required; this summary does not replay ranks or exclude other families, nongeneric points or higher ranks.'}
...
if __name__=='__main__':
    ...
    print('EXACT COHORT BINDING187 CURVES2244 PAIRS25 DUPLICATE PRESENTATIONS',flush=True)
