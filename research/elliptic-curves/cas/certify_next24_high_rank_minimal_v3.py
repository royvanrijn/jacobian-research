...
ROOT=Path(__file__).resolve().parents[2];ART=ROOT/'artifacts/generated-results/elliptic-curves';D=ROOT/'artifacts/local/elliptic-curves/next24-r17-extended-pari-v1';OUT=ART/'next24_high_rank_minimal_proofs_v3.json';OLD=ART/'fresh_r17_paired_results_v1.json';DB=ROOT/'artifacts/local/elliptic-curves/next12-current-catalogue-v1/database.json';IDS=('103b2-021','11952-030','11952-041')

def sources():
    names=['certify_next24_high_rank_minimal_v3.py','memory_rank_certificate.py','certify_compact_r17_candidates.py','elliptic_candidate_record.py','mod2_reduction_independence.py','research_runtime/finite_reduction.py','research_runtime/memory_store.py','research_runtime/store.py']
...
def minimality(model):
    ...
    for p,e in factors:
        ...
        if not (v[0]<4 or v[1]<6 or v[2]<12):
            ...
            if any(all(x.denominator==1 for x in m) for m in candidates):raise ArithmeticError('smaller integral model exists; do not assert minimality')
            row['scaled_invariant_normalized_candidates']=[list(map(str,m)) for m in candidates]
            row['scaled_integral_model_excluded']=True
        rows.append(row)
    return {'invariant_gcd':g,'gcd_factorization':factors,'local_exclusion_rows':rows,'argument':'A nonminimal integral model at p requires v_p(c4)>=4, v_p(c6)>=6 and v_p(discriminant)>=12. Each gcd prime either fails those valuations or has no integral model with scaled invariants c4/p^4,c6/p^6. Every integral Weierstrass equation admits integral translations normalizing a1,a3 to0 or1 and a2 to-1,0 or1: choose s for a1, then r for a2 modulo3, then t for a3. For each of these12 triples c4,c6 uniquely determine a4,a6 by the displayed b-invariant equations. Exhaustively nonintegral candidates exclude a smaller integral model. Primes outside the gcd are excluded by invariant valuations.'}
