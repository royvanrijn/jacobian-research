...
D = ROOT/'artifacts/local/elliptic-curves/small-conductor-fast301-v1'
...
def verify():
    ...
    output = ART/'small_conductor_fast301_recorded_mod2_v1.json'
    checkpoint(folder/'protocol.json', {'input_sha256': cert.hashed(path),
        ...
        'sources': {str(p.relative_to(ROOT)): cert.hashed(p) for p in (
            Path(__file__).resolve(), ROOT/'elliptic-curves/cas/replay_small_conductor_fast301.py',
    ...
    stages = [('charts', ['replay_small_conductor_fast301.py', str(path)], 900),
    ...
    for stage, args, seconds in stages:
        ...
        checkpoint(folder/'result.json', result)
        print('SMALL CONDUCTOR FAST301 VERIFICATION', stage, result['rows'][-1]['status'], flush=True)
