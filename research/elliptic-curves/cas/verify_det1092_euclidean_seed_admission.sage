...
def replay():
    ...
    save('replay-protocol-v2.json', dict(checker_sha256=sha(Path(__file__)), inputs=inputs,
    ...
    assert all(f.degree() <= bound for f, bound in zip([N, V, m, g, b, k], [10, 15, 5, 9, 4, 3]))
...
if __name__ == '__main__':
    ...
    signal.alarm(25)
    result = replay()
    if args.write: save('independent-replay-v2.json', result)
    else: assert result == read(OUT/'independent-replay-v2.json')
