...
def run(preparation, output, continuation):
    ...
    candidate_masks=sorted(x['mask'] for x in family['classes'] if reduce(x['mask'],original_pivots))
    ...
    derivation={'original_masks':sorted(old_masks),'new_masks':sorted(words),
        ...
        'rule':'Use the bound exact maximum generic parity list outside the original productive span. Recheck every minimum with integer and rational solvers. In ascending mask order first extend that span, then fill to16. No specialized higher-point labels.'}
    paths=[catalogue,continuation/'terminal.json',continuation/'verified.json']+[preparation/n for n in sealed['files']]+[preparation/'prepared.json',Path(__file__),
