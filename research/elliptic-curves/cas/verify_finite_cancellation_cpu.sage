...
def main():
        ...
        for arm in plan['arms']:
            ...
            for record in result['charts']:
                ...
                if arm=='finite_selector':
                    ...
                    # The opaque worker hash includes CPU timings, which are
                    # not reproducible and whose full object was not retained.
                    # Replay the actual retained arithmetic vectors, selection
                    # and map; never retag that historical hash.
                    assert row['selection']['features']==[m['features'] for m in prepared['models']]
    ...
    write(CPU/'verified.json',{'status':'PASS_TRANSCRIPTS_SELECTOR_AND_TWO_FINITE_IMPLEMENTATIONS','strict_cpu_gate_passed':gate,
        ...
        'boundary':'Newly executed retrospective controls on12 curves, cached V3 banks, M18-M20 inputs. Every claimed gain independently replayed using complete Sage finite-group cosets and the portable implementation. Literal withheld-point hits are counted separately. The opaque prepared hash includes unretained timings; retained feature vectors, scores and selected maps are all recomputed directly instead, without retagging that hash. Runtime measurements and conditional examples do not prove a population speedup or a new record.'})
