...
def main():
    ...
    for row in p['rows']:
        ...
        geometry(data,maps,initial,{**p,'maps_path':folder/'maps.json'});cloud_check(data,data['charts'],ART/('product24_r17_'+row['id'].replace('-','_')+'_mod2_v1.json'),path);count+=len(data['charts'])
    if count>988 or sum(len(p['generic_masks'][r['family']]) for r in p['rows'])!=988:raise ArithmeticError('fixed988 maximum chart roster differs')
    print('REPLAYED PRODUCT22 CHARTS',count,'GENERIC CENTRES, RAW POINTS AND CLOUD PROVENANCE',flush=True)
