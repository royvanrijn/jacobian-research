...
def main():
    ...
    for row in p['rows']:
        ...
        geometry(data,maps,initial,{**p,'maps_path':folder/'maps.json'});cloud_check(data,data['charts'],ART/('fresh_r17_paired_'+row['id'].replace('-','_')+'_mod2_v1.json'),path);count+=len(data['charts'])
    for directory,seed_id,prefix in [('paired-rank27-adaptive-v1','074d9-007','paired_rank27'),('paired-second27-adaptive-v1','11952-010','paired_second27')]:
    ...
    print('REPLAYED PAIRED GEOMETRY, EXACT CENTRES, RAW POINTS AND CLOUD PROVENANCE',count,'charts',flush=True)
