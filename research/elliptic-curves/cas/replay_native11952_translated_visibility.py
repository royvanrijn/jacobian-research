#!/usr/bin/env python3
"""Independent rational group replay of196 retrospective oracle translations."""
...
ROOT=Path(__file__).resolve().parents[2];ART=ROOT/'artifacts/generated-results/elliptic-curves';INPUT=ART/'native11952_translated_visibility_v1.json';OUT=ART/'native11952_translated_visibility_replay_v1.json';CONTROL=ROOT/'artifacts/local/elliptic-curves/native11952-pari49-control-v1/candidate-00/result.json'
def main():
    if OUT.exists():raise FileExistsError('preserve oracle replay')
    ...
    if len(basis)!=27 or len(oracles)!=2 or source['curve']!=data['curve']:raise ArithmeticError('oracle/baseline dimensions differ')
    ...
    for r in data['proposals']:
        ...
        if not 0<=i<49 or not 0<=k<2 or sign not in (1,-1) or len(r['word'])!=27 or any(type(x)!=int for x in r['word']):raise ArithmeticError('invalid fixed group word')
        ...
        if v!=r['visibility'] or r['published_index']!=[2,27][k]:raise ArithmeticError('exact visibility differs')
        counts[v['status']]+=1
    if roster!=[(i,k,sign) for i in range(49) for k in range(2) for sign in (1,-1)] or dict(counts)!=data['status_counts']:raise ArithmeticError('fixed196 roster differs')
    best=[min((r for r in data['proposals'] if r['oracle_index']==k),key=lambda r:r['visibility'].get('minimum_affine_height') or 10**1000) for k in range(2)]
    ...
    checkpoint(OUT,{'schema':'elliptic-curves.native11952-translated-visibility-replay.v1','status':'PASS','sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in (Path(__file__).resolve(),INPUT,CONTROL,ROOT/'elliptic-curves/cas/search_nagao_u42_skew_height.py',ROOT/'elliptic-curves/cas/search_observability.py',ROOT/'elliptic-curves/cas/memory_rank_certificate.py')},'exact_group_words_checked':196,'status_counts':dict(counts),'best_per_direction':best,'claim_boundary':'Exact rational group words and chart-square/visibility identities. Numerical CVP optimality is not asserted. No prospective oracle leakage, new curve or upper bound.'})
    print('REPLAYED196 NATIVE29 TRANSLATIONS',[(r['published_index'],r['visibility']['minimum_affine_height']) for r in best],flush=True)
