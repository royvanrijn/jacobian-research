...
import score_mw16_outer_bands as scores
...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},'wait_seconds':1200,'stages':[('prepare',120),('run',3600),('check',300),('select',60),('selection_check',60),('validate',300),('validation-check',180)],'rss_bytes':2147483648,'scope':'Wait for all thirty fixed outer MW16 slices and exact replays. Freeze15360 traces, pass first15 runtime gate, two score workers, immutable per-address raw outputs and30-address checkpoints. Replay every score, freeze60 Q-distinct equations at equal four per family per band, replay selection, record and replay disjoint validation. No point search, refill or retries.'})
def launch():
    ...
    try:
        ...
        for name,seconds in p['stages']:
            ...
            ledger['rows'].append({'name':name,'status':'PASS' if ok else 'FAILED_OR_CENSORED','supervision':s});checkpoint(D/'ledger.json',ledger);print('MW16 OUTER SCORE CONTROLLER',name,ledger['rows'][-1]['status'],flush=True)
