#!/usr/bin/env python3
"""Fixed32 per family from the unsearched compact H4096 score population."""
import argparse
from pathlib import Path
from concurrent.futures import ThreadPoolExecutor,as_completed
import certify_compact_r17_candidates as cert
import compact_atlas_specialization as spec
import select_compact192_unsearched as extension
...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';D=ROOT/'artifacts/local/elliptic-curves/compact192-r17-pari-v1';PARITY=ART/'r17_exact_maximum_parity_classes_v1.json';SAGE='/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python'

def sources():
    names=['memory_rank_certificate.py','select_compact192_unsearched.py','verify_compact192_r17_pari_batch.py','compact192_r17_pari_batch.py','prepare_compact192_r17_pari_batch.sage','prospective_half_lattice_v2.sage','compact_atlas_specialization.py','research_runtime/search_state.py','research_runtime/cached_observation_state.py','research_runtime/preloaded_prime_state.py','research_runtime/rotated_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
...
def expected_roster():
    ...
    if selected['status']!='COMPLETE_FROZEN_UNSEARCHED_SELECTION':raise ArithmeticError('terminal score extension required')
    for f in sorted(selected['selection']):
        ...
        if [r['retained_index'] for r in available[:32]]!=selected['selection'][f]:raise ArithmeticError('fixed selection differs')
        for r in available[:32]:rows.append({'id':f+f"-{r['retained_index']:03}",'family':f,'parameter':r['parameter'],'retained_index':r['retained_index'],'arms':['compact4096_unsearched_top32'],'combined_selection_units':r['combined_selection_units']})
    if len(rows)!=192 or len({r['id'] for r in rows})!=192:raise ArithmeticError('fixed192 roster differs')
    return rows

def freeze():
    if (D/'protocol.json').exists():raise FileExistsError('preserve compact192 protocol')
    ...
    if sum(len(masks[r['family']]) for r in rows)!=8640:raise ArithmeticError('fixed generic chart count differs')
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.compact192-r17-pari.v1','sources':sources(),'selection_sha256':cert.hashed(extension.D/'result.json'),'selection_protocol_sha256':cert.hashed(extension.D/'protocol.json'),'selection_replay_sha256':cert.hashed(extension.D/'replay.supervisor.json'),'control_sha256':cert.hashed(ROOT/'artifacts/local/elliptic-curves/native11952-height125-control-v1/125000/verification.json'),'rows':rows,'generic_masks':masks,'height':125000,'seconds_per_chart':10,'admission_prime_bound':997,'stop_rank':28,'map_wall_seconds':120,'worker_wall_seconds':600,'map_roster_wall_seconds':1200,'point_batch_wall_seconds':60000,'geometry_before_search':True,'rss_bytes':1610612736,'maximum_workers':2,'gp_sha256':cert.hashed(Path('/usr/bin/gp')),'gate':'The original compactH4096 population has6144 replayed extended S1 scores and its preceding retention24 cohort contains two new27-point curves. A separately frozen selection now chooses192 exact rational-isomorphism-distinct previously unsearched equations,32 per family, after excluding all528 prior measured equations and21 audited endpoints. This increases point exposure in the productive compact population without changing scores or generating larger-parameter equations. The identical49-map known native control reached28 at125000; this is a visibility and cost gate, not a rank predictor.','selection':'Exactly32 per family in the frozen compact192 selector, unchanged score through32749, selection-band good count, denominator and signed numerator. Validation32771..65521 does not enter ordering. Equation-only previous/within-roster rational-isomorphism exclusions precede point execution; no public rank or point enters selection or maps. No refill of failures or later catalogue matches.','geometry':'All43 or49 exact generic maximum parity classes. Numerical384-bit heights choose representatives only; rational centres and maps are checked exactly.','boundaries':'Exactly192 attempts and at most8640 initial boxes, stop each curve on a provisional28-point independent subgroup pending replay. Each starts from17 generic sections only. Retain all checkpoints, failures and raw points. Catalogue and previous-equation comparison follows terminal batch replay. No complete4096 extended-score coverage, rank upper bound, exact rank, record or universal novelty.'})

def protocol():
    p=cert.read(D/'protocol.json')
    if p['sources']!=sources() or p['selection_sha256']!=cert.hashed(extension.D/'result.json') or p['selection_protocol_sha256']!=cert.hashed(extension.D/'protocol.json') or p['selection_replay_sha256']!=cert.hashed(extension.D/'replay.supervisor.json') or p['rows']!=expected_roster():raise ArithmeticError('frozen compact192 inputs or roster differ')
    return p
...
def replay(index):
    ...
    memory_checked_rank(model,state.basis,state.reductions.primes,state.no_two_torsion_prime);print('REPLAYED COMPACT192 R17',p['rows'][index]['id'],len(data['charts']),'rank >=',state.rank,flush=True)

def maps():
    p=protocol();path=D/'maps-ledger.json'
    if path.exists():raise FileExistsError('preserve full192 geometry roster')
    ledger={'status':'RUNNING','rows':[{**r,'status':'PENDING'} for r in p['rows']]};checkpoint(path,ledger)
    def one(index):
        row=p['rows'][index];folder=D/row['id']
        r=supervise([SAGE,str(CAS/'prepare_compact192_r17_pari_batch.sage'),'--index',str(index)],limits=Limits(p['map_wall_seconds'],p['rss_bytes']),log_path=folder/'maps.log',checkpoint_path=folder/'maps.supervisor.json',cwd=ROOT)
    ...
    with ThreadPoolExecutor(max_workers=p['maximum_workers']) as pool:
        ...
        for f in as_completed(pending):
            i=pending[f];ledger['rows'][i]=f.result();checkpoint(path,ledger);print('COMPACT192 FROZEN MAP',p['rows'][i]['id'],ledger['rows'][i]['status'],flush=True)
...
def batch():
    ...
    if path.exists():raise FileExistsError('preserve compact192 point batch')
    if geometry['status']!='PASS' or [r['id'] for r in geometry['rows']]!=[r['id'] for r in p['rows']]:raise ArithmeticError('all192 geometry choices must precede searches')
    ...
    with ThreadPoolExecutor(max_workers=p['maximum_workers']) as pool:
        pending={pool.submit(one,i):i for i in range(len(p['rows']))}
        for f in as_completed(pending):
            i=pending[f];r=f.result();ledger['rows'][i]=r;checkpoint(path,ledger);print('COMPACT192 POINT ATTEMPT',r['id'],r['status'],r.get('rank_lower_bound'),flush=True)
