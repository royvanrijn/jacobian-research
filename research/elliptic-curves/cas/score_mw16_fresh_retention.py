#!/usr/bin/env python3
"""Target-free extended scores for40960 fresh outer MW16 parameters."""
...
D=ROOT/'artifacts/local/elliptic-curves/mw16-fresh-retention-scores-v1'
OUT=ART/'mw16_fresh_retention_selection_v1.json'
FRESH_PRIMES=[q for q in _primes_up_to(131071) if q>=65537]

def sources():
    paths=[Path(__file__).resolve(),Path(scan.__file__),Path(engine.__file__),Path(models.__file__),Path(scoring.__file__),Path(cert.__file__),scan.core.spec.ATLAS,scan.D/'protocol.json',scan.D/'result.json',scan.D/'replay.json',CAS/'research_runtime/store.py',CAS/'research_runtime/supervisor.py',CAS/'mod2_reduction_independence.py']
...
def prepare():
    ...
    families={r['fibration_id']:r for r in cert.read(scan.core.spec.ATLAS)['families']};pools=[]
    for band in (1,):
        for family in sorted(families):
            ...
            if len(pool)!=8192 or len({r['parameter'] for r in pool})!=8192:raise ArithmeticError('fixed band-family8192 differs')
            pools.append([{**r,'id':f'fresh-{family}-{i:04}','retained_index':i,'model':models.model_at(families[family],r['parameter'])} for i,r in enumerate(pool)])
    rows=[pool[i] for i in range(8192) for pool in pools]
    if len(rows)!=40960:raise ArithmeticError('fixed40960 roster differs')
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.mw16-fresh-retention-scores.v1','sources':sources(),'rows':rows,'selected_curves':60,'seconds_per_curve':20,'score_wall_seconds':3600,'validation_wall_seconds':300,'maximum_workers':4,'checkpoint_block':80,'rss_bytes':1073741824,'gp_sha256':cert.hashed(engine.GP),'fresh_validation_primes':FRESH_PRIMES,'cost_gate_rows':10,'cost_gate_maximum_projected_serial_seconds':8000,'gate':'Ten fresh signed denominator slices beyond4096 and through16384 pass exact annular population and40960-score replay. Eightfold short-score retention tests the demonstrated tail loss on a disjoint prospective population, equally across all five families. No point, catalogue or public-rank data enter the candidate worker.','ordering':'Twelve per family in the single fresh annulus, ordered by combined quantized S1 through65521, good count, denominator and signed numerator. Exact within-finalist Q-isomorphism deduplication in fixed family order, refilling only from the same frozen8192-address family pool. Fresh65537..131071 validation never changes selection.','scope':'All40960 retained candidates from ten unused denominator slices in4096<H<=16384. The first ten rows, two per family, form a runtime gate and are reused once. Four scalar workers,20 seconds per curve,3600-second outer cap and80-address checkpoints. Two direct character sums per curve and exact score replay precede60 finalists and wholly disjoint validation. No retries, compact-region rescan, adaptive parameter expansion, rank prediction or universal novelty.'})
    print('FROZEN40960 FRESH MW16 TRACE ROSTER',flush=True)
def protocol():
...
def run():
    ...
    try:
        ...
        if projected>p['cost_gate_maximum_projected_serial_seconds']:raise ArithmeticError('fixed first10 runtime gate failed')
        result['status']='RUNNING_FROZEN40960';checkpoint(out,result)
        with ThreadPoolExecutor(max_workers=p['maximum_workers']) as pool:
            for start in range(p['cost_gate_rows'],len(p['rows']),p['checkpoint_block']):
                ...
                if len(result['rows'])%800==10:print('MW16 FRESH TRACE PROGRESS',len(result['rows']),flush=True)
        result['status']='COMPLETE_FROZEN40960';checkpoint(out,result)
...
def check():
    ...
    if d['status']!='COMPLETE_FROZEN40960' or d['protocol_sha256']!=cert.hashed(D/'protocol.json') or len(d['rows'])!=40960:raise ArithmeticError('complete score roster required')
    for row,actual in zip(p['rows'],d['rows']):
        if trace(row,p,False)!=actual:raise ArithmeticError('exact score replay differs')
    print('REPLAYED ALL40960 MW16 FRESH SCORES',flush=True)
...
def selected_rows(p,d):
    ...
    for key,pool in sorted(pools.items()):
        ...
        for row in pool:
            ...
            if count==12:break
        if count!=12:raise ArithmeticError('insufficient distinct equations; no population expansion')
...
def validate(check=False):
    ...
    print('MW16 FRESH FINALISTS FRESH VALIDATION60 PASS',flush=True)
