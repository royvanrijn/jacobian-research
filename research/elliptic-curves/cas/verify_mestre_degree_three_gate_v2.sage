...
"""Standalone frame proof, including component rotation for general translates."""
...
def main(path):
    ...
    for i,row in enumerate(frames['rows']):
        ...
        # The first three examples had uniquely determined components. In the
        # wider retained roster several classes share the other intersections.
        # Translation by -S rotates each split In component cycle by -[S].
        base_cross=base*oldG;components=list(base_cross[2:7])
        full=[[2-components[0],components[0]],[2-components[1],components[1]],
              [2-sum(components[2:]),*components[2:]]]
        section_cross=S*oldG
        sc=[int(section_cross[2]),int(section_cross[3]),
            sum(j*int(section_cross[3+j]) for j in (1,2,3))]
        shifted=[[a[(j+c)%len(a)] for j in range(len(a))] for a,c in zip(full,sc)]
        cmp=[shifted[0][1],shifted[1][1],*shifted[2][1:]]
        v=oldinv*vector(QQ,[2,2,*cmp,*cross])
        assert all(c in ZZ for c in v) and v*oldG*v==-2 and list(v)==row['bisection_class']
