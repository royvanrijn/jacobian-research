...
"""Freeze301 exact PARI maps using only the newly discovered26-point subgroup."""
import sys,argparse
...
def main():
    ...
    for i,g in enumerate(p['generic_pool']):
        word=words[i%len(words)];mask=g['mask']|(word<<16);residue=[(mask>>j)&1 for j in range(26)];norm,rep,error=oracle.solve(residue)
        if len(rep)!=26 or any((rep[j]-residue[j])%2 for j in range(26)):raise ArithmeticError('adaptive parity mismatch')
    ...
    data['status']='COMPLETE_DECLARED_MAPS';checkpoint(out,data);print('FROZEN NEW26 ADAPTIVE MAPS',len(data['rows']),flush=True)
