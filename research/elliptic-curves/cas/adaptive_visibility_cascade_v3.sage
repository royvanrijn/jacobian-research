...
def freeze():
    if D.exists(): raise FileExistsError('preserve frozen V3')
    old = ROOT/'artifacts/local/elliptic-curves/adaptive-visibility-cascade-v2'
    D.mkdir(parents=True)
    checkpoint(D/'generic.json', read(old/'generic.json'))
    fixed = read(old/'calibration302/seed.json')
    final = read(old/'calibration302/epoch-13/selection.json')
    checkpoint(D/'fixed-M30.json', {'curve':fixed['curve'], 'points':final['basis']})
    preserved = [p for p in old.rglob('*') if p.is_file()]
    preserved += [CAS/'adaptive_visibility_cascade_v2.sage',CAS/'visibility_lattice_v2.py',
                  ART/'adaptive_visibility_cascade_v2.json']
    checkpoint(D/'v2-preservation.json',{rel(p):sha(p) for p in preserved})
    p = read(old/'protocol.json')
    p.pop('exact_shortlist_per_anchor');p.pop('finalists_per_anchor')
    p.update(schema='adaptive-half-lattice-calibration.v3',
        sources=sources(), inputs={rel(q):sha(q) for q in (D/'generic.json',D/'fixed-M30.json',ORBITS)},
        selection='Per anchor: top4 and bottom4 Babai scores; two nearest each of 16 quantile centres; 8 semantic-coset SHA256 coverage samples and 8 from the lower-score half. Keep all metric boundary ties. Exact CVP all selected. Construct factor-free maps and record coefficient bit sum/max and minimum multiplicity. Finalists: all shallow/deep-norm versus small-bit/high-multiplicity Pareto fronts; nearest each of 8 exact-norm and 8 chart-bit quantile centres; 4 semantic-coset coverage samples. Keep all ties. Unchanged global chart ordering and actual-centre dedup.',
        oracle_boundary='Calibration-informed design. Execution reads only generic MW17, fixed certified V2 M30 points, generic orbit table, own outputs and frozen source. No target point, diagnostic ranks, residual labels or selected orbit IDs. First M30; replay M17 only following independently certified M30 success.',
        invariance='Semantic finite-reduction coset keys do not depend on mask integer encoding. Complete extension enumeration retained. Babai/LLL and chart tie convention remain basis dependent as in V2.')
    checkpoint(D/'protocol.json',p)
    print('FROZEN V3',sha(D/'protocol.json'),flush=True)
...
def run(start):
    ...
    if p['sources'] != sources() or any(sha(ROOT/k) != v for k,v in p['inputs'].items()):
        raise ArithmeticError('frozen V3 source/input changed')
    from research_runtime.memory_store import MemoryFactStore
    ...
    if start==17:
        terminal=read(D/'terminal-M30/terminal.json')
        if terminal['final_rank_lower_bound']<31:raise ArithmeticError('M17 replay requires M30 success')
    ...
    for epoch in range(p['max_epochs']):
        ...
        for j,c in enumerate(centres):
            ...
            if not snapshot.exists():
                checkpoint(snapshot, {'status':'INCREMENTAL_RETAINED_CLOUD','family':'calibration302-v3',
...
if __name__ == '__main__':
    ...
    ap.add_argument('action',choices=['freeze','run','preservation'])
    ap.add_argument('--start',type=int,choices=[17,30],default=30)
    args=ap.parse_args()
    if args.action=='freeze':freeze()
    elif args.action=='run':run(args.start)
    else:
        old=read(D/'v2-preservation.json')
        assert all(sha(ROOT/k)==v for k,v in old.items()),'V2 changed'
        print('V2 UNCHANGED',len(old),'files')
