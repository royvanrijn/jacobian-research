...
ROOT=Path(__file__).resolve().parents[2];ART=ROOT/'artifacts/generated-results/elliptic-curves';LOCAL=ROOT/'artifacts/local/elliptic-curves';OUT=ART/'native29_height_control_v2.json'
...
def expected():
    ...
    for height,folder in FOLDERS.items():
        ...
        for name in ('worker','replay'):
            ...
            if s['outcome']!='completed' or s['returncode']!=0 or cert.hashed(folder/(name+'.log'))!=s['log_sha256']:raise ArithmeticError('history/supervision replay differs')
