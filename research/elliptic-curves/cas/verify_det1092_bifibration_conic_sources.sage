...
def maps_and_incidence(parent, pencil, protocol):
    ...
    for mask in MASKS:
        ...
        # Coefficients explicitly replace u by v; V(R-polynomial) would
        # instead embed that entire polynomial as a constant in K=Q(u).
        fT = V(T.numerator().list())-T*V(T.denominator().list())
        fW = V(W.numerator().list())-W*V(W.denominator().list())
...
def replay():
    ...
    save('replay-protocol-v2.json', dict(checker_sha256=sha(Path(__file__)), seconds=25,
    ...
    return dict(status='PASS_INDEPENDENT_BIFIBRATION_CONICS_AND_FIXED_M18_SEED',
        ...
        replay_protocol_sha256=sha(OUT/'replay-protocol-v2.json'),
