...
def main():
    ...
    for p in rat_bad:
        print(
            f"{PROTOCOL}|stage=factor_prime|p={p}|status=start",
            flush=True,
        )
        t1 = time.monotonic()

        # primes_above avoids asking Sage for reduced ideal generators.
        # We need only the prime ideals themselves, ramification indices,
        # residue degrees and norms.
        Ps = K.primes_above(p)
        ...
        print(
            f"{PROTOCOL}|stage=factor_prime|p={p}|status=complete"
            f"|seconds={elapsed:.6f}|factors={local}",
            flush=True,
        )
    ...
    for i, P in enumerate(known):
        print(
            f"{PROTOCOL}|stage=kummer_point|i={i}|status=start",
            flush=True,
        )
        Q = iso(P)
        if Q.is_zero():
        ...
        for j, Pideal in enumerate(prime_ideals):
            ...
            if velapsed >= 0.25:
                meta = prime_meta[j]
                print(
                    f"{PROTOCOL}|stage=valuation|point={i}|column={j}"
                    f"|p={meta['p']}|e={meta['e']}|f={meta['f']}"
                    f"|seconds={velapsed:.6f}|valuation={v}",
                    flush=True,
                )
        ...
        print(
            f"{PROTOCOL}|stage=kummer_point|i={i}|status=complete"
            f"|seconds={time.monotonic()-point_started:.6f}"
