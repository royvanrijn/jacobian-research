#!/usr/bin/env python3
"""Exact integral models and point transports for unmatched retention curves>=27."""
import argparse
from pathlib import Path
import certify_compact_r17_candidates as cert
import certify_discarded_rank26_minimal as arithmetic
from research_runtime.store import checkpoint
ROOT=Path(__file__).resolve().parents[2];ART=ROOT/'artifacts/generated-results/elliptic-curves';INPUT=ART/'retention24_r17_results_v1.json';OUT=ART/'retention_high_rank_minimal_proofs_v1.json'
def sources():
...
def expected():
    ...
    return {'schema':'elliptic-curves.retention-high-rank-minimal.v1','status':'PASS','sources':sources(),'input_sha256':cert.hashed(INPUT),'curves':rows,'catalogue':d['catalogue'],'previous_equations':d['previous_equations'],'claim_boundary':'Exact independent-point lower bounds, proved global minimal integral models and exact point transports for the unmatched>=27 portion of the fixed24 cohort. Absence from586 pinned catalogue equations and358 earlier measured equations is not universal novelty. No exact rank or conductor is asserted.'}
if __name__=='__main__':
    ...
    if a.check:
        ...
        checkpoint(OUT,r)
    print('EXACT RETENTION HIGH-RANK MINIMAL PROOFS',[(q['id'],q['rank_lower_bound'],q['minimality']['invariant_gcd']) for q in r['curves']],flush=True)
