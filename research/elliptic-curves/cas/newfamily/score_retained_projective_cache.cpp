...
int main(int argc,char** argv) {
 try {
  if (argc!=3) throw std::runtime_error("usage: score_retained_projective_cache CACHE CANDIDATES");
  ...
  for (auto& r:rows) {
   if (!(input>>r.n>>r.d) || r.n==0 || r.n < -131072 || r.n>131072 || r.d<1 || r.d>131072 || std::gcd(r.n,r.d)!=1) throw std::runtime_error("invalid primitive candidate");
  }
  ...
  for (std::uint32_t j=0;j<primes;++j) {
   ...
   if (p<=previous || p<4099 || p>32749 || !prime(p) || length!=static_cast<std::uint32_t>(p+1)) throw std::runtime_error("invalid prime frame");
