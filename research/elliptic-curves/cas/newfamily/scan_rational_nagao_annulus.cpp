// Annular periodic scanner: retain only INNER < max(numerator,denominator).
...
//
// Usage:
//   scan_rational_nagao_tables TABLE NUM DEN KEEP SHARD_ID SHARDS INNER
//
...
int main(int argc, char** argv) {
  if (argc != 8) {
    std::cerr << "usage: scan_rational_nagao_tables TABLE NUM DEN KEEP SHARD_ID SHARDS INNER\n";
    return 2;
  }
  ...
  try {
    ...
    const int shard_id = std::atoi(argv[5]);
    const int shards = std::atoi(argv[6]);
    ...
    const int inner = std::atoi(argv[7]);
    if (inner < 0 || inner >= std::max(numerator_bound, denominator_bound))
      throw std::runtime_error("require 0 <= INNER < max(NUM,DEN)");
    ...
    for (int denominator = 1; denominator <= denominator_bound; ++denominator) {
      ...
      for (int numerator = 1; numerator <= numerator_bound; ++numerator) {
        if (std::max(numerator, denominator) <= inner) continue;
