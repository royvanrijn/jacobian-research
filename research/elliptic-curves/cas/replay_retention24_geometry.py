...
def main():
    ...
    for row in p['rows']:
        ...
        geometry(data,maps,initial,{**p,'maps_path':folder/'maps.json'});cloud_check(data,data['charts'],ART/('retention24_r17_'+row['id'].replace('-','_')+'_mod2_v1.json'),path);count+=len(data['charts'])
    if count!=1080:raise ArithmeticError('fixed1080 chart roster differs')
    print('REPLAYED1080 RETENTION24 CHARTS, GENERIC CENTRES, RAW POINTS AND CLOUD PROVENANCE',flush=True)
