...
"""Bounded near-root strips toward class-2-rank <=16, seeded by four audited box waves."""
import argparse
import hashlib
import json
...
D = ROOT/'artifacts/local/elliptic-curves/small-conductor-class-target-strips-v1'
...
SEED_SOURCE = ROOT/'elliptic-curves/cas/pursue_small_conductor_class_target.sage'
...
def audit_path(wave):
    return ART/('small_conductor_class_target_strip_wave_%03d_v1.json'%wave)
...
def baseline():
    ...
    # Rebuild with this implementation too; the seed's class is mathematically identical.
    result = Matrix(matrix.base)
    for path in [BASE, EXTRA, PREVIOUS]:
        for r in cert.read(path)['relations']:
            result.add(r['ideal_factorization'])
    for k in range(1,SEED_WAVES+1):
        for item in cert.read(seed['audit_path'](k))['chunks']:
            for r in cert.read(ROOT/item['path'])['relations']:
                result.add(r['ideal_factorization'])
...
def prepare(args):
    ...
    paths = [SOURCE,SEED_SOURCE,OLD_SOURCE,BASE,EXTRA,SMALL,PREVIOUS,Path(batch.__file__).resolve(),batch.forms.OUT,
    ...
    checkpoint(path,{'schema':'elliptic-curves.small-conductor-class-target-strip-protocol.v1',
        ...
        'seed_waves':SEED_WAVES,'slope_scale':str(SLOPE_SCALE),
        'gate':'User goal: reduce the GRH-certified small-base quotient to16. Target free columns using thin strips near real zeros of the transformed norm form, to test lower norms per enumerated candidate. Measure exact independent supported rank gain.',
        'population':'For each exact index-q lattice and each of three real norm roots, center u at floor(v*floor(root_in_lattice*2^96)/2^96). For each new integer v<=vmax, try center-1, center, center+1; deduplicate pairs and require primitive(u,v) and mapped(m,n). Stop after a completed target if dimension<=16.',
...
def audit_all(last,write=False):
    ...
    for wave in range(1,last+1):
        ...
        if matrix.dimension()!=p['starting_dimension'] or selection(matrix,wave,p['vmax'],len(p['selection']['targets']))!=p['selection']:
        ...
        record = {'schema':'elliptic-curves.small-conductor-class-target-strip-wave.v1','status':'PASS','wave':wave,
