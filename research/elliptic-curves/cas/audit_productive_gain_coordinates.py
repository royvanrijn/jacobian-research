#!/usr/bin/env python3
"""Retrospective two-map visibility of a certified gain at its winning centre."""
import argparse
...
def run(epoch, output):
    ...
    rows = []
    names = ('factor_free_pari_mapping.sage', 'prepare_extended20_mw16_pari_batch.sage')
    for name in names:
    ...
    paths += [receipt, Path(__file__), *(CAS/n for n in names)]
