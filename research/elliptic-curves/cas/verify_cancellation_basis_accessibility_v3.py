...
def exact_replay(row, plan, folder):
    ...
    for key, word in expected.items():
        ...
        for item in entry['models']:
            ...
            search = PointedQuarticSearch(curve=maps['seed']['curve'], subgroup=[],
    ...
    for target in result['targets']:
        ...
        for key, entry in entries.items():
            ...
            if h <= plan['height']:
                ...
                search = PointedQuarticSearch(curve=maps['seed']['curve'], subgroup=[], centre={'point': entry['point']},
...
def verify(row, plan, folder):
    from cancellation_basis_accessibility import PRIOR
    ...
    need(endpoints['curve'] == maps['seed']['curve'], 'endpoint equation differs')
    for target in result['targets']:
    ...
    return exact_replay(row, plan, folder)
