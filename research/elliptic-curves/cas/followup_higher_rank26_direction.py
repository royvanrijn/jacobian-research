...
def configure(identifier):
    ...
    ID=identifier;D=LOCAL/('higher-rank26-'+ID+'-newdirection-v1');SEED=LOCAL/('higher-rank26-'+ID+'-adaptive-v1')/'result.json';CENSUS=LOCAL/'compact-r17-fresh-generic-census-v1'/ID.split('-')[0]/'generic-census.json'

def sources():
    names=['memory_rank_certificate.py','followup_higher_rank26_direction.py','prepare_higher_rank26_direction.sage','prepare_fresh_r17_pari_batch.sage','fresh_r17_pari_batch.py','prospective_half_lattice_v2.sage','research_runtime/rotated_observation_state.py','research_runtime/cached_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
...
def launch():
    p=protocol()
    for name,cmd,seconds in [('maps',['/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python',str(CAS/'prepare_higher_rank26_direction.sage'),'--id',ID],p['geometry_wall_seconds']),('worker',[sys.executable,str(Path(__file__).resolve()),'worker','--id',ID],p['worker_wall_seconds'])]:
