#!/usr/bin/env python3
"""Verify the generic-only native11952 metric49 control."""
...
def verify():
    ...
    output = ART/'native11952_metric49_recorded_mod2_v1.json'
    checkpoint(folder/'protocol.json', {'input_sha256': cert.hashed(path),
        ...
        'sources': {str(p.relative_to(ROOT)): cert.hashed(p) for p in (
            Path(__file__).resolve(), ROOT/'elliptic-curves/cas/replay_native11952_metric49.py',
    ...
    stages = [('charts', ['replay_native11952_metric49.py', str(path)], 900),
