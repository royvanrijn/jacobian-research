"""V3 selection on an explicitly frozen productive subset, not a full shell bank.
...
def landscape(model, basis, tested, wd, policy, bank, *, reference=False):
    ...
    if (not 0 < generic <= rank or bank['dimension'] != generic
            or bank['status'] != 'COMPLETE_FROZEN_PRODUCTIVE_SUBSET'
            or bank['generic_gram_scale'] != 2 or bank['shells'] != policy['scaled_shells']
            or len(bank['rows']) != 8
            or len({row['mask'] for row in bank['rows']}) != 8):
