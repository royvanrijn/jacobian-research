...
def freeze():
    ...
    save('map-protocol.json',dict(classification='generic-only rational maps, no specialized inputs',
...
def section():
    ...
    while todo:
        def score(item):
            i,s=item;v=vector(ZZ,word);v[i]+=s;return (v*G*v,i,s)
    ...
    save('short-section.json',dict(status='EXACT_GENERIC_SHORT_ALTERNATE_SECTION',word=ww,
        ...
        arithmetic_order=order,map_protocol_sha256=sha(OUT/'map-protocol.json')))
...
def maps(index):
    ...
    save('map-%d.json'%index,dict(status='EXACT_GENERIC_SHORT_TRANSLATION_MAP',index=index,sign=sign,
        ...
        source_sha256=sha(SOURCE),section_sha256=sha(OUT/'short-section.json'),map_protocol_sha256=sha(OUT/'map-protocol.json')))
