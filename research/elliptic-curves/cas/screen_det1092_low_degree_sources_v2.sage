...
def freeze():
    ...
    save('incidence-protocol-v2.json',dict(classification='retrospective exact source-incidence sieve, not a prospective seed selection',
...
def run(prime):
    protocol=read(OUT/'incidence-protocol-v2.json')
    ...
    record=dict(prime=prime,incoming=active,protocol_sha256=sha(OUT/'incidence-protocol-v2.json'))
    ...
    for i in active:
        ...
        if P.is_zero():row['status']='UNKNOWN_REDUCED_SECTION_IS_O'
            ...
            if degree!=2:row['status']='UNKNOWN_MAP_DEGREE_DROP'
                ...
                f=R(num-zstar*den);disc=f[1]**2-4*f[2]*f[0]
