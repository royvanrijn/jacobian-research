...
OLD=ART/'new_high_rank_curve_index_v10.json'
ADDITIONAL=ART/'higher25_followup_results_v1.json'
...
def build(output):
    ...
    cert.write(output,{'schema':'elliptic-curves.new-high-rank-index.v11','sources':sources(),
...
def check(path):
    ...
    for r in data['curves']:
        ...
        seen.add(j)
        print('REPLAYED V11 INVENTORY',r['id'],'rank >=',len(points),flush=True)
    print('EXACT CURVE INVENTORY',len(seen),flush=True)

if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('--output',type=Path,default=ART/'new_high_rank_curve_index_v11.json');p.add_argument('--check',type=Path)
