...
INPUTS=[ART/f'kihara_fresh_point_pilot_fibre{i}_mod2_v1.json' for i in range(4)]
OUT=ART/'kihara_fresh_point_pilot_sage_replay_v1.json'

def check_one(INPUT):
    d=json.loads(INPUT.read_bytes())
    E=EllipticCurve(QQ,d['curve'])
    points=[E([QQ(x),QQ(y)]) for x,y in d['points']]
    if len(points)<14:raise ArithmeticError('complete point cloud required')
    projective=[]
    for P in points:
...
if __name__=='__main__':
    ...
    result={'schema':'elliptic-curves.kihara-fresh-point-pilot-sage.v1','status':'PASS','rows':rows,
