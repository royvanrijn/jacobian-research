...
def main():
    ...
    for row in p['rows']:
        ...
        geometry(data,maps,initial,{**p,'maps_path':folder/'maps.json'});cloud_check(data,data['charts'],ART/('annulus64_r17_'+row['id'].replace('-','_')+'_mod2_v1.json'),path);count+=len(data['charts'])
    if count>3136 or sum(len(p['generic_masks'][r['family']]) for r in p['rows'])!=3136:raise ArithmeticError('fixed3136 maximum chart roster differs')
    print('REPLAYED ANNULUS64 CHARTS',count,'GENERIC CENTRES, RAW POINTS AND CLOUD PROVENANCE',flush=True)
