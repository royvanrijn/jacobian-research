...
ROOT=CAS.parents[1]
PARITY=ROOT/'artifacts/generated-results/elliptic-curves/fresh6_outside_maximum_seed_masks_v1.json'
...
def prepare(seed_path,folder):
    ...
    require(0<len(masks)<=16,'bounded outside-maximum mask roster required')
    paths=[seed_path,dataset/'prepared.json',dataset/'verified.json',dataset/'protocol.json',PARITY]
    names=['run_outside_maximum_seed_confirmation.py','prospective_half_lattice_v3.sage',
    ...
    protocol=dict(schema='fresh6-outside-maximum-first-M18.v1',family=seed['family'],parameter=seed['parameter'],
        ...
        rule='Frozen sampled generic classes outside the maximum-class span, with specialized exact CVP representatives '
