#!/usr/bin/env python3
"""Run one predeclared own-subgroup follow-up after full11952 inventory replay."""
import argparse
import sys
import time
...
D=LOCAL/'full11952-specialized-followup-controller-v1'


def prepare():
    if (D/'protocol.json').exists():raise FileExistsError('preserve one follow-up controller')
    names=['finish_full11952_specialized_followup.py','full11952_specialized_followup.py',
           'prepare_full11952_specialized_followup.sage','audit_full11952_specialized_followup.py',
           'replay_full11952_specialized_geometry.sage']
    paths=[*(CAS/n for n in names),ART/'native28_specialized_parity_adaptive_coverage_v1.json',
           LOCAL/'full11952-inventory-v14-controller-v1/protocol.json']
    checkpoint(D/'protocol.json',{
        'schema':'elliptic-curves.full11952-specialized-followup-controller.v1',
        'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},
        'wait_deadline_seconds':22000,'prepare_seconds':600,
        'maximum_roster':1,'trial_seconds_per_curve':1800,
        'audit_seconds_per_curve':960,'geometry_seconds':1200,
        'rss_bytes':4294967296,
        'scope':'After the fixed full11952 initial cohort has passed catalogue, full-cloud and complete V14 inventory replay, freeze all newly added V14 curves from that cohort with lower bound27. The entire eligible roster must have exactly one entry, stable ID186 at4286/1881; no truncation or refill. This explicit single follow-up changes point exposure using their own additional independent directions. The previously certified known29 control is the visibility gate. Freeze all2048-mask numerical samples and49 maps per curve before any new point search, then execute at most49 boxes at height125000 and ten seconds each with one point worker and target28. Perform exact histories, full-cloud proofs modulo2,3,5 and rational geometry. Any failure or censoring stops the controller without retry. No second wave, automatic height increase, inventory promotion, exact-rank or universal-novelty claim.'})
...
def launch():
    ...
    proof=ART/'new_high_rank_curve_index_v14_memory_replay_v1.json'
    while not proof.exists():
        upstream=cert.read(LOCAL/'full11952-inventory-v14-controller-v1/ledger.json')
        if upstream['status'] not in ('WAITING_FOR_ISOLATED_POINT_PROOFS','RUNNING','PASS') or any(r['status']!='PASS' for r in upstream['rows']):raise ArithmeticError('upstream inventory evidence failure requires review')
    ...
    def stage(name,executable,script,args,seconds):
        ...
        print('FULL11952 SPECIALIZED FOLLOWUP',name,ledger['rows'][-1]['status'],flush=True)
        if not ok:raise ArithmeticError('follow-up failed or censored; no retry')
    stage('freeze',sys.executable,'full11952_specialized_followup.py',['prepare'],p['prepare_seconds'])
    frozen=cert.read(LOCAL/'full11952-specialized-followup-v1/protocol.json');count=len(frozen['rows'])
    ...
    ledger.update(roster=frozen['rows'],point_protocol_sha256=cert.hashed(LOCAL/'full11952-specialized-followup-v1/protocol.json'),maximum_point_boxes=49*count);checkpoint(out,ledger)
    stage('trial',sys.executable,'full11952_specialized_followup.py',['launch'],p['trial_seconds_per_curve']*count)
    stage('clouds',sys.executable,'audit_full11952_specialized_followup.py',[],p['audit_seconds_per_curve']*count)
    stage('geometry','/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python','replay_full11952_specialized_geometry.sage',[],p['geometry_seconds'])
    audit=cert.read(LOCAL/'full11952-specialized-followup-v1/verification-ledger.json')
