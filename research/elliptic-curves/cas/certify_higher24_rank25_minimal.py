...
ROOT=Path(__file__).resolve().parents[2];ART=ROOT/'artifacts/generated-results/elliptic-curves';INPUT=ART/'higher24_r17_results_v1.json';OUT=ART/'higher24_rank25_minimal_proof_v1.json'
def sources():
...
def expected():
    ...
    return {'schema':'elliptic-curves.higher24-rank25-minimal.v1','status':'PASS','sources':sources(),'input_sha256':cert.hashed(INPUT),'curves':rows,'catalogue':d['catalogue'],'previous_equations':d['previous_equations'],'claim_boundary':'Exact independent-point lower bounds, proved global minimal integral models and exact point transports for the unmatched>=25 portion of the fixed24 higher-height cohort. Absence from593 pinned catalogue equations and402 earlier measured equations is not universal novelty. No exact rank or conductor is asserted.'}
