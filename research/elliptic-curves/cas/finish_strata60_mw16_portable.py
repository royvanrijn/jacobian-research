...
def prepare():
    ...
    preflight=batch.ART/'strata60_mw16_portability_preflight_v1.json'
    if cert.read(preflight)['status']!='PASS':raise ArithmeticError('isolated prospective input preflight required')
    paths=[Path(__file__).resolve(),CAS/'package_strata60_mw16_points.py',CAS/'verify_strata60_mw16_points_portable.py',
    ...
    checkpoint(D/'protocol.json',{'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},
        ...
        'stages':[['package','package_strata60_mw16_points.py',300],['isolated-replay','verify_strata60_mw16_points_portable.py',12000]],
