...
ROOT=batch.ROOT;CAS=batch.CAS;D=ROOT/'artifacts/local/elliptic-curves/fresh60-mw16-controller-v1'
def prepare():
    if (D/'protocol.json').exists():raise FileExistsError('preserve fresh60 controller')
    names=['finish_fresh60_mw16_points.py','fresh60_mw16_pari_batch.py','prepare_fresh60_mw16_pari_batch.sage','verify_fresh60_mw16_pari_batch.py','replay_fresh60_mw16_geometry.py','audit_fresh60_mw16_clouds_modl.py','score_mw16_fresh_retention.py','audit_recorded_point_mod2_rank_v3.py','audit_retained_cloud_modl.py','research_runtime/supervisor.py','research_runtime/store.py']
    paths=[*(CAS/n for n in names),batch.extension.D/'controller/protocol.json']
    checkpoint(D/'protocol.json',{'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},'wait_seconds':4800,'rss_bytes':4294967296,'stages':[('freeze','fresh60_mw16_pari_batch.py',['freeze'],120),('maps','fresh60_mw16_pari_batch.py',['maps'],1800),('points','fresh60_mw16_pari_batch.py',['batch'],20000),('verify','verify_fresh60_mw16_pari_batch.py',[],3600),('odd','audit_fresh60_mw16_clouds_modl.py',[],1800),('geometry','replay_fresh60_mw16_geometry.py',[],1200)],'scope':'After40960-score,60-selection and disjoint validation proofs pass, freeze exactly60 new-parameter MW16 point attempts,43 generic16 parity maps each, height125000,10 seconds per chart. All60 map files pass before any point attempt. Two point workers,600 seconds per curve, provisional28 stop; no adaptive wave or retries. Replay all terminal admission histories and retained point clouds modulo2,3,5 and rational geometry. Any catalogue/previous-equation comparison, inventory promotion or portable proof packaging follows separate terminal proofs.'})
def launch():
    p=cert.read(D/'protocol.json')
    if any(cert.hashed(ROOT/n)!=h for n,h in p['sources'].items()):raise ArithmeticError('fixed fresh60 sources changed')
    ...
    try:
        ...
        for name,script,args,seconds in p['stages']:
            ...
            ledger['rows'].append({'name':name,'status':'PASS' if ok else 'FAILED_OR_CENSORED','supervision':s});checkpoint(D/'ledger.json',ledger);print('FRESH60 POINT CONTROLLER',name,ledger['rows'][-1]['status'],flush=True)
