#!/usr/bin/env python3
"""Zero-search calibration and masked curve90 landscape for future V3."""
import argparse
import csv
...
from visibility_future import landscape
...
def sources(old):
    names = ('visibility_future.py', 'visibility_lattice_fast.py', 'v3_warm_engine.py',
             'v3_warm_support.py', 'run_future_visibility_audit.py')
...
def run(mode, folder):
    ...
    if mode == 'calibrate302':
        ...
        proof = seed['proof']
        bank = read(PREP/'parent-bank.json')
        # No read of the withheld point, M27 packet or historical chart data.
        inputs = [PREP/'seed-M26.json', PREP/'parent-bank.json', PREP/'prepared.json',
