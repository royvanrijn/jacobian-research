#!/usr/bin/env python3
"""Bind nine exact incidence cohorts to the current185 equations and duplicate map."""
import argparse
...
NAMES=['compact','latest7','latest8','latest23','retention','extended20_mw16','higher24','inventory101_added','inventory185_added']
...
INDEX=ART/'new_high_rank_curve_index_v13.json'

def result():
    index=cert.read(INDEX);targets={r['id']:r for r in index['curves']};seen=set();counts=Counter();duplicates=[]
    if len(targets)!=185:raise ArithmeticError('fixed185 inventory differs')
    maps=None
    for path in PROOFS:
        data=cert.read(path)
    ...
    if seen!=set(targets) or counts!={'NO_RATIONAL_J_PREIMAGE':2010,'RATIONAL_J_PREIMAGES_CERTIFIED':210} or len(duplicates)!=25:raise ArithmeticError('fixed2220 coverage differs')
    ...
    return {'schema':'elliptic-curves.inventory185-incidence.v1','status':'PASS','sources':sources,'targets_checked':185,'pairs_checked':2220,'status_counts':dict(counts),'same_generic_subgroup_duplicates':duplicates,'historical_rank_metadata':'Incidence targets retain their historical lower bounds; only nondecreasing current bounds are accepted, and all curve, j, family and parameter fields must remain identical. Incidence itself is independent of rank.','claim_boundary':'Combines the nine separately exact-replayed rational j-incidence cohorts and the exact Q(t) unimodular17-section transport. Every additional recorded presentation is the same generic subgroup. The separate incidence/transport replays remain required; this summary does not replay ranks or exclude other families, nongeneric points or higher ranks.'}
...
if __name__=='__main__':
    ...
    print('EXACT COHORT BINDING185 CURVES2220 PAIRS25 DUPLICATE PRESENTATIONS',flush=True)
