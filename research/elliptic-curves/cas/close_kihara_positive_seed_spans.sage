...
D=ROOT/'artifacts/local/elliptic-curves/kihara-positive-seed-spans-v1'
def main():
    ...
    for i in protocol['seed_indices']:
        ...
        for j,P in enumerate(points):
            numerical=B.solve_right(H.column(j)[indices]);coeff=[QQ(Fraction(str(c)).limit_denominator(128)) for c in numerical];den=lcm(c.denominator() for c in coeff);word=[ZZ(c*den) for c in coeff]
