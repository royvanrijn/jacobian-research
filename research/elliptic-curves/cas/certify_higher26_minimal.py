#!/usr/bin/env python3
"""Exact integral models and point transports for unmatched higher-height R17 follow-up curves>=26."""
import argparse
from pathlib import Path
import certify_compact_r17_candidates as cert
import certify_discarded_rank26_minimal as arithmetic
from research_runtime.store import checkpoint
ROOT=Path(__file__).resolve().parents[2];ART=ROOT/'artifacts/generated-results/elliptic-curves';INPUT=ART/'higher25_followup_results_v1.json';OUT=ART/'higher26_minimal_proof_v1.json'
def sources():
    return {**arithmetic.sources(),str(Path(__file__).resolve().relative_to(ROOT)):cert.hashed(Path(__file__).resolve())}
def expected():
    d=cert.read(INPUT);selected=[r for r in d['curves'] if r['rank_lower_bound']>=26 and not r['icarm_matches'] and not r['previous_matches']];rows=[]
    if not selected:raise ArithmeticError('no unmatched>=26 cohort')
    for q in selected:
    ...
    return {'schema':'elliptic-curves.higher26-minimal.v1','status':'PASS','sources':sources(),'input_sha256':cert.hashed(INPUT),'curves':rows,'catalogue':d['catalogue'],'previous_equations':d['previous_equations'],'claim_boundary':'Exact independent-point lower bounds, proved global minimal integral models and exact point transports for the unmatched>=26 portion of the single completed higher-height adaptive follow-up. Absence from593 pinned catalogue equations and402 earlier measured equations is not universal novelty. No exact rank or conductor is asserted.'}
