...
def main():
    ...
    for row in p['rows']:
        ...
        geometry(data,maps,initial,{**p,'maps_path':folder/'maps.json'});cloud_check(data,data['charts'],ART/('compact192_r17_'+row['id'].replace('-','_')+'_mod2_v1.json'),path);count+=len(data['charts'])
    if count>8640 or sum(len(p['generic_masks'][r['family']]) for r in p['rows'])!=8640:raise ArithmeticError('fixed8640 maximum chart roster differs')
    print('REPLAYED COMPACT192 CHARTS',count,'GENERIC CENTRES, RAW POINTS AND CLOUD PROVENANCE',flush=True)
