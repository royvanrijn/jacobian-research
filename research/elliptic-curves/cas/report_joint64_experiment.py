#!/usr/bin/env python3
"""Bind the completed fresh outer selection and fixed64 point outcomes."""
import argparse,json
from pathlib import Path
import certify_compact_r17_candidates as cert
import joint64_r17_pari_batch as batch
from research_runtime.store import checkpoint
ROOT=batch.ROOT;ART=batch.ART;D=batch.D;CONTROL=ROOT/'artifacts/local/elliptic-curves/joint64-controller-v1'
INPUT=ART/'joint64_r17_results_v1.json';OUT=ART/'joint64_experiment_v1.json'
def expected():
    ...
    for r,o in zip(source['curves'],odd['rows']):
        ...
        rows.append({'id':r['id'],'family':r['family'],'parameter':r['parameter'],'rank_lower_bound':r['rank_lower_bound'],
    ...
    return {'schema':'elliptic-curves.joint64-experiment.v1','status':'PASS_STRONGER_ODD_BOUNDS_REQUIRE_REVIEW' if stronger else 'PASS',
        ...
        'claim_boundary':'Eight untouched signed denominator slices in131072<H<=524288 on11952, disjoint from the previous complete square and earlier outer slices. Every primitive address receives all3510 selection primes before retention. All32768 retained combined scores precede4096 exact equation exclusions/scalar scores and64 distinct finalists; wholly disjoint validation never changes selection. Up to3136 generic17-only point charts with all maps before points. The1101 prior-equation and593 catalogue exclusions are independently rechecked after terminal exact point proofs. Matching finite bounds do not prove exact rank, saturation or absence. No universal novelty, rank density, calibrated sensitivity or isolated portability claim.'}
if __name__=='__main__':
    ...
    if a.check:
        ...
        checkpoint(OUT,d)
    print('JOINT64 OUTCOMES',d['status'],d['completed_point_boxes'],[(r['id'],r['rank_lower_bound']) for r in d['rows'] if r['rank_lower_bound']>=22],flush=True)
