...
BATCH=LOCAL/'retained-native19-point-trial-v1';INDEX=ART/'new_high_rank_curve_index_v22.json';GATE=ART/'inventory188_fixed49_point_control_v1.json'
...
def sources():
    names=['audit_retained_native19_trial.py','replay_retained_native19_geometry.sage','export_new_high_rank_curve_index_v14.py','memory_rank_certificate.py','retained_native19_trial.py','prepare_retained_native19_trial.sage','prepare_fresh_r17_pari_batch.sage','fresh_r17_pari_batch.py','prospective_half_lattice_v2.sage','research_runtime/search_state.py','research_runtime/preloaded_prime_state.py','research_runtime/rotated_observation_state.py','research_runtime/cached_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py','replay_retention24_geometry.py','audit_recent_outer26_exposure.py','score_retained_native19.py','audit_retained_native19_intake.py']
...
def prepare():
    ...
    gate=cert.read(GATE)
    if gate['status']!='PASS' or gate['completed_charts']!=49 or gate['rank_lower_bound']!=28:raise ArithmeticError('completed exceptional control required')
    ...
    checkpoint(BATCH/'protocol.json',{'schema':'elliptic-curves.retained-native19-point-trial.v1','sources':sources(),'inputs':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},'seed_hashes':seeds,'rows':rows,'sample_size':2048,'sample_domain':'full11952-specialized-followup-v1','charts':49,'height':125000,'seconds_per_chart':10,'target_rank':28,'rank_stop':False,'gp_sha256':cert.hashed(Path('/usr/bin/gp')),'geometry_wall_seconds':300,'worker_wall_seconds':900,'replay_wall_seconds':600,'rss_bytes':1610612736,'maximum_workers':1,'maximum_point_boxes':98,'gate':'Twelve retained rank19 carrier images had no further point exposure because all failed an earlier400-bit model-budget gate. Their exact19-point proofs and current620-catalogue comparisons replay. A fixed12-equation training-prime score audit is complete. Test the strongest-scoring image and the smallest displayed model as a separate bounded feasibility and direction-discovery experiment in new parameter territory.','centre_policy':'Reuse the calibrated2048 SHA256 parity masks in rank19, nonzero above the17 inherited prefix.384-bit heights rounded at10^6, unimodular LLL and numerical CVP, exact norm/parity checks;49 largest computed norms. Freeze both map files before any point search.','selection':'Exactly native19-08 word[-2,0],3875 coefficient bits, and native19-01 word[1,0],543 bits, from the existing12. No new carrier word, parameter scan, public point input, validation-prime input, outcome-dependent refill or rank stopping.','previous_budget_gate':'This separate trial deliberately admits the two retained equations above400 coefficient bits. The original height-gated experiment and its negative admission result remain unchanged. Point-coordinate height and per-chart time remain125000 and ten seconds.','boundaries':'Exactly98 declared charts,49 per curve, one worker. Preserve failures and timeouts. No automatic retry, next wave, new parameter scan, exact-rank or universal novelty claim.','following_campaign':None})
...
def launch():
    ...
    for index in range(len(p['rows'])):
        ...
        s=supervise(['/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python',str(CAS/'prepare_retained_native19_trial.sage'),'--index',str(index)],limits=Limits(p['geometry_wall_seconds'],p['rss_bytes']),log_path=D/(name+'.log'),checkpoint_path=path,cwd=ROOT)
