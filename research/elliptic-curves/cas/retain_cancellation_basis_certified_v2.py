#!/usr/bin/env python3
"""Read the nested native rank receipt; preserve the frozen first reader.
...
def endpoints():
    need(read(OUT/'preflight.json')['inputs_sha256'] == sha(OUT/'inputs.json'), 'preflight input differs')
...
def bindings():
    ...
    for receipt in supervision['records']:
        ...
        need(verification['status'] == 'PASS' and verification['new_directions'] == result['new_directions'] and verification['rank']['rank'] == result['rank_lower_bound'], 'unverified arm')
        need(receipt['charged_cpu_seconds'] >= result['total_internal_cpu_seconds']-.05, 'CPU meter understates worker')
        ...
        for event in events:
            if event['kind'] == 'prepared':
                ...
                need(verified['status'] == 'PASS_INDEPENDENT_BANK_CERTIFICATE', 'unverified bank')
                need(not (folder/'reference').exists(), 'obsolete reference schema')
                ...
                selection = read(folder/'producer/selection.json'); proof = read(folder/'selection-proof.json')
                need(proof['status'] == 'PASS_INDEPENDENT_BANK_SELECTION_CERTIFICATE' and proof['selection_sha256'] == verified['producer_sha256'], 'selection proof differs')
                for ai, anchor in enumerate(selection['anchors']):
                    for suffix, field in [('full.npz','full_scores_sha256'),('maps.json','maps_sha256')]:
                        need(sha(folder/'producer'/f'anchor-{ai:02d}-{suffix}') == anchor[field], 'bank auxiliary input changed')
                banks[event['epoch']] = read(folder/'bank.json')
                need(banks[event['epoch']]['centres'] == selection['centres'][:plan['maximum_centres']], 'exported bank differs')
    ...
    return {'status': 'PASS_RETAINED_BYTES_AND_INPUT_BINDINGS', 'rows': rows, 'totals': totals,
        ...
        'boundary': 'Retained receipt accounting and input binding. Minimum exhaustion and full selection were independently checked before use; final worker replay binds those records and repeats direct norm/parity and native witnesses. This inspection does not repeat that arithmetic. Tail witnesses assert coordinates, not rational points.'}
...
def audit():
    ...
    new_write(OUT/'dependency-audit.json', {'extra_source_sha256': extra,
        'boundary': 'Predeclared retention and replay entry points. Any extra source would be explicitly reported here, without retagging the primary seal.'})
...
def portable():
    ...
    with tempfile.TemporaryDirectory(prefix='basis-certified-empty-root-') as temp:
        ...
        code = "import sys;sys.path.insert(0,'elliptic-curves');sys.path.insert(0,'elliptic-curves/cas');import retain_cancellation_basis_certified_v2 as r;from verify_cancellation_basis_certificate import verify_arm;import ecsearch.q12o5867_specialization;a=r.bindings();print(a['status'],len(a['rows']))"
