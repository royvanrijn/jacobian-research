"""Dimension-aware V3 landscape for new campaigns; historical V3 is untouched.
...
def landscape(model, basis, tested, wd, policy, bank, *, reference=False):
    ...
    if (not 0 < generic <= rank or bank['dimension'] != generic
            or bank['status'] != 'COMPLETE_EXACT_GENERIC_SHELL_BANK'
            or bank['generic_gram_scale'] != 2 or bank['shells'] != [16, 20]):
