...
// Modular filters are necessary square conditions, including zero and p | denominator.
#include <gmpxx.h>
#include <algorithm>
#include <array>
#include <chrono>
...
int main(int argc, char **argv) {
    ...
    for (int p = 3; p <= 151; p += 2) {
        ...
        for (int d = 0; d < p; ++d) for (int n = 0; n < p; ++n) {
            ...
            filter.allowed[d*p+n] = square[value];
            filter.count += square[value];
        }
    ...
    for (int d = first; d <= last; ++d) {
        ...
        for (size_t i = 0; i < filters.size(); ++i) offsets[i] = (d%filters[i].p)*filters[i].p;
        for (int i = 0; i < word_filters; ++i)
