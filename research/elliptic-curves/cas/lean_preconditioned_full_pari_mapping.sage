"""Exact factor-free preconditioning, then the full PARI minimization call.

This is a bounded coordinate policy; minimality is not independently certified. Run under an
...
def mapping(model, points, centre):
    ff = load('preconditioned_factor_free', CAS/'lean_factor_free_pari_mapping.sage')
    ...
    result = pari('my(m1,m2,C1,C2);C1=hyperellminimalmodel('+C+',&m1);'
    ...
    return dict(centre=centre, raw_coefficients=old['raw_coefficients'],
                ...
                minimization_policy='full PARI call after factor-free preconditioning', claim_boundary='Exact coordinates; no global minimality claim.')
