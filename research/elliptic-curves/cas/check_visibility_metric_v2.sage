...
"""Independent recomputation of every V2 height decision form and LLL transport."""
...
v2 = SourceFileLoader('metric_replay_v2',str(CAS/'adaptive_visibility_cascade_v2.sage')).load_module()
v2.v1.guard()
...
stages = []
for path in sorted((v2.D/'calibration302').glob('epoch-*/selection.json')):
...
checkpoint(v2.D/'metric-replay.json',{'protocol_sha256':v2.sha(v2.D/'protocol.json'),
