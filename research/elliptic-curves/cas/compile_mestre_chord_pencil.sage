...
BUNDLE=ART/'mestre_468_replay_bundle_v1.json'
D=ROOT/'artifacts/local/elliptic-curves/mestre-chord-pencils-v1'
parser=argparse.ArgumentParser();parser.add_argument('--index',type=int,required=True);args=parser.parse_args()
admission=json.loads((D/'admission.json').read_text());selected=admission['rows'][args.index];OUT=D/('pencil'+str(args.index)+'.json')
row=next(r for r in json.loads(BUNDLE.read_text())['rows'] if r['outer_u']=='11');h=row['generic_heights']
R=PolynomialRing(QQ,'T');T=R.gen();K=R.fraction_field();A=R(h['curve_A_coefficients']);B=R(h['curve_B_coefficients']);E=EllipticCurve(K,[A,B])
ns=json.loads((ART/'mestre_rational_ns_gram_v2.json').read_text())['rows'][0]
seed=[E([K(c) for c in coords]) for coords in ns['section_points']]
P=sum((int(w)*point for w,point in zip(selected['word'],seed)),E(0))
...
result={'schema':'elliptic-curves.mestre-chord-pencil.v1','selection':selected,'pencil_index':args.index,'status':'PASS','outer_u':'11','old_A':list(map(str,A.list())),'old_B':list(map(str,B.list())),
...
'sources':{str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__).resolve(),BUNDLE,D/'admission.json',ART/'mestre_rational_ns_gram_v2.json']},
