...
v2 = SourceFileLoader('replayed_v2',str(CAS/'adaptive_visibility_cascade_v2.sage')).load_module()
...
def replay(progress=None):
    ...
    for wd in sorted((d/'calibration302').glob('epoch-*')):
        if not (wd/'stage.json').exists():
            break
        index = len(reports)
        ...
        for ai, record in enumerate(selection['anchors']):
            ...
            order = sorted(range(count),key=lambda i:(-int(norms[i]),keys[i]))[:p['exact_shortlist_per_anchor']]
            ...
            chosen += [r for r in ranked if tuple(r['point']) not in tested][:p['finalists_per_anchor']]
            cosets += count
        ...
        basis = enlarged
        print('REPLAYED V2 EPOCH',{k:v for k,v in reports[-1].items() if k!='checkpoint_hashes'},flush=True)
        if progress:
            checkpoint(progress,{'checker_sha256':v2.sha(Path(__file__)),
                'protocol_sha256':v2.sha(d/'protocol.json'),'stages':reports})
    return {'schema':'visibility-cascade-v2-replay','protocol_sha256':v2.sha(d/'protocol.json'),
