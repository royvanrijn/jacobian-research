...
cert=batch.cert;ROOT=batch.ROOT;D=batch.BATCH;ART=batch.ART;OUT=ART/'mixed_reduced_parent_exposure_v1.json'
def expected():
 p=batch.protocol();search=cert.read(D/'ledger.json');post=cert.read(D/'post-ledger.json');driver=cert.read(D/'run-ledger.json');assert search['status']==post['status']==driver['status']=='PASS'
 assert len(search['rows'])==len(post['rows'])==len(search['maps'])==9
 ...
 inputs={};rows=[];seen=set();total=sum(s['supervision']['wall_seconds'] for s in driver['stages'])+score['total_score_seconds']
 for row in p['rows']:
