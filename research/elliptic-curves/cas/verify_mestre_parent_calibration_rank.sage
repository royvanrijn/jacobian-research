#!/usr/bin/env python3
"""Independent complete finite groups for the twelve parent-calibration clouds."""
import json,hashlib,argparse
from pathlib import Path
from sage.all import QQ,ZZ,GF,EllipticCurve,Matrix,lcm,gcd,PolynomialRing
ROOT=Path(__file__).resolve().parents[2];ART=ROOT/'artifacts/generated-results/elliptic-curves'
INPUTS=[ART/f'mestre_parent_calibration_u{u}_{role}_mod2_v1.json' for u in (11,13,17,19,23,29) for role in ('unit','outer')]
OUT=ART/'mestre_parent_calibration_sage_replay_v1.json'

def check_one(INPUT):
    d=json.loads(INPUT.read_bytes())
    E=EllipticCurve(QQ,d['curve'])
    points=[E([QQ(x),QQ(y)]) for x,y in d['points']]
    if len(points)<11:raise ArithmeticError('complete point cloud required')
    projective=[]
    for P in points:
...
if __name__=='__main__':
    ...
    result={'schema':'elliptic-curves.mestre-parent-calibration-sage.v1','status':'PASS','rows':rows,
