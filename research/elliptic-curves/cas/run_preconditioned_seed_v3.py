#!/usr/bin/env python3
"""Prepared-seed discovery with separately bounded map construction and point searching.
...
import time
import shutil
from preconditioned_map_receipts import obtain as obtain_map
...
from visibility_parent_subset import landscape
...
def guard(protocol):
    ...
    require(sha(Path(protocol['map_python'])) == protocol['map_python_sha256'], 'map launcher changed')
...
def freeze(folder):
    ...
    require(bank['status'] in ('COMPLETE_FROZEN_PRODUCTIVE_SUBSET', 'COMPLETE_FROZEN_PAIRWISE_PARENT_SUBSET'), 'exact parent subset required')
    ...
    require(seed['generic_rank'] in (16, 17) and seed['generic_rank'] < initial_rank < 32, 'native MW16/R17 seed below target required')
    ...
    names = ('run_preconditioned_seed_v3.py', 'preconditioned_map_receipts.py', 'preconditioned_map_worker.py',
             'preconditioned_full_pari_mapping.sage',
             'factor_free_pari_mapping.sage', 'visibility_parent_subset.py',
    ...
    protocol = {'schema': 'prepared-parent-preconditioned-maps.v1', 'initial_rank': initial_rank,
        ...
        'selection': 'V3 diversified extension shortlist over every frozen exact parent class; no per-shell anchor truncation. '
            ...
            'coordinates, skipping identical projective coordinates. Each map has its own5-second '
...
def search(folder):
    ...
    proof = seed['proof']
    require(len(points) == protocol['initial_rank'], 'initial rank differs')
    ...
    for epoch in range(protocol['max_epochs']):
        ...
        charts, skipped, map_attempts, grew = [], [], [], False
        for ci, centre in enumerate(selection['centres']):
            seen_coordinates = set()
            for key in MAPPERS:
                if total >= protocol['max_charts']:
                    break
                mapping, receipt_sha, limited = obtain_map(wd, ci, key, model, points, centre, state, protocol)
                map_attempts.append({'centre_index': ci, 'policy': key, 'receipt_sha256': receipt_sha})
                if limited:
                    skipped.append({'centre_index': ci, 'policy': key, 'reason': limited})
                    continue
        ...
        snapshot = {'status': 'RETAINED_PRODUCTIVE_EPOCH_CLOUD', 'family': protocol['family'],
            'parameter': protocol['parameter'], 'curve': seed['curve'], 'charts': charts, 'final_state': state.record()}
        snap = wd/'cloud.json'
        if snap.exists():
        ...
        censored_maps = sum(s['reason'] in ('strict_wall_timeout', 'strict_rss_limit') for s in skipped)
        if audit['rank_lower_bound'] != len(points) or any(r != len(points) for r in odd.values()):
            ...
            stop = 'CENSORED_FINITE_EXPOSURE' if censored or censored_maps else 'FINITE_POLICY_EXHAUSTED_NO_CERTIFIED_GAIN'
        stage = {'epoch': epoch, 'before': before, 'after': len(points), 'charts': len(charts),
            'skipped': skipped, 'map_attempts': map_attempts, 'censored_maps': censored_maps,
            'last_chart_sha256': previous, 'stop_reason': stop,
...
def replay(folder):
    ...
    for stage in terminal['stages']:
        ...
        schedule, last_ci, seen = [], None, set()
        attempts = stage['map_attempts']
        raw_order = [(ci, key) for ci in range(len(saved['centres'])) for key in MAPPERS]
        require([(a['centre_index'], a['policy']) for a in attempts] == raw_order[:len(attempts)] and
                len(attempts) <= len(raw_order), 'map-attempt prefix differs')
        for attempt in attempts:
            ci, key = attempt['centre_index'], attempt['policy']
            centre = saved['centres'][ci]
            if ci != last_ci:
                seen = set()
                last_ci = ci
            mapping, receipt_sha, limited = obtain_map(wd, ci, key, model, basis, centre,
                                                       state, protocol, replay=True)
            require(receipt_sha == attempt['receipt_sha256'], 'map receipt seal differs')
            if limited:
                skipped.append({'centre_index': ci, 'policy': key, 'reason': limited})
                continue
            coordinate = coordinate_key(mapping)
        ...
        require(skipped == stage['skipped'] and
                sum(s['reason'] in ('strict_wall_timeout', 'strict_rss_limit') for s in skipped)
                == stage['censored_maps'], 'map censoring/deduplication differs')
        require(stage['charts'] == len(schedule), 'map/chart schedule count differs')
        if stage['stop_reason'] in ('FINITE_POLICY_EXHAUSTED_NO_CERTIFIED_GAIN', 'CENSORED_FINITE_EXPOSURE'):
            require(len(attempts) == len(raw_order), 'incomplete map exposure labelled exhausted')
        if stage['stop_reason'] == 'FINITE_POLICY_EXHAUSTED_NO_CERTIFIED_GAIN':
            require(stage['censored_maps'] == 0 and stage['censored'] == 0,
                    'censored exposure labelled complete')
        ...
        tested.update(tuple(saved['centres'][a['centre_index']]['point']) for a in attempts)
        ...
        require(sum(c['search']['status'] != 'bounded_search_complete' for c in cloud['charts'])
                == stage['censored'], 'point-search censor count differs')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--preparation', type=Path, help='Required when starting a new search')
    args = parser.parse_args()
    PREP = args.preparation.resolve() if args.preparation else None
    BANK = PREP
    require(args.folder.exists() or PREP is not None, 'new search needs --preparation')
