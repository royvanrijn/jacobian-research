#!/usr/bin/env python3
"""One independent rank26 proof with global minimal models and novelty checks."""
...
ROOT=Path(__file__).resolve().parents[2];ART=ROOT/'artifacts/generated-results/elliptic-curves';D=ROOT/'artifacts/local/elliptic-curves/discarded12-r17-pari-v1';OUT=ART/'discarded_rank26_minimal_proof_v1.json';OLD=ART/'next24_r17_results_v1.json';DB=ROOT/'artifacts/local/elliptic-curves/next12-current-catalogue-v1/database.json';IDS=('074d9-027',)

def sources():
    names=['certify_discarded_rank26_minimal.py','memory_rank_certificate.py','certify_compact_r17_candidates.py','elliptic_candidate_record.py','mod2_reduction_independence.py','research_runtime/finite_reduction.py','research_runtime/memory_store.py','research_runtime/store.py']
...
def build():
    ...
    for identifier in IDS:
        ...
        r={'id':identifier,'family':d['family'],'parameter':d['parameter'],'rank_lower_bound':len(points),'minimal_curve':list(map(str,model)),'points':[list(map(str,P)) for P in transported],'discovery_curve':d['curve'],'discovery_points':state['reductions']['points'],'rank_certificate':proof,'minimality':minimality(model),'icarm_matches':[q['id'] for q in projection if cert.isomorphic(model,q['ainvs'])],'previous_matches':[q['address'] for q in previous if cert.isomorphic(model,q['curve'])]};check_row(r,projection,previous);rows.append(r);inputs[str(path.relative_to(ROOT))]=cert.hashed(path);print('PROVED MINIMAL DISCARDED CURVE',identifier,'rank >=',len(points),'gcd',r['minimality']['invariant_gcd'],flush=True)
    if any(cert.isomorphic(r['minimal_curve'],s['minimal_curve']) for i,r in enumerate(rows) for s in rows[:i]):raise ArithmeticError('selected curves are not mutually distinct')
    if any(r['icarm_matches'] or r['previous_matches'] for r in rows):raise ArithmeticError('unmatched roster gate failed')
    checkpoint(OUT,{'schema':'elliptic-curves.discarded-rank26-minimal-proof.v1','sources':sources(),'inputs':{**inputs,**{str(p.relative_to(ROOT)):cert.hashed(p) for p in (D/'ledger.json',OLD,DB)}},'curves':rows,'catalogue':{'url':'https://elliptic-rank.icarm.cloud/database.json','raw_sha256':cert.hashed(DB),'equations':projection},'previous_equations':previous,'claim_boundary':'One globally minimal elliptic curve in compact family074d9 at2588/1603, with26 exactly independent rational points, absent from the pinned586-equation catalogue and346 earlier measured equations. No exact rank, exact conductor, universal novelty or new rank28/32 claim.'})
...
def check():
    ...
    print('REPLAYED ONE MINIMAL RANK26 PROOF',flush=True)
