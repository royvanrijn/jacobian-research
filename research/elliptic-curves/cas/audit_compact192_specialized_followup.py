...
def main():
    ...
    if out.exists():raise FileExistsError('preserve fixed compact192 follow-up cloud audit')
    ...
    for index,row in enumerate(p['rows']):
        ...
        mod2=ART/('compact192_specialized_'+row['id'].split('-')[-1]+'_mod2_v1.json');modl=mod2.with_name(mod2.name.replace('_mod2_','_modl_'))
        ...
        checkpoint(out,ledger);print('AUDITED SPECIALIZED COMPACT192',row['id'],len(data['charts']),cloud['rank_lower_bound'],len(cloud['points']),flush=True)
