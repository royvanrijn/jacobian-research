...
save('degree-minima-protocol.json',{'classification':'exact generic-only low-degree translation-orbit calculation',
...
ct=[Fraction(q) for q in ct];limit=Fraction(radius);word=[0]*17;nodes=0;answers=[]
def visit(i,used):
    global nodes
    nodes+=1
...
save('degree-minima-%s.json'%('minus' if args.sign<0 else 'plus'),report)
