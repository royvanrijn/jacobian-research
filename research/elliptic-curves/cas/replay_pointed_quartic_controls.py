#!/usr/bin/env python3
"""Replay retained witnesses for the 1,034 frozen MW16 control boxes.

Check exact coefficients, square hits and mapped points chart by chart;
...
Use --regenerate to repeat the complete bounded point census as a backend
regression. Default replay proves the retained points and relations; it does
not independently prove that a discovery transcript omitted no square hits.
"""
import argparse
...
def main():
    ...
    parser.add_argument("--regenerate", action="store_true", help="explicitly re-sieve all frozen boxes")
    ...
    def replay(job):
        ...
        if not args.regenerate:
            points=exact.check_chart(model,subgroup,saved)
            return {"curve_id":curve_id,"centre":centre_kind,"mask":saved['mask'],
                    "height":saved['height_bound'],"square_hits":len(saved['primitive_square_hits']),
                    "finite_points":len(points),"retained_witness_sha256":sha256(pq.canonical(saved).encode()).hexdigest()}
    ...
    report = {"schema": "elliptic-curves.universal-pointed-control-regression.v1",
              "status": "PASS_EXACT_BOX_AND_QUOTIENT_REPLAY" if args.regenerate else "PASS_RETAINED_POINT_AND_QUOTIENT_WITNESSES", "backend": pq.BACKEND_NAME,
              "census_regenerated":args.regenerate,
