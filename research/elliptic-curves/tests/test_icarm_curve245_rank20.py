...
class IcarmCurve245Rank20Tests(unittest.TestCase):
    ...
    def test_finite_reduction_certificate_has_rank_20(self) -> None:
        coefficients = short_coefficients()
        self.assertEqual(
            find_two_torsion_certificate_prime(coefficients, prime_bound=100),
            7,
        )
        ...
        self.assertEqual(combined_mod2_rank(signatures, 20), 20)

    def test_conductor_has_66_decimal_digits(self) -> None:
        self.assertEqual(len(str(CONDUCTOR)), 66)
        self.assertLess(CONDUCTOR, 10**66)
