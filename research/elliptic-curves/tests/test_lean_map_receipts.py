"""Lean worker regression: forced startup timeout, exact fallback, forged-map rejection."""
import json
...
class LeanMapReceiptsTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.minimized = cls.obtain('preconditioned_full')
    ...
    def obtain(cls, policy, replay=False):
        return maps.obtain(cls.epoch, 0, policy, cls.model, cls.points, cls.centre,
                           cls.state, {**cls.protocol, 'seconds_per_map': 0.005} if policy == 'preconditioned_full' else cls.protocol, replay=replay)
    ...
    def test_timeout_replay_never_restarts_worker(self):
        ...
        with patch.object(maps, 'run', side_effect=AssertionError('worker started during replay')):
            self.assertEqual(self.obtain('preconditioned_full', replay=True), self.minimized)
