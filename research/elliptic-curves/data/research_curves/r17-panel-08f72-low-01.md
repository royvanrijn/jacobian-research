# r17-panel-08f72-low-01

Rank **≥ 21**. Family `08f72` at native parameter `422/743`.

[Full data and certified points](r17-panel-08f72-low-01.json) · [Inventory](../../INVENTORY.md)

Source a-invariants (global minimality has not been certified for this inventory):

```text
0, 0, 0, -4857544067242564666119787269167027039895773825487147, 137792159503599508884132725197899303279268040956450343710582589080592915659686
```

Conductor status: **UNKNOWN**. Minimal discriminant, naive height and Faltings height are uncomputed in this inventory.

Certified point count: 21. This is a rank lower bound; bounded no-gain searches do not prove exact rank.

[Canonical proof note](../../../elliptic-curves/notes/R17_SIXTY_SEED_COMPLEMENT_PANEL_2026-09-09.md) · [Equation and points](../../../artifacts/generated-results/elliptic-curves/r17_60_panel_results_v1.json) · [Independent rank proof](../../../artifacts/generated-results/elliptic-curves/r17_60_panel_results_v1.json)

This finite seed supplement makes no literature-wide novelty or conductor-record claim.

Certified conductor divisor:

```text
122151783390
```

Certified conductor upper bound:

```text
4807303653779893835455291214635036318850847821770749349016999263101984404959371119379106691613058991357735391491169258512370
```

Proved bad primes (incomplete):

```text
2, 3, 5, 7, 13, 41, 53, 59, 349
```

[Conductor certificate](../../../artifacts/generated-results/elliptic-curves/inventory291_conductor_snapshot_v2.json)
