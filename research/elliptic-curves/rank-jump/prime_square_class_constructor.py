#!/usr/bin/env python3
"""Bounded prime-square principal-generator construction on one retained fibre."""
import argparse
...
OUTPUT=r.OUT/'rank_jump_prime_square_class_constructor_v1.json'
WORK=r.ROOT/'artifacts/local/rank-jump-prime-square-class-constructor-v1'
...
def export():
    ...
    data=r.read(pool.INPUT);ref.configure();fm,pts,scale=ref.base.model_data(ref.TOKEN)
    R=PolynomialRing(QQ,'z');f=R(data['cubic_ascending']);c,a=map(QQ,data['affine_map_old_root_from_masked'])
    assert a.is_square() and f(a*R.gen()+c)==a**3*R(list(map(str,fm.list())))
    generic=[]
    for x,y in pts:
        oldx=a*x+c;d=oldx.denominator();assert d.is_square()
        beta=[str(d*oldx),str(-d),'0'];N=d**3*a**3*y**2;assert N.is_square()
        generic.append({'beta_ascending':beta,'norm':str(N)})
    ids=[i for i,c in enumerate(data['columns']) if c['f']==1][:24]
    assert len(ids)==24 and [data['columns'][i]['p'] for i in ids]==sorted(data['columns'][i]['p'] for i in ids)
    r.write_new(INPUT,{'schema':'rank-jump.prime-square-class-constructor-inputs.v1',
        'cubic_ascending':data['cubic_ascending'],'field_discriminant':data['field_discriminant'],
        'S_finite':data['S_finite'],'generic_classes':generic,
        'prime_ideals':[{'retained_column':i,**data['columns'][i]} for i in ids],
        'bindings':bindings([Path(__file__),PROTOCOL,pool.INPUT,ref.INPUT])})
...
def worker():
    ...
    for label,kind,I,extra in targets:
        ...
        norms=[];hits=[]
        for v in ds:
            coords=reduced*vector(ZZ,v);alpha=pari.nfbasistoalg(nf,pari(list(coords)))
    ...
    r.write_new(OUTPUT,{'schema':'rank-jump.prime-square-class-constructor.v1','status':'PASS',
...
def capture():
    ...
    if error:r.write_new(OUTPUT,{'schema':'rank-jump.prime-square-class-constructor.v1','status':'UNKNOWN','reason':error})
    result=r.read(OUTPUT);print(result['status'],[(x['id'],len(x['hits'])) for x in result.get('rows',[])],flush=True)


if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('mode',choices=['export','worker','capture']);args=p.parse_args();globals()[args.mode]()
