#!/usr/bin/env python3
"""Generic-section divisibility at native branch fibres, without twist points."""
import argparse
...
OUTPUT=r.OUT/'rank_jump_branch_divisibility_capacity_v1.json'
WORK=r.ROOT/'artifacts/local/rank-jump-branch-divisibility-capacity-v1'
...
def compute():
    ...
    return {'schema':'rank-jump.branch-divisibility-capacity.v1','status':'PASS','topology':topology,
        'generic_mod_two_dimension':m,'generic_independence_parameter':'0','generic_independence_blocks':generic_blocks,
        'rows':rows,'bindings':bindings([Path(__file__),PROTOCOL,INPUT,Path(real.__file__),Path(r.__file__)]),
