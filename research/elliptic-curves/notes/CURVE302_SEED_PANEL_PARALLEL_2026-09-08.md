...
The scheduling-only continuation runs up to **four independent Sage case
...
Its partial replay restarts with the original checker; no search charts are
repeated for that sealed case. The verified `residual-strict-03` result
(rank31,385 charts) is reused with protocol/terminal hash checks.
...
From `research/`, use the new status command for all active cases:

```sh
python3 elliptic-curves/cas/run_curve302_seed_panel_parallel.py status
```

After reviewing a stopped run, resume with the same manifest and concurrency:
