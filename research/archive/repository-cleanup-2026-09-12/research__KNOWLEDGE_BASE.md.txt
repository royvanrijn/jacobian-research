...
| Positive-control calibration plus separate proof/resource gates | Elkies rank-25--29 fibres and the rank-32 search | The redacted sixteen-fibre refresh ladder validates exact half-lattice recovered rank against displayed jumps `+3` through `+12`: tied-margin Kendall `tau_b=.7503`, exact one-sided `p=60852/2421619200`, and score at least ten on 7/8 of the `+10/+11/+12` tail versus 0/8 below (`p=1/1430`).  High score may schedule search, but low score cannot veto: curve 544 is a `+11` false negative after the bootstrap-only 43 charts.  In production, only a certified rank/Selmer upper bound below 32 excludes a fibre; incomplete descent is scheduling information.  Every search still needs finite recorded limits, and 32 exactly verified independent points prove rank at least 32 without a completed descent.  The sealed 100-row class-quotient laboratory retains its stronger all-features-before-outcomes phase boundary as an experiment-specific control. |
| Generic-rank and jump-size family triage | Parallel rank-32 construction lanes | Treat generic Mordell--Weil rank and the largest exactly calibrated specialization jump as separate axes.  The R17/MW17 atlas is one peer lane; reconstruct curve 398's announced A1/MW16 source and curve 302's unknown parent in parallel.  An atlas miss excludes only the named chart classes. |
| Generic-normalized Kummer class pressure | Published-R17 and alternate-Q80 rank-jump controls | Compare the ideal-square-root image forced by known point classes only after quotienting the specialized generic MW17 contribution.  On the six certified controls the exceptional block adds no bad-valuation rank, so the lower bound reduces to `jump-(r1+r2-1)` and the strata `3,5,6,10..11` follow largely formally.  This localizes much of the extra Kummer information in global 2-class directions and explains descent pressure, but not why rational points appear: prospectively a residual Selmer quotient still mixes new Mordell--Weil directions with `Sha[2]`, so explicit coverings and rational-solubility witnesses are required. |
