...
def validate_lessons(items: list[dict], entries: list[dict], *, check_reviews: bool = False) -> None:
    ...
    for item in items:
        assert set(item) == required, f"{item.get('id')}: invalid lesson schema"
...
def render_algorithms(items: list[dict], entries: list[dict]) -> str:
    ...
    for key, label in AREAS.items():
        group = [item for item in items if item["area"] == key]
        if not group:
            continue
...
def render_outputs(entries: list[dict], items: list[dict], *, work_data: tuple | None = None,
    ...
    intro = ["# Research catalogue", "",
             "<!-- Generated by scripts/research.py render; do not edit. -->", "",
             f"All **{len(entries)} registered claims** and **{len(by_source)} canonical sources** "
             "are indexed here. State, proof assurance, scope and dependency edges come from "
    ...
    for key, label in AREAS.items():
        ...
        for e in group:
            ...
            lines.append(f"| `{e['id']}` | {e['state']}{replacements} | [{cell(e['title'])}](../{e['canonical_source']}) |")
    ...
    intro.extend(["", "## Find previous work before computing", "", "```sh",
                  ...
                  "Search reads current claims, method lessons, source text and archived Markdown. "
                  "`--history` also reads byte-preserved cleanup snapshots. Results label their "
    ...
    known = {e["id"]: e for e in entries}
    for e in entries:
        if not e.get("forbidden_attack_classes"):
            continue
...
def search(query: str, entries: list[dict], items: list[dict], *, selected_area: str | None = None,
    ...
    for e in entries:
        ...
        add("claim", e["id"], e["title"], searchable, e["canonical_source"], area(e["canonical_source"]),
            state=e["state"], replaced_by=e["replaced_by"])
    for item in items:
    ...
    if work_data is not None:
        ...
        for row in research_work.records(data, legacy, entries):
...
def main() -> int:
    ...
    find.add_argument("--history", action="store_true", help="also search preserved cleanup snapshots")
    ...
    if args.command in {"render", "check"}:
        ...
        current_partial = {e['id'] for e in entries if e['state'] == 'partial'}
        ...
        rows = research_work.records(*work_data, entries, args.kind, args.area)
        if args.json:
            print(json.dumps(rows, indent=2, ensure_ascii=False))
        else:
        ...
        rows = research_resources.filtered(resource_rows, args.kind, args.unknown_only)
        if args.json:
