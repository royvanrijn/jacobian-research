#!/usr/bin/env python3
"""Verify completed first-Postnikov descent on the cut-14 half-braid."""
...
JET_ARTIFACT = (
    ...
    / "artifacts"
    / "generated-results"
    / "degree42_ritt_rotated_conormal_jet_237.json"
)
ARTIFACT = (
    ROOT
    / "artifacts"
    / "generated-results"
    / "degree42_ritt_cut14_postnikov_overlap.json"
)
...
def main() -> None:
    ...
    assert jet["thick_composite_omission"] == 14
    assert jet["thin_prime_omission"] == 3
    ...
    assert result["sector_source_mod_base_square"] == {
        "artin_rees_cutoff": 4,
        "cutoff_intersection_generators": 490,
    ...
    output = {
        ...
        "derived_boundary": (
            "This proves the completed comparison through H_1 on the "
            "cut-14 half-braid. It does not compute individual H_i for "
            "i>=2, the cut-21 half-braid, or braid restriction coherence."
        ),
        "reproducing_command": (
            ".venv/bin/python "
    ...
    print("PASS: the completed cut-14 quadratic overlap vanishes")
    print("PASS: the cut-14 first-Postnikov sequence is short exact")
