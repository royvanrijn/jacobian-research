...
def validate(data: dict, legacy: dict, entries: list[dict], root: Path,
    ...
    for action in actions:
        assert set(action) == required, f"{action.get('id')}: invalid work schema"
...
def records(data: dict, legacy: dict, entries: list[dict], kind: str = 'actions',
            selected_area: str | None = None) -> list[dict]:
    if kind == 'actions':
    ...
    return [row for row in rows if not selected_area or row['area'] == selected_area]
...
def render(data: dict, legacy: dict, entries: list[dict], root: Path) -> dict[Path, str]:
    ...
    intro = ['# Unknowns and proposed work', '', header, '',
             ...
             f"**{counts['parked']} parked problems** in the authority. The {len(actions)} work proposals "
             f"include a first step for every active open problem and destinations for all {len(legacy['items'])} "
             'unchecked items recovered from the historical retrospective.', '',
    ...
    for key, label in AREAS.items():
        ...
        count = Counter(e['state'] for e in group)
        intro.append(f"| [{label}](work/{key}.md) | {count['open']} | {count['partial']} | {count['parked']} | {len(group_actions)} |")
    ...
    intro += ['', '## Retrieve the details', '', '```sh',
              ...
              'python3 research/scripts/research.py work --kind partial --area geometry',
              ...
              'the structured support-gate records are available through `--kind support`. '
