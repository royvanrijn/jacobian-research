...
**Current navigation.** For the model `D=u*partial_x+v*partial_y`,
Theorem 6.16 (`LND-CPM`) closes every nonzero principal ideal, and
Theorem 6.1 (`LND-NPF`) closes every ideal containing a power of `(u,v)`.
The bounded plinth searches in Section 6 are retained experiments, not a
current launch order. In particular, the five `LND-NPL` ideals all contain
`(u,v)^3`, and the five `LND-PDP` ideals satisfy the same saturation gate.
The remaining model problem requires genuinely nonprincipal ideals outside
these classes; the separate two-variable nonmonic slice frontier is stated
in Section 4.3. Full obligations are recorded in the
[partial-result review](../knowledge/PARTIAL_REVIEW.md).
...
## 6. Historical search sequence and the resulting all-order theorems

The initial search design used \(D=\partial_s\) and
...
negative search, not an all-order theorem. The subsequent plinth pass used
...
counterexample chart tested a genuinely nonprincipal plinth ideal,
...
After this saturation theorem, the next historical experiment used the
principal ideal \(I=(x)\), whose support is not contained in the plinth locus.
Theorem 6.4 below subsequently closes this target, and Theorem 6.16 closes
every principal ideal for this model.
Here
