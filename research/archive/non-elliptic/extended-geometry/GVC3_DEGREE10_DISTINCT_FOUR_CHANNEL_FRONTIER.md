...
**Current frontier.** Four-channel collision charts are closed over
`Q`; pairwise-distinct four-channel charts remain modular. For the complete
five-channel system, the later calculation below closes the entire nested
`D=0` fibre in characteristic zero at moment five, including its exceptional
normal branch. The unresolved work is the residual `D!=0` boundary, the main
`A*B!=0` chart and the excluded pivot/projective faces. The generic local
lengths 12 and 18 at cutoff four are retained structure, not missing
calculations to repeat before using the moment-five closure.
...
The \(a_3\) chart was selected for its smaller downstream equations.
Quotient saturation by the complete localization product timed out after
60 seconds at each of 101, 103, and 107. The successive saturation below
already resolves that cutoff-three failure and records the growth at each
factor. Reuse it before considering a larger product saturation. The nested
denominator-zero boundary remains a separate projective stratum.
...
Read the retained records before running any command below. These commands
rebuild symbolic systems, may take substantial time and overwrite their
outputs. Even `--compile-only` expands the moment polynomials. Environment
setup is in [`REPRODUCE.md`](../REPRODUCE.md).

The following checkpoints contain the later five-channel results. Filenames
are historical; the cutoff in the JSON is authoritative.

| checkpoint under `artifacts/generated-results/` | reusable conclusion |
|---|---|
| [`component_cycle_modular3`](../artifacts/generated-results/gvc3_degree10_five_channel_a1norm_a4_boundary_a3_pivot_component_cycle_modular3.json) | Cutoff **four**: three-prime support closure, exact generic normal length 12 and the rational local basis. The ambient saturated ideal is not certified. |
| [`hbranch_p101`](../artifacts/generated-results/gvc3_degree10_five_channel_a1norm_a4_boundary_a3_pivot_hbranch_p101.json) | Exact exceptional normal length 18 and elimination of a deeper valid-chart stratum. |
| [`exact_coordinate_collision5_p101`](../artifacts/generated-results/gvc3_degree10_five_channel_a1norm_a4_boundary_a3_pivot_exact_coordinate_collision5_p101.json) | Exact rational cleared restrictions at moment five. Together with the finite inverse argument above, they close `D=0`; a polynomial gcd alone is insufficient. |
| [`omitD_quotient6_p101`](../artifacts/generated-results/gvc3_degree10_five_channel_a1norm_a4_boundary_a3_pivot_omitD_quotient6_p101.json) | A 900-second timeout on the residual chart, with no emptiness or survivor certificate. |

The many-channel runner now rejects Singular diagnostics, incomplete output
and repeated result markers before its existing parsers accept a record.
The generated mathematical programs and their historical program hashes are
unchanged; the execution guard is new and was not used for the retained runs.
