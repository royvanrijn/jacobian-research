...
**Scope correction.** The coefficient locks in Sections 2–4 apply to the full
spaces stated there. Section 5 checks only the normalized zero-linear-\(x\)
slice: target normalization does not eliminate the three residual coefficients
of \(x=bc\). The later [full quadratic theorem](DANIELEWSKI_FULL_QUADRATIC_OBSTRUCTION.md)
(`CL24`) retains those coefficients and closes the entire ambient-quadratic
space. The [all-degree audit](DANIELEWSKI_ALL_DEGREE_OBSTRUCTION_AUDIT.md)
(`CL25`) records the remaining cubic and higher-degree obligation.
...
## 5. The zero-linear-x slice of the minimal escape chart fails
...
are \(a,c,w=b+\text{higher terms}\). A constant target-linear change normalizes
those source-linear coefficients but does not remove the coefficients of
\(x=bc\). On the zero-linear-\(x\) slice, the triple has the form
...
a contradiction. Therefore the normalized zero-linear-\(x\) slice of
\(\mathcal V_{\rm esc}\) contains no constant-Jacobian triple. This calculation
alone does not exclude the three omitted \(x\)-coefficients.

Combining (7) and (12), any candidate in the ambient-quadratic space on that
zero-linear-\(x\) slice must use at least one escape direction \(cx\) or \(xw\) and at least one
...
checks all 68 charts containing at most four extra directions on the same
zero-linear-\(x\) slice. It raises this slice's support lower bound to five: one escape plus four diagonal
...
insufficient. Diagonal quadratic terms alone are also insufficient. The minimal
escape calculation excludes the zero-linear-\(x\) slice. These tests motivated
the quadratic interaction calculation; `CL24` now closes the full quadratic
space, including all residual \(x\)-coefficients.
...
lock, verifies the zero-linear-\(x\) minimal-escape equations and their unit
