...
the finite-generation ideal of this kernel is exactly its conductor
\(s^2t^2k[s,t,P,Q]\) to the invariant algebra on the normalized ambient
ring.  Its infinite monomial SAGBI basis makes the conductor-return ladder
intrinsic, rather than an artifact of a selected generating family.  The
calculation extends to every tensor product of independent cusp axes:
for
\[
 K^{(r)}=\bigotimes_i(k+t_i^2k[t_i,P_i]),
\]
the finite-generation ideal is exactly
\((\prod_i t_i^2)k[t_1,P_1,\ldots,t_r,P_r]\), with \(2^r\) infinite return
ladders.  The same control proves that
the apparent leading pair in every tangent-normalized factorization slice is
disjoint, since \(1=ad+bp\in(a,p)\).

For the exact Keller factorization slices computed so far, the
finite-generation-ideal inventory is therefore complete:

| slice and action | exact invariant algebra | finite-generation ideal |
|---|---|---|
| \((2,3)\), \(D_{10},D_7\) | \(k[K,H,V]\) | the unit ideal |
| \((2,3)\), \(D_7\) | \(k[K,H,V,s]\) | the unit ideal |
| \((2,3)\), \(D_{22}\) | \(k[K,H,V,W]\) | the unit ideal |
| \((2,3)\), \(D_{10}\) | finitely generated by the global-slice theorem | the unit ideal |
| \((2,4)\), \(D_{10},D_0\) | \(k[a,p,U,N,M,C,Q,S]\) | the unit ideal |
| \((2,4)\), \(D_{10},D_0,D_2\) | \(k[a,U,N,M,C]\) | the unit ideal |

Here “unit ideal” is not a claim that the displayed generators are minimal:
it follows from the exact finite-generation theorems.  Thus the
finite-generation ideal adds genuine geometry only before termination.  A
useful future computation must retain the conductor ideals between
successive saturation algebras, not just return a Boolean generation result.
...
2. retain the third divisor forced by its one-variable Jacobian and run a
   three-index conductor ledger, calibrated first against the exact
   \(r=3\) independent-cusp ideal; or
...
At the same time compute the conductor to the normalized generic quotient.
Localizing a conductor element gives a certified lower bound for the
finite-generation ideal; specializations to each one-boundary quotient can
certify the reverse containment, as in the two-cusp calculation.  A bounded
Gröbner or SAGBI search without one of those arbitrary-degree certificates
remains an experiment, not a Hilbert--14 result.

The fork for the three-index ledger is now sharp.  If the three one-axis
specializations tensor-separate, the multiboundary theorem computes the
finite-generation ideal and proves non-finite generation immediately.  If
they do not, the mixed multi-Rees relations must be retained and become the
candidate Keller-specific mechanism.  The symmetric coupling \(xyz=p\)
already fails the affine-source and dualizing tests, so the remaining useful
branch is asymmetric or distributed between source and target.
