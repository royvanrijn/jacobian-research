...
If equality holds, the same component bound gives local dimension at least
eight, while tangent dimension bounds it above by eight. Equality of local
dimension and tangent dimension makes the local ring regular, hence smooth
over `C`, with a unique component through `F`. This follows from the
[regular-local/smoothness criterion](https://stacks.math.columbia.edu/tag/00TS).
The computed tangent dimensions below are much larger than eight, so this
criterion does not settle any of those examples.
...
The full tangent profiles, including the cutoffs in (22), remain computations
over the single prime 32003. Their characteristic-zero ranks are open.
The individual seed degrees `25`, `30,35` and `35,40,45` already have exact
rational certificates in Section 6.1 (`CSK1`); they do not need a new modular
lift. Every polynomial tangent has a finite-degree source trivializer, so a
stable parameter cannot be defined by survival at every first-order cutoff.

The profile script groups coefficient rows of `adj(DF_N)G` by source degree
and inserts them into one descending-degree echelon computation. This reuses
the tangent basis for every cutoff. The retained JSON stores the ranks and
breakpoints, not that basis; rebuilding it is a research calculation. Even
exact rational full profiles would remain source-only and first-order. The
two-sided filtered algebraization and component questions in `OP-CCDM`
would still be open.
