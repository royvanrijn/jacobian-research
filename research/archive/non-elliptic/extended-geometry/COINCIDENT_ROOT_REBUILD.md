# Contact-Strata and Exceptional-Component Theorem from coincident-root geometry

This note proves the contact strata, their exact closure order, the
exceptional components, and their set-theoretic intersections.  Smooth
component normalizations and their branch fibers belong to the separate
[Exceptional-Component Normalization Theorem](COMPONENT_NORMALIZATION.md).
Fix an algebraically closed field `k` of characteristic zero and an integer
`n>=3`.

## 1. Coincident-root quotient models and the seed slice
...
its image.  This is the finiteness input used in the closure arguments below;
the separate component-normalization theorem reuses the same input.
...
## 7. Intersections are common coarsenings
...
`C_mu`, giving the component-intersection formula.  Equation (19)
...
## 8. Consequences and proof boundary
...
handle the seven endpoint-rank irreducibility identities.
...
The finite quotient model, slice nonemptiness, and exact admissible closure
statements used here are proved directly above.  Smooth normalizations and
branch fibers are proved only in the separate component-normalization note.
