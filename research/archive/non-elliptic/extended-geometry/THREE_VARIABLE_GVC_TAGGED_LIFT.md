...
**Historical construction route.** The later characteristic-zero theorem
[GVC3HC](THREE_VARIABLE_HOMOGENEOUS_GVC_COUNTEREXAMPLE.md) supplies a
three-variable homogeneous GVC counterexample. Together with GVC2ENV it
settles the minimum dimension at three; `OP-GVC3-LIFT` is closed. The
calculations below remain useful exclusions of particular lift architectures.
The distinct minimum-degree problem is `OP-GVC3-MIN` in
...
No GVC(3) counterexample is obtained by this tagged lift. The calculations produce four
...
The construction goal is already closed by GVC3HC. Formula (4.4) leaves the
following questions only if a classification of these lift architectures is
explicitly in scope:
...
The odd-only chart is closed. Its two prime components in (7.15) provide
regression faces for item 2; their equations cannot be imposed as constraints
on the full space after restoring the even endpoints. That space needs its
own simultaneous moment ideal. None of these questions is required to
reconstruct the existing GVC3HC counterexample.

## 9. Reproduction

The retained JSON files below are the first replay checkpoints. These
commands rebuild the calculations and overwrite those files; they are not
required for navigation or cleanup. Follow the environment setup in
[`REPRODUCE.md`](../REPRODUCE.md) before an explicitly scoped replay.
...
Gröbner calculations described above. The tagged-lift Singular wrapper now
requires clean interpreter output and a completion marker before accepting
its dimension, basis size and normal form of `1`. The preserved earlier
script and output remain historical evidence, not a fresh execution of that
guard.
