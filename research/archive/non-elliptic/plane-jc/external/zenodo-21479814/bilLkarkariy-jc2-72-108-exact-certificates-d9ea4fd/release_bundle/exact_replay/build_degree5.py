...
S=pickle.load(open('hne0_deg35.pkl','rb'));W=(5,6,5);Q=(6,5,1,0,6,5);U=ec.K(fmpq_poly([0,1]));F=fmpq_poly([26,0,3,3,-1,1]);PHI=fmpq_poly([fmpq(-9725570295901,12623962),fmpq(-1170753213563,971074),fmpq(-387111042229,12623962),fmpq(1578225240619,12623962),fmpq(-469713794365,6311981)])
def ev(p,x,mod):
 o=fmpq_poly([0])
...
for i,e in enumerate(S):
 ...
 for m,c in e.items():
  ...
  if d:ee[m]=d
 out.append(ee)
pickle.dump(out,open('hne0_polred.pkl','wb'),protocol=4);print([len(e) for e in out])
