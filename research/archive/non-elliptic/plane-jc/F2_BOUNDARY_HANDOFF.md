...
> `(5,1)|(3,3)|(3,1,1,1)`, geometric monodromy `A_6`, and a product-one
> meridian relation for that residue cover.  The `A_6` action is four-transitive and primitive, its
...
and the product-one meridian relation on the terminal residue cover
...
It supplies no meridian completion for the unknown global Keller cover.
...
2. factor the known fixed-coordinate `k=1` quintic pullback at unresolved
   boundary points, locate its new divisors, and retain the severe collision
   strata and `k=2..24` unless a theorem forces `k=1`;
...
For the current coefficient frontier, use
[PF2NF1](F2_75_125_NONLINEAR_FORCING.md): carrier specialization, the full
nonlinear circuit and finite formal continuation are already available.
For the precise surviving global obligations, use
[OP-PLANE](NEXT_DEGREE_FRONTIER.md).  This handoff retains the contact and
terminal-row derivation because its replay and downstream proofs use it.
