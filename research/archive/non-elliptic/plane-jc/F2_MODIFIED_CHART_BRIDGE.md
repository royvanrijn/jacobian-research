...
have now been carried.  The coupled rows and thirteen residual functionals
are subsequently specialized in
[PF2CS1](F2_75_125_CARRIER_SPECIALIZATIONS.md) and compiled into the full
nonlinear circuit in [PF2NF1](F2_75_125_NONLINEAR_FORCING.md).  That note also
records finite formal continuation through order 16; its regular truncated
jet fails 50 equations at the target.  An exact component solution or
exclusion and the full Laurent-to-polynomial bridge remain open.  The lower
Laurent tail remains retained.
...
The optional Singular call uses the shared execution guard: interpreter
diagnostics, incomplete execution or a missing exact `UNIT` line fail the
check even when Singular exits zero.  The retained `r=5` result was not
recomputed during repository cleanup.
