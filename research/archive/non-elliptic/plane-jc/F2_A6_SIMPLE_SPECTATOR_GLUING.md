...
also two simple `R` Kummer orbits.  At the time of this experiment their
carrier inertia was unknown.  PF2CW1 has since proved that those points are
unramified on a different, cyclic cubic carrier cover.
...
choosing an orientation of the Kummer generator.  These are exact conditional
permutation packets.  The subsequent unramified-carrier classification
prevents either model from serving as the actual simple-`R` inertia model.
...
identification is an assumption of the historical experiment below, and
PF2CW1 subsequently disproves it for the actual carrier inertia.
...
Thus the stronger conditional model has one degree-eleven unoriented
gluing.  Its proposed identification with simple squarefree-`R` inertia is
already ruled out by PF2CW1.  A different geometric application would need
its own ramification and attachment theorem before using this enumeration.
...
## 7. Historical bridge assumptions

The original experiment needed the following geometric declarations.  Items
1–3 are now resolved against the proposed transposition models by PF2CW1;
this list is retained to explain the failed application, not as a request
to prove those models:
...
covers into the single `P^1` cover assumed here.  Current F2 attachment
obligations belong to the
[global compiler](F2_75_125_GLOBAL_ATTACHMENT_COMPILER.md) and
[OP-PLANE](NEXT_DEGREE_FRONTIER.md).
