...
Thus the carrier points are known.  The later carrier Wronskian theorem
also determines their local inertia: the simple roots are unramified on the
cyclic cubic carrier.  The missing data are the complete global attachments,
as distinguished in Section 8.
...
the unresolved global rows.  The coefficient specialization is also already
completed in [PF2CS1](F2_75_125_CARRIER_SPECIALIZATIONS.md).  On the double
carrier, [PF2NF1](F2_75_125_NONLINEAR_FORCING.md) supplies the full nonlinear
circuit and finite formal jets through order 16 over `F_31` and order 8 over
`F_61`.  The regular order-16 jet still fails 50 equations at `lambda=1`.
The squarefree carrier's 82 later spacings are routed, not eliminated.
Thus the remaining inputs are:

1. obtain an exact solution or exclusion of the specialized double-carrier
   component, or close the remaining squarefree target-and-tail branches;
2. retain the exact cyclic root contribution `27`, then compile noncyclic
   attachments and all possible cancellation centers;
3. factor the known fixed-coordinate `k=1` quintic pullback at the unresolved
   source boundary; retain the severe collision strata and `k=2..24` if
   `k=1` is not forced, as specified by [OP-PLANE](NEXT_DEGREE_FRONTIER.md);
