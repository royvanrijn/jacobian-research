...
The full component circuit and subsequent formal continuation are already
available.  Sections 5–6 record the corrected sparse tangent solve, 153
left-cokernel coordinates, order-16 jets over `F_31`, and an order-8 check
over `F_61`.  The regular order-16 jet still leaves 50 nonzero equations at
`lambda=1`; continuing or reconstructing a branch that satisfies the entire
localized circuit is the next gate.  These are finite formal jets, with no
certified modular point or characteristic-zero realization.
...
For a cheap integrity and metadata check of the retained results:

```bash
python3 plane-jc/cas/audit_f2_retained_checkpoints.py
python3 plane-jc/cas/test_sparse_circuit_modp.py
```

This checks retained hashes and formal-jet metadata, including their failed
target evaluations; it does not replay the mathematical circuits.  The full
research replay commands are:
...
Intentional artifact regeneration uses `--refresh` on the artifact-producing
commands.  The forcing regression also rebuilds the full circuit; only
`test_sparse_circuit_modp.py` above is a tiny backend control.  The full
directional and order-16 formal audits take several minutes because they
replay the complete arithmetic circuit many times.

The compact forcing JSON contains circuit summaries and digests, not a
serialized 710,439-node DAG.  Replay rebuilds it from the pinned compiler,
carrier maps, endpoint reducer and layer classifier; the later checks also
use `sparse_circuit_modp.py`.  Those sources and all six checkpoint outputs
are indexed with PF2NF1.  Preserve the recorded outputs when inspecting
them: their stage-local status text is not the current frontier.  In
particular, the `mod61` record's `field` is `F_61`; its copied `GF(31)`
boundary sentence is a historical metadata error, not its actual field.
The replay comparison recognizes only that copied sentence and the older
`seven-pole-coordinate-zero` label for the exact seven-coordinate gauge.
It still compares the field, every prescribed coordinate, all stored
coefficients, residuals and digests.  This repairs the label-only replay
mismatch without rewriting a checkpoint or running the formal lift.
