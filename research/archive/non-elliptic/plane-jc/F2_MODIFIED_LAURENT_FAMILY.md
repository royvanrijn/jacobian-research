...
The terminal residue theorem in Section 7 is unconditional within the F2
corner-chain setup.  The finite common-power systems in Sections 3–6 are
conditional: their solutions do not supply a polynomial Keller pair.

| Established result | Canonical continuation |
| --- | --- |
| Exact source bands, corrected tangent kernels, descent-eight defect and edge returns through `v^10` | [Corner and layer derivation](F2_75_125_DERIVATION.md) |
| Forced `d=3` chart, exact source-image relations and exclusion of literal negative-tail deletion | [Modified-chart bridge](F2_MODIFIED_CHART_BRIDGE.md) |
| Actual squarefree and double carrier maps; simple squarefree points are unramified | [Carrier Wronskian](F2_75_125_CARRIER_WRONSKIAN_CLASSIFIER.md) |
| Full `294+53` coupled obstruction and `7+6` final coordinates over the carrier number field | [Carrier specialization](F2_75_125_CARRIER_SPECIALIZATIONS.md) |
| `366` circuit equations and subsequent finite formal continuation | [Nonlinear forcing](F2_75_125_NONLINEAR_FORCING.md) |

The double-carrier continuation already reaches order 16 over `F_31` and
order 8 over `F_61`.  Its regular order-16 jet still fails 50 equations at
`lambda=1`.  An exact component solution or exclusion, full Laurent-tail
realization and global attachment remain open.  The squarefree carrier's
82 later first-defect spacings are routed but not eliminated.  Rebuilding
the earlier endpoint/cokernel presentation is not a new research step.
...
and leave `219` Fitting slots at this stage.  PF2CS1 subsequently specializes
the carrier, and PF2NF1 compiles the coupled descents and thirteen residual
functionals, retaining all `294+53` divisibility and quotient conditions.
The remaining component decision and finite formal continuation are recorded
in [that source](F2_75_125_NONLINEAR_FORCING.md).
...
row nor layer zero can be tested in isolation.  PF2NF1 already performs this
carried presentation; its component ideal remains undecided.
...
five visible rows in those coordinates.  Section 2.3 derives the monomial
change to `d=3,h=2`, including its `y^2` coefficient weight.  What is still
unproved is the full common-power polynomial realization after that change;
literal negative-tail deletion is already excluded.
...
`d=2`, this formal geometry still needs the exact source-image and full
Laurent-tail conditions to become either an F2 contradiction or a plane
candidate.  The original source-band ledger is already complete; a support
box alone does not impose those linear relations.
...
## 8. Historical `A_6` permutation-gluing audit

The companion
[`F2_A6_SIMPLE_SPECTATOR_GLUING.md`](F2_A6_SIMPLE_SPECTATOR_GLUING.md)
retains a conditional spectator model.  Its proposed identification with
actual simple-`R` inertia is superseded by PF2CW1.  The historical assumption
is explicit:
...
row once additional simple branch cycles are allowed.  PF2CW1 proves that
the actual simple squarefree carrier points are unramified, so neither
transposition model describes them.  The enumeration remains useful as a
conditional regression, with current global gates in
[F2_75_125_GLOBAL_ATTACHMENT_COMPILER.md](F2_75_125_GLOBAL_ATTACHMENT_COMPILER.md).

## 9. Exact conclusion

No F2 member is excluded by the conditional formal residue systems.  Their
smooth loci and endpoint-binomial sections prove that the compact power
rows alone cannot establish exclusion.  The uniform terminal theorem is
separate and remains valid within the corner-chain setup.

The literal polynomial projection is already excluded at `r=3` by the
length-27 top-gap algebra and its unit Q gap.  The `r=4,5` P-only samples
are also closed; no induction to every `r` follows from those samples.
A new modified-chart theorem must account for the omitted positive-negative
Laurent cross terms and the complete source-image relations.

For `(75,125)`, the coefficient frontier is the specialized nonlinear
component in [PF2NF1](F2_75_125_NONLINEAR_FORCING.md), including its retained
formal jets.  An exclusion of the earliest double-root component must still
handle the later first-defect spacings.  A surviving coefficient branch must
still supply full Laurent realization and the global data in
[OP-PLANE](NEXT_DEGREE_FRONTIER.md).  Further searches in the old 14- or
22-function support boxes do not supply these missing implications.
