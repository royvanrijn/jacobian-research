...
from research_runtime.search_state import raw_state, input_generators, reduction_cache
from research_runtime.pointed_worker import search_box, compiled_worker
...
def sources():
    ...
    runtime=path.parent/'research_runtime'
    dependencies=[path,*(runtime/name for name in ('pointed_worker.py','supervisor.py','arithmetic.py',
        'mw_state.py','search_state.py','finite_reduction.py','binary.py','store.py'))]
    return {**base.provenance(),**{str(p.relative_to(ROOT)):sha256(p.read_bytes()).hexdigest() for p in dependencies}}
...
class PointedQuarticSearch:
    ...
    """
    def __init__(self, curve=None, subgroup=None, centre=None, coordinate_policy=None, *, state=None):
        if state is not None:
            if not isinstance(state,MWState) or curve is not None or subgroup is not None:
                raise ValueError("supply one MWState, or the legacy curve/subgroup adapter")
            curve,subgroup=state.model.coefficients,state.basis
        ...
        self.state = state or raw_state(self.curve,self.subgroup)
    ...
    def verify_record(self, record):
        ...
        current=sources()
        if not set(record["source_hashes"]) <= set(current):
            raise ArithmeticError("checkpoint contains an unknown source dependency")
        for name,expected in record["source_hashes"].items():
            if current[name]!=expected:retained_source(ROOT,name,expected)
        if record.get("mw_state_key",self.state.key)!=self.state.key:
            raise ArithmeticError("checkpoint belongs to a different MWState")
        if any(record[k] for k in
    ...
    def search(self, height, seconds, *, denominator_start=1, denominator_end=None, checkpoint_dir=None):
        ...
        worker, hits = search_box(self.coefficients, height, seconds, first, last)
        lead = self.coefficients[4]
        ...
        for n, den, root in hits:
            for signed in {root, -root}:
                ...
                if p is not None:
                    found.add(p)
        record = {"backend": BACKEND_NAME, "source_hashes": sources(), "mw_state_key":self.state.key, **self.chart_record(), **bounds,
        ...
        if path is not None:
            ...
            atomic_checkpoint(path,document)
        return outcome


    def search_state(self, height, seconds, *, extra_primes=(), **kwargs):
        """Return a new MWState, admitting only independently certified gains."""
        outcome=self.search(height,seconds,**kwargs)
        updated=self.state
        cache=reduction_cache()
        for point in outcome.curve_points:
            updated=updated.adjoin(point,cache=cache,extra_primes=extra_primes)
