#!/usr/bin/env python3
"""Bounded all-class alternate-cover search for Nagao ``T=531/2``.

The exact finite-reduction-certified rank-17 basis is loaded from the pinned
standalone certificate.  All ``2^17-1`` nonzero known mod-2 classes are scored
by their degree-two coordinates; the best twenty receive three optimized
cross-ratio charts and the fixed 50k/250k/1m staged search.

Every returned point is mapped and checked exactly.  Height-pairing relation
proposals count only after exact Fraction group-law replay, and any unresolved
points enter a fresh finite-reduction certificate.  The bounded boxes are not
a complete 2-descent or a rank upper bound.
"""
...
from verify_nagao_rank21_t531 import PARAMETER_T
...
CERTIFICATE_SHA256 = (
    "a3b242525c153b6fb70bcbd51b27243b0e9b6176be197689dd24f231992f48cc"
)
REPRODUCING_COMMAND = (
    "PYTHONPATH=elliptic-curves/cas .venv/bin/python "
    "elliptic-curves/cas/search_nagao_rank21_t531_alternate_covers.py"
)
...
def load_exact_basis(
    ...
    if len(basis) != 17 or any(
        not point_on_short_curve(coefficients, point) for point in basis
    ):
        raise AssertionError("the exact rank-17 basis is incomplete")
    if data["exact_rank_certificate"]["certified_algebraic_rank_lower_bound"] != 17:
        raise AssertionError("the input lacks its exact rank-17 claim")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--certificate-input",
        type=Path,
        default=generated / "elliptic_nagao_rank21_t531_rank17_certificate.json",
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=generated / "elliptic_nagao_rank21_t531_alternate_covers.json",
    )
    return parser
