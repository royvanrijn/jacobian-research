#!/usr/bin/env python3
"""Bounded alternate-cover search from the exact ``T=5777/32`` rank-17 lead.

Every one of the ``2^17-1`` nonzero classes represented by the certified
basis is enumerated in Gray-code order and scored by its known-point degree-
two coordinates.  The best twenty classes receive three optimized
cross-ratio charts, followed by fixed pilot and escalation heights.  Returned
points are mapped to the short curve and checked exactly.  Height-pairing
relation proposals are accepted only after exact Fraction group-law replay;
all unresolved points enter a fresh finite-reduction independence test.
"""
...
from verify_nagao_rank21_t5777 import PARAMETER_T
...
CERTIFICATE_SHA256 = "065a569fe57a2f232cab376954c2f0854d02cb9a80c2ddc19f7596ff011213bc"
REPRODUCING_COMMAND = (
    "PYTHONPATH=elliptic-curves/cas .venv/bin/python "
    "elliptic-curves/cas/search_nagao_rank21_t5777_alternate_covers.py"
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--certificate-input",
        type=Path,
        default=generated / "elliptic_nagao_rank21_t5777_rank17_certificate.json",
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=generated / "elliptic_nagao_rank21_t5777_alternate_covers.json",
    )
    return parser
