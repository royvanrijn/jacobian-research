#!/usr/bin/env python3
"""Exact rank certificate for the rank-21-family lead ``T=1637/12``.

The verifier independently replays the uniform height-1,000,000 Mestre
quartic search, maps every returned point exactly to the short Jacobian, and
reconstructs the pinned 126-point pool.  A height matrix at two precisions is
used only to select sixteen candidates.  Small-prime saturation is followed
by finite-reduction independence, which supplies the unconditional rank
lower bound.  Conductor and root number are recomputed directly by PARI.
"""
...
Q = Fraction
PARAMETER_T = Q(1637, 12)
UNIFORM_HEIGHT = 1_000_000
EXPECTED_SIGNED_POINTS = 252
EXPECTED_NEW_IMAGES = 114
EXPECTED_POOL_SIZE = 126
EXPECTED_HEIGHT_SUBSET = tuple(range(1, 12)) + (13, 14, 15, 16, 17)
TARGET_RANK = 21
REPRODUCING_COMMAND = (
    "PYTHONPATH=elliptic-curves/cas .venv/bin/python "
    "elliptic-curves/cas/verify_nagao_rank21_t1637.py"
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=(
            root
            / "artifacts/generated-results/elliptic_nagao_rank21_t1637_rank16_certificate.json"
        ),
    )
    return parser
...
def main() -> None:
    ...
    if stable_height_rank(height_runs) != 16:
    ...
    if len(saturated_basis) != 16 or any(
        not point_on_short_curve(short_coefficients, point)
    ...
    if exact_rank != 16:
        raise AssertionError("finite reductions did not certify rank 16")
    ...
    artifact: dict[str, Any] = {
        "schema_version": 1,
        "status": "exact_rank16_certificate_complete",
        "theorem": (
            "the Nagao rank-21-family specialization T=1637/12 has "
            "Mordell-Weil rank at least 16 and log conductor below 182.72"
        ),
        "candidate": {
        ...
        "interpretation": {
            "root_number": (
                "root number +1 suggests even analytic rank only under the parity "
                "heuristic; it is not used in the algebraic rank certificate"
            ),
            "next_parity_compatible_target_rank": 22,
        },
        "software": {
