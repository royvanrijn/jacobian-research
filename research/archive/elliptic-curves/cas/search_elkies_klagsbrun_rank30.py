#!/usr/bin/env python3
"""Bounded direct search for a 30th point on the 2024 rank-29 curve.

The rank-17 K3 fibration used for discovery has not been published.  This
script therefore searches several exact birational charts of the record fiber
itself.  A line of slope ``m`` through a known point cuts the curve in two
...
so resolved is tested for an exact rank-30 finite-reduction certificate.
"""
...
REPRODUCING_COMMAND = (
    ...
    "elliptic-curves/cas/search_elkies_klagsbrun_rank30.py"
)
...
def main() -> None:
    ...
    for index, chart in enumerate(charts, 1):
        ...
        if index % 100 == 0 or unexpected:
            print(
                f"charts {index}/{len(charts)}; timeouts={len(timeouts)}; "
    ...
    artifact = {
        "schema_version": 1,
        "artifact_kind": "bounded_elliptic_curve_rank30_point_search",
        "status": (
        ...
        "claim_scope": {
            ...
            "published_fibration_blocker": (
                "The rank-17 K3 fibration and its sections used to discover E29 "
                "have not been published, so this search operates on the fiber."
            ),
        },
        "reproduction": {
