# Failed All-k Transfer Block conductor-ribbon audit: counterexample to the canonical norm theorem
...
[ALL_K_DEFORMATION_AUDIT.md](ALL_K_DEFORMATION_AUDIT.md) and the Boolean-norm
...
The script `scripts/verify_all_k_ribbon_functor.py` performs the following
calculation over `QQ`.
...
generator numbers and Nakayama force `K=0`.  In Failed All-k Transfer Block, the missing hypotheses
...
Until that is done, Master Quotient Theorem and the allocation-intersection statements that use
