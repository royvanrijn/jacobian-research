...
`artifacts/generated-results/` retains compact, pinned EC/K3 reference
artifacts. Restartable checkpoints, solver transcripts and large intermediates
belong in the ignored `artifacts/local/` cache. Other programmes and their
historical generation commands are preserved in the
[programme archive](../../archive/non-elliptic/README.md) and
[archived Makefile](../../archive/non-elliptic/Makefile).
...
- The paused R17 rational-quadratic MW20 programme preserves three bounded
  Nagao ledgers, an exact splitting preflight for their top anchored
  diagnostic, and two compact exact negative subchart certificates.  None is
  a twist-rank upper or lower bound.  In order, the artifacts and SHA-256
  values are:

  - `elkies-k3-r17-rational-quadratic-twist-nagao-h100-v1.json`:
    `8db623ca4f66102a4c470f89c6240520809fb8d2f1e9c737e15103fdc32a22a2`;
  - `elkies-k3-r17-rational-quadratic-twist-pgl8-nagao-h100-v1.json`:
  ...
  - `elkies-k3-r17-mw20-control-transport-anchored-heuristic-top-v1.json`:
  ...
  - `elkies-k3-r17-074d9-tracezero-genus5-polynomial-x-p19-v1.json`:
    `c22068ae09b96b58b5298f943e60f6c00d5a79d04b48d64eb079a3696c10bda2`.

  Replay commands and the exact coordinate/proof boundaries are in
  [`../../elkies-k3/R17_RATIONAL_QUADRATIC_MW20_HANDOFF_2026-09-04.md`](../../elkies-k3/R17_RATIONAL_QUADRATIC_MW20_HANDOFF_2026-09-04.md).

- `elliptic-curves/quotient_geometry_table_v1.json` joins the five usable R17
  controls, sixteen refreshed R17 ladder fibres, and nine A1/MW16 presentations
  to full displayed-quotient height geometry and 230 blind recovery witnesses.
  All 230 pointwise half-lattice projection CVPs agree at two rounding scales,
  and all thirteen nonempty strictly partial initial recoveries reject a scalar
  successive-minimum cutoff.  Replay with
  `sage -python elliptic-curves/cas/build_quotient_geometry_table.sage --check`.
  Its whole-file SHA-256 is
  `9842b049f64035ee7765f954b6db4bc7496478679aaec0f898f87154f039e2db`.
...
- `elliptic-curves/r17_refresh_jump_ladder_protocol_v1.json` and
  ...
  detector may schedule candidates but cannot supply a residual-Selmer upper
  bound.  The same blind run proves curve 478 has rank at least 23.
...
- `elkies-k3-mw17-jump-v2-campaign-v1.json` freezes 2,239 addresses from the
  ...
  Detailed fibre certificates are checkpointed under `artifacts/local/`.
  `elkies-k3-mw17-jump-v2-ledger-v1.json` is the stopped partial leaderboard:
  66 completed leading-`07ca9` fibres, 2,838 completed charts, no timeout or
  backend failure, and measured recovered gain `+0` throughout.  The negative
  signal is bounded by this detector and does not test the remaining 2,173
  frozen addresses.  Replay the merge with
  `elliptic-curves/cas/run_mw17_jump_v2.sage --merge --chunk-count 4`.

- `elkies-k3-mw17-jump-v2-zero-gain-rescue-arm-v1.json` preserves those same
  2,239 addresses and assigns one SHA-256 bucket of eight without reading
  detector outcomes.  Its 264 assigned rows become eligible only after a clean
  exact 43-chart zero.  Each eligible fibre receives the disjoint generic
  class ranks 44--344 in seven batches; after the first certified escape,
...
- `elkies-k3-r17-extreme-anchored-mw18-covers-v1.json` is the exact complete
  split census on the `07ca9` and `08234` smooth rootless rational-bisection
  frames at eight refreshed extreme fibres.  It contains eight refreshed
  nonzero `+11` anchors and the historical rank-28 anchor; its SHA-256 is
  `7b94bf9b3dec377ce118c64e4455c6e2f089e88108cdabc9b3c5ca16c3a94b27`.
  `elkies-k3-r17-extreme-anchored-mw18-nagao-h1000-summary-v1.json` compiles
  the nine uniform bounded Nagao runs, totaling 10,950,912 scored parameters
  and 178 heuristic survivors; its SHA-256 is
  `314fa5f91fd3ee86b5cbc0a25d60e81d3e1ddd3bf0eb3be0f29622b217bb9d45`.
  Replay with the exact compiler and
  `python3 elkies-k3/scripts/summarize_r17_extreme_anchored_mw18_nagao.py --check`.
  The search summary is not a rank certificate.
<!-- status-consumer: EC-K3-R17-EXTREME-ANCHORED-MW18-COVERS 4763babcbf5d923c -->

- `elliptic-curves/a1_mw16_target_free_parameter_candidates_h300_v1.json`
  records 104 exact pairwise nonisomorphic specializations selected from the
  anonymous five-fibration/nine-presentation A1/MW16 template at projective
  height at most 300.  Its SHA-256 is
  `9c776ecd47e5abf06b28f9c0feed1a2293355f6a7505297292b269e44976138d`.
  `elliptic-curves/a1_mw16_target_free_parameter_search_h300_v1.json` merges
  the exact maximum-depth searches: all 856 quartic charts finish through
  height 100,000, returning no affine point, failure, or quotient direction
  beyond MW16.  This is a bounded null result, not a rank upper bound; its
  SHA-256 is
  `c4c17e1a837fe851ac0a878139621be40bc490beb1b00c169ed9755c50aa9586`.
...
- `elkies-k3-custom-ns-rootless-half-lattice-sweep-v1.json` is the complete
  ...
  attached to custom `NS0002`--`NS0048`.  It covers 17,825,792 classes;
  135 frames have `max mu_2=12`, while determinant-1494 `NS0021/F006` has
  one unique norm-14 class and hence `rho_2^2=7/2`.  The certificate retains
  exact integral norm checks, all deepest masks, representative hashes, and
  deterministic 256-bit MPFR audits.  It is lattice geometry only and proves
  no rational marking, point, Selmer bound, or rank jump.  Replay with
...
- `elkies-k3-ns0031-period-group-counterwitness-v1.json` retains the literal
  stable reflection, fixed positive plane and determinant-37 Clifford action
  refuting the former norm-one containment. `make verify-ns0031-period-group`
  checks it with the standard library; it does not decide rational K3 existence.
...
- `elkies-k3-r17-product-survivor-galois-height-gate-v1.json` factors the
  ...
  product survivors.  Every cubic is irreducible, so rational sections meet
  the identity component at all four `I0*` fibres and height eight is exactly
  the direct `P.O=0` degree-`(8,12)` box.  It also records exact degree-four
  constructor maps, exact `u(nP)` values, and bit sizes, selecting
  `19bad:083ad` first by its 124-bit `u(P)`.  The separate extra-prime audit and complete toric
  certificate for that target at `p=151` have SHA-256 values
  ...
  the latter again has Tate factor `(Z-1)(Z+1)` and proves no section.  Replay
  the gate with
  `sage -python elkies-k3/scripts/certify_r17_product_survivor_galois_height_gate.sage --check`;
  its whole-file SHA-256 is
  `d6f5c2bb74531506b2eecdcec6462c44ec79addc5c2fa8e9c12bb7a3cad7fe85`.
  The artifact also verifies one normalized Frobenius-fixed direction at
...
- `elkies-k3-r17-kummer-classgroup-pressure-comparison-v1.json` replays the
  ...
  the comparison is known-point localization data rather than an explanation
  for why rational points occur, an exact class-group dimension, or a
  prospective predictor.  Replay it with
