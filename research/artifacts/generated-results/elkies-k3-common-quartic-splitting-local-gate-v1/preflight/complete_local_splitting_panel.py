...
start=time.process_time();pkt=json.loads(SOURCE.read_text());parents={c['name']:c for c in pkt['parents']};rows=[]
initial=json.loads((OUT/'hensel-preview.json').read_text())
for row in initial['rows']:
 if row['status']=='ROOT':continue
 c=parents[row['parent']];p=row['p']
...
result={'schema':'full-splitting-panel-completion-preflight-v1','source_sha256':sha256(SOURCE.read_bytes()).hexdigest(),
        'script_sha256':sha256(Path(__file__).read_bytes()).hexdigest(),'rows':rows,'cpu_seconds':time.process_time()-start,
        'scope':'The unresolved cubic-splitting witnesses in the retained four-parent fourteen-prime discriminant panel. Same depth7/2000-node limits per cell stage. Discovery only, not a global point search.'}
with (OUT/'panel-completion-preview.json').open('x') as f:json.dump(result,f,indent=2);f.write('\n')
