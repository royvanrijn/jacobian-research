#!/usr/bin/env python3
"""One fixed301-chart adaptive attempt on the independently certified new27 curve."""
import argparse,sys
...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';LOCAL=ROOT/'artifacts/local/elliptic-curves';D=LOCAL/'paired-rank27-adaptive-v1';SEED=LOCAL/'fresh-r17-paired-pari-v1/074d9-007/result.json';PROOF=ART/'new_paired_rank27_proof_v1.json';CENSUS=LOCAL/'compact-r17-fresh-generic-census-v1/074d9/generic-census.json'

def sources():
    names=['followup_paired_rank27.py','prepare_paired_rank27_adaptive.sage','prepare_fresh_r17_pari_batch.sage','fresh_r17_pari_batch.py','prospective_half_lattice_v2.sage','research_runtime/rotated_observation_state.py','research_runtime/cached_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
...
def prepare():
    ...
    seed,proof,census=cert.read(SEED),cert.read(PROOF),cert.read(CENSUS)
    ...
    p=proof['rank_certificate_on_discovery_model'];cert.checked_rank(tuple(map(cert.F,seed['curve'])),[tuple(map(cert.F,P)) for P in points],[r['prime'] for r in p['signatures']],p['no_rational_2_torsion_prime'])
...
def launch():
    p=protocol()
    for name,cmd,seconds in [('maps',['/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python',str(CAS/'prepare_paired_rank27_adaptive.sage')],p['geometry_wall_seconds']),('worker',[sys.executable,str(Path(__file__).resolve()),'worker'],p['worker_wall_seconds'])]:
