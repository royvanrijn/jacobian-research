...
D=ROOT/'artifacts/local/elliptic-curves/outer60-mw16-point-portable-v2'
OUT=ART/'outer60_mw16_point_portable_replay_v2.json'

def main():
    path=ART/'outer60_mw16_point_evidence_v2.json';manifest=cert.read(path);workspace=D/'workspace';folder=D/'verification'
    ...
    p=cert.read(workspace/'artifacts/local/elliptic-curves/outer60-mw16-pari-v1/protocol.json');jobs=[]
    ...
    checkpoint(OUT,{'schema':'elliptic-curves.outer60-mw16-point-portable-replay.v2','status':'PASS','logical_stages':len(jobs),'sources':{str(q.relative_to(ROOT)):cert.hashed(q) for q in paths},'archive_sha256':manifest['archive_sha256'],'ledger':ledger,'claim_boundary':manifest['claim_boundary']})
