...
def replay(path, output):
    ...
    if data['targets'] != [{k:r[k] for k in ('id','curve','j_invariant','family','parameter','rank_lower_bound')} for r in original['curves'] if int(r['id'].split('-')[-1])>101]:
    ...
    if len(maps) != 12 or len(targets)!=84 or len(data['pairs']) != 12*len(targets) or {(r['target'],r['family']) for r in data['pairs']} != {(i,f) for i in targets for f in maps}:
