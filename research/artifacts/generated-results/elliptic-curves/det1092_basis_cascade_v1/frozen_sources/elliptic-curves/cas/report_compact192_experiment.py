...
import select_compact192_unsearched as selection
from research_runtime.store import checkpoint
ROOT=batch.ROOT;ART=batch.ART;OUT=ART/'compact192_experiment_v1.json'
...
def expected():
    ...
    if selected!=json.loads(json.dumps(selection.expected())):raise ArithmeticError('fixed compact192 selection differs')
    maps=read(batch.D/'maps-ledger.json');point=read(batch.D/'ledger.json');v=read(batch.D/'verification-ledger.json');odd=read(batch.D/'odd-cloud-audit/ledger.json');proof=read(ART/'compact192_r17_results_v1.json')
    for label in ('certify','proof-replay','geometry'):terminal(batch.D/'post-batch'/(label+'.supervisor.json'))
    roster=[r['id'] for r in p['rows']]
    if len(roster)!=192 or len(set(roster))!=192 or maps['status']!='PASS' or point['status']!='COMPLETE_FIXED_BATCH_ATTEMPTS' or v['status']!='PASS' or odd['status']!='PASS':raise ArithmeticError('all192 required stages must be terminal')
    for d in (maps,point,v,odd):
        if [r['id'] for r in d['rows']]!=roster:raise ArithmeticError('fixed192 order differs')
    if [r['id'] for r in proof['curves']]!=roster or len(proof['previous_equations'])!=549 or proof['catalogue']['curve_count']!=593:raise ArithmeticError('point proof roster or pinned comparison population differs')
    rows=[]
    ...
    if sum(r['declared_charts'] for r in rows)!=8640:raise ArithmeticError('fixed8640 chart cap differs')
    return {'schema':'elliptic-curves.compact192-experiment.v1','status':'PASS','sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in sorted(paths)},'rows':rows,'saved_compact_score_rows':6144,'selection_skips':len(selected['skipped_before_roster_completion']),'declared_point_boxes':8640,'attempted_point_boxes':sum(r['attempted_charts'] for r in rows),'completed_point_boxes':sum(r['completed_boxes'] for r in rows),'rank_lower_bound_counts':dict(sorted(Counter(r['rank_lower_bound'] for r in rows).items())),'catalogue_snapshot_curves':593,'previous_address_equations':549,'within_batch_isomorphic_pairs':proof['within_batch_isomorphic_pairs'],'catalogue_unmatched_candidates_at_least22':sum(r['rank_lower_bound']>=22 and not r['catalogue_matches'] and not r['previous_matches'] for r in rows),'claim_boundary':'A fixed deeper point-exposure cohort in the already scored compactH4096 population, selected by unchanged S1 with validation excluded from all ties. Previous and within-roster equation exclusions do not assert random sampling. Exact point lower bounds and pinned rational-isomorphism comparisons require separate inventory promotion. These results do not establish whole-curve rank, upper bounds, saturation, score optimality, rank density, point absence or universal novelty.'}
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--check',action='store_true');a=p.parse_args();d=expected()
    if a.check:
        if cert.read(OUT)!=json.loads(json.dumps(d)):raise ArithmeticError('compact192 aggregate differs')
    else:
        if OUT.exists():raise FileExistsError('preserve compact192 experiment aggregate')
        checkpoint(OUT,d)
    print('COMPACT192 EXACT EXPERIMENT',d['rank_lower_bound_counts'],d['completed_point_boxes'],'completed boxes;',d['catalogue_unmatched_candidates_at_least22'],'unmatched >=22 candidates',flush=True)
