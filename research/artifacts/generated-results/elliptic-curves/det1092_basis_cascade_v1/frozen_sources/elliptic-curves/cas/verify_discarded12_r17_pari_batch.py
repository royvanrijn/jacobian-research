...
def verify(index):
    ...
    for label,args,seconds in [('history',[str(CAS/'discarded12_r17_pari_batch.py'),'replay','--index',str(index)],300),('cloud-build',[str(CAS/'audit_recorded_point_mod2_rank_v3.py'),'--input',str(path),'--input-sha256',cert.hashed(path),'--output',str(out),'--prime-bound','997'],120),('cloud-check',[str(CAS/'audit_recorded_point_mod2_rank_v3.py'),'--check',str(out)],120)]:
        ...
        if label=='history' and old.exists():
            ...
            if s['outcome']!='completed' or s['returncode']!=0 or Path(s['command'][1]).resolve()!=CAS/'discarded12_r17_pari_batch.py' or s['command'][2:]!=['replay','--index',str(index)] or cert.hashed(Path(s['log']))!=s['log_sha256'] or bound['result_sha256']!=cert.hashed(path):raise ArithmeticError('prior exact replay cannot be reused')
            checkpoint(record,s)
    ...
    checkpoint(folder/'verification.json',{'status':'PASS','input_sha256':cert.hashed(path),'cloud_certificate':str(out.relative_to(ROOT)),'cloud_sha256':cert.hashed(out),'verifier_sha256':cert.hashed(Path(__file__).resolve()),'steps':steps});print('VERIFIED DISCARDED12 R17',row['id'],'rank >=',cert.read(out)['rank_lower_bound'],flush=True)
