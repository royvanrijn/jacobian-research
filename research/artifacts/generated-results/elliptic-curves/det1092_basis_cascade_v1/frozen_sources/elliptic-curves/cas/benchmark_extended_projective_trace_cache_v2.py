...
D=ROOT/'artifacts/local/elliptic-curves/extended-projective-trace-cache-benchmark-v2';OUT=ART/'extended_projective_trace_cache_benchmark_v2.json'
...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.extended-projective-trace-cache-benchmark.v2','sources':sources(),'family':'11952','model':model,'model_hash':digest(model),'primes':cases,'full_selection_prime_roster':primes,'projective_rows':sum(p+1 for p in cases),'full_selection_projective_rows':sum(p+1 for p in primes),'gp_sha256':cert.hashed(scalar.GP),'gp_seconds_per_prime':20,'rss_bytes':536870912,'outer_wall_seconds':300,'direct_residues':'0,floor(p/3),floor(2p/3),p-1,infinity','projected_single_worker_seconds_gate':1800,'gate':'The known native11952 control changes from hypothetical position435 under the short score to2 under extended S1 among the same967 saved outer candidates. Extending many more short-retained survivors with a separate GP call per curve repeats finite-field fibres. Before any broader survivor campaign, measure whether caching each projective residue once per selection prime is affordable. The family is chosen by this exact calibration issue, not generic rank alone.','supersedes_failed_setup':'Version one placed the GP execution loop on the same line as the function definition, so GP parsed it into the function body and returned no table. The failed call is retained. Version two separates the statements; the eight primes and all budgets are unchanged.', 'scope':'Only eight deterministic prime-table cases, spaced across4099through32749, one worker. Compute allp+1 raw homogeneous finite-field fibres in each case with PARI ellcard, retain singular markers, check every prime/residue/Hasse frame and independently verify five complete character sums per prime. Cost extrapolation is descriptive, not a certified runtime bound. No full extended cache, new parameter search, new selector, point search or automatic escalation occurs in this benchmark.'})
...
def run(check=False):
    ...
    if check:
        if cert.read(D/'result.json')!=data or cert.read(OUT)!={'schema':'elliptic-curves.extended-projective-trace-cache-benchmark-result.v2',**data,'sources':sources(),'claim_boundary':p['scope']}:raise ArithmeticError('projective cache benchmark replay differs')
    else:
        if OUT.exists():raise FileExistsError('preserve benchmark certificate')
        checkpoint(D/'result.json',data);checkpoint(OUT,{'schema':'elliptic-curves.extended-projective-trace-cache-benchmark-result.v2',**data,'sources':sources(),'claim_boundary':p['scope']})
