#!/usr/bin/env python3
"""Generic17-only matched49-map PARI control at two frozen height bounds."""
import argparse,sys
...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';LOCAL=ROOT/'artifacts/local/elliptic-curves';D=LOCAL/'native11952-height-pair-v1';MAPS=LOCAL/'native11952-pari-maps-v1/maps.json';HEIGHTS=(100000,1000000)
def sources():
    names=['native11952_height_pair.py','memory_rank_certificate.py','research_runtime/search_state.py','research_runtime/cached_observation_state.py','research_runtime/preloaded_prime_state.py','research_runtime/rotated_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.native11952-height-pair.v1','sources':sources(),'maps_sha256':cert.hashed(MAPS),'height_bounds':list(HEIGHTS),'seconds_per_chart':10,'worker_wall_seconds':600,'replay_wall_seconds':300,'rss_bytes':1610612736,'maximum_workers':1,'gp_sha256':cert.hashed(Path('/usr/bin/gp')),'gate_source_sha256':cert.hashed(gate),'gate':'A separate196-word retrospective exact audit locates representatives of both missing known-control directions at113933 and918522. Test whether increasing only height exposes those directions in a generic17-only matched control. This is calibration of known points, never new-curve discovery.','policy':'Two independent arms start from only the17 generic points in the frozen49-map file. Use identical map order, backend, admission primes and10second per-chart limits. Attempt all49 charts regardless of gain; no oracle coordinates, words, published point list or rank29 stopping rule enter either worker.','boundaries':'No exact rank, global upper bound or general prospective recovery guarantee. Preserve all partial outputs and failures. A higher-height prospective policy requires completed exact control replays first.'})
def protocol():
