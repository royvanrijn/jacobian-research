...
def launch():
    ...
    for job in p['jobs']:
        ...
        ok=s['outcome']=='completed' and s['returncode']==0;ledger['rows'].append({'name':n,'status':'PASS' if ok else 'FAILED_OR_CENSORED','supervision':s});checkpoint(D/'ledger.json',ledger);print('HIGHER60 PORTABLE',n,ledger['rows'][-1]['status'],flush=True)
        if not ok:
