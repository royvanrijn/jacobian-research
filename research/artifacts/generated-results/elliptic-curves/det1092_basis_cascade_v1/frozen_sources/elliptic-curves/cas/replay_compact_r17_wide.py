#!/usr/bin/env python3
"""Sage-free replay of wide compact R17 transports, charts and rank admissions."""
import argparse
...
def replay(path):
    ...
    if data['status']=='INCOMPLETE_GENERIC_MOD2_CERTIFICATE':
        if state.rank>=17 or data['charts'] or data['centres']:raise ArithmeticError('censored input semantics changed')
    elif data['centres']:
        if initial_points!=points or state.rank!=17:raise ArithmeticError('initial generic basis mismatch')
        ...
        for c in data['centres']:
            ...
            if len(rep)!=17 or any((rep[j]-(c['mask']>>j))%2 for j in range(17)):raise ArithmeticError('centre parity failed')
    ...
    print('REPLAYED RETAINED R17 H4096',data['family'],data['parameter'],'charts',len(data['charts']),'rank >=',state.rank,flush=True)
