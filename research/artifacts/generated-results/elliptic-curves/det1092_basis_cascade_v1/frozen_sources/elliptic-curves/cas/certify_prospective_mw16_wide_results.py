#!/usr/bin/env python3
"""Post-batch exact point certificates and catalogue comparison for the wider MW16 pilot."""
import argparse
import json
...
def build(directory,output):
    ...
    for entry in ledger['rows']:
        ...
        print('CERTIFIED PROSPECTIVE MW16 WIDE',row['family'],row['parameter'],'rank >=',len(points),'catalogue matches',row['icarm_matches'],flush=True)
    duplicates=[]
    for i,r in enumerate(rows):
        for j,s in enumerate(rows[:i]):
            if cert.isomorphic(r['curve'],s['curve']):duplicates.append([j,i])
    cert.write(output,{'schema':'elliptic-curves.prospective-mw16-wide-results.v1','sources':sources(),'curves':rows,'missing_measurements':missing,
        ...
        'claim_boundary':'Independent points give exact lower bounds only. Novelty means no Q-isomorphism in the pinned584-equation catalogue and41 earlier measured equations, not universal novelty. Censored point attempts are not upper bounds. All catalogue reads occurred after the fixed20-address batch.'})
...
def check(path):
    ...
    for row in d['curves']:
        ...
        print('REPLAYED PROSPECTIVE MW16 WIDE CERTIFICATE',row['family'],row['parameter'],'rank >=',row['rank_lower_bound'],flush=True)
