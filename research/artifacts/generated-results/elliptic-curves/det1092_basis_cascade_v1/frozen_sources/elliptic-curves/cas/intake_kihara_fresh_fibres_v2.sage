...
"""Four fixed fresh Kihara fibres with exact index-six seed enlargement."""
...
from ecsearch.kihara import kihara_rank14_replay, _translate_quartic
...
D=ROOT/'artifacts/local/elliptic-curves/kihara-fresh-fibres-v2';SOURCE=ART/'kihara_global_parent_geometry_v1.json'
def intake(index):
 ...
 den=t.denominator()**int(g['chi']);Eint=EllipticCurve(QQ,[den**4*E.a4(),den**6*E.a6()]);points=[Eint([den**2*P[0],den**3*P[1]]) for P in points]
 model=tuple(cert.F(str(c)) for c in Eint.a_invariants())
 translated=_translate_quartic(original.quartic.quartic,original.quartic_origin[0]);q0=QQ(original.quartic_origin[1]);dd,cc,bb=map(QQ,translated[1:4])
 kx=dd*dd/(4*q0*q0)-cc;ky=-dd*kx/(2*q0)-q0*bb
 extra=Eint([(u*den)**2*(kx+c/3),(u*den)**3*ky])
 assert 6*extra==sum(points[:12],Eint(0))
 old_points=list(points);points[0]=extra
 cloud=[tuple(cert.F(str(c)) for c in P.xy()) for P in points]
 ...
 result={'schema':'elliptic-curves.kihara-fresh-seed.v2','status':'PASS' if proof else 'UNRESOLVED_SEED_QUOTIENT','parameter':str(t),'family':'kihara-rank14','curve':list(map(str,model)),
 'original_points':[[str(c) for c in P.xy()] for P in old_points],'added_involution_point':list(map(str,extra.xy())),'exact_relation':'6*K=P1+...+P12','old_subgroup_index_in_new':6,'points':[[str(c) for c in P] for P in cloud],'generic_points':[[str(c) for c in P] for P in cloud],'finite_mod2_rank':rank,'rank_certificate':proof,'rank_lower_bound':14 if proof else None,
 ...
 'scope':'Same fixed four parameters as V1. Natural weighted denominator clearing avoids unnecessary large coefficients. The rational involution point K satisfies 6K=sum of the twelve product-root points, enlarging the seed by exact index six after replacing P1. Full mod2 independence proves rank14; no rank gain is attributed to this saturation correction.'}
