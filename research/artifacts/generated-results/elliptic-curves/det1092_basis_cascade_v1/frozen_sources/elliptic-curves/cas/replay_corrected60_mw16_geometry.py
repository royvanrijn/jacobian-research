...
def main():
    p=batch.protocol();atlas={r['fibration_id']:r for r in cert.read(spec.ATLAS)['families']};count=0
    for row in p['rows']:
        ...
        for c in maps['centres']:
            if len(c['representative'])!=16 or any((c['representative'][j]-(c['mask']>>j))%2 for j in range(16)):raise ArithmeticError('generic parity differs')
        geometry(data,maps,initial,{**p,'maps_path':folder/'maps.json'});cloud_check(data,data['charts'],ART/('corrected60_mw16_'+row['id'].replace('-','_')+'_mod2_v1.json'),path);count+=len(data['charts'])
    if sum(len(cert.read(batch.D/r['id']/'maps.json')['rows']) for r in p['rows'])!=2580:raise ArithmeticError('fixed2580 declared roster differs')
    print('REPLAYED',count,'OF2580 DECLARED MW16 CHARTS, GENERIC CENTRES, RAW POINTS AND CLOUD PROVENANCE',flush=True)
