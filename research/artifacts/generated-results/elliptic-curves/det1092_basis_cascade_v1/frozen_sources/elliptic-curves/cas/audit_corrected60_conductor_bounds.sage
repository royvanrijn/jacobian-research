...
"""Fixed finite conductor bounds for the new corrected MW16 curve."""
...
INPUT=ART/'corrected60_mw16_results_v1.json'
CAT=ROOT/'artifacts/local/elliptic-curves/retention24-current-catalogue-v1/database.json'
D=ROOT/'artifacts/local/elliptic-curves/corrected60-conductor-bounds-v1'
OUT=ART/'corrected60_conductor_bounds_v1.json'
...
def candidates():
    gate=cert.read(ROOT/'artifacts/local/elliptic-curves/corrected60-mw16-pari-v1/post-batch/ledger.json')
    if gate['status']!='PASS' or gate.get('stronger_odd_prime_bounds')!=[]:raise ArithmeticError('terminal point proofs required before conductor comparison')
    return [r for r in cert.read(INPUT)['curves'] if r['rank_lower_bound']>=22 and not r['icarm_matches'] and not r['previous_matches']]

def prepare():
    if (D/'protocol.json').exists():raise FileExistsError('preserve conductor audit protocol')
    rows=candidates()
    if len(rows)!=1 or len({r['id'] for r in rows})!=1:raise ArithmeticError('fixed one new corrected curve required')
    checkpoint(D/'protocol.json',{'sources':sources(),'roster':[r['id'] for r in rows],
        ...
        'scope':'Audit conductor upper bounds for the newly certified corrected-MW16 curve with lower bound23, after terminal point proofs and novelty checks. Exact local Tate data at2,3 and every discriminant prime through10000; leave every larger cofactor unfactored. No parameter search, point search, new rank assertion, descent or automatic factorization escalation. A bound above a recorded benchmark does not exclude a smaller true conductor.'})
...
def expected(write=False):
    ...
    inputs=candidates();catalogue=cert.read(CAT)['curves']
    ...
    result={'schema':'elliptic-curves.corrected60-conductor-bounds.v1','status':'RUNNING',
...
if __name__=='__main__':
    ...
    if a.stage=='prepare':prepare()
        ...
        print('CORRECTED60 NEW-CURVE CONDUCTOR BOUNDS',len(d['rows']),'STRICT IMPROVEMENTS',d['strict_improvement_ids'],flush=True)
