...
"""Bounded normalizer discovery and exact replay for the literal det412 T.
...
def build(witness=None):
    T = matrix(ZZ, [[-2, 1, 0], [1, 2, 2], [0, 2, 84]])
    assert T.det() == -412
    ...
    assert trace.det() == -206**2
    quaternion = QuaternionAlgebra(QQ, 5, QQ(824)/5)
    assert quaternion.discriminant() == 206
    ...
    assert U*T*W == smith and list(smith.diagonal()) == [1,1,412]
    dual = T.inverse()*U.inverse()*vector(ZZ,[0,0,1])
    q = dual*T*dual/2
    units = [u for u in range(412) if gcd(u,412)==1 and (u*u-1)*q in ZZ]
    ...
    def check(p,v):
        ...
        assert all(S[2,j]%412==0 for j in (0,1))
        u = int(S[2,2]%412)
    ...
    if witness is None:
        ...
        assert len(records)==2, ('bounded normalizer miss',sorted(records))
    else:
        for r in witness['prime_normalizers']:
            p=int(r['prime']); records[p]=check(p,r['coordinates'])
        assert sorted(records)==[2,103]
    actions={1:1}
    for p in (2,103):
        actions.update({m*p:u*records[p]['discriminant_unit']%412
                        for m,u in list(actions.items())})
    assert sorted(actions.values())==units and len(units)==4
    stable=[m for m,u in sorted(actions.items()) if u in (1,411)]
    assert stable==[1,206]
    result=dict(schema='det412.literal-marking.v1',T=rows(T),
        clifford_multiplication_table=table,trace_pairing=rows(trace),
        reduced_order_discriminant=206,quaternion_discriminant=206,
