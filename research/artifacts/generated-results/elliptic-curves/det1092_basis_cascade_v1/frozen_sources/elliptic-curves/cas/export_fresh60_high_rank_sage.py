#!/usr/bin/env python3
"""Executable equations and exactly transported points for four outer MW16 fibres."""
import argparse
import certify_compact_r17_candidates as cert
import certify_fresh60_high_rank_minimal as proof

OUT = proof.ART / 'new_fresh60_high_rank_curves.sage'

def main(check):
    data = proof.expected()
    if cert.read(proof.OUT) != data:
        raise ArithmeticError('exact minimal-model certificate differs')
    lines = ['# Four distinct outer MW16 curves with certified rank lower bounds.',
             '# Proof: fresh60_high_rank_minimal_proof_v1.json.',
    ...
    if check:
        ...
        with OUT.open('x') as stream:
            stream.write(text)
    print('FOUR OUTER MW16 EQUATION/POINT EXPORTS PASS', flush=True)
