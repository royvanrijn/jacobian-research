#!/usr/bin/env python3
"""Post-batch exact point certificates and catalogue comparison for the next12 MW16 candidates."""
import argparse
import json
...
def build(directory,output):
    ...
    if ledger['status']!='COMPLETE_FIXED_BATCH_ATTEMPTS' or len(ledger['rows'])!=12 or any(r['status']=='PENDING' for r in ledger['rows']):raise ArithmeticError('catalogue boundary: fixed batch not finished')
    ...
    retained_path=ROOT/'artifacts/generated-results/elliptic-curves/recorded_mod2_admission_audit_v1.json'
    previous_hashes[str(retained_path.relative_to(ROOT))]=cert.hashed(retained_path)
    for case in cert.read(retained_path)['cases']:
        p=ROOT/case['certificate'];record=cert.read(p)
        previous_hashes[str(p.relative_to(ROOT))]=cert.hashed(p)
        previous.append({'address':case['input'],'curve':record['curve']})
    ...
    for entry in ledger['rows']:
        ...
        cloud_path=ROOT/f"artifacts/generated-results/elliptic-curves/prospective_mw16_next12_recorded_mod2_{entry['index']:02}_v1.json"
        cloud_binding=None
        if cloud_path.exists():
            cloud=cert.read(cloud_path)
            if cloud['status']!='COMPLETE_DECLARED_FINITE_AUDIT' or cloud['input_sha256']!=cert.hashed(path) or cloud['curve']!=d['curve']:
                raise ArithmeticError('complete-cloud proof input differs')
            points=tuple(tuple(map(cert.F,p)) for p in cloud['independent_points'])
            proof=cloud['rank_certificate']
            cloud_binding={'path':str(cloud_path.relative_to(ROOT)),'sha256':cert.hashed(cloud_path)}
        else:
        ...
        row={'family':d['family'],'parameter':d['parameter'],'curve':d['curve'],'points':[list(map(str,p)) for p in points],
            ...
            'complete_cloud_certificate':cloud_binding,'completed_charts':len(d['charts']),'search_status':d['status'],'supervision_status':entry['status'],
        ...
        print('CERTIFIED PROSPECTIVE MW16 NEXT12',row['family'],row['parameter'],'rank >=',len(points),'catalogue matches',row['icarm_matches'],flush=True)
    duplicates=[]
    for i,r in enumerate(rows):
        for j,s in enumerate(rows[:i]):
            if cert.isomorphic(r['curve'],s['curve']):duplicates.append([j,i])
    cert.write(output,{'schema':'elliptic-curves.prospective-mw16-next12-results.v1','sources':sources(),'curves':rows,'missing_measurements':missing,
        ...
        'claim_boundary':'Independent points give exact lower bounds only. Novelty means no Q-isomorphism in the pinned584-equation catalogue and the retained earlier measured equations, not universal novelty. Censored point attempts are not upper bounds. All catalogue reads occurred after the fixed12-address batch.'})
...
def check(path):
    ...
    for row in d['curves']:
        ...
        print('REPLAYED PROSPECTIVE MW16 NEXT12 CERTIFICATE',row['family'],row['parameter'],'rank >=',row['rank_lower_bound'],flush=True)
