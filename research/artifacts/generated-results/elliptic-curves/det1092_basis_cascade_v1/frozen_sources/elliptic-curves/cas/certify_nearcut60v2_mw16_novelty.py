...
ROOT=batch.ROOT;ART=batch.ART;D=batch.extension.D/'novelty-controller-v2'
UPSTREAM=batch.extension.D/'accounting-controller-v3'
OUT=ART/'nearcut60v2_mw16_novelty_v1.json'
PREVIOUS=ART/'strata60_mw16_results_v1.json'
REPORT=ART/'nearcut60v2_mw16_experiment_v1.json'
AUDIT=ART/'nearcut60v3_mw16_accounting_replay_v1.json'
...
def expected():
    if cert.read(UPSTREAM/'ledger.json')['status']!='PASS' or cert.read(batch.extension.D/'point-controller-v2/ledger.json')['status']!='COMPLETE_FIXED_RETAINED_TRIAL_AND_ACCOUNTING':
    ...
    if len(old)!=1405 or len(catalogue)!=593:raise ArithmeticError('fixed1405 prior and593 catalogue equations required')
    ...
    return {'schema':'elliptic-curves.nearcut60v2-novelty.v1','status':'PASS','sources':sources,
...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},
        ...
        'scope':'After the fixed comparison and independent accounting finish, exactly compare all60 prospective equations with593 pinned catalogue and1405 prior equations. Report certified>=22 unmatched rows separately; leave unknown ranks unknown. Build and read-only replay, no new point or parameter search.'})
...
if __name__=='__main__':
    ...
    if a.stage in ('prepare','launch'):globals()[a.stage]()
        ...
        if a.stage=='check':
            ...
            checkpoint(OUT,d)
        print('NEARCUT60V2 NOVELTY',d['status'],'UNMATCHED>=22',d['unmatched_high_rank_ids'],flush=True)
