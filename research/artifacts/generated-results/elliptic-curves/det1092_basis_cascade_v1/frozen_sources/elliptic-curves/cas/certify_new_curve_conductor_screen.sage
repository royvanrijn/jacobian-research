...
OUT = ROOT/'artifacts/generated-results/elliptic-curves/new_curve_conductor_screen_v1'


def build(check=False):
    audit = cert.read(INPUT)
    rows = [r for r in audit['rows'] if not r['catalogue_matches']]
    results = []
    for r in rows:
    ...
    summary = {'schema':'elliptic-curves.new-conductor-screen.v1','status':'PASS',
