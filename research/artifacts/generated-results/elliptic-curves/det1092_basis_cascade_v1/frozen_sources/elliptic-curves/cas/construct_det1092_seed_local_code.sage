...
DIR = ART/'det1092_seed_local_code_v1'
...
def build(index):
    ...
    for p in primes:
        ...
        coeffs = [list(map(int,columns.solve_right(M[:,j]))) for j in range(17)]
    ...
    real_coords = [list(map(int,B.solve_right(M[:,j]))) for j in range(17)]
    ...
    if index == 8:
        ...
        for row,engine in zip(local[:-1],engines):
            signatures = matrix(GF(2),[list(engine.signature(betas[j])) for j in row['basis_indices']]).transpose()
