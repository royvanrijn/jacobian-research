...
def prepare():
    ...
    paths=[Path(__file__).resolve(),Path(scan.__file__),scan.annulus.CPP,CAS/'research_runtime/supervisor.py',CAS/'research_runtime/store.py']
    checkpoint(D/'protocol.json',{'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},'stages':[('prepare',120),('benchmark',180),('run',600),('replay',180)],'rss_bytes':2147483648,'scope':'Freeze one disjoint new8-slice protocol using the previously verified annular binary, check ten complete signed11952 frames, pass the first full-slice runtime gate, complete32768 retained scores and replay. Two scanner workers, immutable calls, no retries or point searches.'})
def launch():
    ...
    for name,seconds in p['stages']:
        ...
        checkpoint(D/'ledger.json',ledger);print('NEW11952 CONTROLLER',name,ledger['rows'][-1]['status'],flush=True)
