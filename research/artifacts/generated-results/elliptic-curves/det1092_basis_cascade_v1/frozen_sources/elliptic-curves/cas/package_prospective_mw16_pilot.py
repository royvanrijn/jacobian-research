#!/usr/bin/env python3
"""Portable source, population, census and result evidence for the fixed MW16 pilot."""
import argparse
...
def package(directory,manifest):
    ...
    paths.update((Path(__file__).resolve(),ART/'prospective_mw16_census_audit_v1.json',ART/'prospective_mw16_results_v1.json',
                  ROOT/'elliptic-curves/cas/replay_prospective_mw16_search.py',ROOT/'elliptic-curves/cas/certify_prospective_mw16_results.py',
                  ROOT/'elliptic-curves/cas/run_prospective_mw16_batch.py',ROOT/'elliptic-curves/cas/audit_prospective_mw16_census.py'))
    rows=[]
    ...
    cert.write(manifest,{'schema':'elliptic-curves.prospective-mw16-evidence.v1','builder_sha256':hashed(Path(__file__).resolve()),
    ...
    print('PACKAGED MW16 PILOT',len(rows),'files',archive.stat().st_size,'bytes',flush=True)
