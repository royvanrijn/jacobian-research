#!/usr/bin/env python3
"""Batch finite mod2 admission of every retained point, without another point search."""
import argparse
...
def build(path,output,bound):
    if output.exists():raise FileExistsError('preserve recorded-point audit')
    data=cert.read(path)
    if data['status'] not in ('COMPLETE_DECLARED_PILOT','TARGET_REACHED_PENDING_INDEPENDENT_REPLAY'):raise ArithmeticError('use a terminal immutable transcript')
    ...
    result={'schema':'elliptic-curves.recorded-point-mod2-rank.v1','sources':sources(),'input_path':str(path.resolve().relative_to(ROOT)),
        'input_sha256':cert.hashed(path),'family':data.get('family','published-R17'),'parameter':data['parameter'],'curve':data['curve'],
...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--input',type=Path);p.add_argument('--output',type=Path);p.add_argument('--prime-bound',type=int,default=1000);p.add_argument('--check',type=Path);a=p.parse_args()
    check(a.check) if a.check else build(a.input,a.output,a.prime_bound)
