#!/usr/bin/env python3
"""Standalone independent finite-group ranks on the two native19 clouds and the independent known28 method control."""
...
OUT=ART/'retained_native19_sage_replay_v1.json'

def check_one(INPUT):
    d=json.loads(INPUT.read_bytes())
    E=EllipticCurve(QQ,d['curve'])
    points=[E([QQ(x),QQ(y)]) for x,y in d['points']]
    if len(points)<19:raise ArithmeticError('complete point cloud required')
    projective=[]
    for P in points:
...
if __name__=='__main__':
    ...
    result={'schema':'elliptic-curves.retained-native19-sage.v1','status':'PASS','rows':rows,
