...
OUT=ART/'small_conductor_smaller_base_v2.json'
...
def triangle(base,T,bits,terms):
    ...
    J=R.pi()/2-4*R(RealBallField(bits).catalan_constant())/L+(alternating+error)/L
...
def expected():
    ...
    return {'schema':'elliptic-curves.small-conductor-smaller-base.v2','status':'PASS',
