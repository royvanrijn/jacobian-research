...
def sources():
    return {str(p.relative_to(ROOT)):cert.hashed(p) for p in (Path(__file__).resolve(),ATLAS,MODEL,SECTIONS,INDEX)}
...
def build(output):
    ...
    if unmatched:
        witness = next(r for r in cert.read(INDEX)['curves'] if r['family'] == 'published-R17' and r['parameter'] == '33/119')
        model = tuple(map(cert.F,witness['curve']))
        certified = [tuple(map(cert.F,p)) for p in witness['points']]
        proof = witness['rank_certificate']
        cert.checked_rank(model,certified,[r['prime'] for r in proof['signatures']],proof['no_rational_2_torsion_prime'])
        cert.family_check('33/119',model,certified)
        probe = QQ(33)/119
        E1 = EllipticCurve(QQ,[0,0,0,A(probe),B(probe)])
        points = [E1(p[0](probe),p[1](probe)) for p in old+new]
        pari.default('realprecision',90)
        raw = pari(E1).ellheightmatrix([list(p.xy()) for p in points], precision=320)
        RF = RealField(256)
    ...
    checkpoint(output, {'schema':'elliptic-curves.compact-published-r17-generic-transport.v1',
        ...
        'matrix_determinant':det,'numeric_word_proposals':len(unmatched),'proposal_parameter':'33/119','height_precision_bits':320,
