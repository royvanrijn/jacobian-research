...
OUT=batch.ART/'nearcut60_mw16_portability_preflight_v1.json'
...
def main():
    ...
    paths.update([batch.D/'protocol.json',batch.extension.OUT,batch.extension.D/'protocol.json',
        batch.extension.D/'controller/ledger.json',CAS/'verify_nearcut60_mw16_points.py',
        CAS/'package_nearcut60_mw16_points.py',CAS/'verify_nearcut60_mw16_points_portable.py'])
    ...
    code='import sys;sys.path.insert(0,'+repr(str(workspace/'elliptic-curves/cas'))+');import nearcut60_mw16_pari_batch as b;import verify_nearcut60_mw16_points; p=b.protocol();assert len(p["rows"])==60;assert p["stop_rank"] is None;print("RELOCATED FROZEN60 INPUTS AND PROOF IMPORTS PASS")'
    ...
    checkpoint(OUT,{'schema':'elliptic-curves.nearcut60-portability-preflight.v1','status':'PASS',
    ...
    print('NEARCUT60 PROSPECTIVE PORTABILITY PREFLIGHT PASS',len(paths),'FILES',flush=True)
