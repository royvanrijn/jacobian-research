...
selector=SourceFileLoader('prospective_selector',str(CAS/'select_prospective_mw16_atlas.sage')).load_module()
geometry=selector.geometry
DIRECTORY=selector.DIRECTORY

def sources():
    paths=[Path(__file__).resolve(),CAS/'research_runtime/memory_store.py',CAS/'research_runtime/search_state.py']
...
def prepare(directory):
    ...
    for f in selection['families']:
        ...
        if censuses[f]['protocol_hash']!=digest(selection) or populations[f]['protocol_hash']!=digest(selection):raise ArithmeticError('selection binding changed')
    checkpoint(path,{'schema':'elliptic-curves.prospective-mw16-point-pilot.v1','sources':sources(),
        ...
        'target_free_boundary':selection['target_free_boundary'],'scope':'All20 fixed finalists run regardless of scores, catalogue status or discoveries in other workers. No refill or catalogue prefilter.'})
