#!/usr/bin/env python3
"""Exact binary caches and all4831 target-free saved scalar-score comparisons."""
import argparse, struct, sys, time
from array import array
from math import log
from pathlib import Path
import build_r17_remaining_extended_projective_caches as parent
...
D=ROOT/'artifacts/local/elliptic-curves/r17_remaining-joint-binary-caches-v1'
OUT=ART/'r17_remaining_joint_binary_caches_v1.json'
SAVED=ROOT/'artifacts/local/elliptic-curves/outer131072-r17-extended-v1'
...
def sources():
    paths=[Path(__file__).resolve(),Path(parent.__file__),parent.OUT,parent.D/'replay.json',
           ...
           SAVED/'replay.supervisor.json',reader.BINARY,reader.OUT,
...
def prepare():
    ...
    r=cert.read(SAVED/'replay.supervisor.json')
    if r['outcome']!='completed' or r['returncode']!=0: raise ArithmeticError('complete saved scalar replay required')
    saved=cert.read(SAVED/'protocol.json');scores=cert.read(SAVED/'result.json')
    if scores['status']!='COMPLETE_FROZEN_TRACE_EXTENSION' or len(saved['rows'])!=5798 or len(scores['rows'])!=5798:
        raise ArithmeticError('fixed5798 prior scalar cases required')
    extended={(r['family'],r['prime']):r for r in proof['rows']};families=[]
    for f in p['families']:
        tables={}
        for label,primes in [('short',[q for q in _primes_up_to(4093) if q>=5]),('extended',p['primes'])]:
            rows=[]
            for prime in primes:
                path=(old.PARENT/f['family']/'trace-tables'/f'{prime}.json') if label=='short' else (parent.D/f['family']/str(prime)/'table.json')
                h=cert.hashed(path)
        ...
        for row,score in zip(saved['rows'],scores['rows']):
            if (row['family'],row['retained_index'],row['parameter'])!=(score['family'],score['retained_index'],score['parameter']): raise ArithmeticError('saved scalar ordering differs')
            if row['family']==f['family']:
                fixtures.append({'id':row['family']+'-'+str(row['retained_index']),'numerator':row['numerator'],'denominator':row['denominator'],
                    'short':[row['score_units'],row['good_primes']],
                    'extended':[score['extension_selection_units'],score['extension_good']]})
        if len(fixtures)!={'074d9':970,'07ca9':969,'103b2':969,'08234':964,'08f72':959}[f['family']]: raise ArithmeticError('complete saved family fixture roster required')
        families.append({'family':f['family'],'model_hash':f['model_hash'],'tables':tables,'fixtures':fixtures})
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.r17_remaining-joint-binary-caches.v1','sources':sources(),
        'families':families,'encode_seconds':2400,'replay_seconds':2400,'score_seconds_per_call':60,
        'rss_bytes':1610612736,'maximum_height':524288,
        ...
        'scope':'Encode all562 short and2948 extended prime tables for each of the five R17 families in the existing little-endian R17XS001 format. Replay every byte against the frozen exact tables. Require every short/extended sum and good count on all4831 previously scored target-free candidates to agree with their saved canonical/scalar values using the independent compiled retained-list reader. No public-record input, new parameter scan, point exposure, selector or rank claim. The explicit reader bound524288 prepares later higher-parameter inputs.'})
...
def encode(check=False):
    ...
    for f in p['families']:
        for label,table_rows in f['tables'].items():
            ...
            print('R17 BINARY CACHE',f['family'],label,'BYTE REPLAY' if check else 'ENCODED',flush=True)
...
def score(check=False):
    ...
    for f in p['families']:
        ...
        for label in ('short','extended'):
            ...
            expected=[f"R {i} {r[label][0]} {r[label][1]}" for i,r in enumerate(f['fixtures'])]+[f"S {len(f['fixtures'])} {meta['primes']}"]
            ...
            records.append({'family':f['family'],'label':label,'candidates_checked':len(f['fixtures']),'raw_sha256':cert.hashed(rawpath),'wall_seconds':raw['supervision']['wall_seconds']})
    result={'schema':'elliptic-curves.r17_remaining-joint-binary-caches-result.v1','status':'PASS','sources':sources(),'protocol_sha256':cert.hashed(D/'protocol.json'),'encoding':encoded,'comparisons':records,'candidates_checked':4831,'component_sums_checked':9662,'claim_boundary':p['scope']}
    if check:
    ...
    print('ALL4831 SAVED R17 SHORT/EXTENDED SCORES MATCH',flush=True)
