...
import certify_strata60_high_rank_minimal as proof

OUT = proof.ART / 'new_strata60_high_rank_curves.sage'
...
def main(check):
    ...
    lines = [f'# {len(data["curves"])} distinct higher MW16 curves with certified rank lower bounds.',
             '# Proof: strata60_high_rank_minimal_proof_v1.json.',
    ...
    if check:
        ...
        with OUT.open('x') as stream:
            stream.write(text)
    print('MATCHED SCORE-STRATIFIED MW16 EQUATION/POINT EXPORTS PASS', flush=True)
