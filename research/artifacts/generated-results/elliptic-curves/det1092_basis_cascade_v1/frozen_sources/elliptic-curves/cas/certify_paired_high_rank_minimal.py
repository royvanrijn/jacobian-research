...
ROOT=Path(__file__).resolve().parents[2];ART=ROOT/'artifacts/generated-results/elliptic-curves';D=ROOT/'artifacts/local/elliptic-curves/fresh-r17-paired-pari-v1';OUT=ART/'paired_high_rank_minimal_proofs_v1.json';OLD=ART/'compact_r17_wide_results_v1.json';DB=ROOT/'artifacts/local/elliptic-curves/next12-current-catalogue-v1/database.json';IDS=('074d9-007','074d9-017','07ca9-025','11952-010')

def sources():
    names=['certify_paired_high_rank_minimal.py','memory_rank_certificate.py','certify_compact_r17_candidates.py','elliptic_candidate_record.py','mod2_reduction_independence.py','research_runtime/finite_reduction.py','research_runtime/memory_store.py','research_runtime/store.py']
...
def check_row(r,catalogue,previous):
    ...
    if r['icarm_matches'] or r['previous_matches'] or any(cert.isomorphic(model,q['ainvs']) for q in catalogue) or any(cert.isomorphic(model,q['curve']) for q in previous):raise ArithmeticError('new curve comparison failed')
