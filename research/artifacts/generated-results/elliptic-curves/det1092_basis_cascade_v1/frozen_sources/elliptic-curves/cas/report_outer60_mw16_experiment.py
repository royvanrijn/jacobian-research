...
ROOT=batch.ROOT;ART=batch.ART;D=batch.D;CONTROL=ROOT/'artifacts/local/elliptic-curves/outer60-mw16-controller-v1'
INPUT=ART/'outer60_mw16_results_v1.json';OUT=ART/'outer60_mw16_experiment_v1.json'
def expected():
    ...
    return {'schema':'elliptic-curves.outer60-mw16-experiment.v1','status':'PASS_STRONGER_ODD_BOUNDS_REQUIRE_REVIEW' if stronger else 'PASS',
        ...
        'claim_boundary':'Three disjoint stratified parameter-height bands beyond4096 and through262144, five MW16 families with equal retention and point budgets. All15360 retained extended scores precede60 distinct finalists; wholly disjoint validation never changes selection. Up to2580 generic16-only point charts with all maps before points. Catalogue and917 prior-equation comparison follows terminal exact point proofs. Matching finite bounds do not prove exact rank, saturation or absence. No universal novelty, rank density, calibrated sensitivity or isolated portability claim.'}
if __name__=='__main__':
    ...
    if a.check:
        ...
        checkpoint(OUT,d)
    print('OUTER60 OUTCOMES',d['status'],d['completed_point_boxes'],[(r['id'],r['rank_lower_bound']) for r in d['rows'] if r['rank_lower_bound']>=22],flush=True)
