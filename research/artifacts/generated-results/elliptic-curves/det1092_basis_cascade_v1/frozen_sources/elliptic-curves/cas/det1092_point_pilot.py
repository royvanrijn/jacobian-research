...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';BATCH=ROOT/'artifacts/local/elliptic-curves/det1092-point-pilot-v1';SAGE='/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python'
def sources():
 return {**engine.sources(),**{str((CAS/n).relative_to(ROOT)):cert.hashed(CAS/n) for n in ['det1092_point_pilot.py','prepare_det1092_point_pilot.sage','verify_det1092_point_seeds.sage']}}
def freeze():
 ...
 checkpoint(BATCH/'protocol.json',dict(schema='elliptic-curves.det1092-point-pilot.v1',sources=sources(),inputs={str(x.relative_to(ROOT)):cert.hashed(x) for x in [parentfile,gate]},rows=rows,sample_size=2048,sample_domain='full11952-specialized-followup-v1',charts=49,height=125000,seconds_per_chart=10,rank_stop=False,target_rank=32,maximum_point_boxes=98,geometry_wall_seconds=180,worker_wall_seconds=600,replay_wall_seconds=300,rss_bytes=2147483648,maximum_workers=1,gp_sha256=cert.hashed(Path('/usr/bin/gp')),
 gate='New parallel exact determinant1092 arithmetic MW17 parent with explicit generic sections; full-roster original27-only factor-free known28 regression passed. No extra parameter population, lattice search or recovered record point is required.',
...
def intake():
 ...
 for row in p['rows']:
  ...
  assert state.rank==17 and list(state.basis)==points
...
def launch():
 ...
 for i,row in enumerate(p['rows']):
  configure(i);s=run([SAGE,str(CAS/'prepare_det1092_point_pilot.sage'),'--index',str(i)],limits=Limits(p['geometry_wall_seconds'],p['rss_bytes']),log_path=D/'maps.log',checkpoint_path=D/'maps.supervisor.json',cwd=ROOT)
