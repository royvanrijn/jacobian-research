...
def main():
 import det1092_reduced_chart_point_pilot_v2 as pilot
 d=pilot.BATCH;assert not (d/'run-ledger.json').exists();ledger={'status':'RUNNING','stages':[]};checkpoint(d/'run-ledger.json',ledger)
 checkpoint(d/'run-protocol.json',dict(driver_sha256=cert.hashed(Path(__file__)),pilot_sha256=cert.hashed(CAS/'det1092_reduced_chart_point_pilot_v2.py'),maximum_workers=1,rss_bytes=2147483648,freeze_seconds=60,intake_seconds=180,seed_replay_seconds=120))
 ...
 for name,seconds in [('freeze',60),('intake',180)]:stage(name,[sys.executable,str(CAS/'det1092_reduced_chart_point_pilot_v2.py'),name],seconds)
 fresh=d/'seed-replay';fresh.mkdir(exist_ok=False)
 files=[CAS/'verify_det1092_reduced_point_seeds.sage',d/'parent-sections.json']
 ...
 stage('seed-replay',[SAGE,str(fresh/'verify_det1092_reduced_point_seeds.sage'),'--parent',str(fresh/'parent-sections.json'),'--seeds',str(fresh/'unit-seed.json'),str(fresh/'outer-seed.json'),'--output',str(fresh/'result.json')],120,fresh)
 pilot.launch()
 import complete_det1092_reduced_chart_point_pilot_v2 as proof
 proof.main()
