...
P=ROOT/'artifacts/local/elliptic-curves/joint11952-fresh-scores-v1/bank-protocol.json'
SCAN=ROOT/'artifacts/local/elliptic-curves/joint11952-fresh-annulus-v1/result.json'
OUT=ART/'joint11952_parameter_models_v1.json'
...
def expected():
    ...
    for row in p['rows']:
        ...
        if any(row[k]!=old[k] for k in ('numerator','denominator','parameter','score_units','good_primes','extension_selection_units','extension_good','combined_selection_units','combined_good')):raise ArithmeticError('retained address/score changed')
...
if __name__=='__main__':
    ...
    if a.check:
        ...
        checkpoint(OUT,d)
    print('JOINT11952 ANNULUS MODEL AUDIT',d['curves_checked'],d['distinct_j_invariants'],d['distinct_Q_isomorphism_classes'],'PASS',flush=True)
