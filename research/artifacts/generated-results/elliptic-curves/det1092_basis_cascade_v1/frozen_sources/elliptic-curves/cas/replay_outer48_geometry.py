...
def main():
    ...
    for row in p['rows']:
        ...
        geometry(data,maps,initial,{**p,'maps_path':folder/'maps.json'});cloud_check(data,data['charts'],ART/('outer48_r17_'+row['id'].replace('-','_')+'_mod2_v1.json'),path);count+=len(data['charts'])
    if count>2160 or sum(len(p['generic_masks'][r['family']]) for r in p['rows'])!=2160:raise ArithmeticError('fixed2160 maximum chart roster differs')
    print('REPLAYED OUTER48 CHARTS',count,'GENERIC CENTRES, RAW POINTS AND CLOUD PROVENANCE',flush=True)
