...
Read at most4096 already enumerated rows. No new lattice enumeration,
...
OUT=ART/'det1092_brauer_section_gate_v1'
...
def main():
    ...
    protocol={'classification':'generic-only bounded extraction of section intersection witnesses',
      'rule':'Read at most the first4096 rows of the unchanged old census. Keep norm6 section words when they increase binary rank, stopping at17. Account for every scanned row. Reuse only the unchanged nine-address21-place local panel; no exceptional coordinate is an input.',
      'limits':{'wall_seconds':25,'saved_rows':4096,'retained_words':17,'new_lattice_nodes':0,
    ...
    G=matrix(ZZ,read(parentpath)['generic_height_gram']);chosen=[];ledger=[]
    with census.open() as handle:
        for n,row in enumerate(csv.DictReader(handle,delimiter='\t'),1):
            if n>4096:break
            norm=int(row['minimum_norm']);trial={'saved_row':n,'orbit_mask':int(row['orbit_mask']),'norm':norm}
    ...
    result={'status':'NORM6_ODD_INDEX_SECTION_FRAME' if rank==17 else 'UNKNOWN_WITHIN_ROW_LIMIT',
      ...
      'integer_determinant':det,'chosen':chosen,'ledger':ledger,
    ...
    print(result['status'],'saved rows',len(ledger),'rank',rank,'determinant',det,flush=True)
