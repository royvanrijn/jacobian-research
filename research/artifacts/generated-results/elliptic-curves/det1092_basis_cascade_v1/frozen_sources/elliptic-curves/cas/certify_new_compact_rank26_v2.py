...
def verify(data):
    ...
    for x, y in points:
        if y*y+a1*x*y+a3*y != x**3+model[1]*x*x+model[3]*x+model[4]:
            raise ArithmeticError('transported point is off the integral model')
    if transported != [tuple(map(cert.F, p)) for p in data['discovery_points']]:
        raise ArithmeticError('point transport differs')
    proof = data['rank_certificate_on_discovery_model']
    actual = cert.checked_rank(short, transported, [s['prime'] for s in proof['signatures']],
...
def build(output):
    ...
    proof = cert.checked_rank(short, [tuple(map(cert.F, p)) for p in row['points']], [s['prime'] for s in old['signatures']], old['no_rational_2_torsion_prime'])
    data = {'schema': 'elliptic-curves.new-compact-rank26-proof.v1', 'sources': sources(),
        ...
        'points': [list(map(str, p)) for p in points], 'rank_certificate_on_discovery_model': proof,
