...
D = ROOT/'artifacts/local/elliptic-curves/native11952-pari49-control-v1'
...
def verify():
    ...
    output = ART/'native11952_pari49_recorded_mod2_v1.json'
    checkpoint(folder/'protocol.json', {'input_sha256': cert.hashed(path),
        ...
        'sources': {str(p.relative_to(ROOT)): cert.hashed(p) for p in (
            Path(__file__).resolve(), ROOT/'elliptic-curves/cas/replay_native11952_pari49.py',
    ...
    stages = [('charts', ['replay_native11952_pari49.py', str(path)], 900),
    ...
    for stage, args, seconds in stages:
        ...
        checkpoint(folder/'result.json', result)
        print('NATIVE11952 CONTROL VERIFICATION', stage, result['rows'][-1]['status'], flush=True)
