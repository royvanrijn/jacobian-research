...
def replay(path, output):
    ...
    original = cert.read(ROOT/'artifacts/generated-results/elliptic-curves/new_high_rank_curve_index_v13.json')
    current={r['id']:r for r in original['curves']}
    for target in data['targets']:
        if target['id'] not in current or any(target[k]!=current[target['id']][k] for k in ('id','curve','j_invariant','family','parameter')) or target['rank_lower_bound']>current[target['id']]['rank_lower_bound']:
            raise ArithmeticError('incidence target changed or historical lower bound regressed')
    maps = {row['family']:row for row in data['maps']}
    targets = {row['id']:row for row in data['targets']}
    if len(maps) != 12 or not 1<=len(targets)<=185 or len(data['pairs']) != 12*len(targets) or {(r['target'],r['family']) for r in data['pairs']} != {(i,f) for i in targets for f in maps}:
    ...
    checkpoint(output, {'schema':'elliptic-curves.compact-cross-family-j-incidence-replay.v1',
        ...
        'pairs_checked':12*len(targets),'status_counts':counts,
    ...
    print('REPLAYED RETENTION CROSS-FAMILY J INCIDENCE', counts, flush=True)

def main(output):
    import certify_inventory185_incidence as cohort
    if output.exists():raise FileExistsError('preserve complete185 incidence replay')
    result=cohort.result();folder=output.with_suffix('');summaries=[];seen=set()
    for path in cohort.PROOFS:
        out=folder/(path.stem+'_replay.json');replay(path,out);d=cert.read(path)
        ids={r['id'] for r in d['targets']}
        if len(ids)!=len(d['targets']) or seen&ids:raise ArithmeticError('overlapping incidence cohorts')
        seen.update(ids);summaries.append({'input':str(path.relative_to(ROOT)),'input_sha256':cert.hashed(path),'replay':str(out.relative_to(ROOT)),'replay_sha256':cert.hashed(out)})
    if len(seen)!=185 or sum(cert.read(ROOT/r['replay'])['pairs_checked'] for r in summaries)!=2220:raise ArithmeticError('complete185 product differs')
    checkpoint(output,{'schema':'elliptic-curves.inventory185-all-incidence-replay.v1','status':'PASS','targets_checked':185,'pairs_checked':2220,'cohorts':summaries,'checker_sha256':cert.hashed(Path(__file__).resolve()),'claim_boundary':'Independently recomputes every exact projective j-incidence witness across all nine cohorts and binds them to the current185 equations. Historical lower bounds may only increase. The separate exact generic transport is still needed for equal-subgroup claims; no new points, ranks or family exclusions beyond these twelve presentations.'})
    print('ALL185 CURVES:2220 EXACT J-INCIDENCE PAIRS REPLAYED',flush=True)
