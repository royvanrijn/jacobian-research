...
P=ROOT/'artifacts/local/elliptic-curves/mw16-outer-band-scores-v1/protocol.json'
SCAN=ROOT/'artifacts/local/elliptic-curves/mw16-outer-bands-v1/result.json'
OUT=ART/'mw16_outer_parameter_models_v1.json'
BANDS={1:(4096,16384),2:(16384,65536),3:(65536,262144)}
...
def expected():
    ...
    if len(p['rows'])!=15360 or scan['status']!='COMPLETE_FIXED_STRATIFIED_POPULATION':raise ArithmeticError('fixed retained population required')
    ...
    if len(source)!=15360:raise ArithmeticError('duplicate retained address')
    seen={};pairs=[];groups={};keys=set();jcounts=Counter()
    for row in p['rows']:
        n=row['numerator'];d=row['denominator'];t=F(n,d);b=row['band'];lo,hi=BANDS[b]
    ...
    if len(records)!=15 or any(r['curves']!=1024 or r['positive']!=512 or r['negative']!=512 for r in records):raise ArithmeticError('balanced signed band/family counts differ')
...
if __name__=='__main__':
    ...
    if a.check:
        ...
        checkpoint(OUT,d)
    print('OUTER MODEL AUDIT',d['curves_checked'],d['distinct_j_invariants'],d['distinct_Q_isomorphism_classes'],'PASS',flush=True)
