...
import higher_r17_masked_controls as control
...
OUT=control.ART/'higher_r17_masked_relations_v1.json'
def inputs():
    p=control.protocol();ledger=cert.read(control.D/'ledger.json');prepared=cert.read(control.D/'prepared.json')
    if ledger['status']!='PASS' or len(ledger['rows'])!=6:raise ArithmeticError('all six searches and map replays must finish before oracle access')
...
def main(check=False):
    ...
    for index,(row,raw,model,generic,points) in enumerate(cases):
        ...
        print('MASKED HIGHER RELATION',row['id'],result['recovery_status'],len(points),'points',flush=True)
