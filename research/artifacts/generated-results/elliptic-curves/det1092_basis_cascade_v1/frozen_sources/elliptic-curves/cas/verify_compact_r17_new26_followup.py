#!/usr/bin/env python3
"""Verify the terminal rank26 follow-up, including any censored retained prefix."""
...
D = ROOT/'artifacts/local/elliptic-curves/compact-r17-new26-followup-v1'
...
def verify():
    ...
    output = ART/'compact_r17_new26_followup_recorded_mod2_v1.json'
    checkpoint(folder/'protocol.json', {'input_sha256': cert.hashed(path),
        ...
        'sources': {str(p.relative_to(ROOT)): cert.hashed(p) for p in (
            Path(__file__).resolve(), ROOT/'elliptic-curves/cas/replay_compact_r17_new26_followup.py',
    ...
    stages = [('charts', ['replay_compact_r17_new26_followup.py', str(path)], 900),
    ...
    for stage, args, seconds in stages:
        ...
        checkpoint(folder/'result.json', result)
        print('RANK26 FOLLOWUP VERIFICATION', stage, result['rows'][-1]['status'], flush=True)
