...
ROOT=batch.ROOT;CAS=batch.CAS;D=ROOT/'artifacts/local/elliptic-curves/broad60-mw16-controller-v1'
def prepare():
    if (D/'protocol.json').exists():raise FileExistsError('preserve broad60 controller')
    names=['finish_broad60_mw16_points.py','broad60_mw16_pari_batch.py','prepare_broad60_mw16_pari_batch.sage','verify_broad60_mw16_pari_batch.py','replay_broad60_mw16_geometry.py','audit_broad60_mw16_clouds_modl.py','score_broad_mw16_higher.py','audit_recorded_point_mod2_rank_v3.py','audit_retained_cloud_modl.py','research_runtime/supervisor.py','research_runtime/store.py']
    paths=[*(CAS/n for n in names),batch.extension.D/'controller/protocol.json']
    checkpoint(D/'protocol.json',{'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},'wait_seconds':45000,'rss_bytes':4294967296,'stages':[('freeze','broad60_mw16_pari_batch.py',['freeze'],120),('maps','broad60_mw16_pari_batch.py',['maps'],1800),('points','broad60_mw16_pari_batch.py',['batch'],20000),('verify','verify_broad60_mw16_pari_batch.py',[],3600),('odd','audit_broad60_mw16_clouds_modl.py',[],1800),('geometry','replay_broad60_mw16_geometry.py',[],1200)],'scope':'After all1310720 full-score survivors and10240-scalar-score,60-selection and disjoint validation proofs pass, freeze exactly60 new-parameter MW16 point attempts,43 generic16 parity maps each, height125000,10 seconds per chart. All60 map files pass before any point attempt. Two point workers,600 seconds per curve, provisional28 stop; no adaptive wave or retries. Replay all terminal admission histories and retained point clouds modulo2,3,5 and rational geometry. Any catalogue/previous-equation comparison, inventory promotion or portable proof packaging follows separate terminal proofs.'})
def launch():
    ...
    try:
        ...
        for name,script,args,seconds in p['stages']:
            ...
            ledger['rows'].append({'name':name,'status':'PASS' if ok else 'FAILED_OR_CENSORED','supervision':s});checkpoint(D/'ledger.json',ledger);print('BROAD60 POINT CONTROLLER',name,ledger['rows'][-1]['status'],flush=True)
