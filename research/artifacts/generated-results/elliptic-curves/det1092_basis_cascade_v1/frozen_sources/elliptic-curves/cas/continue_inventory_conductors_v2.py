...
def prepare():
    ...
    rows = data['rows']
    unfinished = {r['id'] for r in load_conductor_inventory() if r['conductor_information']['status'] != 'EXACT'}
    selected = [r['id'] for r in sorted(rows,key=lambda r:(len(r['remaining_cofactor']),-r['rank_lower_bound'],r['id'])) if r['id'] in unfinished]
    protocol = {
        'input': str(INPUT.relative_to(ROOT)),
        'input_sha256': hashlib.sha256(INPUT.read_bytes()).hexdigest(),
        'script_sha256': hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        'all_curve_ids': [r['id'] for r in rows],
        'priority_ids': selected, 'workers': 4,
        'pari_seconds_per_priority': 15, 'ecm_seconds_per_priority': 30,
        'ecm_schedule': [2000]*25 + [11000]*50 + [50000]*75 + [250000]*75 + [1000000]*50,
        'sigma': '3000000 + priority roster index*10000 + attempt',
        'ecm_binary': ECM, 'ecm_binary_sha256': hashlib.sha256(Path(ECM).read_bytes()).hexdigest(),
        'scope': 'User requested continued factorization and an ICARM-like README table for all201 curves. This pass revisits all172 curves lacking local exact conductor proofs, with at most15 seconds PARI plus30 seconds ECM each, four workers, frozen201 inputs, and checkpoints. No point search or rank change.',
...
def run():
    ...
    for identifier in protocol['all_curve_ids']:
    ...
    def worker(item):
        ...
        for attempt in range(state['attempts'],len(protocol['ecm_schedule'])):
            ...
            if state['ecm_seconds'] >= protocol['ecm_seconds_per_priority']:
                break
            argv = [ECM,'-sigma',str(3000000+index*10000+attempt),'-maxmem','256',
