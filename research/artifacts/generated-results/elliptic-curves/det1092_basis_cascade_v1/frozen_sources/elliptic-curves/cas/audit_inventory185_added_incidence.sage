...
"""Exact twelve-presentation incidence for the 84 new inventory curves102through185."""
import argparse
...
def run(output):
    ...
    source_paths = [ART/'compact_six_r17_atlas_v1.json', ART/'compact_five_mw16_atlas_v1.json',
                    ...
                    ART/'new_high_rank_curve_index_v13.json']
    families = []
    ...
    targets = [{k:r[k] for k in ('id', 'curve', 'j_invariant', 'family', 'parameter', 'rank_lower_bound')}
               for r in cert.read(source_paths[3])['curves'] if int(r['id'].split('-')[-1])>101]
    if len(targets)!=84:raise ArithmeticError('exactly84 new inventory additions required')
    maps = []
