...
import extend_product_first_higher as extension
...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';D=ROOT/'artifacts/local/elliptic-curves/productfirst24-r17-pari-v1';PARITY=ART/'r17_exact_maximum_parity_classes_v1.json';SAGE='/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python'

def sources():
    names=['memory_rank_certificate.py','extend_product_first_higher.py','verify_productfirst24_r17_pari_batch.py','productfirst24_r17_pari_batch.py','prepare_productfirst24_r17_pari_batch.sage','prospective_half_lattice_v2.sage','compact_atlas_specialization.py','research_runtime/search_state.py','research_runtime/cached_observation_state.py','research_runtime/preloaded_prime_state.py','research_runtime/rotated_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
...
def expected_roster():
    ...
    for f in sorted(selected['selection']):
        available=[r for r in selected['rows'] if r['family']==f and r['cached_trace'] is None]
        ...
        for r in available[:4]:rows.append({'id':f+f"-{r['retained_index']:03}",'family':f,'parameter':r['parameter'],'retained_index':r['retained_index'],'arms':['product_first_new_retention_top_four'],'combined_selection_units':r['combined_selection_units']})
    if len(rows)!=24 or len({r['id'] for r in rows})!=24:raise ArithmeticError('fixed24 roster differs')
    return rows

def freeze():
    if (D/'protocol.json').exists():raise FileExistsError('preserve productfirst24 protocol')
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.productfirst24-r17-pari.v1','sources':sources(),'selection_sha256':cert.hashed(extension.D/'result.json'),'selection_protocol_sha256':cert.hashed(extension.D/'protocol.json'),'selection_replay_sha256':cert.hashed(extension.D/'replay.supervisor.json'),'control_sha256':cert.hashed(ROOT/'artifacts/local/elliptic-curves/native11952-height125-control-v1/125000/verification.json'),'rows':rows,'generic_masks':masks,'height':125000,'seconds_per_chart':10,'admission_prime_bound':997,'stop_rank':28,'map_wall_seconds':120,'worker_wall_seconds':600,'rss_bytes':1610612736,'maximum_workers':2,'gp_sha256':cert.hashed(Path('/usr/bin/gp')),'gate':'The product-first retention has4338 addresses absent from the original6144 S1 pool. All6144 product-first score extensions replay, reusing1806 cached trace inputs and computing the4338 newly retained addresses. Select four per family only among the newly retained addresses, without using ranks or point outcomes, and expose them to the same generic17-only policy. This tests the retention bottleneck left outside the previous paired second-stage score comparison. The prior identical49-map native control reached28 at125000 versus27 at100000; this is a finite visibility and cost gate, not a predictor for these candidates.','selection':'Exactly four per family among rows absent from the old S1 pool, using product score through32749, good count, denominator, numerator. Validation32771..65521 is excluded even from ties. Public equations, points, ranks and catalogue labels are absent from selection and execution. No refill of repeats or failures.','geometry':'All43 or49 exact generic maximum parity classes. Numerical384-bit heights choose representatives only; rational centres and maps are checked exactly.','boundaries':'Exactly24 attempts and at most1080 initial boxes, stop each curve on a provisional28-point independent subgroup pending replay. Each starts from17 generic sections only. Retain all checkpoints, failures and raw points. Catalogue and previous-equation comparison follows terminal batch replay. No complete32768 coverage, rank upper bound, exact rank, record or universal novelty.'})

def protocol():
    p=cert.read(D/'protocol.json')
    if p['sources']!=sources() or p['selection_sha256']!=cert.hashed(extension.D/'result.json') or p['selection_protocol_sha256']!=cert.hashed(extension.D/'protocol.json') or p['selection_replay_sha256']!=cert.hashed(extension.D/'replay.supervisor.json') or p['rows']!=expected_roster():raise ArithmeticError('frozen productfirst24 inputs or roster differ')
    return p
...
def replay(index):
    ...
    memory_checked_rank(model,state.basis,state.reductions.primes,state.no_two_torsion_prime);print('REPLAYED PRODUCTFIRST24 R17',p['rows'][index]['id'],len(data['charts']),'rank >=',state.rank,flush=True)
...
def batch():
    ...
    def one(index):
        row=p['rows'][index];folder=D/row['id'];mapping=supervise([SAGE,str(CAS/'prepare_productfirst24_r17_pari_batch.sage'),'--index',str(index)],limits=Limits(p['map_wall_seconds'],p['rss_bytes']),log_path=folder/'maps.log',checkpoint_path=folder/'maps.supervisor.json',cwd=ROOT)
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        for future in as_completed(futures):
            i=futures[future];r=future.result();ledger['rows'][i]=r;checkpoint(path,ledger);print('PRODUCTFIRST24 R17 ATTEMPT',r['id'],r['status'],r.get('rank_lower_bound'),flush=True)
