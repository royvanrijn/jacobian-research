...
ROOT=Path(__file__).resolve().parents[2];ART=ROOT/'artifacts/generated-results/elliptic-curves';D=ROOT/'artifacts/local/elliptic-curves/million-height-mw16-portable-v2'
...
def main():
    ...
    supplement=cert.read(ART/'million_height_mw16_population_supplement_v1.json')
    if supplement['base_manifest_sha256']!=cert.hashed(ART/'million_height_mw16_evidence_v1.json') or cert.hashed(ROOT/supplement['archive'])!=supplement['archive_sha256']:raise ArithmeticError('population supplement binding differs')
    with zipfile.ZipFile(ROOT/supplement['archive']) as z:
        if any(Path(n).is_absolute() or '..' in Path(n).parts for n in z.namelist()):raise ArithmeticError('unsafe supplement path')
        z.extractall(workspace)
    for r in supplement['files']:
        if cert.hashed(workspace/r['path'])!=r['sha256']:raise ArithmeticError('supplement member differs')
