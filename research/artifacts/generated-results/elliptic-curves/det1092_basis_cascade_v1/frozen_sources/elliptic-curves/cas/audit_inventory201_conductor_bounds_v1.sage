...
"""Fixed finite conductor bounds for every certified V22 inventory curve."""
...
D=ROOT/'artifacts/local/elliptic-curves/inventory201-conductor-bounds-v1'
OUT=ART/'inventory201_conductor_bounds_v1.json'

def sources():
    paths=[Path(__file__).resolve(),INPUT,CAT,Path(cert.__file__),Path(arithmetic.__file__),ROOT/'elliptic-curves/cas/mod2_reduction_independence.py']
...
def prepare():
    ...
    if len(rows)!=201 or len({r['id'] for r in rows})!=201:raise ArithmeticError('fixed201 required')
    checkpoint(D/'protocol.json',{'sources':sources(),'roster':[r['id'] for r in rows],
        ...
        'motivation':'User requested #628 completion and comparison of all other new high-rank curves against current conductor records.',
        'scope':'Audit all201 V22 conductor upper bounds to close the coverage gap beyond the early31 and next12 audits. Exact local Tate data at2,3 and every discriminant prime through10000; leave every larger cofactor unfactored. This stage performs no factorization or rank search. Separately budgeted factorization may follow for prioritized candidates. A bound above a recorded benchmark does not exclude a smaller true conductor.'})
...
def expected(write=False):
    ...
    byj={}
    for public in catalogue:
        inv=cert.weierstrass_invariants(tuple(map(cert.F,public['ainvs'])))
        byj.setdefault(inv['c4']**3/inv['discriminant'],[]).append(public)
    result={'schema':'elliptic-curves.inventory201-conductor-bounds.v1','status':'RUNNING',
    ...
    for r in inputs:
        short=tuple(map(cert.F,r['curve']));model=arithmetic.integral(short)
        if not cert.isomorphic(short,model):raise ArithmeticError('exact integral translation differs')
        E=EllipticCurve(QQ,[QQ(str(a)) for a in model]);inv=cert.weierstrass_invariants(model)
        ...
        for p in _primes_up_to(protocol['trial_prime_bound']):
            ...
            independent=pari.ellinit([QQ(str(a)) for a in model]).elllocalred(p)
            if f!=int(independent[0]):raise ArithmeticError('independent PARI conductor exponent differs')
        ...
        row={'id':r['id'],'family':r['family'],'parameter':r['parameter'],'rank_lower_bound':r['rank_lower_bound'],
            ...
            'catalogue_matches':sorted(q['id'] for q in byj.get(inv['c4']**3/inv['discriminant'],[]) if cert.isomorphic(model,q['ainvs'])),
            'curve':r['curve'],'integral_curve':list(map(str,model)),'discriminant':str(delta),'local_data':local,'remaining_cofactor':str(remaining),
...
if __name__=='__main__':
    ...
    if a.stage=='prepare':prepare()
        ...
        print('INVENTORY201 CONDUCTOR BOUNDS',len(d['rows']),'STRICT IMPROVEMENTS',d['strict_improvement_ids'],flush=True)
