...
OLD=ART/'new_high_rank_curve_index_v3.json'
ADDITIONAL=ART/'fresh_r17_paired_results_v1.json'
...
def build(output):
    ...
    cert.write(output,{'schema':'elliptic-curves.new-high-rank-index.v4','sources':sources(),
...
def check(path):
    ...
    for r in data['curves']:
        ...
        seen.add(j)
        print('REPLAYED V4 INVENTORY',r['id'],'rank >=',len(points),flush=True)
    print('EXACT CURVE INVENTORY',len(seen),flush=True)

if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('--output',type=Path,default=ART/'new_high_rank_curve_index_v4.json');p.add_argument('--check',type=Path)
