...
OLD=ART/'new_high_rank_curve_index_v18.json'
ADDITIONAL=ART/'corrected60_mw16_results_v1.json'

def sources():
    paths=(Path(__file__).resolve(),Path(cert.__file__).resolve(),ROOT/'elliptic-curves/cas/memory_rank_certificate.py',ROOT/'elliptic-curves/cas/mod2_reduction_independence.py',ROOT/'elliptic-curves/cas/elliptic_candidate_record.py',ART/'corrected60_mw16_point_portable_replay_v1.json')
    return {str(p.relative_to(ROOT)):cert.hashed(p) for p in paths}

def promotion_gate():
    base=ROOT/'artifacts/local/elliptic-curves/corrected60-mw16-pari-v1'
    portable=cert.read(ART/'corrected60_mw16_point_portable_replay_v1.json')
    if portable['status']!='PASS' or portable['logical_stages']!=182:
        raise ArithmeticError('all182 isolated point-proof stages required before inventory promotion')
    ...
    if any(cert.hashed(ROOT/n)!=h for n,h in protocol['sources'].items()):
        raise ArithmeticError('fixed corrected60_mw16 finalization sources differ')
    ...
    if ledger['status']!='PASS' or ledger.get('stronger_odd_prime_bounds')!=[] or [r['name'] for r in ledger['rows']]!=stages:
        raise ArithmeticError('complete corrected60_mw16 proof and review of any stronger odd bounds required')
    ...
    aggregate=cert.read(ART/'corrected60_mw16_experiment_v1.json')
    if aggregate['status']!='PASS' or len(aggregate['rows'])!=60:
        raise ArithmeticError('complete60 aggregate required')
    if any(cert.hashed(ROOT/n)!=h for n,h in aggregate['sources'].items()):
        raise ArithmeticError('corrected60_mw16 aggregate inputs changed')
    incoming=cert.read(ADDITIONAL)
    if incoming['within_batch_isomorphic_pairs'] or len(incoming['curves'])!=60:
        raise ArithmeticError('complete distinct60 point proof required')
...
def build(output):
    ...
    cert.write(output,{'schema':'elliptic-curves.new-high-rank-index.v19','sources':sources(),
...
def check(path):
    ...
    for r in data['curves']:
        ...
        seen.add(j)
        print('REPLAYED V19 INVENTORY',r['id'],'rank >=',len(points),flush=True)
    print('EXACT CURVE INVENTORY',len(seen),flush=True)

if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('--output',type=Path,default=ART/'new_high_rank_curve_index_v19.json');p.add_argument('--check',type=Path)
