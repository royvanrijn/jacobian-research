...
OLD=ART/'new_high_rank_curve_index_v17.json'
ADDITIONAL=ART/'broad60_mw16_results_v1.json'

def sources():
    paths=(Path(__file__).resolve(),Path(cert.__file__).resolve(),ROOT/'elliptic-curves/cas/memory_rank_certificate.py',ROOT/'elliptic-curves/cas/mod2_reduction_independence.py',ROOT/'elliptic-curves/cas/elliptic_candidate_record.py',ART/'broad60_mw16_point_portable_replay_v1.json')
    return {str(p.relative_to(ROOT)):cert.hashed(p) for p in paths}

def promotion_gate():
    base=ROOT/'artifacts/local/elliptic-curves/broad60-mw16-pari-v1'
    portable=cert.read(ART/'broad60_mw16_point_portable_replay_v1.json')
    ...
    if any(cert.hashed(ROOT/n)!=h for n,h in protocol['sources'].items()):
        raise ArithmeticError('fixed broad60_mw16 finalization sources differ')
    ...
    if ledger['status']!='PASS' or ledger.get('stronger_odd_prime_bounds')!=[] or [r['name'] for r in ledger['rows']]!=stages:
        raise ArithmeticError('complete broad60_mw16 proof and review of any stronger odd bounds required')
    ...
    aggregate=cert.read(ART/'broad60_mw16_experiment_v1.json')
    ...
    if any(cert.hashed(ROOT/n)!=h for n,h in aggregate['sources'].items()):
        raise ArithmeticError('broad60_mw16 aggregate inputs changed')
...
def build(output):
    ...
    cert.write(output,{'schema':'elliptic-curves.new-high-rank-index.v18','sources':sources(),
...
def check(path):
    ...
    for r in data['curves']:
        ...
        seen.add(j)
        print('REPLAYED V18 INVENTORY',r['id'],'rank >=',len(points),flush=True)
    print('EXACT CURVE INVENTORY',len(seen),flush=True)

if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('--output',type=Path,default=ART/'new_high_rank_curve_index_v18.json');p.add_argument('--check',type=Path)
