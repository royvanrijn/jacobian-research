...
D=ROOT/'artifacts/local/elliptic-curves/fresh60-mw16-point-portable-v1'
OUT=ART/'fresh60_mw16_point_portable_replay_v1.json'

def main():
    path=ART/'fresh60_mw16_point_evidence_v1.json';manifest=cert.read(path);workspace=D/'workspace';folder=D/'verification'
    if workspace.exists() or OUT.exists():raise FileExistsError('preserve isolated fresh60 point replay')
    ...
    p=cert.read(workspace/'artifacts/local/elliptic-curves/fresh60-mw16-pari-v1/protocol.json');jobs=[]
    if len(p['rows'])!=60:raise ArithmeticError('fixed60 roster required')
    for i,row in enumerate(p['rows']):
        jobs.append((row['id']+'-history',[str(cas/'fresh60_mw16_pari_batch.py'),'replay','--index',str(i)],600))
        for suffix,script in [('mod2','audit_recorded_point_mod2_rank_v3.py'),('modl','audit_retained_cloud_modl.py')]:
            proof=art/('fresh60_mw16_'+row['id'].replace('-','_')+'_'+suffix+'_v1.json')
            jobs.append((row['id']+'-'+suffix,[str(cas/script),'--check',str(proof)],300))
    jobs.extend([('geometry',[str(cas/'replay_fresh60_mw16_geometry.py')],1200),('points-and-equations',[str(cas/'certify_fresh60_mw16_results.py'),'--check'],1200)])
    ...
    for name,args,seconds in jobs:
        ...
        ok=s['outcome']=='completed' and s['returncode']==0;ledger['rows'].append({'name':name,'status':'PASS' if ok else 'FAILED_OR_CENSORED','supervision':s});checkpoint(folder/'ledger.json',ledger);print('ISOLATED FRESH60',name,ledger['rows'][-1]['status'],flush=True)
        if not ok:
    ...
    checkpoint(OUT,{'schema':'elliptic-curves.fresh60-mw16-point-portable-replay.v1','status':'PASS','logical_stages':len(jobs),'sources':{str(q.relative_to(ROOT)):cert.hashed(q) for q in paths},'archive_sha256':manifest['archive_sha256'],'ledger':ledger,'claim_boundary':manifest['claim_boundary']})
    print('ISOLATED FRESH60',len(jobs),'STAGES PASS',flush=True)
