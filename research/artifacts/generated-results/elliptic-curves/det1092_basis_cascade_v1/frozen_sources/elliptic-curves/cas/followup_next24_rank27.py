#!/usr/bin/env python3
"""One fixed301-chart adaptive attempt on the third independently certified new27 curve."""
import argparse,sys
...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';LOCAL=ROOT/'artifacts/local/elliptic-curves';D=LOCAL/'next24-rank27-adaptive-v1';SEED=LOCAL/'next24-r17-extended-pari-v1/11952-041/result.json';PROOF=ART/'next24_high_rank_minimal_proofs_v4.json';CENSUS=LOCAL/'compact-r17-fresh-generic-census-v1/11952/generic-census.json'

def sources():
    names=['memory_rank_certificate.py','followup_next24_rank27.py','prepare_next24_rank27_adaptive.sage','prepare_fresh_r17_pari_batch.sage','fresh_r17_pari_batch.py','prospective_half_lattice_v2.sage','research_runtime/rotated_observation_state.py','research_runtime/cached_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
...
def prepare():
    ...
    seed,census=cert.read(SEED),cert.read(CENSUS);proof=next(r for r in cert.read(PROOF)['curves'] if r['id']=='11952-041')
...
def launch():
    p=protocol()
    for name,cmd,seconds in [('maps',['/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python',str(CAS/'prepare_next24_rank27_adaptive.sage')],p['geometry_wall_seconds']),('worker',[sys.executable,str(Path(__file__).resolve()),'worker'],p['worker_wall_seconds'])]:
