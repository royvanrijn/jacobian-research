...
"""Finite specialized parity sample using only the own certified independent endpoint seed."""
import sys
...
def main():
    r=control.dimension();p=control.protocol();out=control.D/'maps.json'
    ...
    for i,mask in enumerate(control.masks(p)):
        residue=matrix(ZZ,1,r,[(mask>>j)&1 for j in range(r)]);target=[int(v)%2 for v in (residue*inverse).row(0)];norm,rep,error=oracle.solve(target);word=list(map(int,(matrix(ZZ,1,r,rep)*u).row(0)))
        if any((word[j]-(mask>>j))%2 for j in range(r)) or sum(word[j]*g[j,k]*word[k] for j in range(r) for k in range(r))!=norm:raise ArithmeticError('exact specialized parity/norm transport differs')
        sample.append({'parity':mask,'representative':word,'metric_norm':norm,'cvp_error':error,'reduced_representative':list(rep)})
        if (i+1)%127==0:checkpoint(out,data);print('SPECIALIZED SAMPLE',i+1,'of256',flush=True)
    data['centres']=sorted(sample,key=lambda c:(-c['metric_norm'],c['parity']))[:p['charts']];data['status']='RUNNING_MAPS';checkpoint(out,data);mapper.pari.allocatemem(256000000,silent=True)
    for c in data['centres']:data['rows'].append(mapper.mapping(model,points,c));checkpoint(out,data)
    data['status']='COMPLETE_DECLARED_MAPS';checkpoint(out,data);print('FROZEN ENDPOINT12 MAPS FROM256 MASKS',flush=True)
