...
import score_joint_mw16_higher as scores
...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in paths},'wait_seconds':24000,'stages':[('prepare',180),('run',2400),('check',600),('select',60),('selection_check',60),('validate',300),('validation-check',180)],'rss_bytes':2147483648,'scope':'Wait for all twenty untouched higher MW16 slices and81920 exact full3510-prime scores. Freeze10240 within-roster distinct equations,1024 per band/family, and pass first20 runtime gate, use four workers and80-row checkpoints, replay all scores, select six distinct curves per band/family and validate the frozen60 with disjoint primes. No point searches or automatic retries.'})
def launch():
    ...
    try:
        ...
        for name,seconds in p['stages']:
            ...
            ledger['rows'].append({'name':name,'status':'PASS' if ok else 'FAILED_OR_CENSORED','supervision':s});checkpoint(D/'ledger.json',ledger);print('MW16 HIGHER JOINT SCORE CONTROLLER',name,ledger['rows'][-1]['status'],flush=True)
