#!/usr/bin/env python3
"""One fixed301-chart adaptive attempt on the second independently certified new27 curve."""
import argparse,sys
...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';LOCAL=ROOT/'artifacts/local/elliptic-curves';D=LOCAL/'paired-second27-adaptive-v1';SEED=LOCAL/'fresh-r17-paired-pari-v1/11952-010/result.json';PROOF=ART/'paired_high_rank_minimal_proofs_v2.json';CENSUS=LOCAL/'compact-r17-fresh-generic-census-v1/11952/generic-census.json'

def sources():
    names=['followup_paired_second27.py','prepare_paired_second27_adaptive.sage','prepare_fresh_r17_pari_batch.sage','fresh_r17_pari_batch.py','prospective_half_lattice_v2.sage','research_runtime/rotated_observation_state.py','research_runtime/cached_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
...
def prepare():
    ...
    seed,census=cert.read(SEED),cert.read(CENSUS);proof=next(r for r in cert.read(PROOF)['curves'] if r['id']=='11952-010')
    ...
    p=proof['rank_certificate'];cert.checked_rank(tuple(map(cert.F,seed['curve'])),[tuple(map(cert.F,P)) for P in points],[r['prime'] for r in p['signatures']],p['no_rational_2_torsion_prime'])
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.paired-second27-adaptive.v1','sources':sources(),'seed_path':str(SEED.relative_to(ROOT)),'seed_sha256':cert.hashed(SEED),'proof_sha256':cert.hashed(PROOF),'generic_census_path':str(CENSUS.relative_to(ROOT)),'generic_census_sha256':cert.hashed(CENSUS),'generic_pool':[{'mask':r['mask'],'computed_generic_norm':r['norm']} for r in pool],'initial_rank':27,'generic_rank':17,'charts':301,'height':100000,'seconds_per_chart':5,'target_rank':28,'gp_sha256':cert.hashed(Path('/usr/bin/gp')),'geometry_wall_seconds':300,'worker_wall_seconds':900,'rss_bytes':1610612736,'maximum_workers':1,'gate':'The complete23-address paired batch produced the second new rank27 curve with independently replayed point-cloud rank, search histories, global minimality and no catalogue/prior equation match. Its generic17 search has discovered10 further certified directions. This one follow-up tests301 parity centres involving those directions, aiming for one more independently certified point.','centre_policy':'Take301 largest computed generic census norms, mask ties. Pair them cyclically with nonzero10-bit quotient words ordered by Hamming weight then integer. Compute384-bit numerical heights on the exact27-point subgroup and numerical CVP representatives in each declared parity; order descending numerical norm, generic mask, quotient word. These representatives are search choices, not specialized lattice-minimum certificates.','boundaries':'Only points discovered on this prospective curve enter centres. Freeze all rational maps before point searching. Preserve all histories and timeout outputs; no automatic continuation or rank upper bound. Stop at the first worker lower bound28 pending exact replay.'})
...
def replay():
    ...
    cert.checked_rank(model,state.basis,state.reductions.primes,state.no_two_torsion_prime);print('REPLAYED ADAPTIVE NEW27',len(data['charts']),'rank >=',state.rank,flush=True)

def launch():
    p=protocol()
    for name,cmd,seconds in [('maps',['/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python',str(CAS/'prepare_paired_second27_adaptive.sage')],p['geometry_wall_seconds']),('worker',[sys.executable,str(Path(__file__).resolve()),'worker'],p['worker_wall_seconds'])]:
