...
def main(path,directory):
    ...
    for idx,row in enumerate(data['rows']):
        ...
        delta=-16*(4*A**3+27*B**2);assert (A.degree(),B.degree(),delta.degree())==(8,12,20) and delta.gcd(A)==1
        ...
        # 4 kills every component group: all finite fibres are I1/I2 and
        # infinity is split I4. Compute heights without local node charts.
        four=[4*P for P in images]
        def scaled_height(P):
            if not P:return QQ(0)
            n,d=P[0].numerator(),P[0].denominator();return QQ(4+max(d.degree(),n.degree()-4))/16
        H=matrix(QQ,12)
        ...
        for p in (131,239,251):
            ...
            if (av.degree(),bv.degree(),dv.degree())!=(8,12,20) or dv.gcd(av)!=1:
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--input',type=Path,required=True);p.add_argument('--output-directory',type=Path,required=True);a=p.parse_args();main(a.input,a.output_directory)
