...
def replay(path, output):
    ...
    original = cert.read(ROOT/'artifacts/generated-results/elliptic-curves/new_high_rank_curve_index_v8.json')
    if data['targets'] != [{k:r[k] for k in ('id','curve','j_invariant','family','parameter','rank_lower_bound')} for r in original['curves'] if int(r['id'].split('-')[-1])>89]:
