...
"""Exact minimum audit on the172 computed-norm12 classes in the remaining four R17 censuses."""
import argparse
...
D=ROOT/'artifacts/local/elliptic-curves/r17-norm12-exact-minima-remaining4-v1'
...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.r17-norm12-minima.v1','sources':sources(),
        'families':['103b2','074d9','07ca9','08234'],'census_hashes':{f:cert.hashed(PARENT/f/'generic-census.json') for f in ('103b2','074d9','07ca9','08234')},
        'queries_per_family':43,'worker_wall_seconds':300,'worker_rss_bytes':1073741824,'maximum_workers':2,
        'gate':'Fresh numerical CVP censuses record43 norm12 representatives in each of these remaining four families. Check whether these queried representatives are actually minimal, after the first two families passed exact replay of their49 computed-norm12 candidates, before using the numerical geometry to schedule further work.',
        ...
        'claim_boundary':'Only172 queried parity minima, not all2^17 minima, the full covering radius, or any specialized rank. Censored queries remain UNKNOWN.'})
...
def run(family):
    ...
    if len(chosen)!=43:raise ArithmeticError('query roster changed')
