...
def main(index):
    ...
    geometry={'A_degree':int(A.degree()),'B_degree':int(B.degree()),'discriminant_degree':int(delta.degree()),'c4_discriminant_gcd_degree':int(delta.gcd(c4).degree()),'repeated_discriminant_degree':int(delta.gcd(delta.derivative()).degree())}
    print('GEOMETRY',geometry,flush=True)
    data['geometry']=geometry;checkpoint(out,data)
    if A.degree()>8 or B.degree()>12 or not delta:raise ArithmeticError('nonzero K3-sized Weierstrass model required')
    data.update(stage='specialized_subgroup',roots=list(map(str,roots)),A_coefficients=list(map(str,A.list())),B_coefficients=list(map(str,B.list())),quartic_coefficients=[list(map(str,q.list())) for q in quartic],extra_sections=extras,discriminant_degree=int(delta.degree()));checkpoint(out,data)
