#!/usr/bin/env python3
"""Five complete target-free MW16 projective caches for outer-region selection."""
import argparse,time
from pathlib import Path
from concurrent.futures import ThreadPoolExecutor
import benchmark_mw16_extended_projective_tables as benchmark
...
D=ROOT/'artifacts/local/elliptic-curves/mw16-extended-projective-caches-v1'
OUT=ART/'mw16_extended_projective_caches_v1.json'
def sources():
    paths=[Path(__file__).resolve(),benchmark.OUT,benchmark.D/'protocol.json']
...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.mw16-extended-projective-caches.v1','sources':sources(),'families':p['families'],'primes':p['full_prime_roster'],'gp_sha256':p['gp_sha256'],'gp_seconds_per_case':20,'rss_bytes':536870912,'maximum_workers':5,'checkpoint_block_size':80,'build_wall_seconds':7200,'replay_wall_seconds':3600,'reused':reused,'gate':'All five MW16 families pass the same fixed three-prime projective-table cost gate and75 independent character sums. Fresh outer-region discoveries include a26-point fibre beyond the signed512 short prefix; the11952 full-score benchmark independently measures366of512 longer-score leaders lost by a short4096 prefix. Complete height-independent projective tables remove repeated scalar finite-field work and enable scoring new higher parameter bands before short-score truncation. All five families receive equal treatment; no known record enters selection or execution.','scope':'Exactly all five existing MW16 atlas models, every prime4099through32749 and allp+1 raw projective residues. Reuse exactly15 benchmark tables and make at most14725 new bounded PARI calls, five workers and80-case checkpoints. Verify all frames/discriminants/Hasse bounds and five independent character sums per table. Replay all tables without new GP calls. No retry or larger prime range. This cache build includes no parameter scan, point search, new rank inference, restoration of bad raw-model reduction or catalogue input. A subsequent outer-parameter trial requires its own frozen protocol after this replay passes.'})
def protocol():
    ...
    try:
        with ThreadPoolExecutor(max_workers=p['maximum_workers']) as pool:
            for i in range(0,len(roster),p['checkpoint_block_size']):
                ...
                if len(ledger['rows'])%400==0:print('FIVE MW16 CACHE TABLES',len(ledger['rows']),'OF',len(roster),flush=True)
...
def check():
    ...
    print('FIVE COMPLETE MW16 CACHES REPLAY PASS',len(rows),flush=True)
