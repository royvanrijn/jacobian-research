...
"""Finite specialized parity sample using only the own certified19-point seed."""
import sys
...
import retained_native19_trial_v3 as control
import certify_compact_r17_candidates as cert
from research_runtime.store import checkpoint,digest
mapper=SourceFileLoader('specialized_mapper',str(CAS/'prepare_fresh_r17_pari_batch.sage')).load_module()
mapper.mapping=SourceFileLoader('factor_free_mapping_v1',str(CAS/'factor_free_pari_mapping.sage')).load_module().mapping
...
def main():
    ...
    for c in data['centres']:
        data['rows'].append(mapper.mapping(model,points,c));checkpoint(out,data);print('MAP',len(data['rows']),'of49',flush=True)
