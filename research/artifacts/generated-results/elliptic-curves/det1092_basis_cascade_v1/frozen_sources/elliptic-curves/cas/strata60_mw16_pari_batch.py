#!/usr/bin/env python3
"""Identical fixed point exposure for matched retained-score strata."""
import argparse,time
from pathlib import Path
from concurrent.futures import ThreadPoolExecutor,as_completed
import certify_compact_r17_candidates as cert
import compact_mw16_specialization as spec
import select_retained_mw16_score_strata as extension
...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';D=ROOT/'artifacts/local/elliptic-curves/strata60-mw16-pari-v1';SAGE='/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python'

def sources():
    names=['memory_rank_certificate.py','strata60_mw16_pari_batch.py','prepare_strata60_mw16_pari_batch.sage','prospective_half_lattice_v2.sage','compact_mw16_specialization.py','research_runtime/search_state.py','research_runtime/cached_observation_state.py','research_runtime/preloaded_prime_state.py','research_runtime/rotated_observation_state.py','research_runtime/pointed_orbit_compression.py','research_runtime/quotient_only_reduction.py','research_runtime/supervisor.py']
...
def expected_roster():
    ...
    if d['status']!='PASS_FROZEN60_MATCHED_SELECTION' or len(d['selected'])!=60:
        raise ArithmeticError('complete frozen matched60 roster required')
    return d['selected']

def freeze():
    if (D/'protocol.json').exists():raise FileExistsError('preserve matched point protocol')
    extension.completion_gate();extension.protocol()
    gate=cert.read(extension.D/'controller/ledger.json')
    if gate['status']!='PASS_FROZEN60_MATCHED_SELECTION':raise ArithmeticError('matching replay required')
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.strata60-mw16-pari.v1',
...
def protocol():
    ...
    if p['sources']!=sources() or any(p[k]!=cert.hashed(v) for k,v in bindings) or p['rows']!=expected_roster() or p['generic_masks']!=masks():raise ArithmeticError('frozen matched inputs differ')
    return p
...
def replay(index):
    ...
    memory_checked_rank(model,state.basis,state.reductions.primes,state.no_two_torsion_prime);print('REPLAYED STRATA60 MW16',p['rows'][index]['id'],len(data['charts']),'rank >=',state.rank,flush=True)
...
def maps():
    ...
    def one(i):
        ...
        supervision=supervise([SAGE,str(CAS/'prepare_strata60_mw16_pari_batch.sage'),'--index',str(i)],limits=Limits(p['map_wall_seconds'],p['rss_bytes']),log_path=folder/'maps.log',checkpoint_path=folder/'maps.supervisor.json',cwd=ROOT)
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        for f in as_completed(pending):
            i=pending[f];got[i]=f.result();ledger['rows']=[got[j] for j in sorted(got)];checkpoint(path,ledger);print('STRATA60 MAP',got[i]['id'],got[i]['status'],flush=True)
...
def batch():
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        for future in as_completed(futures):
            i=futures[future];r=future.result();ledger['rows'][i]=r;checkpoint(path,ledger);print('STRATA60 MW16 ATTEMPT',r['id'],r['status'],r.get('rank_lower_bound'),flush=True)
