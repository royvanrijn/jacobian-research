...
import certify_scaled13_24_r17_results as cohort
import followup_scaled13_rank25 as follow
from research_runtime.store import checkpoint
ROOT=follow.ROOT;ART=follow.ART;INPUT=ART/'scaled13_24_r17_results_v1.json';OUT=ART/'scaled13_25_followup_results_v1.json'
def sources():
...
def expected():
    old=cert.read(INPUT);follow.configure('11952-300');ledger=cert.read(follow.D/'replay.supervisor.json')
    if ledger['outcome']!='completed' or ledger['returncode']!=0:raise ArithmeticError('terminal exact adaptive history required')
    rows=[];inputs={str(INPUT.relative_to(ROOT)):cert.hashed(INPUT)};families={f['family']:f for f in cert.read(cohort.spec.ATLAS)['families']}
    for identifier in ('11952-300',):
        follow.configure(identifier);base=next(r for r in old['curves'] if r['id']==identifier);result=follow.D/'result.json';data=cert.read(result);cloudpath=ART/('scaled13_rank25_'+identifier.replace('-','_')+'_all_retained_mod2_v1.json');cloud=cert.read(cloudpath);ip=follow.D/'all-retained-point-cloud-only.json';v=follow.D/'cloud-verification-ledger.json'
    ...
    return {'schema':'elliptic-curves.higher25-followup-results.v1','status':'PASS','sources':sources(),'inputs':inputs,'curves':rows,'catalogue':old['catalogue'],'previous_equations':old['previous_equations'],'claim_boundary':'One already catalogue-unmatched curve, with exact point lower bounds from their initial and separately frozen301-centre follow-ups. New bounds do not create new curve IDs. Initial selector/exposure outcomes remain in the original cohort certificate. No exact rank, rank upper bound, saturation, conductor or universal novelty.'}
def main(check):
    result=expected()
    if check:
    ...
    print('EXACT HIGHER25 FOLLOWUP RESULTS',[(r['id'],r['rank_lower_bound']) for r in result['curves']],flush=True)
