...
def sources():
    names=['finish_nearcut60_mw16_experiment.py','nearcut60_mw16_pari_batch.py',
           'prepare_nearcut60_mw16_pari_batch.sage','verify_nearcut60_mw16_points.py',
           'report_nearcut60_mw16_experiment.py','replay_corrected60_mw16_geometry.py',
...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.nearcut60-mw16-controller.v1',
        'sources':sources(),'wait_seconds':100000,'rss_bytes':4294967296,
        'stages':[
            ['freeze','nearcut60_mw16_pari_batch.py',['freeze'],120],
            ['maps','nearcut60_mw16_pari_batch.py',['maps'],4000],
            ['baselines','nearcut60_mw16_pari_batch.py',['baselines'],4000],
            ['points','nearcut60_mw16_pari_batch.py',['batch'],20000],
            ['verify','verify_nearcut60_mw16_points.py',['all'],43000],
            ['report','report_nearcut60_mw16_experiment.py',[],180],
            ['report-check','report_nearcut60_mw16_experiment.py',['--check'],180]],
...
def launch():
    ...
    try:
        ...
        for label,script,args,seconds in p['stages']:
            ...
            print('NEARCUT60 CONTROLLER',label,ledger['rows'][-1]['status'],flush=True)
            if not ok:
                if label in ('maps','baselines'):
                    ...
                    for reporting_args in ([],['--check']):
                        ...
                        r=run([sys.executable,str(CAS/'report_nearcut60_mw16_experiment.py'),*reporting_args],limits=Limits(180,p['rss_bytes']),cwd=ROOT,log_path=D/(suffix+'.log'),checkpoint_path=D/(suffix+'.supervisor.json'))
