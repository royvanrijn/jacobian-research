...
D=LOCAL/'new-rank26-engine-comparison-v2';INPUT=LOCAL/'new-rank26-pari43-v1/candidate-00/result.json';MAPS=LOCAL/'new-rank26-pari-maps-v1/maps.json'
OUTPUT=ART/'new_rank26_engine_comparison_v2.json'
...
def run():
    ...
    for i,(row,modelmap) in enumerate(zip(source['charts'],maps['rows'])):
        ...
        xs={cert.F(line[2:]) for line in completed.stdout.splitlines() if line.startswith('X|')};hits={cert.F(int(a),int(b)) for a,b,_ in r['primitive_square_hits'] if int(b)!=0}
...
def check():
    ...
    for row in result['rows']:
        ...
        if expected!=row['affine_square_coordinates'] or xs!={cert.F(int(n),int(d)) for n,d,_ in r['primitive_square_hits'] if int(d)!=0}:raise ArithmeticError('hit set differs')
