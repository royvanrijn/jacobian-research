...
D=ROOT/'artifacts/local/elliptic-curves/det1092-reduced-seed-diagnosis-v1'
...
a=[val(v) for v in data['a_invariants']];E=EllipticCurve(QQ,a);points=[E([val(v) for v in P]) for P in data['basis_weierstrass_coordinates']];model=tuple(map(cert.F,a));xy=[tuple(map(cert.F,P.xy())) for P in points]
