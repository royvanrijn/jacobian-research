...
def expected():
    ...
    for manifest in population['shards']:
        ...
        for row in by_slice[shard['id']]:
            ...
            keys=('numerator','denominator','score_units','good_primes','extension_selection_units',
                  'extension_good','combined_selection_units','combined_good','displayed_score_units',
                  'displayed_good_primes','local_correction_units','restored_good_primes')
    ...
    return {'schema':'elliptic-curves.higher-mw16-model-and-retention-audit.v1',
        ...
        'variant':'corrected','models_checked':model_count,
...
if __name__=='__main__':
    ...
    output=ART/'corrected_higher_mw16_score_coverage_v1.json';data=expected()
    if a.check:
        if cert.read(output)!=json.loads(json.dumps(data)):raise ArithmeticError('corrected model/coverage proof replay differs')
    else:
        if output.exists():raise FileExistsError('preserve corrected model/coverage proof')
        checkpoint(output,data)
    print('CORRECTED HIGHER MW16 MODEL/RETENTION AUDIT',data['status'],data['models_checked'],data['distinct_j_invariants'],flush=True)
