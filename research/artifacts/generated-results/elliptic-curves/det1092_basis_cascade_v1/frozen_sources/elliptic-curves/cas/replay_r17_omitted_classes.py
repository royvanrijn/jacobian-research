#!/usr/bin/env python3
"""Exact replay of the48 omitted-class charts, orbit witnesses and point admissions."""
import argparse
...
def replay(path):
    ...
    entry=data['parent_input'];source_path=Path(__file__).resolve().parents[2]/entry['input_path']
    if cert.hashed(source_path)!=entry['input_sha256']:raise ArithmeticError('parent source differs')
    source=cert.read(source_path)
    if data['initial_state']!=source['final_state'] or data['metric_gram']!=source['metric_gram']:raise ArithmeticError('initial state or metric differs')
    if state.rank!=data['initial_dimension'] or initial_points[:17]!=points:raise ArithmeticError('generic prefix differs')
    if sorted(c['mask'] for c in data['centres'])!=sorted(protocol['classes'][data['family']]):raise ArithmeticError('omitted class roster differs')
    G=[[round(cert.F(x)*1000000) for x in row] for row in data['metric_gram']]
    if data['centres']!=sorted(data['centres'],key=lambda c:(-c['metric_norm'],c['mask'])):raise ArithmeticError('centre order differs')
    ...
    if data['status']=='COMPLETE_DECLARED_PILOT' and len(data['charts'])!=6:raise ArithmeticError('incomplete declared plan')
    print('REPLAYED OMITTED R17 CLASSES',data['family'],data['parameter'],'charts',len(data['charts']),'rank >=',state.rank,flush=True)
