#!/usr/bin/env python3
"""Exact V4 inventory/CSV replay with an explicit ephemeral finite cache."""
import argparse,csv,json
from pathlib import Path
import certify_compact_r17_candidates as cert
import export_new_high_rank_curve_index_v4 as original
...
def replay(path,output):
    ...
    for r in data['curves']:
        ...
        seen.add(j);ranks.append({'id':r['id'],'rank_lower_bound':len(points)});print('MEMORY REPLAY V4',r['id'],'rank >=',len(points),flush=True)
    ...
    checkpoint(output,{'schema':'elliptic-curves.inventory-v4-memory-replay.v1','status':'PASS','sources':sources,'curves_checked':len(seen),'rank_lower_bounds':ranks,'csv_checked':True,'claim_boundary':'Same original V4 extraction, source bindings, independent finite point certificates, catalogue exclusion and distinctness checks, with explicit MemoryFactStore for ephemeral exact finite computations. The original frozen exporter and its timed-out isolated attempt remain unchanged.'});print('EXACT MEMORY INVENTORY PASS',len(seen),'AND CSV',flush=True)
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--input',type=Path,default=ART/'new_high_rank_curve_index_v4.json');p.add_argument('--output',type=Path,required=True);a=p.parse_args();replay(a.input.resolve(),a.output.resolve())
