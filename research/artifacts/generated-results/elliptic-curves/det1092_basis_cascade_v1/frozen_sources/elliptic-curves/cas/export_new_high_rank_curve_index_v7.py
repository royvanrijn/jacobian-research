...
OLD=ART/'new_high_rank_curve_index_v6.json'
ADDITIONAL=ART/'retention24_r17_results_v1.json'
...
def build(output):
    ...
    cert.write(output,{'schema':'elliptic-curves.new-high-rank-index.v7','sources':sources(),
        ...
        'claim_boundary':'Exact independent-point lower bounds and absence from the pinned 586-equation catalogue. Existing local curve IDs are preserved. These are neither exact ranks nor a proof of universal novelty; the largest certified lower bound is reported by the actual point certificates.'})
...
def check(path):
    ...
    for r in data['curves']:
        ...
        seen.add(j)
        print('REPLAYED V7 INVENTORY',r['id'],'rank >=',len(points),flush=True)
    print('EXACT CURVE INVENTORY',len(seen),flush=True)

if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('--output',type=Path,default=ART/'new_high_rank_curve_index_v7.json');p.add_argument('--check',type=Path)
