...
def main():
 current=L/'prospective-factor-free-portfolio-v2';deadline=time.monotonic()+900
 while True:
  state=cert.read(current/'ledger.json')['status']
  if state=='PASS':break
  if state=='FAILED_OR_CENSORED' or time.monotonic()>deadline:raise ArithmeticError('current fixed portfolio did not complete')
  time.sleep(2)
 # One arithmetic stage at a time, after the existing search driver has reaped all children.
 import complete_factor_free_portfolio as complete
 complete.main()
 import report_factor_free_portfolio as report
 assert not report.OUT.exists();checkpoint(report.OUT,report.expected());print('PORTFOLIO report complete',flush=True)
 import det1092_point_pilot as pilot
 d=pilot.BATCH;assert not (d/'run-ledger.json').exists();ledger={'status':'RUNNING','stages':[]};checkpoint(d/'run-ledger.json',ledger)
 checkpoint(d/'run-protocol.json',dict(driver_sha256=cert.hashed(Path(__file__)),pilot_sha256=cert.hashed(CAS/'det1092_point_pilot.py'),maximum_workers=1,rss_bytes=2147483648,freeze_seconds=60,intake_seconds=180,seed_replay_seconds=120))
 ...
 for name,seconds in [('freeze',60),('intake',180)]:stage(name,[sys.executable,str(CAS/'det1092_point_pilot.py'),name],seconds)
 ...
 pilot.launch()
 import complete_det1092_point_pilot as proof
 proof.main()
