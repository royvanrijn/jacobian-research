...
def main():
    ...
    for row in p['rows']:
        ...
        if sorted(c['mask'] for c in maps['centres'])!=sorted(p['generic_masks'][row['family']]) or (len(data['charts'])!=len(maps['rows']) and not (data['status']=='TARGET_REACHED_PENDING_REPLAY' and data['rank_lower_bound']>=p['stop_rank'])):raise ArithmeticError('full generic chart roster differs')
        ...
        geometry(data,maps,initial,{**p,'maps_path':folder/'maps.json'});cloud_check(data,data['charts'],ART/('productfirst24_r17_'+row['id'].replace('-','_')+'_mod2_v1.json'),path);count+=len(data['charts'])
    if count>1080 or sum(len(p['generic_masks'][r['family']]) for r in p['rows'])!=1080:raise ArithmeticError('fixed1080 maximum chart roster differs')
    print('REPLAYED PRODUCTFIRST24 CHARTS',count,'GENERIC CENTRES, RAW POINTS AND CLOUD PROVENANCE',flush=True)
    print('Admission/archive histories passed separately and are retained.',flush=True)
