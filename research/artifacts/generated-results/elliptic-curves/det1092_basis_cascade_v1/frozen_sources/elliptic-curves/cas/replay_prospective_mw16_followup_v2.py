#!/usr/bin/env python3
"""Replay retained MW16 follow-ups with exact membership and quotient-only caches."""
import argparse
...
def replay(path):
    ...
    family=next(r for r in cert.read(spec.ATLAS)['families'] if r['fibration_id']==data['family'])
    family_check(data,model,points)
    words=sorted(range(1,1<<(dimension-16)),key=lambda w:(w.bit_count(),w))
    expected={(g['mask'],words[i%len(words)],g['mask']|(words[i%len(words)]<<16)) for i,g in enumerate(protocol['generic_pool'])}
    ...
    print('REPLAYED RETAINED MW16 FOLLOWUP',data['family'],data['parameter'],'charts',len(data['charts']),'rank >=',state.rank,flush=True)
