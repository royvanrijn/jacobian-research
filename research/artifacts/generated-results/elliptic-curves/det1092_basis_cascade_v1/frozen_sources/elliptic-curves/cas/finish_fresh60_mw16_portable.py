...
ROOT=batch.ROOT;CAS=batch.CAS;D=ROOT/'artifacts/local/elliptic-curves/fresh60-mw16-point-portable-controller-v1'

def prepare():
    if (D/'protocol.json').exists():raise FileExistsError('preserve portable controller')
    paths=[Path(__file__).resolve(),CAS/'package_fresh60_mw16_points.py',CAS/'verify_fresh60_mw16_points_portable.py',batch.D/'post-batch/protocol.json']
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.fresh60-mw16-portable-controller.v1','sources':{str(q.relative_to(ROOT)):cert.hashed(q) for q in paths},'wait_seconds':35000,'jobs':[{'name':'package','script':'package_fresh60_mw16_points.py','seconds':600},{'name':'isolated-replay','script':'verify_fresh60_mw16_points_portable.py','seconds':20000}],'rss_bytes':4294967296,'scope':'Wait for all fixed60 point searches, admission-history proofs and final exact geometry/mod2/mod3/mod5/equation comparisons to finish successfully. Require no unresolved odd-prime improvement. Then create one standalone point-only bundle and perform182 isolated replay stages. No scanner, score calculation, point search, adaptive follow-up or automatic retry.'})
...
def launch():
    ...
    for job in p['jobs']:
        ...
        ok=s['outcome']=='completed' and s['returncode']==0;ledger['rows'].append({'name':n,'status':'PASS' if ok else 'FAILED_OR_CENSORED','supervision':s});checkpoint(D/'ledger.json',ledger);print('FRESH60 PORTABLE',n,ledger['rows'][-1]['status'],flush=True)
        if not ok:
