#!/usr/bin/env python3
"""Compare exact affine square-hit sets on49 frozen PARI/GMP control boxes."""
...
D=LOCAL/'native11952-engine-comparison-v1';INPUT=LOCAL/'native11952-pari49-control-v1/candidate-00/result.json';MAPS=LOCAL/'native11952-pari-maps-v1/maps.json'
OUTPUT=ART/'native11952_engine_comparison_v1.json'
...
def run():
    ...
    result={'schema':'elliptic-curves.native11952-fixed-box-engine-comparison.v1','status':'RUNNING','protocol_hash':digest(protocol),'rows':[],'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in (Path(__file__).resolve(),INPUT,MAPS)}};checkpoint(OUTPUT,result)
...
def check():
    ...
    if len(result['rows'])!=49 or result['status']!='COMPLETE_DECLARED_COMPARISON':raise ArithmeticError('incomplete comparison')
