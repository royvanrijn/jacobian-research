...
"""Fixed512-target continuation on the certified37638 factor base."""
import argparse
import json
...
OUT = ART/'small_conductor_small_base_targets_v1.json'
AUDIT = ART/'small_conductor_small_base_relations_v1.json'
...
def selection():
    ...
    covered=set()
    small=cert.read(SMALL)
    section=next(r for r in small['records'] if r['cutoff']==BOUND)
    if not section['positive_margin_certified']: raise ArithmeticError('certified generation required')
    for r in section['relation_matrix']['supported_combinations']:
        covered.update(r['result_columns'])
    data,poly,nf,x,w=setup()
    a=int(data['integral_norm_generator']['fixed_a']); M=data['sl2_matrix']
    eligible=[]; seen=set()
    for i,col in enumerate(base['columns']):
        q=col['p']
        if i in covered or i in blocks or col['f']!=1 or col['e']!=1 or q<1000 or q>BOUND or q in seen: continue
        if int(nf.disc())%q==0 or int(data['defining_order_index'])%q==0 or a%q==0: continue
        eligible.append(i);seen.add(q)
    targets=[]
    if len(eligible)<TARGET_COUNT: raise ArithmeticError('too few untouched target ideals')
    for k in range(TARGET_COUNT):
        column=eligible[k*(len(eligible)-1)//(TARGET_COUNT-1)]
...
def prepare():
    ...
    data=selection()
    sources=[SOURCE,BASE,SMALL,batch.forms.OUT,Path(batch.__file__).resolve(),ROOT/'elliptic-curves/cas/audit_bnf_free_s_class_quotient.py']
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.small-conductor-small-base-targets-protocol.v1',
        'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in sources},
        'selection':data,'coefficient_box':64,'smooth_bound':BOUND,'workers':1,'rss_bytes':1610612736,
        'stage_seconds':{'worker':300,'check':300,'audit':180},
        'gate':'The explicit-formula certificate proves generation at37638 under GRH. The33 valid supported rows leave dimension3210. Target512 untouched small-base directions to measure independent row gain, using only fully small-base-smooth values.',
        'selection_rule':'Exclude canonical pivots and covered columns; choose one unramified degree-one ideal per rational prime between1000and37638, excluding index/discriminant/fixed-a divisors. Select512 equally spaced indices in ascending existing-column order, including endpoints.',
    ...
    print('PREPARED512 SMALL-BASE TARGETS',data['targets'][0]['p'],data['targets'][-1]['p'],flush=True)
...
def calculate(check=False):
    ...
    result={'schema':'elliptic-curves.small-conductor-small-base-targets.v1','status':'PASS','protocol':p,
...
def audit(check=False):
    ...
    prior=runpy.run_path(str(ROOT/'elliptic-curves/cas/certify_small_conductor_smaller_base_v2.sage'))
    small=prior['expected']()
    if small!=cert.read(SMALL): raise ArithmeticError('smaller-base proof replay failed')
    base=cert.read(BASE)
    section=next(r for r in small['records'] if r['cutoff']==BOUND)
    if not section['positive_margin_certified']: raise ArithmeticError('generation certificate missing')
    ...
    old=[sum(1<<c for c in r['result_columns']) for r in section['relation_matrix']['supported_combinations']]
    blocks={max(r['parity_columns']):set(r['parity_columns']) for r in base['canonical_rational_relations']}
    old_rank=packed_rank(old)
    if old_rank!=33: raise ArithmeticError('baseline small-base rank differs')
    free_columns=section['relation_matrix']['free_columns_before_norm_relations']
    new=[];relations=[];hit=set();projections=[]
    ...
    for record in summary['smooth_records']:
        ...
        for j in range(factor.nrows()):
            ...
            if e<0 or int(ideal[0])>BOUND: raise ArithmeticError('relation outside certified small base')
            row.append([column,e])
        ...
        new.append(sum(1<<c for c in sparse))
    ...
    gain=packed_rank(old+new)-old_rank;dimension=free_columns-old_rank-gain
    unique={(m,n) if (n>0 or n==0 and m>0) else (-m,-n) for m,n in [(r['m'],r['n']) for r in relations]}
    sources=[SOURCE,BASE,SMALL,OUT,D/'scalar_replay.json',D/'protocol.json']
    result={'schema':'elliptic-curves.small-conductor-small-base-relations.v1','status':'PASS',
            ...
            'previous_quotient_dimension':free_columns-old_rank,'initial_supported_rank':old_rank,'final_supported_rank':old_rank+gain,'new_quotient_dimension':dimension,
