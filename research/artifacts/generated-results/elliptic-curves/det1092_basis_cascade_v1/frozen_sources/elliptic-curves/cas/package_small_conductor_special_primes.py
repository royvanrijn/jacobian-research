#!/usr/bin/env python3
"""Package the64-target special-prime trial and replay its complete relation audit."""
...
PREFIX = 'small_conductor_special_primes'
...
def package():
    ...
    result = ART/'small_conductor_special_prime_relations_v1.json'
    if cert.read(result)['status']!='PASS':
        raise ArithmeticError('matrix certificate required')
    base = ART/'small_conductor_norm_batch_evidence_v1.zip'
    ...
    files = {Path(__file__).resolve(),Path(batch.__file__).resolve(),ART/(PREFIX+'_v1.json'),result,
    ...
    checkpoint(MANIFEST,{'schema':'elliptic-curves.small-conductor-special-primes-evidence.v1',
                         'zip_sha256':cert.hashed(ZIP),'zip_bytes':ZIP.stat().st_size,
                         'files':sorted(entries,key=lambda r:r['path']),
                         'requirements':'Sage with PARI2.17.3. All scalar smoothness replay records are included. Isolated replay rebuilds the original factor base, deterministic target selection, lattice maps and all new principal-ideal relations; it does not repeat the already completed scalar smoothness run.'})
...
def replay():
    ...
    destination=D.with_name('small-conductor-special-primes-isolated-v1')
    ...
    command=['/home/royvanrijn/.local/bin/sage','-python','elliptic-curves/cas/target_small_conductor_prime_ideals.sage','audit-check']
    ...
    checkpoint(REPORT,{'schema':'elliptic-curves.small-conductor-special-primes-isolated.v1',
                       'status':'PASS' if passed else 'FAILED_OR_CENSORED',
                       'zip_sha256':manifest['zip_sha256'],'supervision':outcome})
    print('ISOLATED MATRIX REPLAY','PASS' if passed else 'FAILED_OR_CENSORED',flush=True)
