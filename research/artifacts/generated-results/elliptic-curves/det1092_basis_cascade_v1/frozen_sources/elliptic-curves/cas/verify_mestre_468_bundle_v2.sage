...
"""Standalone V2 replay, including direct generic halving on all six parents.
...
def main(path):
    ...
    for row in data['rows']:
        ...
        assert A[8]!=0 and 4*A[8]**3+27*B[12]**2==0
        infinity_node=-3*B[12]/(2*A[8]);assert (3*infinity_node).is_square()
        ...
        generic_cloud=[P[0]]
        for point in P[1:]:
            halves=(point-P[0]).division_points(2)
            assert len(halves)==1 and 2*halves[0]==point-P[0]
            generic_cloud.append(halves[0])
        ...
        assert all(point[0](1)==scale**2*Q[0] and point[1](1)==scale**3*Q[1] for point,Q in zip(generic_cloud,cloud))
