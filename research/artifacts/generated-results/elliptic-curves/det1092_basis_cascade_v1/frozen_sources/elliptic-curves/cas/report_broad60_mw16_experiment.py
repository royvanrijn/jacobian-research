...
ROOT=batch.ROOT;ART=batch.ART;D=batch.D;CONTROL=ROOT/'artifacts/local/elliptic-curves/broad60-mw16-controller-v1'
INPUT=ART/'broad60_mw16_results_v1.json';OUT=ART/'broad60_mw16_experiment_v1.json'
def expected():
    ...
    return {'schema':'elliptic-curves.broad60-mw16-experiment.v1','status':'PASS_STRONGER_ODD_BOUNDS_REQUIRE_REVIEW' if stronger else 'PASS',
        ...
        'claim_boundary':'320 untouched signed denominator slices in16384<H<=65536 and65536<H<=262144, disjoint from the earlier outer cohort, five MW16 families with equal retention and point budgets. All10240 retained extended scores precede60 distinct finalists; wholly disjoint validation never changes selection. Up to2580 generic16-only point charts with all maps before points. Catalogue and1225 prior-equation comparison follows terminal exact point proofs. Matching finite bounds do not prove exact rank, saturation or absence. No universal novelty, rank density, calibrated sensitivity or isolated portability claim.'}
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--check',action='store_true');a=p.parse_args();d=expected()
    if a.check:
        if cert.read(OUT)!=json.loads(json.dumps(d)):raise ArithmeticError('broad60 aggregate differs')
    else:
        if OUT.exists():raise FileExistsError('preserve broad60 aggregate')
        checkpoint(OUT,d)
    print('BROAD60 OUTCOMES',d['status'],d['completed_point_boxes'],[(r['id'],r['rank_lower_bound']) for r in d['rows'] if r['rank_lower_bound']>=22],flush=True)
