#!/usr/bin/env python3
"""Export independently rechecked mod2 point bases for the completed matched cohort."""
...
ROOT=batch.ROOT;ART=batch.ART;OUT=ART/'strata60_mw16_results_v1.json'
NOVELTY=ART/'strata60_mw16_novelty_v1.json'
...
def expected():
    gate=cert.read(batch.extension.D/'novelty-controller/ledger.json')
    ...
    if novelty['status']!='PASS' or report['status']!='COMPLETE_FIXED_COMPARISON' or len(novelty['rows'])!=60 or novelty['within_cohort_isomorphic_pairs']:raise ArithmeticError('complete distinct certified cohort required')
    ...
    return {'schema':'elliptic-curves.strata60-results.v1','status':'PASS','sources':sources(),'point_bindings':bindings,
        ...
        'claim_boundary':'All60 matched retained-score curves have rechecked independent mod2 point bases, exact generic transports and post-terminal Q-isomorphism comparisons with593 pinned catalogue and1345 prior equations. All odd bounds are covered by the exported mod2 bounds. This export follows the completed fixed experiment; it changes no allocation, score, budget or policy criterion. No exact rank or universal novelty.'}
...
if __name__=='__main__':
    ...
    if a.check:
        ...
        if OUT.exists():raise FileExistsError('preserve matched point export')
        cert.write(OUT,d)
    print('STRATA60 EXACT POINT EXPORT PASS',len(d['curves']),flush=True)
