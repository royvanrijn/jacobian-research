...
Versions1 and2 remain untouched. This version uses the explicit memory finite cache for the final standalone certificates; returned arithmetic and proof schema are unchanged apart from this version/source binding.
"""
import argparse
...
def sources():
    paths=[Path(__file__).resolve(),Path(cert.__file__).resolve(),Path(memory.__file__).resolve()]
...
def build(path,output,bound,expected_hash):
    ...
    result={'schema':'elliptic-curves.recorded-point-mod2-rank.v3','sources':sources(),'input_path':str(path.resolve().relative_to(ROOT)),
    ...
    proof=memory.checked_rank(model,selected,primes,data['final_state']['state']['no_two_torsion_prime'])
...
def check(path):
    ...
    old=data['rank_certificate'];actual=memory.checked_rank(model,[points[i] for i in indices],[r['prime'] for r in old['signatures']],old['no_rational_2_torsion_prime'])
