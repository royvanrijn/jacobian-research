...
import annulus11952_masked_controls_v2 as run
...
def main():
    ...
    for row in p['rows']:
        ...
        if state.rank!=17 or tuple(tuple(map(cert.F,P)) for P in state.basis)!=generic:raise ArithmeticError('all17 generic points must independently certify before masking')
