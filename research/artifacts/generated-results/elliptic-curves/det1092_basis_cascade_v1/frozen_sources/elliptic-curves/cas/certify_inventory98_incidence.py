...
INDEX=ART/'new_high_rank_curve_index_v8.json'
...
def result():
    ...
    return {'schema':'elliptic-curves.inventory98-incidence.v1','status':'PASS','sources':sources,'targets_checked':98,'pairs_checked':1176,'status_counts':dict(counts),'same_generic_subgroup_duplicates':duplicates,'claim_boundary':'Combines the six separately exact-replayed rational j-incidence cohorts and the exact Q(t) unimodular17-section transport. Every additional recorded presentation is the same generic subgroup. The separate incidence/transport replays remain required; this summary does not replay ranks or exclude other families, nongeneric points or higher ranks.'}
