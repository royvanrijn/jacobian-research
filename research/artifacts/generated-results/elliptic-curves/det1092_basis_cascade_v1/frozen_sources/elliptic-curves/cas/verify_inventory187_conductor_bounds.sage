...
"""Independent PARI Tate replay of the187-curve Sage generic local audit."""
...
INPUT=ART/'inventory187_conductor_bounds_v2.json'
OUT=ART/'inventory187_conductor_bounds_pari_replay_v1.json'
...
def expected():
    ...
    inventory=cert.read(ART/'new_high_rank_curve_index_v14.json')['curves']
    if len(inventory)!=187 or [r['id'] for r in inventory]!=[r['id'] for r in d['rows']]:raise ArithmeticError('whole inventory required')
    counts=[]
    ...
    single=cert.read(ART/'full11952_conductor_bound_v1.json')
    new=next(r for r in d['rows'] if r['id']==single['inventory_id'])
    if single['minimal_model']!=new['integral_curve'] or single['conductor_upper_bound']!=new['conductor_upper_bound']:
        raise ArithmeticError('separate new27 conductor certificate differs')
    return {'schema':'elliptic-curves.inventory187-conductor-pari-replay.v1','status':'PASS',
        'sources':{str(p.relative_to(ROOT)):cert.hashed(p) for p in (Path(__file__).resolve(),INPUT,ART/'full11952_conductor_bound_v1.json')},
        'pari_version':str(pari.version()),'curves':187,'local_pairs':sum(r['local_pairs'] for r in counts),'rows':counts,
        'claim_boundary':'Independent PARI elllocalred agrees with every one of the Sage generic Tate local results and reconstructs all187 conductor upper bounds. The new27 separate bound agrees. This does not factor the residual cofactors, verify external catalogue conductors, reprove ranks or assert conductor records.'}
