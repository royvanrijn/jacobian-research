"""Independent same-parent replay. Reuses the verified V3 landscape checker.

No point search is invoked. Original 302 and warm-11952 checkers/artifacts are
not edited. Numeric traversal, exact historical MWState binding and mod-2/3/5
checks are retained, including no-gain and censored epochs.
"""
...
def replay_case(ctx):
    ...
    result = {'schema':'det1092-v3-independent-replay.v1','case':ctx.case,'sources':ctx.sources,
              'protocol_sha256':sha(folder/'protocol.json'),'terminal_sha256':sha(folder/'replay-M17/terminal.json'),
              'rank_lower_bound':state.rank,'stages':reports,'charts':terminal['charts'],'claim_boundary':c.CLAIM}
    ...
    verified = {'schema':'det1092-v3-result.v1','status':'PASS_INDEPENDENT_DET1092_REPLAY',
        'case':ctx.case,'parameter':read(folder/'seed-input.json')['parameter'],'initial_rank':17,
        'rank_lower_bound':state.rank,'gain':state.rank-17,'stop_reason':terminal['stop_reason'],
