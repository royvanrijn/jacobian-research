...
ROOT=Path(__file__).resolve().parents[2];CAS=ROOT/'elliptic-curves/cas';ART=ROOT/'artifacts/generated-results/elliptic-curves';BATCH=ROOT/'artifacts/local/elliptic-curves/prospective-factor-free-portfolio-v1';SAGE='/home/royvanrijn/.local/share/jacobian-sage-10.9/bin/python'
def sources():
 return {**engine.sources(),**{str((CAS/n).relative_to(ROOT)):cert.hashed(CAS/n) for n in ['prospective_factor_free_portfolio.py','prepare_factor_free_portfolio.sage']}}
def freeze():
 ...
 for r in retained27:
  prior=next(a for a in audit['rows'] if a['id']==r['id']);folder=ROOT/prior['folder'];seedpath=folder/'seed.json';oldmaps=folder/'maps.json';newmaps=ROOT/'artifacts/local/elliptic-curves/factor-free-rank27-box-audit-v1'/r['id']/'maps.json';paths += [seedpath,oldmaps,newmaps]
  assert prior['counts']=={'PROVED_NEW_COORDINATE_EXPOSURE':49}
  add(r['id'],cert.read(seedpath),'X948','all retained unmatched rank27',dict(previous_maps=str(oldmaps.relative_to(ROOT)),factor_free_maps=str(newmaps.relative_to(ROOT))))
 ...
 checkpoint(BATCH/'protocol.json',dict(schema='elliptic-curves.prospective-factor-free-portfolio.v1',sources=sources(),inputs=inputs,rows=rows,sample_size=2048,sample_domain='full11952-specialized-followup-v1',charts=49,height=125000,seconds_per_chart=10,rank_stop=False,target_rank=32,maximum_point_boxes=539,geometry_wall_seconds=180,worker_wall_seconds=600,replay_wall_seconds=300,rss_bytes=2147483648,maximum_workers=1,gp_sha256=cert.hashed(Path('/usr/bin/gp')),
 gate='Full49 original27-only factor-free blind control recovered28 and passed independent exact mod2/3/5, point and map replays. Seven retained27 factor-free49 rosters have proved new finite-coordinate exposure. Two unexposed26 curves are selected by existing training scores. Two new-parent fibres use improved own14/12 subgroups outside their earlier centre spans.',
...
def launch():
 ...
 for i,row in enumerate(p['rows']):
  configure(i);s=run([SAGE,str(CAS/'prepare_factor_free_portfolio.sage'),'--index',str(i)],limits=Limits(p['geometry_wall_seconds'],p['rss_bytes']),log_path=D/'maps.log',checkpoint_path=D/'maps.supervisor.json',cwd=ROOT)
