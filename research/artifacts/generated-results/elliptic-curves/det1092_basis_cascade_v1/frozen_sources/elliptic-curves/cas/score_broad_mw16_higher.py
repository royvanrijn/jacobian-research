...
import scan_broad_mw16_higher_annuli as scan
...
D=ROOT/'artifacts/local/elliptic-curves/broad-mw16-higher-scores-v1'
OUT=ART/'broad_mw16_higher_selection_v1.json'
    ...
    for band in (2,3):
        for family in sorted(families):
            ...
            for i,r in enumerate(candidates):
                row={**r,'id':f'broad-b{band}-{family}-{i:04}','retained_index':i,'model':models.model_at(families[family],r['parameter'])}
...
def prepare():
    ...
    checkpoint(D/'protocol.json',{'schema':'elliptic-curves.mw16-broad-higher-scores.v1','sources':sources(),'rows':rows,'prospective_skips':skips,
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=p['maximum_workers']) as pool:
            for start in range(p['cost_gate_rows'],len(p['rows']),p['checkpoint_block']):
                ...
                if len(result['rows'])%800==20:print('MW16 BROADER TRACE PROGRESS',len(result['rows']),flush=True)
...
def check():
    ...
    print('REPLAYED ALL10240 MW16 BROADER SCORES',flush=True)
...
def validate(check=False):
    ...
    print('MW16 BROADER FINALISTS FRESH VALIDATION60 PASS',flush=True)
