#!/usr/bin/env python3
"""Fail-closed certified discovery yield, completed exposure and retained discovery accounting."""
import argparse
from collections import Counter
from pathlib import Path
import nearcut60_mw16_pari_batch as batch
import verify_nearcut60_mw16_points as proof
import certify_compact_r17_candidates as cert
from research_runtime.store import checkpoint
ROOT=batch.ROOT; D=batch.D; ART=batch.ART
OUT=ART/'nearcut60_mw16_experiment_v1.json'
ARMS=('nearcutoff',)
...
def expected():
    ...
    for selected in p['rows']:
        ...
        rows.append(row)
    if len(rows)!=60 or Counter(r['arm'] for r in rows)!=Counter({a:60 for a in ARMS}):raise ArithmeticError('complete fixed allocation differs')
    return {'schema':'elliptic-curves.retained-nearcutoff-experiment.v1',
        'status':'COMPLETE_FIXED_RETAINED_TRIAL' if all(r['certified_gain'] is not None for r in rows) else 'COMPLETE_ACCOUNTING_WITH_UNRESOLVED_OUTCOMES',
        ...
        'family_band_blocks':{b:[r['id'] for r in rows if r['block']==b] for b in sorted({r['block'] for r in rows})},
...
if __name__=='__main__':
    ...
    if a.check:
        ...
        checkpoint(OUT,data)
    print('RETAINED NEARCUTOFF TRIAL',data['status'],data['totals']['certified_gain_sum_known'],flush=True)
