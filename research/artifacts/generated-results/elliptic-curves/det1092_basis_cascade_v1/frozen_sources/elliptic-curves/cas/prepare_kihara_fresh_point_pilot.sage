...
def maps(index):
    control.configure(index);p=control.protocol();out=control.D/'maps.json';rank=14
    ...
    for i,mask in enumerate(control.masks(p)):
        ...
        if (i+1)%511==0:checkpoint(out,data);print('PARITIES',i+1,'of2048',flush=True)
    ...
    print(control.ROW['id'],'FROZEN49 MAPS FROM2048 PARITIES',flush=True)
