...
their other minimum rows remain `UNKNOWN`.  For the seventeen selected `11952` products, the later
[arithmetic rank-zero closure](R17_PRODUCT_REGULATOR_OBSTRUCTION_SWEEP_2026-09-05.md)
gives `A-=Gamma_d=H_d=0`; their full Tate quotients are no longer unknown.
This does not compute full two-Selmer groups or settle quotients on other
products. The earlier zero-class certificate remains a separate, weaker
result.
