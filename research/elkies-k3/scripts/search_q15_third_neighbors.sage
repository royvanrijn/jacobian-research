...
ap=argparse.ArgumentParser(description="Low-q third-generation neighbor search from q=15 MW4 node.")
...
FRAME=Path("elkies-k3/data/fibrations/q15_mw4_frame.txt")
M=matrix(ZZ,[[ZZ(x) for x in ln.split()] for ln in FRAME.read_text().splitlines() if ln.strip()])
assert M.det()==948
U=matrix(ZZ,[[0,1],[1,0]])
NS=block_diagonal_matrix(U,-M)

print(f"Q15NBR|stage=start|q={a.qmin}..{a.qmax}|det={M.det()}",flush=True)
...
def vectors_of_qnorm(n):
    ...
    rec(dim-1,RR(0))
    print(f"Q15NBR|stage=enumerate|q={n}|pairs={len(found)}",flush=True)
    return found
...
for q in range(a.qmin,a.qmax+1):
    for v in vectors_of_qnorm(q):
        for aa in divisors(q):
            ...
            if f*NS*f!=0 or primitive_div(f)!=1:
                continue
            P=build_frame(f)
            if P is None:
                continue
            ...
            try:
                key=(rr,rc,rd,tuple(P.LLL_gram().list()))
            except Exception:
                key=(rr,rc,rd,tuple(P.list()))
            if key in seen:
                continue
            seen.add(key)
            ...
            if mw<=4:
                print(f"Q15NBR|cand={len(res)}|q={q}|ab={aa},{bb}|root_rank={rr}|roots={rc}|rootdet={rd}|MW={mw}",flush=True)

res.sort(key=lambda z:(z[0],-z[2],z[3],z[4]))
print(f"Q15NBR|stage=summary|unique={len(res)}",flush=True)

for i,z in enumerate(res[:a.report],1):
    mw,rr,rc,rd,q,aa,bb,v,P=z
    print(f"Q15NBR_BEST|rank={i}|MW={mw}|root_rank={rr}|roots={rc}|rootdet={rd}|q={q}|ab={aa},{bb}|v={v}",flush=True)
    print(f"Q15NBR_BEST|frame_gram={P}",flush=True)

out=Path("artifacts/local/elkies-k3/q15-third-neighbors.txt")
...
print(f"Q15NBR|stage=done|out={out}",flush=True)
