...
"""Scan every eligible residual-class fixed-coordinate shell in N(2A9+D6).

For all three nonidentity classes of the exact cyclic order-four residual
group, test every rank-seven coordinate direct summand of a pinned LLL basis
of the primitive fixed lattice. Apply the determinant-5000,
...
def build(catalog, residual, determinant_bound, minimum_mw_rank):
    ...
    ] == [("2A9_D6-C02", 0), ("2A9_D6-C03", 32), ("2A9_D6-C04", 32)]
    assert class_scans[1]["accounting"][
        "accepted_seed_mw_rank_distribution"
    ] == {"13": 9, "15": 19, "17": 4}
    assert class_scans[2]["accounting"][
        "accepted_seed_mw_rank_distribution"
    ] == {"13": 9, "15": 19, "17": 4}
    return {
        "schema": "elkies-k3.2a9-d6-fixed-coordinate-shell-probe.v1",
        "status": "PASS_EXACT_PRE_RESIDUAL_QUOTIENT_2A9_D6_COORDINATE_SHELL_SCAN",
        "proof_scope": {
            "proved": (
...
def main():
    ...
    if arguments.check:
        if not output.exists() or output.read_text() != encoded:
            raise SystemExit("2A9+D6 coordinate-shell probe is stale")
    else:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(encoded)
    print(
        "2A9_D6_SHELL|classes={}|subsets={}|seeds={}|status=PASS_EXACT".format(
