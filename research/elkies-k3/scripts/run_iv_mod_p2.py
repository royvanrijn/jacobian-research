...
O=Path("artifacts/local/elkies-k3/iv-mod-p2"); O.mkdir(parents=True,exist_ok=True)
stem=f"p{a.p}-seed{a.seed}"
inp=O/(stem+".ms"); out=O/(stem+".solve"); log=O/(stem+".log")

r=subprocess.run([
    "sage","elkies-k3/scripts/export_iv_mod_p2_system.sage",
...
try:
    ...
    print(f"IVMOD|stage=solve|exit={rr.returncode}|seconds={time.time()-t0:.2f}",flush=True)
    if out.exists():
        print("IVMOD_SOLVE|"+out.read_text(errors="replace")[-4000:].replace("\n"," | "),flush=True)
except subprocess.TimeoutExpired:
    print(f"IVMOD|stage=solve|status=TIMEOUT|seconds={time.time()-t0:.1f}",flush=True)
