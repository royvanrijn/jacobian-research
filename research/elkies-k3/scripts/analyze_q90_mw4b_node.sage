...
print(f"MW4BNODE|stage=input|rank={F.rank()}|det={F.det()}|positive={F.is_positive_definite()}",flush=True)
...
print(f"MW4BNODE|stage=roots|half={len(half)}|signed={len(roots)}|rank={R.rank()}|det={abs(G.det())}",flush=True)

def ade_solutions(rank,nroots,det):
    atoms=[]
    for n in range(1,18):
        atoms.append((f"A{n}",n,n*(n+1),n+1))
    for n in range(4,18):
        atoms.append((f"D{n}",n,2*n*(n-1),4))
    for n,nr,d in [(6,72,3),(7,126,2),(8,240,1)]:
        atoms.append((f"E{n}",n,nr,d))
    sols=[]
    def rec(start,rleft,nleft,dleft,cur):
        if rleft==0:
            if nleft==0 and dleft==1:
...
print(f"MW4BNODE|stage=ADE|solutions={sols}",flush=True)
...
print(f"MW4BNODE|stage=shioda|MW_rank={mw_rank}|rootdet={rootdet}|regulator_if_torsion1={reg_t1}",flush=True)

out=Path("artifacts/local/elkies-k3/q90-mw4b-node.txt")
out.parent.mkdir(parents=True,exist_ok=True)
out.write_text(
    f"ADE={sols}\nMW_rank={mw_rank}\nrootdet={rootdet}\nregulator_t1={reg_t1}\n"
    f"root_basis=\n{B}\nroot_gram=\n{G}\n"
)
print(f"MW4BNODE|stage=done|out={out}",flush=True)
