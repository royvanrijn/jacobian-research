...
"""Scan every eligible residual-class fixed-coordinate shell in N(6D4).

For each nonidentity matrix conjugacy class of the exact hexacode residual
...
def build(catalog, residual, determinant_bound, minimum_mw_rank):
    ...
    assert residual["schema"] == (
        "elkies-k3.6d4-hexacode-residual-group.v1"
    )
    assert residual["status"] == (
        "PASS_EXACT_6D4_HEXACODE_AND_RESIDUAL_GROUP"
    )
    assert residual["residual_group"]["order"] == 2160
    ambient = matrix(
        ZZ,
        next(
            row["gram"]
    ...
    return {
        "schema": "elkies-k3.6d4-fixed-coordinate-shell-probe.v1",
        "status": "PASS_EXACT_PRE_RESIDUAL_QUOTIENT_6D4_COORDINATE_SHELL_SCAN",
        "proof_scope": {
            "proved": (
...
def main():
    ...
    payload["inputs"] = {
        ...
        str(SCAN_COMMON_SOURCE.resolve().relative_to(ROOT)): digest(
            SCAN_COMMON_SOURCE
        ),
    ...
    if arguments.check:
        if not output.exists() or output.read_text() != encoded:
            raise SystemExit("6D4 coordinate-shell probe is stale")
    else:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(encoded)
    print(
        "6D4SHELL|classes={}|subsets={}|seeds={}|status=PASS_EXACT".format(
