...
"""Canonicalize the exact N(2A9+D6) fixed-coordinate shell scan.

Quotient every accepted coordinate seed by the complete cyclic order-four
...
PROBE = (
    ROOT
    / "artifacts/generated-results/elkies-k3-2a9-d6-fixed-coordinate-shell-probe-v1.json"
)
DEFAULT_OUTPUT = (
    ROOT
...
def canonicalize(catalog, residual, probe):
    ...
    assert residual["schema"] == "elkies-k3.2a9-d6-residual-group.v1"
    assert residual["status"] == "PASS_EXACT_2A9_D6_GLUE_AND_RESIDUAL_GROUP"
    assert probe["schema"] == "elkies-k3.2a9-d6-fixed-coordinate-shell-probe.v1"
    assert probe["status"] == (
        "PASS_EXACT_PRE_RESIDUAL_QUOTIENT_2A9_D6_COORDINATE_SHELL_SCAN"
    )
    ...
    assert residual["residual_group"]["order"] == len(element_rows) == 4
        ...
        group.append(
            {
                ...
                "a9_component_permutation_zero_based": element[
                    "a9_component_permutation_zero_based"
                ],
                "a9_component_diagram_variant_indices": element[
                    "a9_component_diagram_variant_indices"
                ],
                "d6_diagram_variant_index": element["d6_diagram_variant_index"],
    ...
    for orbit_index, orbit in enumerate(raw_orbits):
            ...
            stabilizer.append(
                {
                    ...
                    "a9_component_permutation_zero_based": group_row[
                        "a9_component_permutation_zero_based"
                    ],
                    "a9_component_diagram_variant_indices": group_row[
                        "a9_component_diagram_variant_indices"
                    ],
                    "d6_diagram_variant_index": group_row[
        ...
        orbit_records.append(
            {
                "orbit_id": f"2A9D6-CF-O{orbit_index + 1:04d}",
    ...
    surfaces = surface_rows("2A9_D6", orbit_records)
    ...
    return {
        ...
        "backend_id": "ROOTED-2A9_D6",
        "residual_group": {
            "order": len(group),
            "a9_component_permutation_image_order": residual["residual_group"][
...
def build(catalog, residual, probe):
    ...
    return {
        "schema": "elkies-k3.2a9-d6-fixed-coordinate-shells.v1",
        "status": "PASS_EXACT_DECLARED_2A9_D6_FIXED_COORDINATE_SHELLS_T_NS_FIRST",
        "proof_scope": {
            "proved": (
                "complete cyclic order-four residual lift group; every 7-of-r "
...
def main():
    ...
    if arguments.check:
        if not output.exists() or output.read_text() != encoded:
            raise SystemExit("2A9+D6 canonical shell artifact is stale")
    else:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(encoded)
    print(
        "2A9_D6_CANON|seeds={}|orbits={}|surfaces={}|partners={}|frames={}|status=PASS_EXACT".format(
