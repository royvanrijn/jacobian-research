...
# ---------------------------------------------------------------------------
# Exact digit-by-digit lifting of the historical point in Z_73^2.
#
# The special-fibre local algebra has tangent dimension one and length two.
# At each p-adic digit the rank-one linearization leaves only 73 possible
# tangent corrections.  Enumerate those exactly modulo 73^k.
# ---------------------------------------------------------------------------
...
def qq_mod(value,modulus):
    value = QQ(value)
    den = int(value.denominator()) % modulus
    if gcd(den,modulus) != 1:
        raise ZeroDivisionError
    return (
        (int(value.numerator()) % modulus)
        * inverse_mod(den,modulus)
    ) % modulus

def lift_j(exponent):
    # Hensel lift j=17, j^2=-3, from mod 73 to mod 73^exponent.
    root = 17
    modulus = P
    while modulus < P^exponent:
        quotient = ((root*root+3)//modulus) % P
        derivative = (2*root) % P
        digit = (-quotient*inverse_mod(derivative,P)) % P
        root += digit*modulus
        modulus *= P
    assert (root*root+3) % modulus == 0
    return int(root % modulus)

def k_mod_power(value,modulus,jroot):
    value = K(value)
    parts = list(value)+[QQ(0),QQ(0)]
    return (
        qq_mod(parts[0],modulus)
        + jroot*qq_mod(parts[1],modulus)
    ) % modulus

def eval_power(poly,x,y,modulus,jroot):
    total = 0
    for exponent,coefficient in Cpar(poly).dict().items():
        e0,e1 = map(int,tuple(exponent))
        total += (
            k_mod_power(coefficient,modulus,jroot)
            * pow(int(x),e0,modulus)
            * pow(int(y),e1,modulus)
        )
    return int(total % modulus)

# Jacobian modulo 73 at the seed.
def eval73(poly,x,y):
    return eval_power(poly,int(x),int(y),P,17)

J = matrix(GF(P),[
    [
        eval73(eq.derivative(c0v),seed_c0,seed_c1),
        eval73(eq.derivative(c1v),seed_c0,seed_c1),
    ]
    for eq in equations
])
assert J.rank() == 1

pivot_row = next(i for i in range(J.nrows()) if J[i,0] != 0)
j00 = int(J[pivot_row,0])
j01 = int(J[pivot_row,1])
inv_j00 = int(inverse_mod(j00,P))

print(
    "Q80FINAL73DIGITSETUP|seed=({}, {})|jacobian={}|pivot_row={}|"
    "tangent_kernel={}|status=PASS_RANK_ONE_DIGIT_SETUP".format(
        seed_c0,seed_c1,
        tuple(tuple(int(v) for v in row) for row in J.rows()),
        pivot_row,
        tuple(int(v) for v in J.right_kernel().basis()[0]),
    ),
    flush=True,
)

solutions = [(seed_c0,seed_c1)]
modulus = P
current_exponent = 1
MAX_EXPONENT = 12
...
for exponent in range(1,MAX_EXPONENT):
    ...
    jroot = lift_j(exponent+1)
    next_solutions = {}

    for x,y in solutions:
        values = [
            eval_power(eq,x,y,next_modulus,jroot)
            for eq in equations
        ]
        if any(value % modulus for value in values):
            raise ArithmeticError(
                f"stored lift is not a solution mod 73^{exponent}"
            )
        rhs = [
            (value//modulus) % P
            for value in values
        ...
        # For each tangent digit b, the pivot equation determines a uniquely.
        for b in range(P):
            a = (
                -(rhs[pivot_row] + j01*b)
                * inv_j00
            ...
            # Linearized compatibility with all three equations.
            compatible = True
            for row in range(J.nrows()):
                if (
                    rhs[row]
                    + int(J[row,0])*a
                    + int(J[row,1])*b
            ...
            if all(
                eval_power(eq,xx,yy,next_modulus,jroot) == 0
                for eq in equations
            ):
                next_solutions[(xx % next_modulus,yy % next_modulus)] = (
                    xx % next_modulus,yy % next_modulus
                )

    solutions = sorted(next_solutions.values())
    modulus = next_modulus
    current_exponent = exponent+1

    compact = None
    if len(solutions) <= 12:
        compact = tuple(solutions)

    print(
        "Q80FINAL73DIGIT|exponent={}|modulus_bits={}|solution_count={}|"
        "solutions={}|status={}".format(
            exponent+1,ZZ(modulus).nbits(),len(solutions),compact,
            "PASS_Z73_LIFTS" if solutions else "NO_Z73_LIFT",
        ),
        flush=True,
    )
    ...
    if len(solutions) > 5000:
        raise ArithmeticError(
            "73-adic lift tree exceeded 5000 nodes; local multiplicity "
            "is larger than the length-two diagnostic suggested"
        )

artifact = OUT / "q80-final-q6-char0-73adic-digit-tree.json"
artifact.write_text(json.dumps({
    "status":(
        "PASS_Z73_FINAL_HORIZONTAL_LIFT_TREE"
        if solutions else
        "NO_Z73_LIFT_RAMIFIED_BRANCH_INDICATED"
    ),
    "last_exponent":int(current_exponent),
    "solution_count":len(solutions),
    "solutions":[[str(x),str(y)] for x,y in solutions],
    "modulus":str(modulus),
},indent=2,default=int)+"\n")

print(
    "Q80FINAL73DIGITFINAL|solution_count={}|json={}|status={}".format(
        len(solutions),artifact,
        "PASS_Z73_FINAL_HORIZONTAL_LIFT_TREE"
        if solutions else
        "NO_Z73_LIFT_RAMIFIED_BRANCH_INDICATED",
    ),
