...
# ===========================================================================
# Calibrate the one global square-root sign at U=2.
# ===========================================================================
...
cal_diag=[]
for sign in (+1,-1):
    result,reason=direct_AJ(cal_tau,sign)
    if result is None:
        cal_diag.append((sign,reason))
        continue
    E13,AJ,Q24=result
    try:
        Qref=ref_q24(cal_tau,E13)
    except ZeroDivisionError:
        cal_diag.append((sign,"reference_pole"))
        continue
    ax,ay=AJ.xy()
    qx,qy=Q24.xy()
    cal_diag.append((sign,f"AJ={int(ax)},{int(ay)};q24={int(qx)},{int(qy)};match={int(Q24==Qref)}"))
    if Q24==Qref:
        hits.append(sign)

print(
    "Q24D46DIRECTGLOBAL_CAL|"
    f"diagnostic={cal_diag}|matches={hits}|status=DIAGNOSTIC",
    flush=True,
)
if len(hits)!=1:
    raise ArithmeticError(f"global direct-map sign calibration count {len(hits)}")
global_sign=hits[0]

print(
    f"Q24D46DIRECTGLOBAL_ORIENT|U=2|scale_root_sign={global_sign:+d}|"
    "criterion=AJ_direct(Qmap-S3)+2G1==independent_q24|"
    "status=PASS_UNIQUE_GLOBAL_SIGN",
    flush=True,
)
...
# ===========================================================================
# Exact full-function crosscheck against independent q24.
# ===========================================================================
...
print(
    ...
    "crosscheck=IDENTICAL|status=PASS_MODULAR_Q24_FROM_DIRECT_DEGREE46_BRIDGE",
    flush=True,
)
