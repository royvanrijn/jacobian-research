...
def json_safe(value):
    # Recursively convert Sage scalars/containers to stdlib-JSON values.
    ...
    # Sage Integer is not necessarily a Python int.
    try:
        if value in ZZ:
            return int(value)
    ...
    # Preserve exact non-integral rationals losslessly.
    try:
        rational = QQ(value)
    ...
    # Diagnostics must never kill the scientific computation.
    return str(value)
...
# Fail fast before any shell enumeration if Sage/Python JSON semantics change.
_json_selftest = {
    "sage_integer": ZZ(7),
    "sage_rational": QQ(5) / QQ(7),
    "nested": (ZZ(1), QQ(3) / QQ(2), (ZZ(-4),)),
}
_json_encoded = json.dumps(
    json_safe(_json_selftest),
    sort_keys=True,
    separators=(",", ":"),
)
_json_decoded = json.loads(_json_encoded)
assert _json_decoded["sage_integer"] == 7
assert _json_decoded["sage_rational"] == "5/7"
assert _json_decoded["nested"] == [1, "3/2", [-4]]
print(
    "Q80R5SCORE|json_serializer=PASS_SAGE_SAFE_JSON",
    flush=True,
)
