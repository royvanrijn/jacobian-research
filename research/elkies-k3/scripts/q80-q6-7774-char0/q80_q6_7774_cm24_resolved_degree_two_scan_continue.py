...
for row in candidates:
    ...
    if determinant == 0:
        # The two RR generators have the same marked-chord direction.
        # Hence f0/f1=a0/a1=b0/b1 lies in F_73(V): the purported
        # "new base" factors through the old base and cannot have D.F=2.
        base_ratio = None
        if b1 != 0:
            base_ratio = b0/b1
            assert a1 != 0
            assert a0/a1 == base_ratio
        elif b0 != 0:
            base_ratio = a0/a1
        else:
        ...
        print(
            f"Q807774RRDEGEN|candidate={row['name']}|mw={row['mw']}|"
            f"kernel={tuple(tuple(map(int,rw)) for rw in kernel.rows())}|"
            f"old_base_ratio={base_ratio}|"
            "status=PASS_REJECT_DEGENERATE_BASE_PENCIL",
            flush=True,
        )
...
degenerate_count = sum(
    1 for result in summary if result[6] == "degenerate"
)
quartic_count = sum(
    1 for result in summary if result[6] == 4
)
root_data_values = tuple(sorted(
    set(
        result[7] for result in summary
...
print(
    ...
    f"distinct_root_data={root_data_values}|"
...
nondegenerate = tuple(
    result for result in summary
    if result[6] != "degenerate"
)
quartic_results = tuple(
    result for result in summary
    if result[6] == 4
)

if len(quartic_results) == 1:
    winner = quartic_results[0]
    print(
        f"Q807774RRSELECT|candidate={winner[0]}|mw={winner[1]}|"
        f"root_data={winner[7]}|MW={winner[8]}|"
        "status=PASS_UNIQUE_Q6_7774_SPECIALIZED_MARKING",
        flush=True,
    )
elif quartic_results:
    print(
        f"Q807774RRSELECT|quartic_candidates="
        f"{tuple(result[0] for result in quartic_results)}|"
        "status=PASS_MULTIPLE_Q6_7774_SPECIALIZED_MARKINGS",
        flush=True,
    )
else:
    print(
        f"Q807774RRSELECT|nondegenerate="
        f"{tuple(result[0] for result in nondegenerate)}|"
        "status=PASS_NO_QUARTIC_Q6_7774_MARKING",
        flush=True,
    )
