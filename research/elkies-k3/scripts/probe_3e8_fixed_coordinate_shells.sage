...
"""Scan every eligible residual-class fixed-coordinate shell in N(3E8).

For each nonidentity matrix conjugacy class of the exact S3 residual group,
...
RESIDUAL = ROOT / "artifacts/generated-results/elkies-k3-3e8-residual-group-v1.json"
DEFAULT_OUTPUT = (
    ROOT
...
def build(catalog, residual, determinant_bound, minimum_mw_rank):
    ...
    assert residual["schema"] == "elkies-k3.3e8-residual-group.v1"
    assert residual["status"] == "PASS_EXACT_3E8_RESIDUAL_GROUP"
    ...
    return {
        "schema": "elkies-k3.3e8-fixed-coordinate-shell-probe.v1",
        "status": "PASS_EXACT_PRE_RESIDUAL_QUOTIENT_3E8_COORDINATE_SHELL_SCAN",
        "proof_scope": {
            "proved": (
                "all rank-seven coordinate summands of pinned LLL bases for "
                "every nonidentity residual matrix conjugacy class pass through "
...
def main():
    ...
    if arguments.check:
        if not output.exists() or output.read_text() != encoded:
            raise SystemExit("3E8 coordinate-shell probe is stale")
    else:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(encoded)
    print(
        "3E8SHELL|classes={}|subsets={}|seeds={}|status=PASS_EXACT".format(
