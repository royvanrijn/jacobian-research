...
src=json.loads((P/'explicit-source-certificate.json').read_text());F=matrix(ZZ,src['frame_gram']);U=matrix(ZZ,[[0,1],[1,0]]);NS=block_diagonal_matrix(U,-F);z=json.loads((P/'source-rootless-nef.json').read_text());assert z['status']=='PASS_FULL_NEF_ROOTLESS_SOURCE_TRANSPORT';M=matrix(ZZ,z['transport']);G=matrix(ZZ,z['frame']);h=vector(ZZ,z['ample_class']);D=M.row(0)
...
a=argparse.ArgumentParser();a.add_argument('--check',action='store_true');args=a.parse_args();target=P/'physical-mw17-degree183-certificate.json'
