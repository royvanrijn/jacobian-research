...
ap=argparse.ArgumentParser(description="Fourth-generation tiny-neighbor search from the MW3 node.")
ap.add_argument("--qs",default="2,3,4,6,8,9")
ap.add_argument("--enum-cap",type=int,default=6000)
...
qs=[ZZ(x) for x in a.qs.split(",") if x.strip()]
print(f"MW3NBR|stage=start|qs={qs}|det={M.det()}",flush=True)
...
def vectors_of_qnorm(n):
    ...
    def rec(k,partial):
        ...
        x[k]=ZZ(0)
    rec(dim-1,RR(0))
    print(f"MW3NBR|stage=enumerate|q={n}|pairs={len(found)}",flush=True)
    return found
...
for q in qs:
    for v in vectors_of_qnorm(q):
        for aa in divisors(q):
            ...
            if mw<=3:
                print(f"MW3NBR|cand={len(res)}|q={q}|ab={aa},{bb}|root_rank={rr}|roots={rc}|rootdet={rd}|MW={mw}",flush=True)

res.sort(key=lambda z:(z[0],-z[2],z[3],z[4]))
print(f"MW3NBR|stage=summary|unique={len(res)}",flush=True)
for i,z in enumerate(res[:a.report],1):
    mw,rr,rc,rd,q,aa,bb,v,P=z
    print(f"MW3NBR_BEST|rank={i}|MW={mw}|root_rank={rr}|roots={rc}|rootdet={rd}|q={q}|ab={aa},{bb}|v={v}",flush=True)
    print(f"MW3NBR_BEST|frame_gram={P}",flush=True)
