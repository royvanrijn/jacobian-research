...
"""Probe the NS0030 semistable MW1 source fibre ansatz modulo p.

The selected pole-zero source has profile ``2I2+I3+2I7+3I1``.  Normalize
the identity-component I3 and I2 supports to zero and one, put the depth-one
I2 and I7 supports at lambda and mu, and put the depth-two I7 at infinity.
For every normalized A polynomial, solve the 21 nodal branch jets for the 13
B coefficients and retain exact squarefree residual cubics.
"""
...
DEFAULT_OUTPUT = ROOT / "artifacts/generated-results/elkies-k3-lattice-foundry-ns0030-source-ansatz-mod5.json"
SOURCE_ID = "NS0030-S001"
ORDERS = (3, 2, 2, 7, 7)
...
parser.add_argument("--mu-value", type=int)
parser.add_argument("--max-a-samples-per-support-pair", type=int, default=0)
...
fixed_mu = None if args.mu_value is None else field(args.mu_value)
if (fixed_lambda is None) != (fixed_mu is None):
    raise SystemExit("specify both --lambda-value and --mu-value, or neither")
if fixed_lambda is not None and (
    fixed_lambda in (0, 1)
    or fixed_mu in (0, 1)
    or fixed_lambda == fixed_mu
):
    raise SystemExit("lambda and mu must be distinct and outside {0,1}")

source_payload = json.loads(SOURCE.read_text())
source_row = next(
    row
    for row in source_payload["sources"]
    if row["ns_id"] == "NS0030" and row["source_id"] == SOURCE_ID
)
source = source_row["source"]
assert source["root_type"] == "2A1+A2+2A6"
assert source["mw_height_gram"] == [["17/14"]]
assert source["root_lattice_primitive"] and source["torsion"] == 1
assert source_row["determinant"] == 714
...
assert sorted(component_corrections) == sorted(
    [(2, 3, QQ(0)), (1, 2, QQ(0)), (1, 2, QQ(1) / 2),
     (6, 7, QQ(6) / 7), (6, 7, QQ(10) / 7)]
)
...
assert pole["mw_height"] == "17/14"

ring = PolynomialRing(field, "t")
t = ring.gen()
square_roots = {value * value: value for value in field}
per_pair_total = args.prime**8
if args.sample_stride and math.gcd(args.sample_stride, per_pair_total) != 1:
...
available = [value for value in field if value not in (0, 1)]
support_pairs = (
    [(fixed_lambda, fixed_mu)]
    if fixed_lambda is not None
    else [(left, right) for left in available for right in available if left != right]
)
examples = []
pair_records = []
accounting = {
...
for lambda_value, mu_value in support_pairs:
    rows = []
    ...
    for jet in range(ORDERS[4]):
        rows.append([field(index == 12 - jet) for index in range(13)])
    hermite = matrix(field, rows)
    if hermite.nrows() != 21 or hermite.rank() != 13:
        raise ArithmeticError("unexpected NS0030 Hermite rank")
    compatibility = hermite.left_kernel().basis_matrix()
    if compatibility.nrows() != 8:
        raise ArithmeticError("unexpected NS0030 compatibility codimension")

    local = {key: 0 for key in accounting}
    sample_limit = args.max_a_samples_per_support_pair or per_pair_total
    if args.sample_stride:
        coefficient_iterator = (
            (
                (args.sample_offset + sample_index * args.sample_stride) % per_pair_total,
                coefficient_digits(
                    (args.sample_offset + sample_index * args.sample_stride) % per_pair_total
                ),
            )
        ...
        coefficient_iterator = itertools.islice(
            enumerate(itertools.product(range(args.prime), repeat=8)), sample_limit
        )
    ...
    for coefficient_index, digits in coefficient_iterator:
        ...
        series = (
            ...
            taylor_coefficients(coefficients, mu_value, ORDERS[3]),
            [coefficients[8 - jet] for jet in range(ORDERS[4])],
        )
        ...
        for signs in itertools.product((1, -1), repeat=4):
            ...
            orders = (
                ...
                order_at(discriminant_core, mu_value),
            ...
            divisor = (
                t**ORDERS[0]
                * (t - 1) ** ORDERS[1]
                * (t - lambda_value) ** ORDERS[2]
                * (t - mu_value) ** ORDERS[3]
            )
            residual, remainder = discriminant_core.quo_rem(divisor)
            if remainder or residual.degree() != 3:
                raise ArithmeticError("unexpected NS0030 residual discriminant")
            if any(residual(point) == 0 for point in (0, 1, lambda_value, mu_value)):
                continue
                ...
                examples.append(
                    {
                        ...
                        "mu": int(mu_value),
                        "coefficient_index_within_support_pair": coefficient_index,
                        "branch_signs_at_one_lambda_mu_infinity": list(signs),
                        ...
                        "geometric_fibre_profile": "2I2+I3+2I7+3I1",
                    }
                )
    for key in accounting:
        accounting[key] += local[key]
    exhausted = local["normalized_A_samples"] == per_pair_total and not args.max_a_samples_per_support_pair
    pair_records.append(
        {
            "lambda": int(lambda_value),
            "mu": int(mu_value),
...
exhausted = all(record["exhausted"] for record in pair_records)
output = {
    "schema": "elkies-k3.lattice-foundry-ns0030-source-ansatz-modp.v1",
    "status": (
    ...
    "prime": args.prime,
    "scan": {
        "support_pairs": pair_records,
        "exhausted": exhausted,
        "normalized_A_polynomials_per_support_pair": per_pair_total,
    },
    "accounting": accounting | {"stored_examples": len(examples)},
    "ansatz": {
        "short_weierstrass": "y^2=x^3+A(t)x+B(t)",
        "degree_bounds": {"A": 8, "B": 12},
        "normalization": "A(0)=-3; supports 0:I3, 1:I2, lambda:I2, mu:I7, infinity:I7",
        "hermite_conditions": 21,
        "B_coefficient_rank": 13,
        "compatibility_equations_on_A_and_supports": 8,
    ...
    "proof_boundary": {
        ...
        "not_proved": (
            "The pole-zero MW section, NS0030 marking, characteristic-zero lift, "
...
if args.check:
    if output_path.read_text() != serialized:
        raise SystemExit("NS0030 source-ansatz artifact is stale")
else:
...
print(
    "FOUNDRYNS0030ANSATZ|"
    f"p={args.prime}|pairs={len(pair_records)}|samples={accounting['normalized_A_samples']}|"
