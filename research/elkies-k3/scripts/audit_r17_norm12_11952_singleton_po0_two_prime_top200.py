#!/usr/bin/env python3
"""Audit two distinct exhaustive P.O=0 shells on the top 200 singleton twists."""
...
PRIMARY = GENERATED / (
    "elkies-k3-r17-norm12-11952-singleton-twist-po0-top200-campaign-v1.json"
)
SECONDARY = GENERATED / (
    "elkies-k3-r17-norm12-11952-singleton-twist-po0-"
    "top200-distinct-second-campaign-v1.json"
)
...
OUTPUT = GENERATED / (
    "elkies-k3-r17-norm12-11952-singleton-twist-po0-"
    "two-prime-top200-audit-v1.json"
)
...
def main() -> None:
    ...
    if (
        ...
        or primary["candidate_limit"] != 200
        or secondary["candidate_limit"] != 200
    ...
    result = {
        "schema": "elkies-k3.r17-norm12-11952-singleton-po0-two-prime-top200.v1",
        "status": "PASS_EXACT_BOUNDED_TWO_PRIME_TOP200_NO_ISOLATED_SECOND_SECTION",
        "character_count": 200,
        "complete_shell_count": 400,
        ...
        "proof_boundary": (
            "For each of the 200 ranked characters, the two finite-field primes "
            "are distinct and each exported P.O=0 polynomial-section shell is "
            "complete. Every isolated extra point in all 400 shells has an exact "
            ...
            "chosen primes. Sections with P.O>0, the remaining 38947 singleton "
            "characters, and non-singleton characters are outside this result."
        ),
        "reproducing_command": (
            ".venv/bin/python elkies-k3/scripts/"
            "audit_r17_norm12_11952_singleton_po0_two_prime_top200.py --check"
        ),
    }
    ...
    if args.check:
        if not output.is_file() or output.read_text() != serialized:
            raise ArithmeticError("stored two-prime top-200 audit differs from replay")
    else:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(serialized)
    print(
        "R17SINGLETONPO0TWO_PRIME_TOP200"
