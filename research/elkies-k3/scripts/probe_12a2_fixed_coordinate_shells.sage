...
"""Scan every eligible residual-class fixed-coordinate shell in N(12A2).

For every nonidentity matrix conjugacy class of the exact order-190080
...
def build(catalog, residual, determinant_bound, minimum_mw_rank):
    ...
    assert residual["schema"] == (
        "elkies-k3.12a2-ternary-golay-residual-group.v1"
    )
    assert residual["status"] == (
        "PASS_EXACT_12A2_TERNARY_GOLAY_AND_FULL_RESIDUAL_GROUP"
    )
    assert residual["residual_group"]["order"] == 190080
    assert residual["residual_group"]["component_permutation_image_order"] == 95040
    assert residual["residual_group"]["central_diagram_kernel_order"] == 2
    ambient = matrix(
        ZZ,
        next(
            row["gram"]
    ...
    for row in class_scans:
        accepted_mw_distribution.update(
            {
                int(rank): count
                for rank, count in row["accounting"][
                    "accepted_seed_mw_rank_distribution"
                ].items()
            }
        )
    assert accepted_mw_distribution == Counter(
        {12: 17, 13: 120, 14: 79, 15: 6, 17: 15}
    )
    return {
        "schema": "elkies-k3.12a2-fixed-coordinate-shell-probe.v1",
        "status": "PASS_EXACT_PRE_RESIDUAL_QUOTIENT_12A2_COORDINATE_SHELL_SCAN",
        "proof_scope": {
            "proved": (
...
def main():
    ...
    if arguments.check:
        if not output.exists() or output.read_text() != encoded:
            raise SystemExit("12A2 coordinate-shell probe is stale")
    else:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(encoded)
    print(
        "12A2SHELL|classes={}|subsets={}|seeds={}|status=PASS_EXACT".format(
