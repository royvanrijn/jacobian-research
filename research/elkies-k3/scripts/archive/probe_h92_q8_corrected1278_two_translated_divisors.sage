...
"""
Exact two-translated-divisor E7 principal-part probe on the corrected H92
1278-column movable q8 ambient, restricted to the 14-dimensional global survivor.
...
For an actual corrected global term
    f = t^(64-d)/h_rev(t)^16 * x^a * m^b
...
A common polynomial clearing has L-order 16+8=24.  We compute the image
modulo L^24 exactly using the monic surface equation Y^2=H(L,U), i.e. as a
...
translated divisors on the common 14-dimensional survivor.

Run:
  sage -python ~/Downloads/probe_h92_q8_corrected1278_two_translated_divisors.sage
...
OUTPUT_PATH = GEN / f"zz-h92-q8-corrected1278-two-translated-divisors-mod-{P}.json"
...
def compute_chart(chart_name):
    ...
    # Rank-two coefficient module A + A*Y, A=GF(p)[L,U], truncated mod L^24.
    ...
    #   C=t^16*h_rev^16*Nx^8*yp_den^8,
    # the L order is 16+8=24. The remaining t exponent is 80-d.
    t1_powers = [ONE]
    ...
    record = {
        ...
        "remaining_if_alone": int(14 - rank),
...
print(
    f"CORRECTED_TRANSLATED_START|prime={P}|ambient=1278|survivors=14|threshold=L^{MOD_L}",
    flush=True,
)
...
payload = {
    "schema": "elkies-k3.h92-q8-corrected1278-two-translated-divisors-modp.v1",
    "status": "EXPERIMENTAL_MODULAR_CORRECTED1278_TWO_TRANSLATED_DIVISOR_IMAGES",
    "prime": int(P),
    "common": {
        ...
        "helper_normalization": (
            "The q6^8 helper shift i=d-8 is stripped. No extra helper t^8 "
            "is imposed in these reduced-divisor regularity tests."
        ),
        "method": (
            "Exact rank-two surface-module principal parts modulo L^24; "
            "no Groebner basis."
        ),
    },
    "divisors": records,
    ...
    "boundary": (
        "These are the corrected two translated t-divisor principal-part images on the "
        "p-modular corrected global survivor. A combined rank 12 would leave the expected "
        "two-dimensional pencil, but still requires a second-prime regression and "
...
print(
    "Q8CORRECTEDTWOTRANSLATED|"
    f"prime={P}|global_survivor=14|"
