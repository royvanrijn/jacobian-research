...
For each ordinary child-base value tau in GF(p):
  ...
  6. The quartic covariant map is the degree-4 2-covering, not the
     degree-one identification with the Jacobian.  With old O as origin,
       phi(P)=QO + epsilon*2[P-O].
     Hence form
       W = TraceCov(C) - 26*QO - 24*QA = epsilon*2*S3,
     and recover the unique rational half of W.
...
def rational_halves(E, target):
    """Return all P in E(F) with 2P=target, using the exact x(2P) quartic."""
    if target.is_zero():
        # We do not use zero targets in this reconstruction.
        raise ArithmeticError("unexpected zero target in halving")
    target_x, unused_target_y = target.xy()
    a = E.a4()
    b = E.a6()
    ...
    halves = []
    for factor, multiplicity in halving.factor():
    ...
    # Remove duplicates without assuming point hashability.
    unique = []
    for point in halves:
        if all(point != prior for prior in unique):
            unique.append(point)
    return unique
...
def section_at_tau(tau):
    ...
    # IMPORTANT: the binary-quartic covariant map is the 2-covering:
    #
    #   phi(P) = QO + epsilon*2*[P-O].
    #
    # Therefore traceC here is the sum of COVARIANT images, and
    #
    #   traceC - 50*QO = epsilon*2*AJ(C),
    #   QA-QO           = epsilon*2*A.
    #
    # Since AJ(C)=S3+24*A:
    #
    #   W = traceC - 26*QO - 24*QA = epsilon*2*S3.
    double_s3 = traceC - 26*QO - 24*QA
    halves = rational_halves(E, double_s3)
    if len(halves) != 1:
        raise ArithmeticError(
            f"doubled S3 has {len(halves)} rational halves, expected one"
        )
    S3 = halves[0]
    assert 2*S3 == double_s3
    if S3.is_zero():
        raise ArithmeticError("halved S3 specialized to zero")

    # Independent local regression of the 2-covering interpretation:
    # QA-QO must itself be twice a rational point whenever the old A=-P1
    # section has good specialization.
    Ahalves = rational_halves(E, QA-QO)
    if len(Ahalves) != 1:
        raise ArithmeticError(
            f"QA-QO has {len(Ahalves)} rational halves, expected one"
        )
    assert 2*Ahalves[0] == QA-QO
...
if IK.nrows() != 1:
    diagnostic = []
    # Find any low-complexity rational profiles supported by all samples.
    # Restrict to total unknown count <= number of equations, so nullity is
    # evidence rather than a dimension-count artifact.
    for dden in range(0, 61):
        for dnum in range(0, 61):
            if dnum + dden + 2 > len(samples):
                continue
            test = matrix(F, [
                [tau**j for j in range(dnum+1)]
                + [-sx*tau**j for j in range(dden+1)]
                for tau, sx, unused_sy in samples
            ])
            nullity = test.ncols() - test.rank()
            if nullity:
                diagnostic.append((dnum, dden, nullity))
        if diagnostic:
            break
    raise ArithmeticError(
        "expected x profile 46/42 failed with kernel dimension {}; "
        "first supported low-degree profiles={}".format(IK.nrows(), diagnostic[:8])
    )
rv = IK[0]
...
payload = {
    ...
    "method": {
        ...
        "orientation": (
            "local m->infinity fixes QO/QA; exact finite-field halving removes "
            "the covariant map's multiplication-by-2"
        ),
        "formula": (
            "covariant map phi(P)=QO+epsilon*2[P-O]; "
            "W=TraceCov(C)-26*QO-24*QA=epsilon*2*S3; "
            "recover S3 up to global sign by exact halving"
        ),
    ...
    "orientation_boundary": (
        "The halving recovers epsilon*S3. The global epsilon sign of the "
        "2-covering is not fixed here; either sign has the same x-coordinate "
        "and height/pole profile and will be fixed by the downstream q8 marking."
    ),
    "profile": {
    ...
    "boundary": (
        "Exact over GF(100003)(T) by trace sampling, 2-cover halving, "
        "interpolation, and identity verification. "
