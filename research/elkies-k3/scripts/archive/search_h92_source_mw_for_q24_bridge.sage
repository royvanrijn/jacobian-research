...
# The known source height form in the (P1,P2) orientation has +3 cross
# pairing; our first basis vector is -P1, hence the cross term is -3.
assert H0 == matrix(QQ, [[QQ(21)/2, -3], [-3, 46]]), H0

# III* has exactly two multiplicity-one components: affine and E7_7 in the
# source numbering.  II* has only the affine multiplicity-one component.
highest_e7 = vector(ZZ, (2, 2, 3, 4, 3, 2, 1))
assert E7_CARTAN * highest_e7 == vector(ZZ, [0, 0, 0, 0, 0, 0, 1])
patterns = [
    vector(ZZ, [0]*15),
    vector(ZZ, [0]*6 + [1] + [0]*8),
]
...
print(
    "H3MWQ24_SETUP|basis=(-P1,P2)|height={}|basis_reconstruction=PASS".format(H0),
    flush=True,
)
...
for pattern_index, p in enumerate(patterns):
    correction = -QQ(p*Groot.inverse()*p)
    root_part = vector(QQ, p)*Groot.inverse()*source_roots
    constant = QQ(O0*ns*F8) + correction + QQ(root_part*ns*F8)
...
for word in ((1, 0), (0, 1)):
    ...
    root_part = vector(QQ, p)*Groot.inverse()*source_roots
    formula = (
        QQ(n*H0*n) + linear*n + QQ(O0*ns*F8)
        + correction + QQ(root_part*ns*F8)
    )
    assert formula == P*ns*F8

print(
    "H3MWQ24_BOUND|threshold={}|center={}|box={}|patterns={}".format(
        args.threshold,
        ",".join(map(str, center)),
        ";".join(
