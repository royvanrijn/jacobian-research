...
parser.add_argument(
    "--mode",
    choices=("global-product", "finite-corner", "local-normal-form"),
    default="global-product",
    help=(
        "global-product is a fast exact sufficient membership test in the actual "
        "polynomial ideal (surface,t^T); finite-corner is the weaker one-way "
        "Artinian obstruction; local-normal-form asks Singular for the exact ds local basis"
    ),
)
...
t_field = x_p_t.parent()
t_formal = t_field.gen()
r = x_p_t/t_formal**2
s = y_p_t/t_formal**3
assert r.valuation() == 0 and s.valuation() == 0

# IMPORTANT: x_p_t.parent() is QQ(t), not QQ[t].  Keep all finite
# coefficient expansions below in the polynomial ring so .list() is valid.
t_poly = r.numerator().parent()
...
assert h.degree() == 4
h_reverse = t_poly(list(reversed(h.list())))
assert h_reverse(0)
...
for chart in sorted(pullbacks["charts"], key=lambda item: item["name"]):
    ...
    def eval_t_poly(value):
        # Be deliberately strict here: these inputs must be honest QQ[t]
        # polynomials.  This catches accidental fraction-field coercions early.
        value = t_poly(value)
        return ring(sum(
    ...
    if args.trace:
        print(
            "Q8QFRAME_NODE|prime={}|chart={}|stage=standard_basis|mode={}|"
            "t_exponents={}".format(
                args.prime, name, args.mode, ",".join(str(v) for v in t_exponents)
            ),
            flush=True,
        )
    ...
    if args.mode == "local-normal-form":
        ...
        principal_description = "(surface,({})^{})".format(t_monomial, T)
    elif args.mode == "global-product":
        # Exact sufficient test, but much cheaper than local ds.  The ideal
        # here is literally (surface,t_monomial^T) in the polynomial ring.
        # Zero reduction proves global polynomial-ideal membership and hence
        # local membership.  A nonzero remainder is only inconclusive because
        # localization may invert additional units.
        singular.eval("ring q8qframe={},(Z,U,Y),dp;".format(args.prime))
        singular.eval("poly surface={};".format(surface))
        singular.eval("ideal principal=surface,({})^{};".format(t_monomial, T))
        singular.eval("ideal standard_principal=std(principal);")
        principal_description = "(surface,({})^{}) [global-product]".format(
            t_monomial, T
        )
    else:
        # Fast one-way obstruction copied from the repository's existing
        # q8 E7-node finite-corner mode.
        if not (t_exponents[0] > 0 and t_exponents[1] > 0):
            raise ArithmeticError(
                "finite-corner mode expects positive Z,U exponents in t; got {}".format(
                    t_exponents
                )
            )
        ...
        singular.eval("ring q8qframe={},(Z,U,Y),dp;".format(args.prime))
        singular.eval("poly surface={};".format(surface))
        z_power = T*t_exponents[0]
        u_power = T*t_exponents[1]
        singular.eval(
            "ideal principal=surface,Z^{},U^{};".format(z_power, u_power)
        )
        singular.eval("ideal standard_principal=std(principal);")
        principal_description = "(surface,Z^{},U^{})".format(z_power, u_power)

    if args.trace:
        print(
            "Q8QFRAME_NODE|prime={}|chart={}|stage=standard_basis_done|mode={}".format(
                args.prime, name, args.mode
            ),
            flush=True,
        )
    ...
    for index, numerator in enumerate(numerators):
        ...
        if args.trace:
            print(
                "Q8QFRAME_NODE|prime={}|chart={}|candidate={}|pre_terms={}|"
                "terms={}".format(
                    args.prime, name, index, pre_terms, len(remainder.dict())
                ),
                flush=True,
            )
    ...
    node_records.append({
        "chart": name,
        "mode": args.mode,
        "principal_ideal": principal_description,
        "rows": len(rows),
        "rank": node_rank,
        "kernel_dimension": node_kernel,
        "zero_remainder_candidates": zero_certificates,
        "all_candidates_certified_in_global_product_ideal": (
            args.mode == "global-product" and zero_certificates == len(candidates)
        ),
    })
    print(
...
payload = {
    ...
    "status": (
        "PASS_MODULAR_Q8_QFRAME_E7_NODE_GLOBAL_PRODUCT_MEMBERSHIP"
        if args.mode == "global-product"
        and all(node["all_candidates_certified_in_global_product_ideal"] for node in node_records)
        else (
            "EXPERIMENTAL_Q8_QFRAME_E7_NODE_FINITE_CORNER_OBSTRUCTION"
            if args.mode == "finite-corner"
            else (
                "EXPERIMENTAL_Q8_QFRAME_E7_NODE_LOCAL_NORMAL_FORM"
                if args.mode == "local-normal-form"
                else "INCOMPLETE_Q8_QFRAME_E7_NODE_GLOBAL_PRODUCT_MEMBERSHIP"
            )
        )
    ),
    ...
    "boundary": (
        (
            "Every zero remainder in global-product mode is an exact polynomial-"
            "ideal membership certificate modulo the displayed good prime, hence "
            "also a local node-membership certificate. Nonzero remainders in this "
            "mode are inconclusive under localization."
            if args.mode == "global-product"
            else (
                "This is a fast one-way Artinian corner obstruction in the q-frame. "
                "Failure is a rigorous local rejection, but survival does not prove "
                "membership in the full local (surface,t^T) ideal."
                if args.mode == "finite-corner"
                else
                "This is the exact modular local-normal-form E7-node probe in the "
                "q-frame. Characteristic-zero lifting remains separate."
            )
        )
    ),
}
...
print(
    "Q8QFRAME|prime={}|mode={}|ambient=11|nodes={}|stacked_rows={}|rank={}|kernel={}|"
    "status={}".format(
        args.prime,
        args.mode,
        len(node_records),
