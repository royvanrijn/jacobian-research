...
# The exact cleared identity is Rnorm*h*Dy == Ny (mod Nx).
normalizer = (ny * (h*dy).inverse_mod(nx)).mod(nx)
assert (normalizer*h*dy - ny) % nx == 0
...
q_inf_order = h.degree()-2
assert q_inf_order == 8
r_over_nx_order = nx.degree()-normalizer.degree()
...
required = 2
max_s = bbase_order - required
assert max_s == 6

# A t term more polar than every possible s*abase term cannot cancel.
most_polar_s_a = abase_order - max_s
max_t = tbase_order - most_polar_s_a
assert max_t >= 0
