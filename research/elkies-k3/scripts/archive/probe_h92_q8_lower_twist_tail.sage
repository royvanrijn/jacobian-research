...
# Inspect the four false lower-twist sections in the generic B families.
# Group coefficients by actual numerator degree d and family (a,b), and print
# the highest nonzero degree layers.  Also test the characteristic adapted
# combinations x*m^j - m^(j+2) at each degree.
family_keys = sorted({
    (int(entry["x_power"]), int(entry["m_power"])) for entry in Bm1
})
family_col = {}
for j, entry in enumerate(Bm1):
    family_col[(
...
for r in range(hm1):
    ...
    nonzero_degrees = sorted({
        int(Bm1[j]["actual_u_power"])
        for j, c in enumerate(vec)
        if c
    }, reverse=True)
    top = nonzero_degrees[:8]
    print(
        f"LOWERTWIST_SECTION|index={r}|top_degrees={','.join(map(str, top))}",
        flush=True,
    )
    layers = []
    for d in top:
        coeffs = {}
        for a,b in family_keys:
            j = family_col.get((d,a,b))
            if j is not None and vec[j]:
                coeffs[f"{'x' if a else 'm'}:{b}"] = int(vec[j])
        # Detect exact z_j direction pairs at this same degree:
        # x*m^j - m^(j+2).
        z_matches = []
        for jpow in range(8):
            ix = family_col.get((d,1,jpow))
            im = family_col.get((d,0,jpow+2))
            if ix is None or im is None:
                continue
            cx = vec[ix]
            cm = vec[im]
            if cx and cm and cm == -cx:
                # Require no claim of exclusivity; just record exact pair.
                z_matches.append(jpow)
        print(
            "LOWERTWIST_LAYER|"
            f"section={r}|d={d}|"
            f"support={json.dumps(coeffs, sort_keys=True, separators=(',',':'))}|"
            f"z_pairs={','.join(map(str,z_matches)) if z_matches else '-'}",
            flush=True,
        )
        layers.append({
            "degree": d,
            "support": coeffs,
            "z_pair_indices": z_matches,
        })
    tail_records.append({"section": r, "layers": layers})

# Compute the span of highest-degree family coefficient vectors, degree by degree.
all_top_degrees = sorted({
    rec["degree"] for section in tail_records for rec in section["layers"]
}, reverse=True)
for d in all_top_degrees[:10]:
    rows = []
    labels = [(0,b) for b in range(10)] + [(1,b) for b in range(8)]
    for r in range(hm1):
        row = []
        for a,b in labels:
            j = family_col.get((d,a,b))
            row.append(Fm1[r,j] if j is not None else finite.zero())
        rows.append(row)
    Mtail = matrix(finite, rows)
    if Mtail.rank():
        print(
            f"LOWERTWIST_TAILRANK|d={d}|rank={Mtail.rank()}|rows=4|families=18",
            flush=True,
        )
...
out.write_text(json.dumps({
    ...
    "lower_twist_tail_records": tail_records,
