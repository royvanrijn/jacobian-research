...
for the already-computed 18-dimensional true-global survivor.

For each component, with normal N and valuations
    vt=ord_N(t), vx=ord_N(x), vm=ord_N(m), vg=ord_N(g),
an actual ambient term u^d/h^18 * x^a*m^b has corrected reduced order

    r = vg + (72-d)*vt + a*vx + b*vm.

The q6^9 helper shift i=d-9 is already stripped.
...
rank-8 translated/affine divisor obstruction.

Run:
  sage -python ~/Downloads/probe_h92_q8_true1600_zu_root_full_jets.sage
...
OUTPUT_PATH = GEN / f"zz-h92-q8-true1600-zu-root-full-jets-mod-{P}.json"
...
def compute_component(component, chart_name, normal_name):
    ...
    # t0^17 * nx0^9 * yp_den^9 * h^18.
    # h^18 cancels because every true ambient column has k=18.
    family_factor = {}
    for a in (0, 1):
...
print(
    f"ZUROOTFULLJETS_START|prime={P}|ambient=1600|survivors=18|components=4",
    flush=True,
)
...
for seq, (component, chart_name, normal_name) in enumerate(COMPONENTS, 1):
    ...
    if P == 43 and component == "E7_5" and record["restricted_rank"] != 0:
        raise SystemExit(
...
payload = {
    "schema": "elkies-k3.h92-q8-true1600-zu-root-full-jets-modp.v1",
    "status": "EXPERIMENTAL_MODULAR_TRUE1600_ZU_ROOT_FULL_NORMAL_JETS",
    "prime": int(P),
    "common": {
    ...
    "boundary": (
        ...
        "16, the remaining certified Y-normal components E7_1,E7_2,E7_3 need "
...
print(
    "Q8TRUEZUROOTFULLJETS|"
    f"prime={P}|global_survivor=18|translated_rank={translated_rank}|"
    f"combined_rank={combined_rank}|remaining={18-combined_rank}|"
    "status=EXPERIMENTAL_MODULAR_TRUE1600_ZU_ROOT_FULL_NORMAL_JETS",
    flush=True,
)
