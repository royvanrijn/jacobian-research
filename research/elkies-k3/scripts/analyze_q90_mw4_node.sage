...
print(f"MW4NODE|stage=input|rank={F.rank()}|det={F.det()}|positive={F.is_positive_definite()}",flush=True)
...
B=L.basis_matrix()
G=B*F*B.transpose()
print(f"MW4NODE|stage=roots|half={len(half)}|signed={len(roots)}|rank={R.rank()}|det={abs(G.det())}",flush=True)

# ADE classification by rank / root count / determinant.
...
print(f"MW4NODE|stage=ADE|solutions={sols}",flush=True)
...
print(f"MW4NODE|stage=shioda|MW_rank={mw_rank}|rootdet={rootdet}|regulator_if_torsion1={reg_t1}",flush=True)

# Possible torsion orders from Shioda discriminant formula:
# det(MW heights)=948*tors^2/rootdet.
for tors in divisors(12):
    reg=QQ(948*tors*tors)/QQ(rootdet)
    print(f"MW4NODE|torsion_candidate={tors}|regulator={reg}",flush=True)

out=Path("artifacts/local/elkies-k3/q90-mw4-node.txt")
out.parent.mkdir(parents=True,exist_ok=True)
out.write_text(f"ADE={sols}\nMW_rank={mw_rank}\nrootdet={rootdet}\nregulator_t1={reg_t1}\nroot_basis=\n{B}\nroot_gram=\n{G}\n")
print(f"MW4NODE|stage=done|out={out}",flush=True)
