...
"""Scan every eligible residual-class fixed-coordinate shell in N(3D8).

For both nonidentity matrix conjugacy classes of the exact glue-preserving S3
residual group, test every rank-seven coordinate direct summand of a pinned
LLL basis of the primitive fixed lattice. Apply the determinant-5000,
...
def build(catalog, residual, determinant_bound, minimum_mw_rank):
    ...
    assert residual["schema"] == "elkies-k3.3d8-glue-residual-group.v1"
    assert residual["status"] == "PASS_EXACT_3D8_GLUE_AND_RESIDUAL_GROUP"
    assert residual["residual_group"]["order"] == 6
    ambient = matrix(
        ZZ,
        next(
            row["gram"]
    ...
    ] == [("3D8-C02", 40), ("3D8-C03", 0)]
    assert class_scans[0]["accounting"][
        "accepted_seed_mw_rank_distribution"
    ] == {"12": 28, "13": 12}
    return {
        "schema": "elkies-k3.3d8-fixed-coordinate-shell-probe.v1",
        "status": "PASS_EXACT_PRE_RESIDUAL_QUOTIENT_3D8_COORDINATE_SHELL_SCAN",
        "proof_scope": {
            "proved": (
                "all rank-seven coordinate summands of pinned LLL bases for "
                "both nonidentity residual matrix conjugacy classes pass "
                "through the declared lattice gates"
            ),
            "not_proved": (
...
def main():
    ...
    if arguments.check:
        if not output.exists() or output.read_text() != encoded:
            raise SystemExit("3D8 coordinate-shell probe is stale")
    else:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(encoded)
    print(
        "3D8SHELL|classes={}|subsets={}|seeds={}|status=PASS_EXACT".format(
