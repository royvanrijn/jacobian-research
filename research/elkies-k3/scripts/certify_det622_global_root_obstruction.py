"""All-Niemeier energy and equality obstruction for every determinant622 frame in the pinned genus."""
from sage.all import *
import argparse,json,hashlib,functools
from pathlib import Path
R=Path(__file__).resolve().parents[2];P=R/'artifacts/generated-results/elkies-k3-det622-root-gate-v1'
mark=R/'artifacts/generated-results/elkies-k3-det622-inose-source-v1/certificate.json'
...
F=block_diagonal_matrix(matrix(ZZ,CartanMatrix(['E',8])),matrix(ZZ,CartanMatrix(['E',8])),matrix(ZZ,[[622]]))
K=matrix(ZZ,[[2,1,-1,1,1,-1,1],[1,2,0,1,0,0,0],[-1,0,2,-1,-1,0,-1],[1,1,-1,2,0,0,1],[1,0,-1,0,2,-1,1],[-1,0,0,0,-1,12,1],[1,0,-1,1,1,1,16]])
assert K.det()==622 and K.is_positive_definite() and all(K[i,i]%2==0 for i in range(7))
...
Schur=K[5:,5:]-K[5:,:5]*K[:5,:5].inverse()*K[:5,5:];assert Schur==matrix(QQ,[[QQ(43)/4,QQ(7)/4],[QQ(7)/4,QQ(59)/4]]) and Schur.trace()==QQ(51)/2

def gen(G):
 d,u,v=G.smith_form();assert d.diagonal()==[1]*(G.nrows()-1)+[622]
 z=G.inverse()*u.inverse()*vector(ZZ,[0]*(G.nrows()-1)+[1]);return z,z*G*z/2
kf,qk=gen(K);ff,qf=gen(F);mult=next(a for a in range(622) if gcd(a,622)==1 and qk+a*a*qf in ZZ)
Z=block_diagonal_matrix(K,F);z=vector(QQ,list(kf)+list(mult*ff));B=matrix(ZZ,[list(622*v) for v in identity_matrix(ZZ,24).rows()]+[list(622*z)]).row_module().basis_matrix()/622;N=B*Z*B.transpose()
...
for a in anchors['anchors']:
 ...
 if types==[['A',7],['A',7],['D',5]]:
  ...
  lower=2*a7_min+bound('D',5);assert lower==Schur.trace()
  # Equality forces u^2=39/8 on each A7; its D5 norm is at most1.
  remaining_u=Schur[0,0]-2*QQ(39)/8;assert remaining_u==1
  # Orthogonal roots contain D4 (or D5 for u=0), so v^2 >=14.
  remaining_v=Schur[1,1]-2*QQ(39)/8;assert remaining_v==5 and remaining_v<14
...
out={'schema':'elkies-k3.det622-global-root-obstruction.v1','status':'PASS_ALL_FRAMES_ROOTFUL','NS_determinant':622,'source_NS':'U+E8(-1)^2+<-622>','auxiliary_gram':rows(K),'D5_basis_change':rows(J),'projected_two_vector_gram':rows(Schur),'available_trace':'51/2','discriminant_glue':{'q_aux':str(qk),'q_frame':str(qf),'multiplier':mult,'index':622,'unimodular_Gram':rows(N)},'anchor_cases':results,'energy_bounds':energy_records,'minimum_required_trace':str(minimum),'borderline_case':'2A7+2D5 excluded by A7 equality and D5 dual small-vector bounds','A7_centroid_cases':a7_cases,'A7_equalities':sharp_equalities,'D5_dual_norm_le_one_twice_coordinates':small_d5,'inputs':[{'path':str(p.relative_to(R)),'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in [mark,cat,anc]],'theorem_inputs':['Primitive discriminant anti-isometry gluing','Niemeier classification','Complete16 D5 anchor orbits in retained EC-K3-H3-ROOTLESS-J2-COMPLETE','Written two-vector Weyl-support energy lemma','Written A7 centered integer-pair bound and equality; D5 dual short-vector argument'],'conclusion':'Every positive even rank17 lattice with the pinned frame discriminant form has a root; no MW17 fibration on the full determinant622 NS. Full rational rank19 marking remains valid.','scope':'Only this actual NS genus, not every lattice with determinant622.'}
...
print('PASS det622 globally rootful:15 strict energy exclusions and one sharp equality obstruction. Rational marking existence survives; MW17 excluded.')
