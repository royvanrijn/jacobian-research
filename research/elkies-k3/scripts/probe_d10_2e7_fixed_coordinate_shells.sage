...
"""Scan the residual-involution fixed-coordinate shell in N(D10+2E7).

For the simultaneous D10 diagram involution and E7-component swap generating
the exact order-two residual group, test every rank-seven coordinate direct
summand of a pinned LLL basis of the primitive rank-sixteen fixed lattice.
Apply the determinant-5000, discriminant-length-three, MW12--17, and
nontrivial mod-two complement-action gates.
...
RESIDUAL = (
    ROOT
    / "artifacts/generated-results/elkies-k3-d10-2e7-residual-group-v1.json"
)
DEFAULT_OUTPUT = (
    ROOT
...
def build(catalog, residual, determinant_bound, minimum_mw_rank):
    ...
    assert residual["schema"] == "elkies-k3.d10-2e7-residual-group.v1"
    assert residual["status"] == "PASS_EXACT_D10_2E7_GLUE_AND_RESIDUAL_GROUP"
    assert residual["residual_group"]["order"] == 2
    ambient = matrix(
        ZZ,
        next(
            row["gram"]
    ...
    assert class_scans[0]["accounting"]["coordinate_subsets_tested"] == 11440
    assert class_scans[0]["residual_conjugacy_class_id"] == "D10_2E7-C02"
    assert class_scans[0]["accounting"][
        "high_mw_mod2_accepted_seeds"
    ] == 0
    return {
        "schema": "elkies-k3.d10-2e7-fixed-coordinate-shell-probe.v1",
        "status": (
        ...
        "proof_scope": {
            "proved": (
                ...
                "the nonidentity residual matrix conjugacy class pass through "
                "the declared lattice gates"
            ),
            "not_proved": (
        ...
        "parameters": {
            ...
            "maximum_mw_rank": 17,
            "seed_language": (
                "7-of-16 coordinate direct summands of one pinned LLL basis "
                "for the residual-involution fixed lattice"
            ),
        },
        "residual_group_order": residual["residual_group"]["order"],
        "d10_diagram_image_order": residual["residual_group"][
            "d10_diagram_image_order"
        ],
        "e7_component_permutation_image_order": residual["residual_group"][
...
def main():
    ...
    if arguments.check:
        if not output.exists() or output.read_text() != encoded:
            raise SystemExit("D10+2E7 coordinate-shell probe is stale")
    else:
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(encoded)
    print(
        "D10_2E7_SHELL|classes={}|subsets={}|seeds={}|status=PASS_EXACT".format(
