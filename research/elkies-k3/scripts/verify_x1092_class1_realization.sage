...
ART=parser.parse_args().artifacts
P='x1092_class1_realization_'
...
iso=mat(mark['class1_representative_columns_in_complement'])
target=mat(census['rootless_classes'][0]['gram'])
...
with zipfile.ZipFile(ART/'det1092_pruned_anchor_packets_v1.zip') as z:
    requested=mat(json.loads(z.read('anchor-16.json'))['embeddings'][33]['gram'])
req=mat(mark['embedding33_frame_rows_in_source_NS'])
...
print('PASS full NS transport, class1 isometry, distinct frame, nef fibre and rational zero',flush=True)
...
compact_points=[Ec([dec(c,K) for c in p]) for p in compact['basis_weierstrass_coordinates']]
for i in range(4):
    carrier=read('carrier_'+str(i));word=vector(ZZ,carrier['word'])
    assert word*hg*word==10
    conic=carrier['conic'];h0,nx0,ny0,qc=[Ru(conic[k]) for k in ('h','nx','ny','q')]
    P0=sum((-ZZ(c0)*p for c0,p in zip(word,compact_points)),Ec(0))
    assert P0==Ec(K(nx0/h0**2),K(ny0/h0**3)) and h0.degree()==3
    x0,x1,y0,y1=[Ru(p) for p in conic['maps']]
    assert y0*y0+y1*y1*qc==x0**3+3*x0*x1*x1*qc+Ec.a4()*x0+Ec.a6()
    assert 2*y0*y1==3*x0*x0*x1+x1**3*qc+Ec.a4()*x1
    assert qc.degree() in (1,2) and qc.gcd(qc.derivative()).degree()==0
    assert carrier['parameters_tested']==0 and carrier['constructed_strict_class']=='UNKNOWN'
print('PASS four generic carrier equations/maps; arithmetic strict classes remain UNKNOWN',flush=True)
