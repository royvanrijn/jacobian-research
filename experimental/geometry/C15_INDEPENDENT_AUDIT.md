# C15 independent audit and explicit consequences

## Corrected theorem

Over any characteristic-zero field, the normalized C01 map is connected to an
...
The production scripts use SymPy. `audit_c15_independent.py` instead implements
sparse polynomial arithmetic and rational Gaussian elimination using only the
Python standard library. Starting with the three C01 coordinates, it recomputes
...
- Campbell, Theorem 5, applies to a nondegenerate polynomial map and explicitly
  ...
  maps. C01 is polynomial, normalized to fix zero with identity linear part,
...
Let `F=(F_3/2,F_2,F_1)` be the normalized three-dimensional C01 map and
